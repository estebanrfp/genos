...
const requestedWorkers = Number.parseInt(process.env.GENOS_E2E_WORKERS ?? "", 10);
const e2eWorkers =
  Number.isFinite(requestedWorkers) && requestedWorkers > 0
    ? Math.min(16, requestedWorkers)
    : defaultWorkers;
const verboseE2E = process.env.GENOS_E2E_VERBOSE === "1";

const baseTest = baseConfig.test ?? {};
const exclude = (baseTest.exclude ?? []).filter((p) => p !== "**/*.e2e.test.js");
...
export default defineConfig({
  ...
  test: {
    ...
    silent: !verboseE2E,
    include: ["test/**/*.e2e.test.js", "src/**/*.e2e.test.js"],
    exclude,
  },
});
