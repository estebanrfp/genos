...
describe("resolveAgentRoute", () => {
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/genosos-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("parentPeer binding inheritance (thread support)", () => {
  ...
  const defaultParentPeer = { kind: "channel", id: "parent-channel-123" };
  function makeDiscordPeerBinding(agentId, peerId) {
    return {
      agentId,
      match: {
        channel: "discord",
  ...
  function makeDiscordGuildBinding(agentId, guildId) {
    return {
      agentId,
      match: {
        channel: "discord",
...
describe("role-based agent routing", () => {
  function makeDiscordRoleBinding(agentId, params = {}) {
    return {
      agentId,
      match: {
        channel: "discord",
