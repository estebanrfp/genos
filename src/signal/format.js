let normalizeUrlForComparison = function (url) {
  ...
  clampStyles = function (styles, maxLength) {
    const clamped = [];
  ...
  applyInsertionsToStyles = function (spans, insertions) {
  ...
  sliceSignalStyles = function (styles, start, end) {
    const sliced = [];
    for (const style of styles) {
  ...
  splitSignalFormattedText = function (formatted, limit) {
  ...
  findBreakIndex = function (window) {
    let lastNewline = -1;
