let resolveRuntime = function (opts) {
    return opts.runtime ?? createNonExitingRuntime();
  },
  normalizeAllowList = function (raw) {
    return normalizeStringEntries(raw);
  },
  resolveSignalReactionTargets = function (reaction) {
    const targets = [];
  ...
  isSignalReactionMessage = function (reaction) {
    if (!reaction) {
      return false;
    }
  ...
  shouldEmitSignalReactionNotification = function (params) {
  ...
  buildSignalReactionSystemEventText = function (params) {
...
async function fetchAttachment(params) {
  ...
  if (attachment.size && attachment.size > params.maxBytes) {
    throw new Error(
      `Signal attachment ${attachment.id} exceeds ${(params.maxBytes / 1048576).toFixed(0)}MB limit`,
    );
  }
  const rpcParams = {
    id: attachment.id,
...
export async function monitorSignalProvider(opts = {}) {
  ...
  const startupTimeoutMs = Math.min(
    120000,
    Math.max(1000, opts.startupTimeoutMs ?? accountInfo.config.startupTimeoutMs ?? 30000),
  );
