let pathForTest = function () {
    const file = path.join(os.tmpdir(), `genosos-log-${crypto.randomUUID()}.log`);
...
describe("logger helpers", () => {
  ...
  it("formats messages through runtime log/error", () => {
    const log = vi.fn();
    const error = vi.fn();
    const runtime = { log, error, exit: vi.fn() };
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `genosos-${today}.log`);
    const oldPath = path.join(DEFAULT_LOG_DIR, "genosos-2000-01-01.log");
