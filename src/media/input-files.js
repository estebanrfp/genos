let rejectOversizedBase64Payload = function (params) {
  ...
  decodeTextContent = function (buffer, charset) {
  ...
  clampText = function (text, maxChars) {
...
export const DEFAULT_INPUT_IMAGE_MAX_BYTES = 10485760;
export const DEFAULT_INPUT_FILE_MAX_BYTES = 5242880;
export const DEFAULT_INPUT_FILE_MAX_CHARS = 200000;
export const DEFAULT_INPUT_MAX_REDIRECTS = 3;
export const DEFAULT_INPUT_TIMEOUT_MS = 1e4;
...
export async function fetchWithGuard(params) {
  const { response, release } = await fetchWithSsrFGuard({
    ...
    init: { headers: { "User-Agent": "GenosOS-Gateway/1.0" } },
  });
  try {
    if (!response.ok) {
