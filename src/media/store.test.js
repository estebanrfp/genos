...
describe("media store", () => {
  ...
  let envSnapshot;
  beforeAll(async () => {
    envSnapshot = captureEnv(["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "GENOS_STATE_DIR"]);
    home = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-test-home-"));
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.genosv1${path.sep}media`);
  ...
  it("saves buffers and enforces size limit", async () => {
    await withTempStore(async (store) => {
      ...
      const huge = Buffer.alloc(5242881);
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, home) => {
      ...
      vi.doMock("./mime.js", async () => {
        const actual = await vi.importActual("./mime.js");
        return {
          ...actual,
  ...
  describe("saveMediaBuffer with originalFilename", () => {
    ...
    it("truncates long original filenames", async () => {
      await withTempStore(async (store) => {
        ...
        const saved = await store.saveMediaBuffer(buf, "text/plain", "inbound", 5242880, longName);
