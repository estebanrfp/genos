...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot(params) {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-ui-"));
    try {
  ...
  function parseBootstrapPayload(end) {
  ...
  it("sets security headers for Control UI responses", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = handleControlUiHttpRequest({ url: "/", method: "GET" }, res, {
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = handleControlUiHttpRequest(
          { url: CONTROL_UI_BOOTSTRAP_CONFIG_PATH, method: "GET" },
          res,
          {
            root: { kind: "resolved", path: tmp },
            config: {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: `/genosos${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" },
          res,
          {
            basePath: "/genosos",
            root: { kind: "resolved", path: tmp },
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/genosos");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/genosos/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
