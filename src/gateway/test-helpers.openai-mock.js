let extractLastUserText = function (input) {
    for (let i = input.length - 1; i >= 0; i -= 1) {
      ...
      if (Array.isArray(content)) {
        const text = content
          .filter(
            (c) =>
              !!c && typeof c === "object" && c.type === "input_text" && typeof c.text === "string",
          )
          .map((c) => c.text)
          .join("\n")
          .trim();
  ...
  extractToolOutput = function (input) {
