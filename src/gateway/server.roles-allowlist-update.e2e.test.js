...
const connectNodeClient = async (params) => {
  const token = process.env.GENOS_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("GENOS_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway role enforcement", () => {
  test("enforces operator and node permissions", async () => {
    ...
    try {
      ...
      const binsRes = await rpcReq(nodeWs, "skills.bins", {});
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".genosv1", "restart-sentinel.json");
...
describe("gateway node command allowlist", () => {
  test("enforces command allowlists across node clients", async () => {
    const waitForConnectedCount = async (count) => {
      await expect
        .poll(
          async () => {
            const listRes = await rpcReq(ws, "node.list", {});
