let normalizeLogin = function (login) {
    return login.trim().toLowerCase();
  },
  headerValue = function (value) {
    return Array.isArray(value) ? value[0] : value;
  },
  resolveTailscaleClientIp = function (req) {
    if (!req) {
      return;
    }
  ...
  resolveRequestClientIp = function (req, trustedProxies) {
    if (!req) {
      return;
    }
  ...
  getTailscaleUser = function (req) {
    ...
    return {
      ...
      name,
      profilePic:
        typeof profilePic === "string" && profilePic.trim() ? profilePic.trim() : undefined,
    };
  },
  hasTailscaleProxyHeaders = function (req) {
    if (!req) {
      return false;
    }
    return Boolean(
  ...
  isTailscaleProxyRequest = function (req) {
    if (!req) {
      return false;
    }
    return isLoopbackAddress(req.socket?.remoteAddress) && hasTailscaleProxyHeaders(req);
  },
  authorizeTrustedProxy = function (params) {
...
export function resolveGatewayAuth(params) {
  ...
  const token = authConfig.token ?? env.GENOS_GATEWAY_TOKEN ?? undefined;
  const password = authConfig.password ?? env.GENOS_GATEWAY_PASSWORD ?? undefined;
...
export function assertGatewayAuthConfigured(auth) {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or GENOS_GATEWAY_TOKEN)",
    );
  }
...
export async function authorizeGatewayConnect(params) {
  ...
  if (auth.mode === "token") {
    ...
    if (!safeEqualSecret(connectAuth.token, auth.token)) {
      limiter?.recordFailure(ip, rateLimitScope);
      auditEvent("gateway.auth.failure", { reason: "token_mismatch", ip });
  ...
  if (auth.mode === "password") {
    ...
    if (!safeEqualSecret(password, auth.password)) {
      limiter?.recordFailure(ip, rateLimitScope);
      auditEvent("gateway.auth.failure", { reason: "password_mismatch", ip });
