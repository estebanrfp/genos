...
vi.mock("./session-utils.js", () => ({
  loadSessionEntry: vi.fn((sessionKey) => buildSessionLookup(sessionKey)),
  pruneLegacyStoreKeys: vi.fn(),
  resolveGatewaySessionStoreTarget: vi.fn(({ key }) => ({
    canonicalKey: key,
    storeKeys: [key],
  })),
}));
...
describe("voice transcript events", () => {
  beforeEach(() => {
    ...
    agentCommandMock.mockResolvedValue({ status: "ok" });
...
describe("agent request events", () => {
  beforeEach(() => {
    ...
    loadSessionEntryMock.mockImplementation((sessionKey) => buildSessionLookup(sessionKey));
  });
