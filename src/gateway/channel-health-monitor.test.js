let createMockChannelManager = function (overrides) {
    return {
  ...
  snapshotWith = function (accounts) {
    const channels = {};
    ...
    for (const [channelId, accts] of Object.entries(accounts)) {
      ...
      channelAccounts[channelId] = resolved;
      const firstId = Object.keys(accts)[0];
      if (firstId) {
        channels[channelId] = resolved[firstId];
      }
    }
    return { channels, channelAccounts };
  },
  createSnapshotManager = function (accounts, overrides) {
    return createMockChannelManager({
      getRuntimeSnapshot: vi.fn(() => snapshotWith(accounts)),
      ...overrides,
    });
  },
  startDefaultMonitor = function (manager, overrides = {}) {
  ...
  managedStoppedAccount = function (lastError) {
    return {
      running: false,
      enabled: true,
      configured: true,
...
async function startAndRunCheck(manager, overrides = {}) {
...
describe("channel-health-monitor", () => {
  ...
  it("does not run before the grace period", async () => {
    ...
    const monitor = startDefaultMonitor(manager, { startupGraceMs: 60000 });
