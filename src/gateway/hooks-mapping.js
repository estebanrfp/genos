let normalizeHookMapping = function (mapping, index, transformsDir) {
  ...
  mappingMatches = function (mapping, ctx) {
  ...
  buildActionFromMapping = function (mapping, ctx) {
  ...
  mergeAction = function (base, override, defaultAction) {
    if (!override) {
    ...
    if (kind === "wake") {
      ...
      const mode =
        override.mode === "next-heartbeat" ? "next-heartbeat" : (baseWake?.mode ?? "now");
  ...
  resolveTransformFn = function (mod, exportName) {
  ...
  resolvePath = function (baseDir, target) {
    if (!target) {
      return path.resolve(baseDir);
    }
    return path.isAbsolute(target) ? path.resolve(target) : path.resolve(baseDir, target);
  },
  resolveContainedPath = function (baseDir, target, label) {
  ...
  resolveOptionalContainedPath = function (baseDir, target, label) {
    const trimmed = target?.trim();
    if (!trimmed) {
      return path.resolve(baseDir);
    }
  ...
  normalizeMatchPath = function (raw) {
    if (!raw) {
      return;
    }
  ...
  renderOptional = function (value, ctx) {
    if (!value) {
      return;
    }
  ...
  resolveTemplateExpr = function (expr, ctx) {
    if (expr === "path") {
      return ctx.path;
    }
    if (expr === "now") {
  ...
  getByPath = function (input, pathExpr) {
    if (!pathExpr) {
      return;
    }
    const parts = [];
