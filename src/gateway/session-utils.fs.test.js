let registerTempSessionStore = function (prefix, assignPaths) {
  let dir = "";
  beforeAll(() => {
...
describe("readFirstUserMessageFromTranscript", () => {
  let tmpDir;
  let storePath;
  registerTempSessionStore("genosos-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    expect(marker.__genosos?.kind).toBe("compaction");
    expect(marker.__genosos?.id).toBe("comp-1");
...
describe("readSessionPreviewItemsFromTranscript", () => {
  let tmpDir;
  let storePath;
  registerTempSessionStore("genosos-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
  function writeTranscriptLines(sessionId, lines) {
    const transcriptPath = path.join(tmpDir, `${sessionId}.jsonl`);
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf-8");
  }
  function readPreview(sessionId, maxItems = 3, maxChars = 120) {
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses GENOS_HOME instead of os.homedir()", () => {
    vi.stubEnv("GENOS_HOME", "/srv/genosos-home");
    vi.stubEnv("HOME", "/home/other");
    const candidates = resolveSessionTranscriptCandidates("sess-1", undefined);
    const fallback = candidates[candidates.length - 1];
    expect(fallback).toBe(
      path.join(path.resolve("/srv/genosos-home"), ".genosv1", "sessions", "sess-1.jsonl"),
    );
  });
});
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile when storePath agent context differs", () => {
    const storePath = "/tmp/genosos/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/genosos/agents/ops/sessions/sess-safe.jsonl";
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/genosos/agents/main/sessions/sessions.json",
    );
...
describe("archiveSessionTranscripts", () => {
  let tmpDir;
  let storePath;
  registerTempSessionStore("genosos-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
  beforeAll(() => {
    vi.stubEnv("GENOS_HOME", tmpDir);
  });
  afterAll(() => {
    vi.unstubAllEnvs();
  });
