let redactWebhookUrl = function (url) {
    try {
  ...
  resolveCronWebhookTarget = function (params) {
...
export function buildGatewayCronService(params) {
  ...
  const cronEnabled = process.env.GENOS_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;
  ...
  const cron = new CronService({
    ...
    sendToSession: ({ sessionKey, message, jobId }) => {
      callGateway({
        method: "chat.send",
        ...
        cronLogger.warn(
          { err: String(err), jobId, sessionKey },
          "cron: direct session delivery failed",
        );
      });
    },
    deleteCronSession: ({ sessionKey, jobId }) => {
      callGateway({
        ...
        .then(() => {
          params.broadcast("sessions.changed", {}, { dropIfSlow: true });
        })
        .catch((err) => {
          cronLogger.warn({ err: String(err), jobId, sessionKey }, "cron: session cleanup failed");
        });
    },
    log: getChildLogger({ module: "cron", storePath }),
    onEvent: (evt) => {
      params.broadcast("cron", evt, { dropIfSlow: true });
      if (evt.action === "started" || evt.action === "finished") {
        params.broadcast("sessions.changed", {}, { dropIfSlow: true });
      }
