let createRuntimeStore = function () {
    return {
      aborts: new Map(),
      tasks: new Map(),
      runtimes: new Map(),
  ...
  isAccountEnabled = function (account) {
  ...
  resolveDefaultRuntime = function (channelId) {
    const plugin = getChannelPlugin(channelId);
    return plugin?.status?.defaultRuntime ?? { accountId: DEFAULT_ACCOUNT_ID };
  },
  cloneDefaultRuntime = function (channelId, accountId) {
...
export function createChannelManager(opts) {
  ...
  const setRuntime = (channelId, accountId, patch) => {
    ...
    if (typeof patch.connected === "boolean" && patch.connected !== current.connected) {
      opts.onChannelStatusChange?.(channelId, accountId, next);
    }
    return next;
  };
