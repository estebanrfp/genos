...
describe("runBootOnce", () => {
  ...
  const mockAgentUpdatesMainSession = (storePath, sessionKey) => {
    agentCommand.mockImplementation(async (opts) => {
  ...
  it("skips when BOOT.md is missing", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-boot-"));
