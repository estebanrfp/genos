...
describe("gateway server hooks", () => {
  test("handles auth, wake, and agent flows", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      const call = cronIsolatedRun.mock.calls[0]?.[0];
      ...
      await waitForSystemEvent();
      const routedCall = cronIsolatedRun.mock.calls[0]?.[0];
      ...
      await waitForSystemEvent();
      const fallbackCall = cronIsolatedRun.mock.calls[0]?.[0];
  ...
  test("respects hooks session policy for request + mapping session keys", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      await waitForSystemEvent();
      const defaultCall = cronIsolatedRun.mock.calls[0]?.[0];
      ...
      await waitForSystemEvent();
      const mappedCall = cronIsolatedRun.mock.calls[0]?.[0];
  ...
  test("enforces hooks.allowedAgentIds for explicit agent routing", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      await waitForSystemEvent();
      const noAgentCall = cronIsolatedRun.mock.calls[0]?.[0];
      ...
      await waitForSystemEvent();
      const allowedCall = cronIsolatedRun.mock.calls[0]?.[0];
      ...
      const deniedBody = await resDenied.json();
      ...
      const mappedDeniedBody = await resMappedDenied.json();
