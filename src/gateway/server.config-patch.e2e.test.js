...
describe("gateway config methods", () => {
  const seedAgentsConfig = async (list) => {
  ...
  const readConfigHash = async () => {
    const snapshotRes = await rpcReq(ws, "config.get", {});
  ...
  it("rejects config.patch when raw is not an object", async () => {
    const res = await rpcReq(ws, "config.patch", {
      raw: "[]",
    });
...
describe("gateway server sessions", () => {
  it("filters sessions by agentId", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-sessions-agents-"));
    ...
    const homeSessions = await rpcReq(ws, "sessions.list", {
      includeGlobal: false,
    ...
    const workSessions = await rpcReq(ws, "sessions.list", {
      includeGlobal: false,
      includeUnknown: false,
      agentId: "work",
    });
