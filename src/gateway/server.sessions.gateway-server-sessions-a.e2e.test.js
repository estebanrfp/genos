let expectActiveRunCleanup = function (requesterSessionKey, expectedQueueKeys, sessionId) {
  ...
  const clearedKeys = sessionCleanupMocks.clearSessionQueues.mock.calls[0]?.[0];
...
vi.mock("../auto-reply/reply/queue.js", async () => {
  const actual = await vi.importActual("../auto-reply/reply/queue.js");
  return {
    ...actual,
...
vi.mock("../auto-reply/reply/abort.js", async () => {
  const actual = await vi.importActual("../auto-reply/reply/abort.js");
  return {
    ...actual,
...
vi.mock("../hooks/internal-hooks.js", async () => {
  const actual = await vi.importActual("../hooks/internal-hooks.js");
  return {
    ...actual,
...
const openClient = async (opts) => await harness.openClient(opts);
async function createSessionStoreDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-sessions-"));
  const storePath = path.join(dir, "sessions.json");
  testState.sessionStorePath = storePath;
  return { dir, storePath };
}
async function writeSingleLineSession(dir, sessionId, content) {
  await fs.writeFile(
...
async function getMainPreviewEntry(ws) {
  const preview = await rpcReq(ws, "sessions.preview", { keys: ["main"], limit: 3, maxChars: 120 });
...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    ...
    expect(badThinking.error?.message ?? "").toMatch(/invalid thinkinglevel/i);
    ws.close();
  });
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("sessions.preview resolves legacy mixed-case main alias with custom mainKey", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-sessions-preview-alias-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("sessions.resolve and mutators clean legacy main-alias ghost keys", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-sessions-cleanup-alias-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
    testState.agentsConfig = { list: [{ id: "ops", default: true }] };
    testState.sessionConfig = { mainKey: "work" };
    ...
    const writeRawStore = async (store) => {
      await fs.writeFile(storePath, `${JSON.stringify(store, null, 2)}\n`, "utf-8");
    };
    const readStore = async () => JSON.parse(await fs.readFile(storePath, "utf-8"));
    await writeRawStore({
      "agent:ops:MAIN": { sessionId, updatedAt: Date.now() - 2000 },
      "agent:ops:Main": { sessionId, updatedAt: Date.now() - 1000 },
    });
    const { ws } = await openClient();
    ...
    const reset = await rpcReq(ws, "sessions.reset", { key: "main" });
    expect(reset.ok).toBe(true);
  ...
  test("sessions.reset emits internal command hook with reason", async () => {
    ...
    const event = sessionHookMocks.triggerInternalHook.mock.calls[0]?.[0];
    if (!event) {
