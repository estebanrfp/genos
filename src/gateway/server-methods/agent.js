let isGatewayErrorShape = function (value) {
...
import { buildSessionResetPrompt } from "../../auto-reply/reply/session-reset-prompt.js";
...
export const agentHandlers = {
  agent: async ({ params, respond, context, client, isWebchatConnect }) => {
    ...
    if (resetCommandMatch && requestedSessionKey) {
      ...
      const localeMatch = postResetMessage.match(/^\[locale:([^\]]+)\]$/);
      if (postResetMessage && !localeMatch) {
        message = postResetMessage;
      } else {
        message = buildSessionResetPrompt(localeMatch?.[1] ?? params.locale);
    ...
    if (requestedSessionKey) {
      ...
      // Notify sidebar when inter-agent messages create/update sessions
  ...
  "agent.identity.get": ({ params, respond }) => {
    ...
    respond(
      true,
      { ...identity, avatar: avatarValue, ownerDisplayName: cfg.owner?.displayName ?? null },
      undefined,
    );
  },
