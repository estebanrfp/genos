...
export const agentsHandlers = {
  ...
  "agents.create": async ({ params, respond, context }) => {
    ...
    // Generate opaque UUID hex as agent ID (like GenosOS Pro) — decouples identity from name
    const agentId = generateAgentDirId();
    ...
    // Generate opaque directory ID for agent config/sessions storage
    const dirId = generateAgentDirId();
    const stateDir = resolveStateDir(process.env);
    // Workspace: workspace/{kebab-case-name}/ (like GenosOS Pro)
    const workspaceDir = explicitWorkspace
      ? resolveUserPath(explicitWorkspace)
      : path.join(stateDir, "workspace", toKebabCase(rawName));
    ...
    await updateSessionStore(sessionStorePath, (store) => {
      ...
      store[mainSessionKey] = {
        ...
        createdAt: now,
        updatedAt: now,
        label: `Session ${new Date(now).toLocaleDateString()}`,
        initializing: true,
      };
    });
  ...
  "agents.rename": async ({ params, respond, context }) => {
    ...
    const cfg = loadConfig();
    const agentId = normalizeAgentId(String(params.agentId ?? ""));
    const newName = String(params.name ?? params.newId ?? "").trim();
    if (!newName) {
    ...
    // UUID-based IDs: rename only updates the display name, not the ID or session keys
    const oldName = list[index].name ?? agentId;
    const nextList = [...list];
    nextList[index] = { ...nextList[index], name: newName };
    const nextConfig = { ...cfg, agents: { ...cfg.agents, list: nextList } };
    await writeConfigFile(nextConfig);

    // Rename workspace folder if derived from name (like GenosOS Pro)
    const stateDir = resolveStateDir(process.env);
    const oldDir = path.join(stateDir, "workspace", toKebabCase(oldName));
    const newDir = path.join(stateDir, "workspace", toKebabCase(newName));
    if (oldDir !== newDir) {
      try {
        await fs.rename(oldDir, newDir);
        console.log(
          `[agents] workspace renamed: ${toKebabCase(oldName)} → ${toKebabCase(newName)}`,
        );
      } catch {
        // Workspace may not exist yet or use explicit path — skip silently
      }
    }

    context.broadcast?.("sessions.changed", {}, { dropIfSlow: true });
    respond(true, { ok: true, agentId, name: newName }, undefined);
  },
