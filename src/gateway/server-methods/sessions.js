let requireSessionKey = function (key, respond) {
    const raw =
      typeof key === "string"
        ? key
        : typeof key === "number"
  ...
  resolveGatewaySessionTargetFromKey = function (key) {
    const cfg = loadConfig();
    const target = resolveGatewaySessionStoreTarget({ cfg, key });
    return { cfg, target, storePath: target.storePath };
  },
  migrateAndPruneSessionStoreKey = function (params) {
  ...
  archiveSessionTranscriptsForSession = function (params) {
    if (!params.sessionId) {
      return [];
    }
...
async function ensureSessionRuntimeCleanup(params) {
  ...
  const ended = await waitForEmbeddedPiRunEnd(params.sessionId, 15000);
  if (ended) {
    return;
  }
  return errorShape(
...
export const sessionsHandlers = {
  "sessions.list": ({ params, respond, context }) => {
    ...
    const result = listSessionsFromStore({
      ...
      store,
      opts: p,
      activeRunSessionKeys: context?.chatAbortControllers
        ? new Set([...context.chatAbortControllers.values()].map((r) => r.sessionKey))
        : undefined,
    });
  ...
  "sessions.compactionSummary": ({ params, respond }) => {
    ...
    if (!entry?.sessionFile) {
      respond(true, { summary: null }, undefined);
      return;
    }
  ...
  "sessions.create": async ({ params, respond, context }) => {
    ...
    const result = await updateSessionStore(storePath, async (store) => {
      ...
      if (store[canonicalKey]) {
        return { ok: true, entry: store[canonicalKey], existed: true };
      }
      const now = Date.now();
      const entry = { sessionId: randomUUID(), createdAt: now, updatedAt: now };
      if (p.label) {
        entry.label = String(p.label).trim().slice(0, 64);
      }
      store[canonicalKey] = entry;
      return { ok: true, entry, existed: false };
    });
    if (!result.ok) {
      respond(false, undefined, result.error ?? "failed to create session");
      return;
    }
  ...
  "sessions.compact": async ({ params, respond }) => {
    ...
    secureWriteFileSync(filePath, `${keptLines.join("\n")}\n`);
    await updateSessionStore(storePath, (store) => {
