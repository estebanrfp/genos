...
describe("sanitizeChatSendMessageInput", () => {
  it("rejects null bytes", () => {
    expect(sanitizeChatSendMessageInput("before\0after")).toEqual({
      ok: false,
  ...
  it("strips unsafe control characters while preserving tab/newline/carriage return", () => {
    const result = sanitizeChatSendMessageInput(`a\x01b	c
d\re\x07f\x7F`);
    expect(result).toEqual({ ok: true, message: "ab\tc\nd\ref" });
  });
  it("normalizes unicode to NFC", () => {
    expect(sanitizeChatSendMessageInput("Cafe\u0301")).toEqual({ ok: true, message: "Caf\xE9" });
  });
});
describe("gateway chat transcript writes (guardrail)", () => {
  it("does not append transcript messages via raw fs.appendFileSync(transcriptPath, ...)", () => {
    const chatJs = fileURLToPath(new URL("./chat.js", import.meta.url));
    const src = fs.readFileSync(chatJs, "utf-8");
...
describe("exec approval handlers", () => {
  ...
  async function resolveExecApproval(params) {
    return params.handlers["exec.approval.resolve"]({
      params: { id: params.id, decision: "allow-once" },
      respond: params.respond,
...
describe("gateway healthHandlers.status scope handling", () => {
  ...
  async function runHealthStatus(scopes) {
    const respond = vi.fn();
    const status = await import("../../commands/status.js");
...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "genosos-logs-"));
    const older = path.join(tempDir, "genosos-2026-01-20.log");
    const newer = path.join(tempDir, "genosos-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "genosos-2026-01-22.log") });
    const respond = vi.fn();
