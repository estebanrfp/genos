...
export async function resolveSessionKeyFromResolveParams(params) {
  ...
  if (list.sessions.length === 0) {
    // Fallback: if agentId is provided, derive the canonical session key directly.
    // Agent sessions are created lazily — the key `agent:{id}:{label}` is valid even
    // before the first message. This allows sessions_send to reach implicit sessions.
    if (p.agentId) {
      const agentId = normalizeAgentId(p.agentId);
      const derivedKey = `agent:${agentId}:${parsedLabel.label.toLowerCase()}`;
