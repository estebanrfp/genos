...
async function expectNoForwardedInvoke(hasInvoke) {
  await new Promise((resolve) => setImmediate(resolve));
  await new Promise((resolve) => setImmediate(resolve));
  expect(hasInvoke()).toBe(false);
}
async function getConnectedNodeId(ws) {
  const nodes = await rpcReq(ws, "node.list", {});
  expect(nodes.ok).toBe(true);
  const nodeId = nodes.payload?.nodes?.find((n) => n.connected)?.nodeId ?? "";
  expect(nodeId).toBeTruthy();
  return nodeId;
}
async function requestAllowOnceApproval(ws, command) {
...
describe("node.invoke approval bypass", () => {
  ...
  const connectOperator = async (scopes) => {
  ...
  const connectOperatorWithNewDevice = async (scopes) => {
  ...
  const connectLinuxNode = async (onInvoke) => {
  ...
  test("binds system.run approval flags to exec.approval decision (ignores caller escalation)", async () => {
    ...
    const node = await connectLinuxNode((payload) => {
      ...
      lastInvokeParams = JSON.parse(raw);
    });
