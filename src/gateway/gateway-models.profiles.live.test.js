let parseFilter = function (raw) {
  ...
  assertNoReasoningTags = function (params) {
    if (!params.text) {
      return;
    }
  ...
  extractPayloadText = function (result) {
    const record = result;
  ...
  isGoogleModelNotFoundText = function (text) {
    const trimmed = text.trim();
    if (!trimmed) {
      return false;
    }
  ...
  isGoogleishProvider = function (provider) {
    return provider === "google" || provider.startsWith("google-");
  },
  isRefreshTokenReused = function (error) {
    return /refresh_token_reused/i.test(error);
  },
  isChatGPTUsageLimitErrorMessage = function (raw) {
    const msg = raw.toLowerCase();
    return msg.includes("hit your chatgpt usage limit") && msg.includes("try again in");
  },
  isInstructionsRequiredError = function (error) {
    return /instructions are required/i.test(error);
  },
  isOpenAIReasoningSequenceError = function (error) {
    const msg = error.toLowerCase();
    return msg.includes("required following item") && msg.includes("reasoning");
  },
  isToolNonceRefusal = function (error) {
    const msg = error.toLowerCase();
  ...
  buildLiveGatewayConfig = function (params) {
    ...
    return {
      ...
      models: Object.keys(providers).length > 0 ? { ...baseModels, providers } : baseModels,
    };
  },
  sanitizeAuthConfig = function (params) {
  ...
  buildMinimaxProviderOverride = function (params) {
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.GENOS_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.GENOS_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.GENOS_LIVE_GATEWAY_PROVIDERS);
...
async function runAnthropicRefusalProbe(params) {
  ...
  const probe = await params.client.request(
    "agent",
    {
  ...
  const followup = await params.client.request(
    "agent",
    {
      sessionKey: params.sessionKey,
...
async function runGatewayModelSuite(params) {
  const previous = {
    configPath: process.env.GENOS_CONFIG_PATH,
    token: process.env.GENOS_GATEWAY_TOKEN,
    skipChannels: process.env.GENOS_SKIP_CHANNELS,
    skipGmail: process.env.GENOS_SKIP_GMAIL_WATCHER,
    skipCron: process.env.GENOS_SKIP_CRON,
    skipCanvas: process.env.GENOS_SKIP_CANVAS_HOST,
    agentDir: process.env.GENOS_AGENT_DIR,
  ...
  const agentId = "dev";
  const hostAgentDir = resolveGenosOSAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-live-state-"));
  ...
  process.env.GENOS_AGENT_DIR = tempAgentDir;
  process.env.PI_CODING_AGENT_DIR = tempAgentDir;
  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  const nonceA = randomUUID();
  const nonceB = randomUUID();
  const toolProbePath = path.join(workspaceDir, `.genos-live-tool-probe.${nonceA}.txt`);
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-live-"));
  const tempConfigPath = path.join(tempDir, "genosos.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.GENOS_CONFIG_PATH = tempConfigPath;
  await ensureGenosOSModelsJson(nextCfg);
  ...
  try {
    ...
    for (const [index, model] of params.candidates.entries()) {
      ...
      for (let attempt = 0; attempt < attemptMax; attempt += 1) {
        ...
        try {
          ...
          const toolProbe = await client.request(
            "agent",
            {
              sessionKey,
              idempotencyKey: `idem-${runIdTool}-tool`,
              message: `GenosOS live tool probe (local, safe): use the tool named \`read\` (or \`Read\`) with JSON arguments {"path":"${toolProbePath}"}. Then reply with the two nonce values you read (include both).`,
              thinking: params.thinkingLevel,
              deliver: false,
            },
            { expectFinal: true },
          ...
          if (params.extraToolProbes) {
            ...
            const execReadProbe = await client.request(
              "agent",
              {
                sessionKey,
                idempotencyKey: `idem-${runIdTool}-exec-read`,
                message: `GenosOS live tool probe (local, safe): use the tool named \`exec\` (or \`Exec\`) to run this command: mkdir -p "${tempDir}" && printf '%s' '${nonceC}' > "${toolWritePath}". Then use the tool named \`read\` (or \`Read\`) with JSON arguments {"path":"${toolWritePath}"}. Finally reply including the nonce text you read back.`,
                thinking: params.thinkingLevel,
                deliver: false,
              },
              { expectFinal: true },
          ...
          if (params.extraImageProbes && model.input?.includes("image")) {
            ...
            const imageProbe = await client.request(
              "agent",
              {
                ...
                message:
                  "Look at the attached image. Reply with exactly two tokens separated by a single space: (1) the animal shown or written in the image, lowercase; (2) the code printed in the image, uppercase. No extra text.",
                attachments: [
                  {
                    mimeType: "image/png",
    ...
    process.env.GENOS_CONFIG_PATH = previous.configPath;
    process.env.GENOS_GATEWAY_TOKEN = previous.token;
    process.env.GENOS_SKIP_CHANNELS = previous.skipChannels;
    process.env.GENOS_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.GENOS_SKIP_CRON = previous.skipCron;
    process.env.GENOS_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.GENOS_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.GENOS_STATE_DIR = previous.stateDir;
  }
}
describeLive("gateway live (dev agent, profile keys)", () => {
  it("runs meaningful prompts across models with available keys", async () => {
    ...
    const rawModels = process.env.GENOS_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const authStorage = discoverAuthStorage(agentDir);
    const modelRegistry = discoverModels(authStorage, agentDir);
    const anthropic = modelRegistry.find("anthropic", "claude-opus-4-5");
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.genos-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
    const port = await getFreeGatewayPort();
    const server = await startGatewayServer(port, {
      bind: "loopback",
    ...
    try {
      ...
      const toolProbe = await client.request(
        "agent",
        {
          ...
          message: `Call the tool named \`read\` (or \`Read\` if \`read\` is unavailable) with JSON arguments {"path":"${toolProbePath}"}. Then reply with exactly: ${nonceA} ${nonceB}. No extra text.`,
      ...
      const followup = await client.request(
        "agent",
        {
          ...
          message: `What are the values of nonceA and nonceB in "${toolProbePath}"? Reply with exactly: ${nonceA} ${nonceB}.`,
          thinking: THINKING_LEVEL,
          deliver: false,
        },
        { expectFinal: true },
