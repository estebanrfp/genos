...
describe("resolveTailnetDnsHint", () => {
  const prevTailnetDns = { value: undefined };
  beforeEach(() => {
    prevTailnetDns.value = process.env.GENOS_TAILNET_DNS;
    delete process.env.GENOS_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.GENOS_TAILNET_DNS = prevTailnetDns.value;
    }
  });
  test("returns env hint when disabled", async () => {
    process.env.GENOS_TAILNET_DNS = "studio.tailnet.ts.net.";
