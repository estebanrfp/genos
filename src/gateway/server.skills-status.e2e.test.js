...
describe("gateway skills.status", () => {
  it("does not expose raw config values to operator.read clients", async () => {
    const envSnapshot = captureEnv(["GENOS_BUNDLED_SKILLS_DIR"]);
    process.env.GENOS_BUNDLED_SKILLS_DIR = path.join(process.cwd(), "skills");
    ...
    try {
      await withServer(async (ws) => {
        ...
        const res = await rpcReq(ws, "skills.status", {});
