...
vi.mock("../agents/genosos-tools.js", () => {
  const toolInputError = (message) => {
  ...
  return {
    createGenosOSTools: () => tools,
  };
});
const { handleToolsInvokeHttpRequest } = await import("./tools-invoke-http.js");
let pluginHttpHandlers = [];
let sharedPort = 0;
let sharedServer;
beforeAll(async () => {
...
const resolveGatewayToken = () => TEST_GATEWAY_TOKEN;
...
const invokeAgentsList = async (params) => {
  const body = { tool: "agents_list", action: "json", args: {} };
...
const invokeAgentsListAuthed = async (params = {}) =>
  invokeAgentsList({
    port: sharedPort,
...
describe("POST /tools/invoke", () => {
  ...
  it("routes tools invoke before plugin HTTP handlers", async () => {
    const pluginHandler = vi.fn(async (_req, res) => {
      res.statusCode = 418;
      res.end("plugin");
      return true;
    });
