...
const hoisted = vi.hoisted(() => {
  ...
  let onHotReload = null;
  let onRestart = null;
  const startGatewayConfigReloader = vi.fn((opts) => {
...
describe("gateway hot reload", () => {
  let prevSkipChannels;
  let prevSkipGmail;
  let prevSkipProviders;
  beforeEach(() => {
    prevSkipChannels = process.env.GENOS_SKIP_CHANNELS;
    prevSkipGmail = process.env.GENOS_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.GENOS_SKIP_PROVIDERS;
    process.env.GENOS_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.GENOS_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.GENOS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.GENOS_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.GENOS_SKIP_PROVIDERS;
    } else {
      process.env.GENOS_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
...
describe("gateway agents", () => {
  it("lists configured agents via agents.list RPC", async () => {
    ...
    await connectOk(ws);
    const res = await rpcReq(ws, "agents.list", {});
