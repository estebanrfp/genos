let resetGatewayCallMocks = function () {
  ...
  setGatewayNetworkDefaults = function (port = 18789) {
    resolveGatewayPort.mockReturnValue(port);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
  },
  makeRemotePasswordGatewayConfig = function (remotePassword, localPassword = "from-config") {
    return {
      gateway: {
        mode: "remote",
...
describe("callGateway error details", () => {
  ...
  it("does not overflow very large timeout values", async () => {
    ...
    vi.useFakeTimers();
    let errMessage = "";
    const promise = callGateway({ method: "health", timeoutMs: 2592010000 }).catch((caught) => {
      errMessage = caught instanceof Error ? caught.message : String(caught);
    });
...
describe("callGateway url override auth requirements", () => {
  let envSnapshot;
  beforeEach(() => {
    envSnapshot = captureEnv(["GENOS_GATEWAY_TOKEN", "GENOS_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    ...
    process.env.GENOS_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  let envSnapshot;
  beforeEach(() => {
    envSnapshot = captureEnv(["GENOS_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("prefers env password over local config password", async () => {
    process.env.GENOS_GATEWAY_PASSWORD = "from-env";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
...
describe("callGateway token resolution", () => {
  let envSnapshot;
  beforeEach(() => {
    envSnapshot = captureEnv(["GENOS_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
