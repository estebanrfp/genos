...
describe("gateway server chat", () => {
  test("sanitizes inbound chat.send message text and rejects null bytes", async () => {
    ...
    expect(nullByteRes.error?.message ?? "").toMatch(/null bytes/i);
    const spy = vi.mocked(getReplyFromConfig);
    spy.mockClear();
    const spyCalls = spy.mock.calls;
    const callsBeforeSanitized = spyCalls.length;
    const sanitizedRes = await rpcReq(ws, "chat.send", {
      sessionKey: "main",
      message: "Cafe\u0301\x07\tline",
      idempotencyKey: "idem-sanitized-1",
    });
    expect(sanitizedRes.ok).toBe(true);
    await waitFor(() => spyCalls.length > callsBeforeSanitized);
    const ctx = spyCalls.at(-1)?.[0];
    expect(ctx?.Body).toBe("Caf\xE9\tline");
    expect(ctx?.RawBody).toBe("Caf\xE9\tline");
    expect(ctx?.BodyForCommands).toBe("Caf\xE9\tline");
  });
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const timeoutCall = spyCalls.at(-1)?.[1];
      ...
      expect(blockedRes.error?.message ?? "").toMatch(/send blocked/i);
      testState.sessionStorePath = undefined;
      testState.sessionConfig = undefined;
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-gw-"));
      ...
      expect(agentBlockedRes.error?.message ?? "").toMatch(/send blocked/i);
      testState.sessionStorePath = undefined;
      testState.sessionConfig = undefined;
      spy.mockClear();
      ...
      const imgOnlyOpts = spyCalls.at(-1)?.[1];
      expect(imgOnlyOpts?.images).toEqual([{ type: "image", data: pngB64, mimeType: "image/png" }]);
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-gw-"));
  ...
  test("routes chat.send slash commands without agent runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-gw-"));
    try {
