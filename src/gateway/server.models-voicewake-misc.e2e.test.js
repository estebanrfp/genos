...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir) => {
    ...
    process.env.GENOS_STATE_DIR = path.join(homeDir, ".genosv1");
  ...
  test(
    ...
    { timeout: 60000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-home-"));
      const restoreHome = setTempHome(homeDir);
      const initial = await rpcReq(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["genosos", "claude", "computer"]);
      ...
      expect(changed.payload?.triggers).toEqual(["hi", "there"]);
      const after = await rpcReq(ws, "voicewake.get");
      expect(after.ok).toBe(true);
      expect(after.payload?.triggers).toEqual(["hi", "there"]);
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".genosv1", "settings", "voicewake.json"), "utf8"),
      );
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    ...
    expect(first.payload?.triggers).toEqual(["genosos", "claude", "computer"]);
    const broadcastP = onceMessage(
      nodeWs,
      (o) => o.type === "event" && o.event === "voicewake.changed",
    );
    const setRes = await rpcReq(ws, "voicewake.set", {
      triggers: ["genosos", "computer"],
    });
    expect(setRes.ok).toBe(true);
    const broadcast = await broadcastP;
    expect(broadcast.event).toBe("voicewake.changed");
    expect(broadcast.payload?.triggers).toEqual(["genosos", "computer"]);
    nodeWs.close();
    restoreHome();
  });
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["GENOS_CANVAS_HOST_PORT", "GENOS_GATEWAY_TOKEN"]);
    try {
      ...
      process.env.GENOS_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("send dedupes by idempotencyKey", { timeout: 60000 }, async () => {
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    ...
    if (!configPath) {
      throw new Error("Missing GENOS_CONFIG_PATH");
    }
