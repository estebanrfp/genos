let sendJson = function (res, status, body) {
    res.statusCode = status;
    res.setHeader("Content-Type", "application/json; charset=utf-8");
    res.end(JSON.stringify(body));
  },
  isCanvasPath = function (pathname) {
    return (
  ...
  hasAuthorizedWsClientForIp = function (clients, clientIp) {
  ...
  writeUpgradeAuthFailure = function (socket, auth) {
    if (auth.rateLimited) {
...
import { CANVAS_HOST_PATH, CANVAS_WS_PATH } from "../canvas-host/server.js";
...
import { handleWebAuthnRequest } from "./webauthn-http.js";
const HOOK_AUTH_FAILURE_LIMIT = 20;
const HOOK_AUTH_FAILURE_WINDOW_MS = 60000;
...
export function createHooksRequestHandler(opts) {
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-GenosOS-Token header (query parameters are not allowed).",
      );
      return true;
    }
...
export function createGatewayHttpServer(opts) {
  ...
  async function handleRequest(req, res) {
    ...
    try {
      ...
      if (
        await handleWebAuthnRequest(req, res, {
          auth: resolvedAuth,
          trustedProxies,
          rateLimiter,
        })
      ...
      if (handleControlUiMediaRequest(req, res)) {
        return;
      }
      if (controlUiEnabled) {
        if (
