let collectWsRestMeta = function (meta) {
    const restMeta = [];
    if (!meta) {
      return restMeta;
    }
  ...
  buildWsHeadline = function (params) {
  ...
  compactPreview = function (input, maxLen = 160) {
  ...
  logWsOptimized = function (direction, kind, meta) {
  ...
  logWsCompact = function (direction, kind, meta) {
    const now = Date.now();
    const connId = typeof meta?.connId === "string" ? meta.connId : undefined;
    const id = typeof meta?.id === "string" ? meta.id : undefined;
    const method = typeof meta?.method === "string" ? meta.method : undefined;
...
import { DEFAULT_WS_SLOW_MS, getGatewayWsLogStyle, WS_SILENT_METHODS } from "./ws-logging.js";
...
export function logWs(direction, kind, meta) {
  ...
  // Skip high-frequency polling methods at all verbosity levels
  if (typeof meta?.method === "string" && WS_SILENT_METHODS.has(meta.method)) {
    return;
  }
