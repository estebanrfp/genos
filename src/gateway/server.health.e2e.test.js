...
describe("gateway server health/presence", () => {
  test("connect + health + presence + status succeed", { timeout: 60000 }, async () => {
  ...
  test("broadcasts heartbeat events and serves last-heartbeat", async () => {
    const { ws } = await harness.openClient();
    const waitHeartbeat = onceMessage(ws, (o) => o.type === "event" && o.event === "heartbeat");
  ...
  test("presence includes client fingerprint", async () => {
    ...
    const entries = Array.isArray(presencePayload)
      ? presencePayload
      : Array.isArray(presencePayload?.presence)
