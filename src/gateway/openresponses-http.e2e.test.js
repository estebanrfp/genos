let parseSseEvents = function (text) {
  const events = [];
...
async function writeGatewayConfig(config) {
  ...
  if (!configPath) {
    throw new Error("GENOS_CONFIG_PATH is required for gateway config tests");
  }
  await fs.mkdir(path.dirname(configPath), { recursive: true });
  await fs.writeFile(configPath, JSON.stringify(config, null, 2), "utf-8");
}
async function postResponses(port, body, headers) {
...
async function ensureResponseConsumed(res) {
  if (res.bodyUsed) {
    return;
  }
  try {
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("rejects when disabled (default + config)", { timeout: 120000 }, async () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    const mockAgentOnce = (payloads, meta) => {
      agentCommand.mockReset();
      agentCommand.mockResolvedValueOnce({ payloads, meta });
    };
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "genosos", input: "hi" }),
      });
      ...
      const missingModelJson = await resMissingModel.json();
      expect(missingModelJson.error?.type).toBe("invalid_request_error");
      await ensureResponseConsumed(resMissingModel);
      mockAgentOnce([{ text: "hello" }]);
      const resHeader = await postResponses(
        port,
        { model: "genosos", input: "hi" },
        { "x-genosos-agent-id": "beta" },
      );
      expect(resHeader.status).toBe(200);
      const optsHeader = agentCommand.mock.calls[0]?.[0];
      expect(optsHeader?.sessionKey ?? "").toMatch(/^agent:beta:/);
      await ensureResponseConsumed(resHeader);
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "genosos:beta", input: "hi" });
      expect(resModel.status).toBe(200);
      const optsModel = agentCommand.mock.calls[0]?.[0];
      expect(optsModel?.sessionKey ?? "").toMatch(/^agent:beta:/);
      ...
      const optsUser = agentCommand.mock.calls[0]?.[0];
      expect(optsUser?.sessionKey ?? "").toContain("openresponses-user:alice");
      ...
      const optsString = agentCommand.mock.calls[0]?.[0];
      expect(optsString?.message).toBe("hello world");
      ...
      const optsArray = agentCommand.mock.calls[0]?.[0];
      expect(optsArray?.message).toBe("hello there");
      ...
      const optsSystemDeveloper = agentCommand.mock.calls[0]?.[0];
      const extraSystemPrompt = optsSystemDeveloper?.extraSystemPrompt ?? "";
      ...
      const optsInstructions = agentCommand.mock.calls[0]?.[0];
      const instructionPrompt = optsInstructions?.extraSystemPrompt ?? "";
      ...
      const optsHistory = agentCommand.mock.calls[0]?.[0];
      const historyMessage = optsHistory?.message ?? "";
      ...
      const optsFunctionOutput = agentCommand.mock.calls[0]?.[0];
      const functionOutputMessage = optsFunctionOutput?.message ?? "";
      ...
      const optsInputFile = agentCommand.mock.calls[0]?.[0];
      const inputFileMessage = optsInputFile?.message ?? "";
      const inputFilePrompt = optsInputFile?.extraSystemPrompt ?? "";
      ...
      const optsToolNone = agentCommand.mock.calls[0]?.[0];
      expect(optsToolNone?.clientTools).toBeUndefined();
      ...
      const optsToolChoice = agentCommand.mock.calls[0]?.[0];
      const clientTools = optsToolChoice?.clientTools ?? [];
      ...
      const optsMaxTokens = agentCommand.mock.calls[0]?.[0];
      expect(optsMaxTokens?.streamParams?.maxTokens).toBe(123);
      ...
      const shapeJson = await resShape.json();
      ...
      const noUserJson = await resNoUser.json();
      expect(noUserJson.error?.type).toBe("invalid_request_error");
  ...
  it("blocks unsafe URL-based file/image inputs", async () => {
    ...
    const blockedPrivateJson = await blockedPrivate.json();
    ...
    const blockedMetadataJson = await blockedMetadata.json();
    ...
    const blockedSchemeJson = await blockedScheme.json();
  ...
  it("enforces URL allowlist and URL part cap for responses inputs", async () => {
    ...
    try {
      ...
      const allowlistBlockedJson = await allowlistBlocked.json();
    ...
    try {
      ...
      const maxUrlBlockedJson = await maxUrlBlocked.json();
