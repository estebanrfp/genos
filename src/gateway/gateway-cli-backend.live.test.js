let randomImageProbeCode = function (len = 6) {
  ...
  parseJsonStringArray = function (name, raw) {
  ...
  parseImageMode = function (raw) {
    ...
    throw new Error("GENOS_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
  },
  withMcpConfigOverrides = function (args, mcpConfigPath) {
    const next = [...args];
...
const CLI_LIVE = isTruthyEnvValue(process.env.GENOS_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.GENOS_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.GENOS_LIVE_CLI_BACKEND_RESUME_PROBE);
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.GENOS_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `GENOS_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const providerDefaults =
      providerId === "claude-cli"
        ...
        : providerId === "codex-cli"
          ...
          : null;
    const cliCommand = process.env.GENOS_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(`GENOS_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`);
    }
    const baseCliArgs =
      parseJsonStringArray(
        "GENOS_LIVE_CLI_BACKEND_ARGS",
        process.env.GENOS_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`GENOS_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "GENOS_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.GENOS_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.GENOS_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.GENOS_LIVE_CLI_BACKEND_IMAGE_MODE);
    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "GENOS_LIVE_CLI_BACKEND_IMAGE_MODE requires GENOS_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-live-cli-"));
    const disableMcpConfig = process.env.GENOS_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
