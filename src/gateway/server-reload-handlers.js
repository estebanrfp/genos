  ...
  const applyHotReload = async (plan, nextConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (GENOS_SKIP_GMAIL_WATCHER=1)"),
      });
    }
    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.GENOS_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.GENOS_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (GENOS_SKIP_CHANNELS=1 or GENOS_SKIP_PROVIDERS=1)",
        );
      } else {
