let createLimiterSpy = function () {
  const check = vi.fn((_ip, _scope) => ({ allowed: true, remaining: 10, retryAfterMs: 0 }));
...
describe("gateway auth", () => {
  it("resolves token/password from GENOSOS gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          GENOS_GATEWAY_TOKEN: "env-token",
          GENOS_GATEWAY_PASSWORD: "env-password",
        },
      }),
    ).toMatchObject({
      mode: "password",
  ...
  it("does not resolve legacy GENOS_LEGACY gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          GENOS_GATEWAY_TOKEN: "legacy-token",
          GENOS_GATEWAY_PASSWORD: "legacy-password",
        },
      }),
    ).toMatchObject({
      mode: "token",
