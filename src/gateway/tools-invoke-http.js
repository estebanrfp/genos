let resolveSessionKeyFromBody = function (body) {
    if (typeof body.sessionKey === "string" && body.sessionKey.trim()) {
      return body.sessionKey.trim();
    }
    return;
  },
  resolveMemoryToolDisableReasons = function (cfg) {
  ...
  mergeActionIntoArgsIfSupported = function (params) {
  ...
  isToolInputError = function (err) {
    if (err instanceof ToolInputError) {
      return true;
    }
    return (
      typeof err === "object" && err !== null && "name" in err && err.name === "ToolInputError"
    );
  };
import { createGenosOSTools } from "../agents/genosos-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(req, res, opts) {
    ...
    if (reasons.length > 0) {
      ...
      sendJson(res, 400, {
        ...
        error: {
          ...
          message: `memory tools are disabled in tests${suffix}. Enable by setting plugins.slots.memory="memory-core" (and ensure plugins.enabled is not false).`,
        },
      });
      return true;
    }
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-genosos-message-channel") ?? "");
  const accountId = getHeader(req, "x-genosos-account-id")?.trim() || undefined;
  const {
    agentId,
    globalPolicy,
  ...
  const allTools = createGenosOSTools({
