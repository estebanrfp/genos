let expectChannels = function (call, channel) {
    expect(call.channel).toBe(channel);
    expect(call.messageChannel).toBe(channel);
  },
  readAgentCommandCall = function (fromEnd = 1) {
    const calls = vi.mocked(agentCommand).mock.calls;
    return calls.at(-fromEnd)?.[0] ?? {};
  },
  expectAgentRoutingCall = function (params) {
  ...
  sendAgentWsRequest = function (socket, params) {
    socket.send(
      JSON.stringify({
        type: "req",
        id: params.reqId,
...
async function sendAgentWsRequestAndWaitFinal(socket, params) {
...
describe("gateway server agent", () => {
  ...
  test("agent dedupe survives reconnect", { timeout: 60000 }, async () => {
