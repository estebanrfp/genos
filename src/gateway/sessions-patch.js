let invalid = function (message) {
    return { ok: false, error: errorShape(ErrorCodes.INVALID_REQUEST, message) };
  },
  normalizeExecHost = function (raw) {
  ...
  normalizeExecSecurity = function (raw) {
    const normalized = raw.trim().toLowerCase();
...
export async function applySessionsPatchToStore(params) {
  ...
  const next = existing
    ? {
        ...
        // Preserve original createdAt — never update it on patch
        createdAt: existing.createdAt ?? existing.updatedAt ?? now,
      }
    : { sessionId: randomUUID(), createdAt: now, updatedAt: now };
  ...
  if ("routingTier" in patch) {
    const raw = patch.routingTier;
    if (raw === null) {
      delete next.routingTier;
    } else if (raw !== undefined) {
      const trimmed = String(raw).trim().toLowerCase();
      if (trimmed !== "simple" && trimmed !== "normal" && trimmed !== "complex") {
        return invalid('invalid routingTier (use "simple"|"normal"|"complex")');
      }
      next.routingTier = trimmed;
    }
  }
  ...
  if ("model" in patch) {
    ...
    if (raw === null) {
      ...
      // When routing is active, always write override so routing respects explicit choice
      const routingEnabled = cfg.agents?.defaults?.model?.routing?.enabled === true;
      applyModelOverrideToSessionEntry({
        entry: next,
        selection: {
          provider: resolved.ref.provider,
          model: resolved.ref.model,
          isDefault: matchesDefault && !routingEnabled,
        },
      });
    }
  }
