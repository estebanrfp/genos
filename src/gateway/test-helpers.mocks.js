...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `genosos-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};
export const setTestConfigRoot = (root) => {
  testConfigRoot.value = root;
  process.env.GENOS_CONFIG_PATH = path.join(root, "genosos.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "genosos.json");
  const hashConfigRaw = (raw) =>
    crypto
      .createHash("sha256")
      .update(raw ?? "")
      .digest("hex");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "genosos-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadGenosOSPlugins: () => pluginRegistryState.registry,
  };
});
