...
describe("isTrustedProxyAddress", () => {
  ...
  describe("CIDR subnet matching", () => {
    ...
    it("handles mixed exact IPs and CIDR notation", () => {
      ...
      expect(isTrustedProxyAddress("192.168.1.1", proxies)).toBe(true);
      expect(isTrustedProxyAddress("10.42.0.59", proxies)).toBe(true);
      expect(isTrustedProxyAddress("172.19.5.100", proxies)).toBe(true);
      expect(isTrustedProxyAddress("10.43.0.1", proxies)).toBe(false);
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("returns false for invalid CIDR notation", () => {
      expect(isTrustedProxyAddress("10.42.0.59", ["10.42.0.0/33"])).toBe(false);
      expect(isTrustedProxyAddress("10.42.0.59", ["10.42.0.0/-1"])).toBe(false);
      expect(isTrustedProxyAddress("10.42.0.59", ["invalid/24"])).toBe(false);
    });
