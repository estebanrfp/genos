...
describe("shared/frontmatter", () => {
  ...
  test("resolveGenosOSManifestBlock parses JSON5 metadata and picks genosos block", () => {
    const frontmatter = {
      metadata: "{ genosos: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveGenosOSManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });
  test("resolveGenosOSManifestBlock returns undefined for invalid input", () => {
    expect(resolveGenosOSManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(resolveGenosOSManifestBlock({ frontmatter: { metadata: "not-json5" } })).toBeUndefined();
    expect(
      resolveGenosOSManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
