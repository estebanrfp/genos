...
async function requireRiskAcknowledgement(params) {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "GenosOS is a hobby project and still in beta. Expect sharp edges.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "If you\u2019re not comfortable with basic security and access control, don\u2019t run GenosOS.",
      ...
      "",
      "Must read: https://docs.genos.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
  ...
  await prompter.intro("GenosOS onboarding");
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("genosos doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }
  const quickstartHint = `Configure details later via ${formatCliCommand("genosos configure")}.`;
