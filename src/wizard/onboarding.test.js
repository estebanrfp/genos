let createWizardPrompter = function (overrides) {
    const select = vi.fn(async (_params) => "quickstart");
    return {
      intro: vi.fn(async () => {}),
      outro: vi.fn(async () => {}),
      note: vi.fn(async () => {}),
...
const runTui = vi.hoisted(() => vi.fn(async (_options) => {}));
...
describe("runOnboardingWizard", () => {
  ...
  let suiteCase = 0;
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-onboard-suite-"));
  });
  afterAll(async () => {
