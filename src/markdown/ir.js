let createMarkdownIt = function (options) {
  ...
  createTextToken = function (base, content) {
    return { ...base, type: "text", content, children: undefined };
  },
  applySpoilerTokens = function (tokens) {
  ...
  injectSpoilersIntoInline = function (tokens) {
    const result = [];
  ...
  resolveRenderTarget = function (state) {
    return state.table?.currentCell ?? state;
  },
  appendText = function (state, value) {
    if (!value) {
      return;
    }
  ...
  openStyle = function (state, style) {
    const target = resolveRenderTarget(state);
    target.openStyles.push({ style, start: target.text.length });
  },
  closeStyle = function (state, style) {
    const target = resolveRenderTarget(state);
  ...
  appendParagraphSeparator = function (state) {
  ...
  appendListPrefix = function (state) {
  ...
  renderInlineCode = function (state, content) {
    if (!content) {
      return;
    }
    const target = resolveRenderTarget(state);
  ...
  renderCodeBlock = function (state, content) {
    let code = content ?? "";
    if (!code.endsWith("\n")) {
      code = `${code}\n`;
    }
  ...
  handleLinkClose = function (state) {
    const target = resolveRenderTarget(state);
  ...
  appendCell = function (state, cell) {
    if (!cell.text) {
      return;
    }
  ...
  appendCellTextOnly = function (state, cell) {
    if (!cell.text) {
      return;
    }
    state.text += cell.text;
  },
  renderTableAsBullets = function (state) {
    if (!state.table) {
      return;
    }
  ...
  renderTableAsCode = function (state) {
    if (!state.table) {
      return;
    }
    const headers = state.table.headers.map(trimCell);
  ...
  renderTokens = function (tokens, state) {
    for (const token of tokens) {
      switch (token.type) {
        case "inline":
          if (token.children) {
...
`;
  ...
  closeRemainingStyles = function (target) {
    for (let i = target.openStyles.length - 1; i >= 0; i -= 1) {
  ...
  clampStyleSpans = function (spans, maxLength) {
    const clamped = [];
  ...
  clampLinkSpans = function (spans, maxLength) {
    const clamped = [];
    for (const span of spans) {
      const start = Math.max(0, Math.min(span.start, maxLength));
      const end = Math.max(start, Math.min(span.end, maxLength));
  ...
  mergeStyleSpans = function (spans) {
  ...
  sliceStyleSpans = function (spans, start, end) {
    if (spans.length === 0) {
      return [];
    }
    const sliced = [];
  ...
  sliceLinkSpans = function (spans, start, end) {
    if (spans.length === 0) {
      return [];
    }
    const sliced = [];
