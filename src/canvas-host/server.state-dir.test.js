...
describe("canvas host state dir defaults", () => {
  ...
  it("uses GENOS_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("GenosOS Canvas");
    } finally {
      await handler.close();
