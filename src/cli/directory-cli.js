let parseLimit = function (value) {
  ...
  buildRows = function (entries) {
    return entries.map((entry) => ({
      ID: entry.id,
      Name: entry.name?.trim() ?? "",
    }));
  },
  printDirectoryList = function (params) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genosos directory self --channel slack", "Show the connected account identity."],
          [
            'genosos directory peers list --channel slack --query "alice"',
            "Search contact/user IDs by name.",
          ],
          ["genosos directory groups list --channel discord", "List available groups/channels."],
          [
            "genosos directory groups members --channel discord --group-id <id>",
            "List members for a specific group.",
          ],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/directory", "docs.genos.ai/cli/directory")}\n`,
    )
    .action(() => {
