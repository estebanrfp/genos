let createRunMessageAction = function () {
    const fakeCommand = { help: vi.fn() };
    return createMessageCliHelpers(fakeCommand, "discord").runMessageAction;
  },
  expectNoAccountFieldInPassedOptions = function () {
    const passedOpts = messageCommandMock.mock.calls?.[0]?.[0];
...
const hasHooksMock = vi.fn((_hookName) => false);
const runGatewayStopMock = vi.fn(async (_event, _ctx) => {});
const runGlobalGatewayStopSafelyMock = vi.fn(async (params) => {
...
async function runSendAction(opts = {}) {
...
describe("runMessageAction", () => {
  beforeEach(() => {
    ...
    exitMock.mockReset().mockImplementation(() => {
      throw new Error("exit");
    });
  });
