...
${formatHelpExamples([
  ['genosos agent --to +15555550123 --message "status update"', "Start a new session."],
  ['genosos agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'genosos agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'genosos agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['genosos agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'genosos agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.genos.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.genos.ai/cli/agents")}\n`,
    );
  agents
    .command("list")
...
${formatHelpExamples([
  [
    'genosos agents set-identity --agent main --name "GenosOS" --emoji "\uD83E\uDD9E"',
    "Set name + emoji.",
  ],
  ["genosos agents set-identity --agent main --avatar avatars/genosos.png", "Set avatar path."],
  [
    "genosos agents set-identity --workspace ~/.genosv1/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "genosos agents set-identity --identity-file ~/.genosv1/workspace/IDENTITY.md --agent main",
