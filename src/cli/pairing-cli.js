let parseChannel = function (raw, channels) {
  const value = (
    typeof raw === "string"
      ? raw
...
export function registerPairingCli(program) {
  ...
  const pairing = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/pairing", "docs.genos.ai/cli/pairing")}\n`,
    );
  pairing
    .command("list")
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!opts.channel && !code) {
        throw new Error(
          `Usage: ${formatCliCommand("genosos pairing approve <channel> <code>")} (or: ${formatCliCommand("genosos pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("genosos pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
      const accountId = String(opts.account ?? "").trim();
