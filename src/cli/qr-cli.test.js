let createRemoteQrConfig = function (params) {
  return {
    gateway: {
...
const qrGenerate = vi.fn((_input, _opts, cb) => {
  cb("ASCII-QR");
});
...
describe("registerQrCli", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("GENOS_GATEWAY_TOKEN", "");
    vi.stubEnv("GENOS_GATEWAY_TOKEN", "");
    vi.stubEnv("GENOS_GATEWAY_PASSWORD", "");
    vi.stubEnv("GENOS_GATEWAY_PASSWORD", "");
  });
  afterEach(() => {
    vi.unstubAllEnvs();
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("genosos devices approve <requestId>");
  });
