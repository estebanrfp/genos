...
const updateCommand = vi.fn(async (_opts) => {});
const updateStatusCommand = vi.fn(async (_opts) => {});
const updateWizardCommand = vi.fn(async (_opts) => {});
...
vi.mock("./update-cli/update-command.js", () => ({
  updateCommand: (opts) => updateCommand(opts),
}));
vi.mock("./update-cli/status.js", () => ({
  updateStatusCommand: (opts) => updateStatusCommand(opts),
}));
vi.mock("./update-cli/wizard.js", () => ({
  updateWizardCommand: (opts) => updateWizardCommand(opts),
}));
vi.mock("../runtime.js", () => ({
  defaultRuntime,
}));
