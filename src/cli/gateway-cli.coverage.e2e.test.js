...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints human output with details on new lines", async () => {
    ...
    expect(out).toContain("- Studio genosos.internal.");
    expect(out).toContain("  tailnet: studio.tailnet.ts.net");
    expect(out).toContain("  host: studio.genos.internal");
    expect(out).toContain("  ws: ws://studio.genos.internal:18789");
  });
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ GENOS_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
