let shouldReportPortUsage = function (status, rpcOk) {
  if (status !== "busy") {
    return false;
  }
  if (rpcOk === true) {
...
export async function gatherDaemonStatus(opts) {
  ...
  const rpc = opts.probe
    ? await probeGatewayStatus({
        url: probeUrl,
        token:
          opts.rpc.token || mergedDaemonEnv.GENOS_GATEWAY_TOKEN || daemonCfg.gateway?.auth?.token,
        password:
          opts.rpc.password ||
          mergedDaemonEnv.GENOS_GATEWAY_PASSWORD ||
