let sanitizeDaemonStatusForJson = function (status) {
...
export function printDaemonStatus(status, opts) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("genosos doctor")}" (or "${formatCliCommand("genosos doctor --repair")}").`,
      ),
    );
  }
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("genosos gateway install --force")}\` from the same --profile / GENOS_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.GENOS_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("genosos gateway install")}`),
    );
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("genosos status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.genos.ai/troubleshooting`);
}
