...
vi.mock("./update-cli/restart-helper.js", () => ({
  prepareRestartScript: (...args) => prepareRestartScript(...args),
  runRestartScript: (...args) => runRestartScript(...args),
}));
...
const { resolveGenosOSPackageRoot } = await import("../infra/genosos-root.js");
...
describe("update-cli", () => {
  let fixtureRoot = "";
  let fixtureCount = 0;
  const createCaseDir = async (prefix) => {
    const dir = path.join(fixtureRoot, `${prefix}-${fixtureCount++}`);
    return dir;
  };
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-update-tests-"));
  });
  afterAll(async () => {
  ...
  const mockPackageInstallStatus = (root) => {
    vi.mocked(resolveGenosOSPackageRoot).mockResolvedValue(root);
  ...
  const expectUpdateCallChannel = (channel) => {
    const call = vi.mocked(runGatewayUpdate).mock.calls[0]?.[0];
    expect(call?.channel).toBe(channel);
    return call;
  };
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("genosos-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveGenosOSPackageRoot).mockReset();
    ...
    vi.mocked(resolveGenosOSPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("genosos");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    prepareRestartScript.mockResolvedValue("/tmp/genosos-restart-test.sh");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("GenosOS update status");
  });
  ...
  it("honors --tag override", async () => {
    const tempDir = await createCaseDir("genosos-update");
    vi.mocked(resolveGenosOSPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "npm",
      steps: [],
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["GENOS_UPDATE_IN_PROGRESS"]);
    const randomSpy = vi.spyOn(Math, "random").mockReturnValue(0);
    try {
      delete process.env.GENOS_UPDATE_IN_PROGRESS;
      vi.mocked(runGatewayUpdate).mockResolvedValue(mockResult);
      vi.mocked(runDaemonRestart).mockResolvedValue(true);
      vi.mocked(defaultRuntime.log).mockClear();
      await updateCommand({});
  ...
  it("persists update channel when --channel is set", async () => {
    ...
    const call = vi.mocked(writeConfigFile).mock.calls[0]?.[0];
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("genosos-update-wizard");
    const envSnapshot = captureEnv(["GENOS_GIT_DIR"]);
    try {
      setTty(true);
      process.env.GENOS_GIT_DIR = tempDir;
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: "/test/path",
        installKind: "package",
        packageManager: "npm",
