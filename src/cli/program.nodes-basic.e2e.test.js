let formatRuntimeLogCallArg = function (value) {
...
describe("cli program (nodes basics)", () => {
  function mockGatewayWithIosNodeListAnd(method, result) {
    callGateway.mockImplementation(async (...args) => {
      const opts = args[0] ?? {};
  ...
  it("runs nodes list and calls node.pair.list", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith("Pending: 0 \xB7 Paired: 0");
  });
      ...
      if (opts.method === "node.pair.list") {
        return {
          ...
          paired: [
            ...
            { nodeId: "n2", lastConnectedAtMs: now - 172800000 },
          ],
        };
      }
      return { ok: true };
  ...
  it("runs nodes status and calls node.list", async () => {
    ...
    const output = runtime.log.mock.calls.map((c) => formatRuntimeLogCallArg(c[0])).join("\n");
    expect(output).toContain("Known: 1 \xB7 Paired: 1 \xB7 Connected: 1");
  ...
  it("runs nodes status and shows unpaired nodes", async () => {
    ...
    await program.parseAsync(["nodes", "status"], { from: "user" });
    const output = runtime.log.mock.calls.map((c) => formatRuntimeLogCallArg(c[0])).join("\n");
    expect(output).toContain("Known: 1 \xB7 Paired: 0 \xB7 Connected: 1");
