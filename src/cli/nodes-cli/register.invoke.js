let normalizeExecSecurity = function (value) {
  ...
  normalizeExecAsk = function (value) {
    const normalized = value?.trim().toLowerCase();
  ...
  resolveExecDefaults = function (cfg, agentId) {
      ...
      .action(async (command, opts) => {
        await runNodesCommand("run", async () => {
          ...
          if (approvedByAsk && approvalId) {
            invokeParams.params.runId = approvalId;
          }
          ...
          const payload =
            typeof result === "object" && result !== null ? result.payload : undefined;
