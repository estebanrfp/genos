    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genosos nodes status", "List known nodes with live status."],
          ["genosos nodes pairing pending", "Show pending node pairing requests."],
          ['genosos nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["genosos nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.genos.ai/cli/nodes")}\n`,
    );
