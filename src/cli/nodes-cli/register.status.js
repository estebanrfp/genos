let formatVersionLabel = function (raw) {
  ...
  resolveNodeVersions = function (node) {
  ...
  formatNodeVersions = function (node) {
  ...
  parseSinceMs = function (raw, label) {
    ...
    nodes
      ...
      .action(async (opts) => {
        await runNodesCommand("status", async () => {
          ...
          defaultRuntime.log(
            `Known: ${filtered.length}${filteredLabel} \xB7 Paired: ${pairedCount} \xB7 Connected: ${connectedCount}`,
          );
          ...
          const rows = filtered.map((n) => {
            ...
            return {
              ...
              Status: `${paired} \xB7 ${connected}${since}`,
              Caps: caps,
            };
          });
          defaultRuntime.log(
    ...
    nodes
      ...
      .action(async (opts) => {
        await runNodesCommand("describe", async () => {
          ...
          const status = `${paired ? ok("paired") : warn("unpaired")} \xB7 ${connected ? ok("connected") : muted("disconnected")}`;
          const tableWidth = Math.max(60, (process.stdout.columns ?? 120) - 1);
          const rows = [
    ...
    nodes
      ...
      .action(async (opts) => {
        await runNodesCommand("list", async () => {
          ...
          defaultRuntime.log(
            `Pending: ${pendingRows.length} \xB7 Paired: ${filteredPaired.length}${filteredLabel}`,
          );
          if (opts.json) {
            defaultRuntime.log(
