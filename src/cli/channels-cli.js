let runChannelsCommand = function (action) {
    return runCommandWithRuntime(defaultRuntime, action);
  },
  runChannelsCommandWithDanger = function (action, label) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genosos channels list", "List configured channels and auth profiles."],
          ["genosos channels status --probe", "Run channel status checks and probes."],
          [
            "genosos channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["genosos channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/channels", "docs.genos.ai/cli/channels")}\n`,
    );
  channels
    .command("list")
