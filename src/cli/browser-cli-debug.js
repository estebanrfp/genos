let runBrowserDebug = function (action) {
    return runCommandWithRuntime(defaultRuntime, action, (err) => {
      defaultRuntime.error(danger(String(err)));
      defaultRuntime.exit(1);
    });
  },
  resolveDebugQuery = function (params) {
    return {
      targetId:
        typeof params.targetId === "string" ? params.targetId.trim() || undefined : undefined,
...
export function registerBrowserDebugCommands(browser, parentOpts) {
  ...
  trace
    ...
    .option(
      "--out <path>",
      "Output path within genosos temp dir (e.g. trace.zip or /tmp/genosos/trace.zip)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .action(async (opts, cmd) => {
      const parent = parentOpts(cmd);
