let parsePositiveInt = function (value, fallback) {
    if (!value) {
      return fallback;
    }
  ...
  formatLogLine = function (raw, opts) {
  ...
  emitGatewayError = function (err, opts, mode, rich, emitJsonLine, errorLine) {
    const details = buildGatewayConnectionDetails({ url: opts.url });
    const message = "Gateway not reachable. Is it running and accessible?";
    const hint = `Hint: run \`${formatCliCommand("genosos doctor")}\`.`;
...
async function fetchLogs(opts, cursor, showProgress) {
  ...
  const maxBytes = parsePositiveInt(opts.maxBytes, 250000);
...
export function formatLogTimestamp(value, mode = "plain", localTime = false) {
  if (!value) {
    return "";
  }
...
export function registerLogsCli(program) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.genos.ai/cli/logs")}\n`,
    );
