let renderQrAscii = function (data) {
  ...
  readDevicePairPublicUrlFromConfig = function (cfg) {
    ...
    .action(async (opts) => {
      try {
        ...
        const lines = [
          ...
          "Scan this with the GenosOS iOS app (Onboarding -> Scan QR).",
          "",
        ];
        if (opts.ascii !== false) {
        ...
        lines.push(
          ...
          `  ${theme.command("genosos devices list")}`,
          `  ${theme.command("genosos devices approve <requestId>")}`,
        );
