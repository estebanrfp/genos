...
const runAcpClientInteractive = vi.fn(async (_opts) => {});
const serveAcpGateway = vi.fn(async (_opts) => {});
const defaultRuntime = {
  error: vi.fn(),
  exit: vi.fn(),
};
vi.mock("../acp/client.js", () => ({
  runAcpClientInteractive: (opts) => runAcpClientInteractive(opts),
}));
vi.mock("../acp/server.js", () => ({
  serveAcpGateway: (opts) => serveAcpGateway(opts),
}));
vi.mock("../runtime.js", () => ({
  defaultRuntime,
}));
