let resolveFileNpmSpecToLocalPath = function (raw) {
  ...
  formatPluginLine = function (plugin, verbose = false) {
    const status =
  ...
  applySlotSelectionForPlugin = function (config, pluginId) {
  ...
  createPluginInstallLogger = function () {
    return {
      info: (msg) => defaultRuntime.log(msg),
      warn: (msg) => defaultRuntime.log(theme.warn(msg)),
    };
  },
  enablePluginInConfig = function (config, pluginId) {
    return {
      ...config,
      plugins: {
        ...config.plugins,
  ...
  logSlotWarnings = function (warnings) {
...
export function registerPluginsCli(program) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.genos.ai/cli/plugins")}\n`,
    );
  plugins
    .command("list")
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.genos.ai/plugin");
      lines.push("");
