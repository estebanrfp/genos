...
describe("waitForever", () => {
  it("creates an unref'ed interval and returns a pending promise", () => {
    ...
    expect(setIntervalSpy).toHaveBeenCalledWith(expect.any(Function), 1e6);
...
describe("shouldSkipRespawnForArgv", () => {
  it("skips respawn for help/version calls", () => {
    expect(shouldSkipRespawnForArgv(["node", "genosos", "--help"])).toBe(true);
    expect(shouldSkipRespawnForArgv(["node", "genosos", "-V"])).toBe(true);
  });
  it("keeps respawn path for normal commands", () => {
    expect(shouldSkipRespawnForArgv(["node", "genosos", "status"])).toBe(false);
  });
});
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "genosos.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("genosos.internal");
    } finally {
      log.mockRestore();
    }
  });
...
describe("parseByteSize", () => {
  it("parses bytes with units", () => {
    ...
    expect(parseByteSize("2gb")).toBe(2147483648);
  });
  it("parses shorthand units", () => {
    expect(parseByteSize("5k")).toBe(5120);
...
describe("parseDurationMs", () => {
  ...
  it("parses days suffix", () => {
    expect(parseDurationMs("2d")).toBe(172800000);
  });
