let normalizeUploadPaths = function (paths) {
...
export function registerBrowserFilesAndDownloadsCommands(browser, parentOpts) {
  ...
  browser
    ...
    .argument(
      "<paths...>",
      "File paths to upload (must be within GenosOS temp uploads dir, e.g. /tmp/genosos/uploads/file.pdf)",
    )
  ...
  browser
    ...
    .argument(
      "[path]",
      "Save path within genosos temp downloads dir (default: /tmp/genosos/downloads/...; fallback: os.tmpdir()/genosos/downloads/...)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
  ...
  browser
    ...
    .argument(
      "<path>",
      "Save path within genosos temp downloads dir (e.g. report.pdf or /tmp/genosos/downloads/report.pdf)",
    )
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",
