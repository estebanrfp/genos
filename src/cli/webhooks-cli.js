let parseGmailSetupOptions = function (raw) {
  ...
  parseGmailRunOptions = function (raw) {
    const common = parseGmailCommonOptions(raw);
    return {
  ...
  parseGmailCommonOptions = function (raw) {
    return {
  ...
  gmailOptionsFromCommon = function (common) {
    return {
      topic: common.topic,
...
export function registerWebhooksCli(program) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.genos.ai/cli/webhooks")}\n`,
    );
  const gmail = webhooks.command("gmail").description("Gmail Pub/Sub hooks (via gogcli)");
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + GenosOS hooks")
    ...
    .option("--hook-url <url>", "GenosOS hook URL")
    .option("--hook-token <token>", "GenosOS hook token")
