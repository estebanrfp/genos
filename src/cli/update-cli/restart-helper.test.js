...
describe("restart-helper", () => {
  ...
  async function prepareAndReadScript(env) {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'genosos-gateway.service'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });
    it("uses GENOS_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        GENOS_PROFILE: "default",
        GENOS_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.genos.gateway'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });
    it("uses GENOS_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      const { scriptPath, content } = await prepareAndReadScript({
        GENOS_PROFILE: "default",
        GENOS_LAUNCHD_LABEL: "com.custom.genos",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.genos'");
      await cleanupScript(scriptPath);
    });
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "GenosOS Gateway"');
      expect(content).toContain('schtasks /Run /TN "GenosOS Gateway"');
      expect(content).toContain('del "%~f0"');
      await cleanupScript(scriptPath);
    });
    it("uses GENOS_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const { scriptPath, content } = await prepareAndReadScript({
        GENOS_PROFILE: "default",
        GENOS_WINDOWS_TASK_NAME: "GenosOS Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "GenosOS Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "GenosOS Gateway (custom)"');
      await cleanupScript(scriptPath);
    });
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        GENOS_PROFILE: "production",
      });
      expect(content).toContain("genosos-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    it("uses custom profile in macOS launchd label", async () => {
      ...
      process.getuid = () => 502;
      const { scriptPath, content } = await prepareAndReadScript({
        GENOS_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.genos.staging");
      await cleanupScript(scriptPath);
    });
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "GenosOS Gateway (production)"');
      await cleanupScript(scriptPath);
    });
