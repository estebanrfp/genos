let getStepLabel = function (step) {
    return STEP_LABELS[step.name] ?? step.name;
  },
  formatStepStatus = function (exitCode) {
    if (exitCode === 0) {
...
const STEP_LABELS = {
  ...
  "genosos doctor entry": "Checking doctor entrypoint",
  "genosos doctor": "Running doctor checks",
  "git rev-parse HEAD (after)": "Verifying update",
  "global update": "Updating via package manager",
  "global install": "Installing global package",
};
export function createUpdateProgress(enabled) {
  if (!enabled) {
    return {
      progress: {},
      stop: () => {},
