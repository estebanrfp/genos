...
import { checkShellCompletionStatus, ensureCompletionCacheExists } from "../completion-check.js";
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("genosos completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function runGitUpdate(params) {
  ...
  const effectiveTimeout = params.timeoutMs ?? 1200000;
...
async function maybeRestartService(params) {
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        try {
          ...
          if (report.summary.critical > 0 || report.summary.warnings > 0) {
            defaultRuntime.log(
              theme.warn(
                `Doctor: ${report.summary.critical} critical, ${report.summary.warnings} warnings, ${report.summary.fixed} auto-fixed`,
              ),
            );
          } else {
            defaultRuntime.log(
              theme.muted(`Doctor: all checks passed, ${report.summary.fixed} auto-fixed`),
            );
          }
        } catch (err) {
      ...
      if (!params.opts.json && restartInitiated) {
        ...
        defaultRuntime.log(
          theme.muted(
            `Verify with \`${replaceCliName(formatCliCommand("genosos gateway status"), CLI_NAME)}\` once the gateway is back.`,
          ),
        );
        defaultRuntime.log("");
      }
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("genosos gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("genosos doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("genosos gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("genosos gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateCommand(opts) {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating GenosOS..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this GenosOS install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("genosos doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("genosos gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g genosos@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g genosos@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
