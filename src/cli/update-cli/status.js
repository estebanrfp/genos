let formatGitStatusLine = function (params) {
...
export async function updateStatusCommand(opts) {
  ...
  const rows = [
    ...
    {
      Item: "Update",
      Value: updateAvailability.available ? theme.warn(`available \xB7 ${updateLine}`) : updateLine,
    },
  ];
  defaultRuntime.log(theme.heading("GenosOS update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
