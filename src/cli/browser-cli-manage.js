let runBrowserCommand = function (action) {
    return runCommandWithRuntime(defaultRuntime, action, (err) => {
      defaultRuntime.error(danger(String(err)));
      defaultRuntime.exit(1);
    });
  },
  logBrowserTabs = function (tabs, json) {
    if (json) {
...
async function runBrowserToggle(parent, params) {
  ...
  const name = status.profile ?? "genosos";
...
export function registerBrowserManageCommands(browser, parentOpts) {
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "genosos"}`,
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (genosos|extension). Default: genosos")
