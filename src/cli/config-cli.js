let isIndexSegment = function (raw) {
    return /^[0-9]+$/.test(raw);
  },
  ...
  setAtPath = function (root, path, value) {
    let current = root;
  ...
  unsetAtPath = function (root, path) {
    let current = root;
    for (let i = 0; i < path.length - 1; i += 1) {
      const segment = path[i];
      if (!current || typeof current !== "object") {
  ...
  parseRequiredPath = function (path) {
...
async function loadValidConfig(runtime = defaultRuntime) {
  ...
  runtime.error(`Run \`${formatCliCommand("genosos doctor")}\` to repair, then retry.`);
  runtime.exit(1);
  return snapshot;
}
...
export function registerConfigCli(program) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.genos.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
