let parsePortWithFallback = function (value, fallback) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "genosos node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["genosos node status", "Check node host service status."],
          ["genosos node install", "Install the node host service."],
          ["genosos node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.genos.ai/cli/node")}\n`,
    );
  node
    .command("run")
