let renderNodeServiceStartHints = function () {
    const base = [formatCliCommand("genosos node install"), formatCliCommand("genosos node start")];
  ...
  buildNodeRuntimeHints = function (env = process.env) {
  ...
  resolveNodeDefaults = function (opts, config) {
...
export async function runNodeDaemonInstall(opts) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("genosos node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    GENOS_LOG_PREFIX: baseEnv.GENOS_LOG_PREFIX ?? "node",
  };
