let run = function (cmd, args, opts) {
  ...
  writeFileSudoIfNeeded = function (filePath, content) {
    try {
  ...
  mkdirSudoIfNeeded = function (dirPath) {
    try {
  ...
  zoneFileNeedsBootstrap = function (zonePath) {
  ...
  ensureImportLine = function (corefilePath, importGlob) {
...
export function registerDnsCli(program) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.genos.ai/cli/dns")}\n`,
    );
  dns
    .command("setup")
    .description(
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. genosos.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.genosv1/genosos.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by genosos dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.genosv1/genosos.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
