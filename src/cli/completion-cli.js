let isCompletionShell = function (value) {
    return COMPLETION_SHELLS.includes(value);
  },
  sanitizeCompletionBasename = function (value) {
  ...
  resolveCompletionCacheDir = function (env = process.env) {
    const stateDir = resolveStateDir(env, os.homedir);
    return path.join(stateDir, "completions");
  },
  getCompletionScript = function (shell, program) {
    if (shell === "zsh") {
  ...
  formatCompletionSourceLine = function (shell, binName, cachePath) {
    if (shell === "fish") {
      return `source "${cachePath}"`;
    }
    return `source "${cachePath}"`;
  },
  isCompletionProfileHeader = function (line) {
    return line.trim() === "# GenosOS Completion";
  },
  isCompletionProfileLine = function (line, binName, cachePath) {
  ...
  isSlowDynamicCompletionLine = function (line, binName) {
    return (
      line.includes(`<(${binName} completion`) ||
      (line.includes(`${binName} completion`) && line.includes("| source"))
    );
  },
  updateCompletionProfile = function (content, binName, cachePath, sourceLine) {
    ...
    const block = `# GenosOS Completion\n${sourceLine}`;
    const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
    return { next, changed: next !== content, hadExisting };
  },
  getShellProfilePath = function (shell) {
  ...
  generateZshCompletion = function (program) {
...
`;
  ...
  generateZshSubcmdList = function (cmd) {
  ...
  generateZshSubcommands = function (program, prefix) {
    let script = "";
...
`;
  ...
  generateBashCompletion = function (program) {
    const rootCmd = program.name();
    return `
...
`;
  ...
  generateBashSubcommand = function (cmd) {
    return `${cmd.name()})
  ...
  generatePowerShellCompletion = function (program) {
    const rootCmd = program.name();
...
`;
  ...
  generateFishCompletion = function (program) {
    const rootCmd = program.name();
    let script = "";
    const visit = (cmd, parents) => {
      const cmdName = cmd.name();
...
export async function completionCacheExists(shell, binName = "genosos") {
...
export async function isCompletionInstalled(shell, binName = "genosos") {
...
export async function usesSlowDynamicCompletion(shell, binName = "genosos") {
  const profilePath = getShellProfilePath(shell);
  if (!(await pathExists(profilePath))) {
    return false;
  }
...
export function registerCompletionCli(program) {
  program
    ...
    .option("--write-state", "Write completion scripts to $GENOS_STATE_DIR/completions (no stdout)")
...
export async function installCompletion(shell, yes, binName = "genosos") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
