let appendClawHubHint = function (output, json) {
    if (json) {
      return output;
    }
    return `${output}\n\nTip: use \`npx clawhub\` to search, install, and sync skills.`;
  },
  formatSkillStatus = function (skill) {
    if (skill.eligible) {
  ...
  formatSkillName = function (skill) {
    const emoji = skill.emoji ?? "\uD83D\uDCE6";
    return `${emoji} ${theme.command(skill.name)}`;
  },
  formatSkillMissingSummary = function (skill) {
    const missing = [];
...
export function formatSkillsList(report, opts) {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("genosos skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(report, skillName, opts) {
  ...
  if (!skill) {
    ...
    return appendClawHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("genosos skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  if (opts.json) {
