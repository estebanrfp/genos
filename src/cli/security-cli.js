let formatSummary = function (summary) {
  const rich = isRich();
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genosos security audit", "Run a local security audit."],
          ["genosos security audit --deep", "Include best-effort live Gateway probe checks."],
          ["genosos security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["genosos security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.genos.ai/cli/security")}\n`,
    );
  security
    .command("audit")
    ...
    .action(async (opts) => {
      ...
      const lines = [];
      lines.push(heading("GenosOS security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("genosos security audit --deep")}`));
      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("genosos security audit --fix")}`));
        if (!fixResult) {
