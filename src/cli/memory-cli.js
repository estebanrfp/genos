let formatSourceLabel = function (source, workspaceDir, agentId) {
    if (source === "memory") {
  ...
  resolveAgent = function (cfg, agent) {
  ...
  resolveAgentIds = function (cfg, agent) {
    const trimmed = agent?.trim();
    if (trimmed) {
      return [trimmed];
    }
  ...
  formatExtraPaths = function (workspaceDir, extraPaths) {
...
import { registerMemoryEncryptCli } from "./memory-encrypt-cli.js";
...
export async function runMemoryStatus(opts) {
  ...
  for (const result of allResults) {
    ...
    const indexedLabel =
      totalFiles === null
        ? `${filesIndexed}/? files \xB7 ${chunksIndexed} chunks`
        : `${filesIndexed}/${totalFiles} files \xB7 ${chunksIndexed} chunks`;
    if (opts.index) {
    ...
    if (status.sourceCounts?.length) {
      ...
      for (const entry of status.sourceCounts) {
        ...
        const counts =
          total === null
            ? `${entry.files}/? files \xB7 ${entry.chunks} chunks`
            : `${entry.files}/${total} files \xB7 ${entry.chunks} chunks`;
...
export function registerMemoryCli(program) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["genosos memory status", "Show index and provider status."],
          ["genosos memory index --force", "Force a full reindex."],
          ['genosos memory search --query "deployment notes"', "Search indexed memory entries."],
          ["genosos memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.genos.ai/cli/memory")}\n`,
    );
  memory
    .command("status")
          ...
          run: async (manager) => {
            try {
              ...
              const buildLabel = () => {
                ...
                const eta = formatEta();
                return eta
                  ? `${lastLabel} \xB7 elapsed ${elapsed} \xB7 eta ${eta}`
                  : `${lastLabel} \xB7 elapsed ${elapsed}`;
              };
              if (!syncFn) {
                defaultRuntime.log("Memory backend does not support manual reindex.");
                return;
  ...
  // Memory encryption subcommands
  registerMemoryEncryptCli(memory);
}
