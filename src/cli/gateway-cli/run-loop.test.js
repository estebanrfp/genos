let removeNewSignalListeners = function (signal, existing) {
...
const waitForActiveTasks = vi.fn(async (_timeoutMs) => ({ drained: true }));
...
vi.mock("../../process/command-queue.js", () => ({
  ...
  waitForActiveTasks: (timeoutMs) => waitForActiveTasks(timeoutMs),
...
describe("runGatewayLoop", () => {
  it("restarts after SIGUSR1 even when drain times out, and resets lanes for the new iteration", async () => {
    ...
    const startedFirst = new Promise((resolve) => {
    ...
    const startedSecond = new Promise((resolve) => {
    ...
    const beforeSigusr1 = new Set(process.listeners("SIGUSR1"));
    ...
    try {
      ...
      await new Promise((resolve) => setImmediate(resolve));
      expect(waitForActiveTasks).toHaveBeenCalledWith(30000);
