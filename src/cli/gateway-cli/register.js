let runGatewayCommand = function (action, label) {
  ...
  parseDaysOption = function (raw, fallback = 30) {
  ...
  resolveGatewayRpcOptions = function (opts, command) {
  ...
  renderCostUsageSummary = function (summary, days, rich) {
    const totalCost = formatUsd(summary.totals.totalCost) ?? "$0.00";
    const totalTokens = formatTokenCount(summary.totals.totalTokens) ?? "0";
    const lines = [
      colorize(rich, theme.heading, `Usage cost (${days} days)`),
      `${colorize(rich, theme.muted, "Total:")} ${totalCost} \xB7 ${totalTokens} tokens`,
    ];
    ...
    if (latest) {
      ...
      lines.push(
        `${colorize(rich, theme.muted, "Latest day:")} ${latest.date} \xB7 ${latestCost} \xB7 ${latestTokens} tokens`,
      );
    }
    return lines;
  };
      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["genosos gateway run", "Run the gateway in the foreground."],
            ["genosos gateway status", "Show service status and probe reachability."],
            ["genosos gateway discover", "Find local and wide-area gateway beacons."],
            ["genosos gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.genos.ai/cli/gateway")}\n`,
      ),
  );
      ...
      await runGatewayCommand(async () => {
        ...
        defaultRuntime.log(
          colorize(
            rich,
            theme.muted,
            `Found ${deduped.length} gateway(s) \xB7 domains: ${domains.join(", ")}`,
          ),
        );
        if (deduped.length === 0) {
          return;
