let createMockSessionContent = function (entries) {
    return entries
      .map((entry) => {
        if ("role" in entry) {
  ...
  makeSessionMemoryConfig = function (tempDir, messages) {
    return {
...
async function runNewWithPreviousSession(params) {
  const tempDir = await makeTempWorkspace("genosos-session-memory-");
