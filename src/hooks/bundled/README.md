...
This directory contains hooks that ship with GenosOS. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.genosv1/genosos.json`:
...
2. Restart gateway: `pkill -9 -f 'genosos.*gateway' && pnpm genosos gateway`
3. Enable the hook: `genosos hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.genos.ai/automation/hooks
