...
homepage: https://docs.genos.ai/automation/hooks#command-logger
metadata:
  {
    "genosos":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.genosv1/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.genosv1/logs/commands.log ~/.genosv1/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/genosos`:
   ```
   /home/username/.genos/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
