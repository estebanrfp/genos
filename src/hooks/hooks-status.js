let resolveHookKey = function (entry) {
    return entry.metadata?.hookKey ?? entry.hook.name;
  },
  normalizeInstallOptions = function (entry) {
  ...
  buildHookStatus = function (entry, config, eligibility) {
    const hookKey = resolveHookKey(entry);
    const hookConfig = resolveHookConfig(config, hookKey);
    const managedByPlugin = entry.hook.source === "genosos-plugin";
