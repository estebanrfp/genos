let mockConfig = function (storePath, overrides) {
  ...
  mockGatewaySuccessReply = function (text = "hello") {
  ...
  mockLocalAgentReply = function (text = "local") {
...
async function withTempStore(fn, overrides) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-agent-cli-"));
...
describe("agentCliCommand", () => {
  it("uses a timer-safe max gateway timeout when --timeout is 0", async () => {
    await withTempStore(async () => {
      ...
      const request = vi.mocked(callGateway).mock.calls[0]?.[0];
      expect(request.timeoutMs).toBe(2147000000);
    });
  });
