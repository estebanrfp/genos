let createLegacyProviderConfig = function (params) {
    return {
      models: {
        providers: {
          [params.providerId]: {
  ...
  createConfigWithFallbacks = function () {
    return {
      agents: {
        defaults: {
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under GENOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("genosos-oauth-");
    ...
    const parsed = await readAuthProfilesForAgent(env.agentDir);
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to GENOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("genosos-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
