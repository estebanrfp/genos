...
const debugHealth = (...args) => {
  if (isTruthyEnvValue(process.env.GENOS_DEBUG_HEALTH)) {
...
const formatDurationParts = (ms) => {
  ...
  const units = [
    { label: "w", size: 604800000 },
    { label: "d", size: 86400000 },
    { label: "h", size: 3600000 },
    { label: "m", size: 60000 },
    { label: "s", size: 1000 },
  ];
...
export async function healthCommand(opts, runtime) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.GENOS_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
