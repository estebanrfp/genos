let parseIntOrNull = function (value) {
    const s =
      typeof value === "string"
        ? value.trim()
...
export function resolveAuthForTarget(cfg, target, overrides) {
  ...
  const envToken = process.env.GENOS_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.GENOS_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
