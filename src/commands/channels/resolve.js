let resolvePreferredKind = function (kind) {
    if (!kind || kind === "auto") {
      return;
    }
    if (kind === "user") {
  ...
  detectAutoKind = function (input) {
    ...
    if (/^(user|discord|slack|matrix|msteams|teams|googlechat|google-chat|gchat):/i.test(trimmed)) {
      return "user";
    }
    return "group";
  },
  formatResolveResult = function (result) {
