let appendEnabledConfiguredLinkedBits = function (bits, account) {
  ...
  appendModeBit = function (bits, account) {
    if (typeof account.mode === "string" && account.mode.length > 0) {
      bits.push(`mode:${account.mode}`);
    }
  },
  appendTokenSourceBits = function (bits, account) {
  ...
  appendBaseUrlBit = function (bits, account) {
    if (typeof account.baseUrl === "string" && account.baseUrl) {
      bits.push(`url:${account.baseUrl}`);
    }
  },
  buildChannelAccountLine = function (provider, account, bits) {
...
export function formatGatewayChannelsStatusLines(payload) {
  ...
  if (issues.length > 0) {
    ...
    lines.push(`- Run: ${formatCliCommand("genosos doctor")}`);
    lines.push("");
  }
  lines.push(
