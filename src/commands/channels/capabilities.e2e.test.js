let resetOutput = function () {
    logs.length = 0;
    errors.length = 0;
  },
  buildPlugin = function (params) {
...
vi.mock("./shared.js", () => ({
  ...
  formatChannelAccountLabel: vi.fn(({ channel, accountId }) => `${channel}:${accountId}`),
}));
...
describe("channelsCapabilitiesCommand", () => {
  ...
  it("prints Slack bot + user scopes when user token is configured", async () => {
    const plugin = buildPlugin({
      ...
      probe: { ok: true, bot: { name: "genosos" }, team: { name: "team" } },
    });
