let normalizeTimeout = function (raw, fallback = 1e4) {
  ...
  formatSupport = function (capabilities) {
    if (!capabilities) {
      return "unknown";
    }
    const bits = [];
  ...
  summarizeDiscordTarget = function (raw) {
    if (!raw) {
      return;
    }
  ...
  formatDiscordIntents = function (intents) {
    if (!intents) {
      return "unknown";
    }
    return [
  ...
  formatProbeLines = function (channelId, probe) {
    const lines = [];
...
async function resolveChannelReports(params) {
  ...
  const actions = Array.from(
    new Set(["send", "broadcast", ...listedActions.map((action) => String(action))]),
  );
