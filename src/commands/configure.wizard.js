...
async function runGatewayHealthCheck(params) {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.GENOS_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.GENOS_GATEWAY_PASSWORD;
  ...
  try {
    await healthCommand({ json: false, timeoutMs: 1e4 }, params.runtime);
  } catch (err) {
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from genosos.json",
        },
      ],
...
export async function runConfigureWizard(opts, runtime = defaultRuntime) {
  try {
    ...
    intro(opts.command === "update" ? "GenosOS update wizard" : "GenosOS configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.genos.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("genosos doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.GENOS_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.GENOS_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.GENOS_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.GENOS_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.GENOS_GATEWAY_TOKEN;
