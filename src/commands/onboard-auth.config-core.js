let applyMoonshotProviderConfigWithBaseUrl = function (cfg, baseUrl) {
...
/**
 * Apply credential to config — re-reads fresh config when using providers format,
 * falls back to legacy in-memory applyAuthProfileConfig otherwise.
 * @param {object} cfg
 * @param {object} params
 * @returns {Promise<object>} updated config
 */
export async function applyCredentialToConfig(cfg, params) {
  const { hasCredentialsInProviders } = await import("../agents/auth-profiles/store.js");
  if (hasCredentialsInProviders(cfg) || !Object.keys(cfg.auth?.profiles ?? {}).length) {
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const snapshot = await readConfigFileSnapshot();
    return snapshot.config ?? cfg;
  }
  return applyAuthProfileConfig(cfg, params);
}
