...
export async function statusAllCommand(runtime, opts) {
  await withProgress({ label: "Scanning status --all\u2026", total: 11 }, async (progress) => {
    ...
    const gatewayProbe = await probeGateway({
      url: connection.url,
      auth: probeAuth,
      timeoutMs: Math.min(5000, opts?.timeoutMs ?? 1e4),
    }).catch(() => null);
    ...
    const health = gatewayReachable
      ? await callGateway({
          method: "health",
          timeoutMs: Math.min(8000, opts?.timeoutMs ?? 1e4),
          ...callOverrides,
    ...
    const channelsStatus = gatewayReachable
      ? await callGateway({
          ...
          params: { probe: false, timeoutMs: opts?.timeoutMs ?? 1e4 },
          timeoutMs: Math.min(8000, opts?.timeoutMs ?? 1e4),
          ...callOverrides,
        }).catch(() => null)
      : null;
    ...
    const gatewayStatus = gatewayReachable
      ...
      : gatewayProbe?.error
        ...
        : "unreachable";
    const gatewayAuth = gatewayReachable ? ` \xB7 auth ${formatGatewayAuthUsed(probeAuth)}` : "";
    ...
    const overviewRows = [
        ...
        : { Item: "Dashboard", Value: "disabled" },
      {
        ...
        Value:
          tailscaleMode === "off"
            ? `off${tailscale.backendState ? ` \xB7 ${tailscale.backendState}` : ""}${tailscale.dnsName ? ` \xB7 ${tailscale.dnsName}` : ""}`
            : tailscale.dnsName && tailscaleHttpsUrl
              ? `${tailscaleMode} \xB7 ${tailscale.backendState ?? "unknown"} \xB7 ${tailscale.dnsName} \xB7 ${tailscaleHttpsUrl}`
              : `${tailscaleMode} \xB7 ${tailscale.backendState ?? "unknown"} \xB7 magicdns unknown`,
      },
      ...
      {
        Item: "Gateway",
        Value: `${gatewayMode}${remoteUrlMissing ? " (remote.url missing)" : ""} \xB7 ${gatewayTarget} (${connection.urlSource}) \xB7 ${gatewayStatus}${gatewayAuth}`,
      },
      { Item: "Security", Value: `Run: ${formatCliCommand("genosos security audit --deep")}` },
      gatewaySelfLine
      ...
      daemon
        ? {
            ...
            Value: !daemon.installed
              ...
              : `${daemon.label} ${daemon.installed ? "installed \xB7 " : ""}${daemon.loadedText}${daemon.runtime?.status ? ` \xB7 ${daemon.runtime.status}` : ""}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ""}`,
          }
      ...
      nodeService
        ? {
            ...
            Value: !nodeService.installed
              ...
              : `${nodeService.label} ${nodeService.installed ? "installed \xB7 " : ""}${nodeService.loadedText}${nodeService.runtime?.status ? ` \xB7 ${nodeService.runtime.status}` : ""}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ""}`,
          }
        : { Item: "Node service", Value: "unknown" },
      {
        Item: "Agents",
        Value: `${agentStatus.agents.length} total \xB7 ${agentStatus.bootstrapPendingCount} bootstrapping \xB7 ${aliveAgents} active \xB7 ${agentStatus.totalSessions} sessions`,
      },
    ];
