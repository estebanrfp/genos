let mockRepoLocalPathExists = function () {
  ...
  expectPluginLoadedFromLocalPath = function (result) {
...
async function runInitialValueForChannel(channel) {
...
describe("ensureOnboardingPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@genosos/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@genosos/zalo" }),
    );
  });
