let resolveSshTargetHint = function () {
    const user = process.env.USER || process.env.LOGNAME || "user";
    const conn = process.env.SSH_CONNECTION?.trim().split(/\s+/);
    const host = conn?.[2] ?? "<host>";
    return `${user}@${host}`;
  },
  shouldSkipBrowserOpenInTests = function () {
    if (process.env.VITEST) {
      return true;
    }
    return false;
  },
  summarizeError = function (err) {
...
export function formatControlUiSshHint(params) {
  ...
  return [
    ...
    authedUrl,
    "Docs:",
    "https://docs.genos.ai/gateway/remote",
    "https://docs.genos.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export async function waitForGatewayReachable(params) {
  const deadlineMs = params.deadlineMs ?? 15000;
