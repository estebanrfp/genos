...
describe("channels command", () => {
  ...
  it("adds a non-default telegram account", async () => {
    ...
    const next = configMocks.writeConfigFile.mock.calls[0]?.[0];
  ...
  it("disables a default provider account when remove has no delete flag", async () => {
    ...
    const promptSpy = vi.spyOn(prompterModule, "createClackPrompter").mockReturnValue(prompt);
  ...
  it("includes external auth profiles in JSON output", async () => {
    ...
    const payload = JSON.parse(runtime.log.mock.calls[0]?.[0]);
  ...
  it("surfaces Discord privileged intent issues in channels status output", () => {
    ...
    expect(lines.join("\n")).toMatch(/Run: (?:genosos|genosos)( --profile isolated)? doctor/);
  });
        ...
        telegram: [
          {
            ...
            enabled: true,
            configured: true,
            probe: { ok: true, bot: { username: "genosos_bot" } },
          },
        ],
      },
    });
    expect(lines.join("\n")).toMatch(/bot:@genosos_bot/);
  });
