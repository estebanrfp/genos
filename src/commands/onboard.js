...
export async function onboardCommand(opts, runtime = defaultRuntime) {
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://docs.genos.ai/security",
        `Re-run with: ${formatCliCommand("genosos onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected \u2014 GenosOS runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.genos.ai/windows",
      ].join("\n"),
    );
  }
