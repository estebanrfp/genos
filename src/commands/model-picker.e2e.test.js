let expectRouterModelFiltering = function (options) {
  ...
  createSelectAllMultiselect = function () {
    return vi.fn(async (params) => params.options.map((option) => option.value));
  };
...
const getCustomProviderApiKey = vi.hoisted(() =>
  vi.fn(() => {
    return;
  }),
);
...
describe("promptDefaultModel", () => {
  ...
  it("supports configuring vLLM during onboarding", async () => {
    ...
    const select = vi.fn(async (params) => {
      const vllm = params.options.find((opt) => opt.value === "__vllm__");
      return vllm?.value ?? "";
    });
    const text = vi
      .fn()
    ...
    const prompter = makePrompter({ select, text });
    const config = { agents: { defaults: {} } };
...
describe("promptModelAllowlist", () => {
  ...
  it("filters to allowed keys when provided", async () => {
    ...
    expect(options.map((opt) => opt.value)).toEqual(["anthropic/claude-opus-4-5"]);
  });
});
