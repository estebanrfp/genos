...
describe("pickAsset", () => {
  ...
  describe("edge cases", () => {
    ...
    it("falls back to first archive for unknown platform", () => {
      const result = pickAsset(SAMPLE_ASSETS, "freebsd", "x64");
      expect(result).toBeDefined();
      expect(result.name).toMatch(/\.tar\.gz$/);
    });
...
describe("extractSignalCliArchive", () => {
  it("rejects zip slip path traversal", async () => {
    const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-signal-install-"));
    try {
      ...
      await expect(extractSignalCliArchive(archivePath, extractDir, 5000)).rejects.toThrow(
  ...
  it("extracts tar.gz archives", async () => {
    ...
    try {
      ...
      await fs.mkdir(extractDir, { recursive: true });
      await extractSignalCliArchive(archivePath, extractDir, 5000);
