let createDefaultSessionStoreEntry = function () {
    return {
  ...
  createUnknownUsageSessionStore = function () {
    return {
      "+1000": {
        updatedAt: Date.now() - 60000,
        inputTokens: 2000,
  ...
  createChannelIssueCollector = function (channel) {
    return (accounts) =>
      accounts
  ...
  createErrorChannelPlugin = function (params) {
    return {
      id: params.id,
      meta: {
        id: params.id,
...
let envSnapshot;
beforeAll(() => {
  envSnapshot = captureEnv(["GENOS_PROFILE"]);
  process.env.GENOS_PROFILE = "isolated";
});
afterAll(() => {
  envSnapshot.restore();
});
async function withUnknownUsageStore(run) {
...
vi.mock("../config/sessions.js", () => ({
  ...
  resolveFreshSessionTotalTokens: vi.fn((entry) =>
...
vi.mock("../infra/genosos-root.js", () => ({
  resolveGenosOSPackageRoot: vi.fn().mockResolvedValue("/tmp/genosos"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/genosos/pnpm-lock.yaml",
      markerPath: "/tmp/genosos/node_modules/.modules.yaml",
    },
    registry: { latestVersion: "0.0.0" },
  }),
  formatGitInstallLabel: vi.fn(() => "main \xB7 @ deadbeef"),
...
describe("statusCommand", () => {
  ...
  it("prints unknown usage in formatted output when totalTokens is missing", async () => {
    await withUnknownUsageStore(async () => {
      ...
      const logs = runtimeLogMock.mock.calls.map((c) => String(c[0]));
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime);
    const logs = runtimeLogMock.mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("GenosOS status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("genosos status --all") ||
          l.includes("genosos --profile isolated status --all") ||
          l.includes("genosos status --all") ||
          l.includes("genosos --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.GENOS_GATEWAY_TOKEN = "abcd1234";
    try {
  ...
  it("includes sessions across agents in JSON output", async () => {
    ...
    expect(payload.sessions.recent.some((sess) => sess.key === "agent:ops:main")).toBe(true);
    if (originalAgents) {
