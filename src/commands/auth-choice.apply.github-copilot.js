...
import { loadValidConfigOrThrow } from "./models/shared.js";
export async function applyAuthChoiceGitHubCopilot(params) {
  ...
  // Credential already saved by githubCopilotLoginCommand (providers or legacy format).
  // Re-read config to pick up the fresh state and avoid stale overwrites.
  nextConfig = await loadValidConfigOrThrow();
