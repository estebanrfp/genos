let mockConfig = function (home, storePath, agentOverrides, telegramOverrides, agentsList) {
  ...
  writeSessionStoreSeed = function (storePath, sessions) {
...
async function withTempHome(fn) {
  return withTempHomeBase(fn, { prefix: "genosos-agent-" });
}
beforeEach(() => {
  vi.clearAllMocks();
...
describe("agentCommand", () => {
  ...
  it("prints JSON payload when requested", async () => {
    await withTempHome(async (home) => {
      ...
      const logged = runtime.log.mock.calls.at(-1)?.[0];
