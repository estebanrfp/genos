let resolveFallbackRetryPrompt = function (params) {
    if (!params.isFallbackRetry) {
      return params.body;
    }
    return "Continue where you left off. The previous model attempt failed or timed out.";
  },
  runAgentAttempt = function (params) {
...
export async function agentCommand(opts, runtime = defaultRuntime, deps = createDefaultDeps()) {
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("genosos agents list")}" to see configured agents.`,
      );
    }
  }
