...
export async function appendStatusAllDiagnosis(params) {
  ...
  if (params.sentinel?.payload) {
    ...
    lines.push(
      `  ${muted(`${summarizeRestartSentinel(params.sentinel.payload)} \xB7 ${formatTimeAgo(Date.now() - params.sentinel.payload.ts)}`)}`,
    );
  } else {
  ...
  }
  {
    ...
    const label =
      params.tailscaleMode === "off"
        ? `Tailscale: off \xB7 ${backend}${params.tailscale.dnsName ? ` \xB7 ${params.tailscale.dnsName}` : ""}`
        : `Tailscale: ${params.tailscaleMode} \xB7 ${backend}${params.tailscale.dnsName ? ` \xB7 ${params.tailscale.dnsName}` : ""}`;
  ...
  if (params.skillStatus) {
    ...
    emitCheck(
      `Skills: ${eligible} eligible \xB7 ${missing} missing \xB7 ${params.skillStatus.workspaceDir}`,
      missing === 0 ? "ok" : "warn",
    );
  }
  ...
  if (params.channelsStatus) {
    ...
    for (const issue of params.channelIssues.slice(0, 12)) {
      const fixText = issue.fix ? ` \xB7 fix: ${issue.fix}` : "";
      lines.push(
  ...
  lines.push("Troubleshooting: https://docs.genos.ai/troubleshooting");
  lines.push("");
}
