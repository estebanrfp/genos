let buildScopeSelection = function (opts) {
...
async function removeMacApp(runtime, dryRun) {
  ...
  await removePath("/Applications/GenosOS.app", runtime, {
    dryRun,
    label: "/Applications/GenosOS.app",
  });
}
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled({
      ...
      options: [
        ...
        { value: "state", label: "State + config", hint: "~/.genos" },
        { value: "workspace", label: "Workspace", hint: "agent files" },
        {
          value: "app",
          label: "macOS app",
          hint: "/Applications/GenosOS.app",
        },
      ],
