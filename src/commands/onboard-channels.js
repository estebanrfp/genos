let formatAccountLabel = function (accountId) {
    return accountId === DEFAULT_ACCOUNT_ID ? "default (primary)" : accountId;
  },
  resolveQuickstartDefault = function (statusByChannel) {
    let best = null;
...
async function collectChannelStatus(params) {
  ...
  const fallbackStatuses = listChatChannels()
    ...
    .map((meta) => {
      ...
      return {
        ...
        selectionHint: configured ? "configured \xB7 plugin disabled" : "not configured",
        quickstartScore: 0,
      };
    });
  const catalogStatuses = catalogEntries.map((entry) => ({
    ...
    selectionHint: "plugin \xB7 install",
    quickstartScore: 0,
  }));
...
async function noteChannelPrimer(prompter, channels) {
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("genosos pairing approve <channel> <code>")}`,
      'Public DMs require dmPolicy="open" + allowFrom=["*"].',
      "Multi-user DMs: run: " +
        formatCliCommand('genosos config set session.dmScope "per-channel-peer"') +
  ...
  const selectPolicy = async (policy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`genosos pairing approve ${policy.channel} <code>`)}`,
...
export async function setupChannels(cfg, runtime, prompter, options) {
  ...
  const resolveDisabledHint = (channel) => {
    ...
    } else if (typeof next.channels?.[channel]?.enabled === "boolean") {
  ...
  const buildSelectionOptions = (entries) =>
    entries.map((entry) => {
      ...
      const hint =
        [status?.selectionHint, disabledHint].filter(Boolean).join(" \xB7 ") || undefined;
      return {
        value: entry.meta.id,
    ...
    const choice = await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("genosos channels add")}\``,
        },
      ],
