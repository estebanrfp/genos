let inferSshTargetFromRemoteUrl = function (rawUrl) {
...
export async function gatewayStatusCommand(opts, runtime) {
  ...
  for (const p of probed) {
    ...
    if (p.probe.ok && p.self) {
      ...
      const platform = p.self.platform ? ` \xB7 ${p.self.platform}` : "";
      const version = p.self.version ? ` \xB7 app ${p.self.version}` : "";
