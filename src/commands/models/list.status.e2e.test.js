...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveGenosOSAgentDir: vi.fn().mockReturnValue("/tmp/genosos-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/genosos-agent"),
    ...
    listProfilesForProvider: vi.fn((s, provider) => {
      return Object.entries(s.profiles)
        .filter(([, cred]) => cred.provider === provider)
        .map(([id]) => id);
    }),
    resolveAuthProfileDisplayLabel: vi.fn(({ profileId }) => profileId),
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/genosos-agent/auth-profiles.json"),
    resolveEnvApiKey: vi.fn((provider) => {
      if (provider === "openai") {
        return {
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveGenosOSAgentDir: mocks.resolveGenosOSAgentDir,
}));
...
async function withAgentScopeOverrides(overrides, run) {
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/genosos-agent");
    }
  }
}
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveGenosOSAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/genosos-agent/auth-profiles.json");
    ...
    const providers = payload.auth.providers;
    ...
    expect(payload.auth.providersWithOAuth.some((e) => e.startsWith("anthropic"))).toBe(true);
    expect(payload.auth.providersWithOAuth.some((e) => e.startsWith("openai-codex"))).toBe(true);
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/genosos-agent-custom",
      },
      async () => {
        ...
        const payload = JSON.parse(String(localRuntime.log.mock.calls[0]?.[0]));
        expect(payload.agentId).toBe("jeremiah");
        expect(payload.agentDir).toBe("/tmp/genosos-agent-custom");
  ...
  it("labels defaults when --agent has no overrides", async () => {
    ...
    await withAgentScopeOverrides(
      ...
      async () => {
        await modelsStatusCommand({ agent: "main" }, localRuntime);
        const output = localRuntime.log.mock.calls.map((call) => String(call[0])).join("\n");
