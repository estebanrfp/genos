let guardPromptCancel = function (value, runtime) {
    if (isCancel(value)) {
  ...
  sortScanResults = function (results) {
  ...
  sortImageResults = function (results) {
    return results.slice().toSorted((a, b) => {
  ...
  compareScanMetadata = function (a, b) {
  ...
  printScanSummary = function (results, runtime) {
  ...
  printScanTable = function (results, runtime) {
    const header = [
      pad("Model", MODEL_PAD),
      pad("Tool", 10),
      pad("Image", 10),
