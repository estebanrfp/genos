...
export async function modelsStatusCommand(opts, runtime) {
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveGenosOSAgentDir();
  ...
  runtime.log(
    `${labelWithSource("Default", agentId ? (agentModelPrimary ? "agent" : "defaults") : undefined)}${colorize(rich, theme.muted, ":")} ${colorize(rich, theme.success, displayDefault)}`,
  );
  runtime.log(
    `${labelWithSource(`Fallbacks (${fallbacks.length || 0})`, agentId ? (agentFallbacksOverride !== undefined ? "agent" : "defaults") : undefined)}${colorize(rich, theme.muted, ":")} ${colorize(rich, fallbacks.length ? theme.warn : theme.muted, fallbacks.length ? fallbacks.join(", ") : "-")}`,
  );
  runtime.log(
    `${labelWithSource("Image model", agentId ? "defaults" : undefined)}${colorize(rich, theme.muted, ":")} ${colorize(rich, imageModel ? theme.accentBright : theme.muted, imageModel || "-")}`,
  );
  runtime.log(
    `${labelWithSource(`Image fallbacks (${imageFallbacks.length || 0})`, agentId ? "defaults" : undefined)}${colorize(rich, theme.muted, ":")} ${colorize(rich, imageFallbacks.length ? theme.accentBright : theme.muted, imageFallbacks.length ? imageFallbacks.join(", ") : "-")}`,
  );
  runtime.log(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("genosos models auth setup-token")}\` or \`${formatCliCommand("genosos configure")}\`.`
          : `Run \`${formatCliCommand("genosos configure")}\` or set an API key env var.`;
  ...
  if (probeSummary) {
    ...
    if (probeSummary.results.length === 0) {
      ...
      const rows = sorted.map((result) => {
        ...
        const statusLabel = `${status}${colorize(rich, theme.muted, ` \xB7 ${latency}`)}${detailLabel}`;
        return {
