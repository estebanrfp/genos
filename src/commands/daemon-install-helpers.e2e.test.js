let mockNodeGatewayPlanFixture = function (params = {}) {
  const {
    workingDirectory = "/Users/me",
    version = "22.0.0",
    supported = true,
    warning,
    serviceEnvironment = { GENOS_PORT: "3000" },
  } = params;
...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/genosos/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\genosos\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/genosos/dist/cli/index.js"])).toBe(false);
  });
});
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ GENOS_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.GENOS_PORT).toBe("3000");
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:genosos|genosos)( --profile isolated)? gateway install/,
    );
  });
});
