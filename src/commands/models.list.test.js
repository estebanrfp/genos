...
let modelsListCommand;
const loadConfig = vi.fn();
const ensureGenosOSModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/genosos-agent/auth.json" });
const resolveGenosOSAgentDir = vi.fn().mockReturnValue("/tmp/genosos-agent");
const ensureAuthProfileStore = vi.fn().mockReturnValue({ version: 1, profiles: {} });
const listProfilesForProvider = vi.fn().mockReturnValue([]);
const resolveAuthProfileDisplayLabel = vi.fn(({ profileId }) => profileId);
...
describe("models list/status", () => {
  ...
  function configureModelAsConfigured(model) {
    loadConfig.mockReturnValue({
      agents: {
        defaults: {
          model,
  ...
  function configureGoogleAntigravityModel(modelId) {
    configureModelAsConfigured(`google-antigravity/${modelId}`);
  }
  function makeGoogleAntigravityTemplate(id, name) {
    return {
  ...
  function enableGoogleAntigravityAuthProfile() {
    listProfilesForProvider.mockImplementation((_, provider) =>
      provider === "google-antigravity" ? [{ id: "profile-1" }] : [],
    );
  }
  function parseJsonLog(runtime) {
  ...
  async function expectZaiProviderFilter(provider) {
  ...
  function setDefaultZaiRegistry(params = {}) {
  ...
  function expectAntigravityModel(payload, params) {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/genosos-agent");
  });
  ...
  it("models list falls back to auth heuristics when getAvailable returns invalid shape", async () => {
    await runAvailabilityFallbackCase({
      setup: () => {
        modelRegistryState.available = { bad: true };
      },
