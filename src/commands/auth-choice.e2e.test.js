...
describe("applyAuthChoice", () => {
  ...
  function createSelectFirstOption() {
    return vi.fn(async (params) => params.options[0]?.value);
  }
  function createNoopMultiselect() {
    return vi.fn(async () => []);
  }
  function createApiKeyPromptHarness(overrides = {}) {
  ...
  async function readAuthProfile(profileId) {
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireGenosOSAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
...
describe("resolvePreferredProviderForAuthChoice", () => {
  ...
  it("returns undefined for unknown choices", () => {
    expect(resolvePreferredProviderForAuthChoice("unknown")).toBeUndefined();
  });
});
