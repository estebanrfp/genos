let hasAuthForProvider = function (provider, cfg, store) {
  ...
  createProviderAuthChecker = function (params) {
  ...
  resolveConfiguredModelRaw = function (cfg) {
  ...
  resolveConfiguredModelKeys = function (cfg) {
    const models = cfg.agents?.defaults?.models ?? {};
    return Object.keys(models)
      .map((key) => String(key ?? "").trim())
      .filter((key) => key.length > 0);
  },
  normalizeModelKeys = function (values) {
    const seen = new Set();
    const next = [];
  ...
  addModelSelectOption = function (params) {
    ...
    params.options.push({
      value: key,
      label: key,
      hint: hints.length > 0 ? hints.join(" \xB7 ") : undefined,
    });
    params.seen.add(key);
  },
  isAnthropicLegacyModel = function (entry) {
    return (
