let resolveNodeRunner = function () {
    if (hasBinary("pnpm")) {
  ...
  extractLine = function (lines, prefix) {
  ...
  normalizeSnippet = function (raw, fallback) {
  ...
  parseSearchOutput = function (raw) {
  ...
  buildMarkdown = function (query, results) {
  ...
  formatLinkLabel = function (link) {
    return link.replace(/^https?:\/\//i, "");
  },
  renderRichResults = function (query, results, runtime) {
...
const SEARCH_TOOL = "https://docs.genos.ai/mcp.SearchGenosOS";
...
export async function docsSearchCommand(queryParts, runtime) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.genos.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('genosos docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.genos.ai/");
      runtime.log(`Search: ${formatCliCommand('genosos docs "your query"')}`);
    }
    return;
  }
