...
describe("runInteractiveOnboarding", () => {
  ...
  it("restores terminal state without resuming stdin on success", async () => {
    ...
    await runInteractiveOnboarding({}, runtime);
  ...
  it("restores terminal state without resuming stdin on cancel", async () => {
    ...
    await expect(runInteractiveOnboarding({}, runtime)).rejects.toBe(exitError);
    ...
    const exitOrder = runtime.exit.mock.invocationCallOrder[0] ?? Number.MAX_SAFE_INTEGER;
