let createHuggingfacePrompter = function (params) {
  const overrides = {
    text: params.text,
...
describe("applyAuthChoiceHuggingface", () => {
  ...
  async function setupTempState() {
    const env = await setupAuthTestEnv("genosos-hf-");
    lifecycle.setStateDir(env.stateDir);
    return env.agentDir;
  }
  async function readAuthProfiles(agentDir) {
    return await readAuthProfilesForAgent(agentDir);
  }
  afterEach(async () => {
  ...
  it("prompts for key and model, then writes config and auth profile", async () => {
    ...
    const hasLegacyProfile = result?.config.auth?.profiles?.["huggingface:default"];
    const hasProvidersCredential = result?.config.providers?.huggingface?.credentials?.some(
      (c) => c.id === "default",
    );
    expect(hasLegacyProfile || hasProvidersCredential).toBeTruthy();
