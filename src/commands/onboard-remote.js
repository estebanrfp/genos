let pickHost = function (beacon) {
    return beacon.host || beacon.tailnetDns || beacon.lanHost;
  },
  buildLabel = function (beacon) {
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.genos.ai/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.genos.ai/gateway/remote",
          ].join("\n"),
          "SSH tunnel",
        );
      }
