let parseTimeoutSeconds = function (opts) {
    const raw =
  ...
  formatPayloadForLog = function (payload) {
    const lines = [];
    if (payload.text) {
...
const NO_GATEWAY_TIMEOUT_MS = 2147000000;
export async function agentViaGatewayCommand(opts, runtime) {
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("genosos agents list")}" to see configured agents.`,
      );
    }
  }
  const timeoutSeconds = parseTimeoutSeconds({ cfg, timeout: opts.timeout });
  const gatewayTimeoutMs =
    timeoutSeconds === 0 ? NO_GATEWAY_TIMEOUT_MS : Math.max(1e4, (timeoutSeconds + 30) * 1000);
