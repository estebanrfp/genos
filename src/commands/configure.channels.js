...
export async function removeChannelConfigWizard(cfg, runtime) {
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in genosos.json.",
          `Tip: \`${formatCliCommand("genosos channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    next = {
      ...next,
      channels: Object.keys(nextChannels).length ? nextChannels : undefined,
    };
    note(
