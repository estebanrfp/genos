let summarizeInstallFailure = function (message) {
  ...
  formatSkillHint = function (skill) {
  ...
  upsertSkillEntry = function (cfg, skillKey, patch) {
...
export async function setupSkills(cfg, workspaceDir, runtime, prompter) {
  ...
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("genosos doctor")}\` to review skills + requirements.`,
      );
      runtime.log("Docs: https://docs.genos.ai/skills");
    }
  }
