let resolveMemoryPluginStatus = function (cfg) {
    ...
    async (progress) => {
      ...
      const tailscaleDns =
        tailscaleMode === "off"
          ...
          : await getTailnetHostname((cmd, args) =>
              runExec(cmd, args, { timeoutMs: 1200, maxBuffer: 200000 }),
            ).catch(() => null);
      ...
      const gatewayProbe = remoteUrlMissing
        ...
        : await probeGateway({
            ...
            timeoutMs: Math.min(opts.all ? 5000 : 2500, opts.timeoutMs ?? 1e4),
          }).catch(() => null);
      ...
      const channels = await buildChannelsTable(cfg, {
        showSecrets:
          (process.env.GENOS_SHOW_SECRETS?.trim() ?? process.env.GENOS_SHOW_SECRETS?.trim()) !==
          "0",
      });
      progress.tick();
