let trimOutput = function (text, maxChars) {
  ...
  extractSherpaOnnxText = function (raw) {
  ...
  commandBase = function (command) {
    return path.parse(command).name;
  },
  findArgValue = function (args, keys) {
  ...
  hasArg = function (args, keys) {
    return args.some((arg) => keys.includes(arg));
  },
  resolveWhisperOutputPath = function (args, mediaPath) {
  ...
  resolveWhisperCppOutputPath = function (args) {
  ...
  normalizeProviderQuery = function (options) {
    if (!options) {
      return;
    }
    const query = {};
  ...
  buildDeepgramCompatQuery = function (options) {
    if (!options) {
      return;
    }
    const query = {};
  ...
  normalizeDeepgramQueryKeys = function (query) {
  ...
  resolveProviderQuery = function (params) {
  ...
  resolveEntryRunOptions = function (params) {
...
export async function runCliEntry(params) {
  ...
  const outputDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-media-cli-"));
