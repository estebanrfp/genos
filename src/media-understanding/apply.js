let xmlEscapeAttr = function (value) {
    return value.replace(/[<>&"']/g, (char) => XML_ESCAPE_MAP[char] ?? char);
  },
  escapeFileBlockContent = function (value) {
    return value
      .replace(/<\s*\/\s*file\s*>/gi, "&lt;/file&gt;")
      .replace(/<\s*file\b/gi, "&lt;file");
  },
  sanitizeMimeType = function (value) {
    if (!value) {
      return;
    }
  ...
  resolveFileLimits = function (cfg) {
  ...
  appendFileBlocks = function (body, blocks) {
  ...
  resolveUtf16Charset = function (buffer) {
  ...
  decodeLegacyText = function (buffer) {
    let output = "";
  ...
  isMostlyPrintable = function (text) {
    return getTextStats(text).printableRatio > 0.85;
  },
  looksLikeLegacyTextBytes = function (buffer) {
    if (buffer.length === 0) {
      return false;
    }
  ...
  looksLikeUtf8Text = function (buffer) {
  ...
  decodeTextSample = function (buffer) {
    if (!buffer || buffer.length === 0) {
      return "";
    }
  ...
  guessDelimitedMime = function (text) {
    if (!text) {
      return;
    }
  ...
  resolveTextMimeFromName = function (name) {
    if (!name) {
      return;
    }
  ...
  isBinaryMediaMime = function (mime) {
    if (!mime) {
      return false;
    }
