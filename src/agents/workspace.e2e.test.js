...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses GENOS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      GENOS_HOME: "/srv/genosos-home",
      HOME: "/home/other",
    });
    expect(dir).toBe(path.join(path.resolve("/srv/genosos-home"), ".genosv1", "workspace"));
  });
});
const WORKSPACE_STATE_PATH_SEGMENTS = [".genosv1", "workspace-state.json"];
async function readOnboardingState(dir) {
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("genosos-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  const getMemoryEntries = (files) =>
    files.filter((file) =>
      [DEFAULT_MEMORY_FILENAME, DEFAULT_MEMORY_ALT_FILENAME].includes(file.name),
    );
  const expectSingleMemoryEntry = (files, content) => {
