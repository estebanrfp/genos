let unexpectedGatewayMethod = function (method) {
    throw new Error(`unexpected method: ${String(method)}`);
  },
  getNodesTool = function () {
    const tool = createGenosOSTools().find((candidate) => candidate.name === "nodes");
    if (!tool) {
      throw new Error("missing nodes tool");
    }
    return tool;
  },
  mockNodeList = function (commands) {
    return {
...
const NODE_ID = "mac-1";
const BASE_RUN_INPUT = { action: "run", node: NODE_ID, command: ["echo", "hi"] };
async function executeNodes(input) {
  return getNodesTool().execute("call1", input);
}
beforeEach(() => {
  ...
  it("requests approval and retries with allow-once decision", async () => {
    ...
    callGateway.mockImplementation(async ({ method, params }) => {
      ...
      if (method === "exec.approval.request") {
        ...
        approvalId = typeof params?.id === "string" ? (params.id ?? null) : null;
