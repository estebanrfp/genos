let isFallbackAbortError = function (err) {
  ...
  shouldRethrowAbort = function (err) {
    return isFallbackAbortError(err) && !isTimeoutError(err);
  },
  createModelCandidateCollector = function (allowlist) {
    const seen = new Set();
    const candidates = [];
  ...
  throwFallbackFailureSummary = function (params) {
  ...
  resolveImageFallbackCandidates = function (params) {
  ...
  resolveFallbackCandidates = function (params) {
    ...
    // NOTE: implicit "default model" fallback intentionally removed.
    // Only user-configured fallbacks (agents.defaults.model.fallbacks) are used.
    // If a model fails with no configured fallbacks, the error propagates to chat.
    return candidates;
  },
  resolveProbeThrottleKey = function (provider, agentDir) {
    const scope = String(agentDir ?? "").trim();
    return scope ? `${scope}${PROBE_SCOPE_DELIMITER}${provider}` : provider;
  },
  shouldProbePrimaryDuringCooldown = function (params) {
...
const MIN_PROBE_INTERVAL_MS = 30000;
const PROBE_MARGIN_MS = 120000;
...
export async function runWithModelFallback(params) {
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    if (authStore) {
      ...
      // All profiles for this provider are paused (disabled) — fail fast, no API call
      if (profileIds.length === 0 && hasProfilesForProvider(authStore, candidate.provider)) {
        attempts.push({
          provider: candidate.provider,
          model: candidate.model,
          error: `All credentials for ${candidate.provider} are paused`,
          reason: "disabled",
        });
        continue;
      }
