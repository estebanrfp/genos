...
const announceSpy = vi.fn(async (_params) => true);
...
describe("subagent registry steer restarts", () => {
  ...
  it("suppresses announce for interrupted runs and only announces the replacement run", async () => {
    ...
    const announce = announceSpy.mock.calls[0]?.[0] ?? {};
  ...
  it("retries deferred parent cleanup after a descendant announces", async () => {
    ...
    announceSpy.mockImplementation(async (params) => {
      const typed = params;
    ...
    await flushAnnounce();
    const childRunIds = announceSpy.mock.calls.map((call) => (call[0] ?? {}).childRunId);
