...
describe("before_tool_call loop detection behavior", () => {
  ...
  function createWrappedTool(name, execute, loopDetectionContext = enabledLoopDetectionContext) {
    return wrapToolWithBeforeToolCallHook({ name, execute }, loopDetectionContext);
  }
  async function withToolLoopEvents(run, filter = () => true) {
    const emitted = [];
  ...
  function createPingPongTools(options) {
    const readExecute = options?.withProgress
      ? vi.fn().mockImplementation(async (toolCallId) => ({
  ...
  async function runPingPongSequence(readTool, listTool, count) {
  ...
  it("does not block known poll loops when output progresses", async () => {
    const execute = vi.fn().mockImplementation(async (toolCallId) => {
      return {
