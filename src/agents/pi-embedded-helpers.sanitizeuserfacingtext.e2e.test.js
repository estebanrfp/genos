  ...
  it.each([
    {
      ...
      assert: (value) => expect(value).toBe("defaulttoolid"),
    },
    {
      modeLabel: "strict",
    ...
    },
    {
      ...
      assert: (value) => expect(value).toMatch(/^[a-zA-Z0-9]{9}$/),
    },
...
describe("downgradeOpenAIReasoningBlocks", () => {
  it("keeps reasoning signatures when followed by content", () => {
    ...
    expect(downgradeOpenAIReasoningBlocks(input)).toEqual(input);
  });
  it("drops orphaned reasoning blocks without following content", () => {
    ...
    expect(downgradeOpenAIReasoningBlocks(input)).toEqual([{ role: "user", content: "next" }]);
  });
  it("drops object-form orphaned signatures", () => {
    ...
    expect(downgradeOpenAIReasoningBlocks(input)).toEqual([]);
  });
  ...
  it("is idempotent for orphaned reasoning cleanup", () => {
    ...
    const once = downgradeOpenAIReasoningBlocks(input);
    const twice = downgradeOpenAIReasoningBlocks(once);
