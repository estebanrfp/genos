...
const waitForCalls = async (getCount, count, timeoutMs = 2000) => {
  await vi.waitFor(
    () => {
...
describe("sessions tools", () => {
  it("uses number (not integer) in tool schemas for Gemini compatibility", () => {
    const tools = createGenosOSTools();
    const byName = (name) => {
    ...
    const schemaProp = (toolName, prop) => {
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createGenosOSTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
    ...
    const cronDetails = cronOnly.details;
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createGenosOSTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
    ...
    const withToolsDetails = withTools.details;
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect((details.bytes ?? 0) <= 81920).toBe(true);
  ...
  it("sessions_history resolves sessionId inputs", async () => {
    ...
    const historyCall = callGatewayMock.mock.calls.find(
      (call) => call[0].method === "chat.history",
    );
  ...
  it("sessions_send supports fire-and-forget and wait", async () => {
    ...
    expect(typeof waited.details.runId).toBe("string");
    ...
    expect(
      agentCalls.some(
        (call) =>
          ...
          call.params?.extraSystemPrompt?.includes("Agent-to-agent message context"),
      ),
    ).toBe(true);
    expect(
      agentCalls.some(
        (call) =>
          typeof call.params?.extraSystemPrompt === "string" &&
          call.params?.extraSystemPrompt?.includes("Agent-to-agent reply step"),
      ),
    ).toBe(true);
    expect(
      agentCalls.some(
  ...
  it("subagents steer sends guidance to a running run", async () => {
    ...
    try {
      ...
      const steerWaitIndex = callGatewayMock.mock.calls.findIndex(
        (call) => call[0].method === "agent.wait" && call[0].params?.runId === "run-steer",
      );
      ...
      expect(callGatewayMock.mock.calls[steerWaitIndex]?.[0]).toMatchObject({
        method: "agent.wait",
        params: { runId: "run-steer", timeoutMs: 5000 },
        timeoutMs: 7000,
      });
