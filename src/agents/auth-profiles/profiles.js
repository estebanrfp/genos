...
export function listProfilesForProvider(store, provider) {
  ...
  return Object.entries(store.profiles)
    .filter(
      ([, cred]) => normalizeProviderId(cred.provider) === providerKey && cred.disabled !== true,
    )
    .map(([id]) => id);
}

/** Returns true if ANY profile (including disabled ones) exists for this provider. */
export function hasProfilesForProvider(store, provider) {
  const providerKey = normalizeProviderId(provider);
