let recordSummary = function (summary, seen, bucket, value) {
  ...
  resolvePatchFileOps = function (options) {
    if (options.sandbox) {
  ...
  resolvePathFromCwd = function (filePath, cwd) {
    return path.resolve(cwd, filePath);
  },
  toDisplayPath = function (resolved, cwd) {
  ...
  checkPatchBoundariesLenient = function (lines) {
  ...
  checkPatchBoundariesStrict = function (lines) {
  ...
  parseOneHunk = function (lines, lineNumber) {
    if (lines.length === 0) {
  ...
  parseUpdateFileChunk = function (lines, lineNumber, allowMissingContext) {
    if (lines.length === 0) {
      throw new Error(
