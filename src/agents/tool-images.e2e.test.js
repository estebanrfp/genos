...
describe("tool image sanitizing", () => {
  ...
  it("shrinks oversized images to <=5MB", async () => {
    ...
    const bigPng = await sharp(raw, {
      ...
      .toBuffer();
    expect(bigPng.byteLength).toBeGreaterThan(5242880);
    const blocks = [
      {
        type: "image",
    ...
    expect(size).toBeLessThanOrEqual(5242880);
    expect(image.mimeType).toBe("image/jpeg");
  }, 20000);
  it("sanitizes image arrays and reports drops", async () => {
    const png = await createWidePng();
    const images = [{ type: "image", data: png.toString("base64"), mimeType: "image/png" }];
