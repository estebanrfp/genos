...
describe("buildWorkspaceSkillsPrompt", () => {
  ...
  it("gates by bins, config, and always", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"genosos":{"requires":{"bins":["fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "anybin-skill"),
      name: "anybin-skill",
      description: "Needs any bin",
      metadata: '{"genosos":{"requires":{"anyBins":["missingbin","fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "config-skill"),
      name: "config-skill",
      description: "Needs config",
      metadata: '{"genosos":{"requires":{"config":["browser.enabled"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "always-skill"),
      name: "always-skill",
      description: "Always on",
      metadata: '{"genosos":{"always":true,"requires":{"env":["MISSING"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "env-skill"),
      name: "env-skill",
      description: "Needs env",
      metadata: '{"genosos":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
    });
    try {
      ...
      await fs.chmod(fakebinPath, 493);
  ...
  it("uses skillKey for config lookups", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"genosos":{"skillKey":"alias"}}',
    });
    const prompt = buildWorkspaceSkillsPrompt(workspaceDir, {
      managedSkillsDir: path.join(workspaceDir, ".managed"),
