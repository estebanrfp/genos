let computeReplacements = function (originalLines, filePath, chunks) {
    const replacements = [];
    let lineIndex = 0;
  ...
  applyReplacements = function (lines, replacements) {
    const result = [...lines];
  ...
  seekSequence = function (lines, pattern, start, eof) {
    if (pattern.length === 0) {
      return start;
    }
  ...
  linesMatch = function (lines, pattern, start, normalize) {
  ...
  normalizePunctuation = function (value) {
