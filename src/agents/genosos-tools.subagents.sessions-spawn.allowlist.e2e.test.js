...
describe("genosos-tools: subagents (sessions_spawn allowlist)", () => {
  function setAllowAgents(allowAgents) {
    setSessionsSpawnConfigOverride({
      session: {
        mainKey: "main",
        scope: "per-sender",
  ...
  function mockAcceptedSpawn(acceptedAt) {
    let childSessionKey;
  ...
  async function executeSpawn(callId, agentId) {
