let captureBedrockEnv = function () {
    return {
  ...
  restoreBedrockEnv = function (previous) {
...
async function withEnvUpdates(updates, run) {
...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const envSnapshot = captureEnv(["GENOS_STATE_DIR", "GENOS_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-oauth-"));
    try {
      process.env.GENOS_STATE_DIR = tempDir;
      process.env.GENOS_AGENT_DIR = path.join(tempDir, "agent");
      process.env.PI_CODING_AGENT_DIR = process.env.GENOS_AGENT_DIR;
      const oauthDir = path.join(tempDir, "credentials");
      await fs.mkdir(oauthDir, { recursive: true, mode: 448 });
      await fs.writeFile(
      ...
      const store = ensureAuthProfileStore(process.env.GENOS_AGENT_DIR, {
      ...
      const authData = JSON.parse(authProfiles);
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-auth-"));
    try {
