let createState = function () {
  ...
  recordSuccessfulCall = function (state, toolName, params, result, index) {
  ...
  recordSuccessfulPingPongCalls = function (params) {
  ...
  expectPingPongLoop = function (loopResult, expected) {
...
const enabledLoopDetectionConfig = { enabled: true };
const shortHistoryLoopConfig = {
  enabled: true,
  historySize: 4,
};
describe("tool-loop-detection", () => {
  describe("hashToolCall", () => {
    ...
    it("keeps hashes fixed-size even for large params", () => {
      const payload = { data: "x".repeat(20000) };
  ...
  describe("detectToolCallLoop", () => {
    ...
    it("does not flag ping-pong when alternation is broken", () => {
      ...
      recordToolCall(state, "write", { path: "/tmp/out.txt" }, "c1");
    ...
    it("records fixed-size result hashes for large tool outputs", () => {
      ...
      recordToolCallOutcome(state, {
        ...
        toolCallId,
        result: {
          content: [{ type: "text", text: "y".repeat(40000) }],
          details: { status: "running", totalLines: 1, totalChars: 40000 },
        },
      });
