let normalizeSources = function (sources, sessionMemoryEnabled) {
  ...
  resolveStorePath = function (agentId, raw, agentDir) {
    if (!raw) {
      // Prefer agent dir (opaque UUID) over agentId-derived path
      if (agentDir) {
        return path.join(path.dirname(agentDir), "memory.sqlite");
      }
  ...
  mergeConfig = function (defaults, overrides, agentId, agentDir) {
    ...
    const store = {
      ...
      path: resolveStorePath(agentId, overrides?.store?.path ?? defaults?.store?.path, agentDir),
      vector,
    };
    const chunking = {
...
export function resolveMemorySearchConfig(cfg, agentId) {
  ...
  const resolved = mergeConfig(defaults, overrides, agentId, agentCfg?.agentDir?.trim());
  if (!resolved.enabled) {
    return null;
  }
  return resolved;
