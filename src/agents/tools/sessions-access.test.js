...
describe("resolveSessionToolsVisibility", () => {
  it("defaults to tree when unset or invalid", () => {
    expect(resolveSessionToolsVisibility({})).toBe("tree");
    expect(
      resolveSessionToolsVisibility({
...
describe("sandbox session-tools context", () => {
  it("defaults sandbox visibility clamp to spawned", () => {
    expect(resolveSandboxSessionToolsVisibility({})).toBe("spawned");
  });
...
describe("createAgentToAgentPolicy", () => {
  it("allows cross-agent access by default (enabled unless explicitly false)", () => {
  ...
  it("denies cross-agent access when explicitly disabled", () => {
    const policy = createAgentToAgentPolicy({ tools: { agentToAgent: { enabled: false } } });
