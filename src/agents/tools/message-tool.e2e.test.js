let mockSendResult = function (overrides = {}) {
...
vi.mock("../../infra/outbound/message-action-runner.js", async () => {
  const actual = await vi.importActual("../../infra/outbound/message-action-runner.js");
  return {
    ...actual,
...
describe("message tool schema scoping", () => {
  ...
  it("hides discord components when scoped to telegram", () => {
    ...
    const buttonItemProps = properties.buttons?.items?.items?.properties ?? {};
...
describe("message tool reasoning tag sanitization", () => {
  it("strips <think> tags from text field before sending", async () => {
    ...
    const tool = createMessageTool({ config: {} });
    await tool.execute("1", {
      action: "send",
