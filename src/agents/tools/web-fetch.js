let resolveFetchConfig = function (cfg) {
  ...
  resolveFetchEnabled = function (params) {
    if (typeof params.fetch?.enabled === "boolean") {
      return params.fetch.enabled;
    }
    return true;
  },
  resolveFetchReadabilityEnabled = function (fetch) {
    if (typeof fetch?.readability === "boolean") {
      return fetch.readability;
    }
    return true;
  },
  resolveFetchMaxCharsCap = function (fetch) {
    const raw =
  ...
  resolveFetchMaxResponseBytes = function (fetch) {
    const raw =
  ...
  resolveFirecrawlConfig = function (fetch) {
    if (!fetch || typeof fetch !== "object") {
      return;
    }
  ...
  resolveFirecrawlApiKey = function (firecrawl) {
    const fromConfig =
  ...
  resolveFirecrawlEnabled = function (params) {
    if (typeof params.firecrawl?.enabled === "boolean") {
      return params.firecrawl.enabled;
    }
    return Boolean(params.apiKey);
  },
  resolveFirecrawlBaseUrl = function (firecrawl) {
    const raw =
  ...
  resolveFirecrawlOnlyMainContent = function (firecrawl) {
    if (typeof firecrawl?.onlyMainContent === "boolean") {
      return firecrawl.onlyMainContent;
    }
    return true;
  },
  resolveFirecrawlMaxAgeMs = function (firecrawl) {
    const raw =
  ...
  resolveFirecrawlMaxAgeMsOrDefault = function (firecrawl) {
  ...
  resolveMaxChars = function (value, fallback, cap) {
    const parsed = typeof value === "number" && Number.isFinite(value) ? value : fallback;
    const clamped = Math.max(100, Math.floor(parsed));
    return Math.min(clamped, cap);
  },
  resolveMaxRedirects = function (value, fallback) {
    const parsed = typeof value === "number" && Number.isFinite(value) ? value : fallback;
    return Math.max(0, Math.floor(parsed));
  },
  looksLikeHtml = function (value) {
  ...
  formatWebFetchErrorDetail = function (params) {
  ...
  redactUrlForDebugLog = function (rawUrl) {
    try {
  ...
  wrapWebFetchContent = function (value, maxChars) {
    if (maxChars <= 0) {
  ...
  wrapWebFetchField = function (value) {
    if (!value) {
      return value;
    }
    return wrapExternalContent(value, { source: "web_fetch", includeWarning: false });
  },
  buildFirecrawlWebFetchPayload = function (params) {
  ...
  normalizeContentType = function (value) {
    if (!value) {
      return;
    }
  ...
  toFirecrawlContentParams = function (params) {
  ...
  resolveFirecrawlEndpoint = function (baseUrl) {
