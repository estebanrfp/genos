let resolveRunStatus = function (entry) {
    if (!entry.endedAt) {
      return "running";
    }
  ...
  resolveModelRef = function (entry) {
  ...
  resolveSubagentTarget = function (runs, token, options) {
  ...
  resolveStorePathForKey = function (cfg, key, parsed) {
    return resolveStorePath(cfg.session?.store, {
      agentId: parsed?.agentId,
    });
  },
  resolveSessionEntryForKey = function (params) {
  ...
  resolveRequesterKey = function (params) {
    ...
    const spawnedBy =
      typeof callerEntry?.spawnedBy === "string" ? callerEntry.spawnedBy.trim() : "";
    return {
...
async function killSubagentRun(params) {
  ...
  if (killed) {
    // Trigger sessions.changed broadcast so sidebar updates in real-time
    callGateway({
