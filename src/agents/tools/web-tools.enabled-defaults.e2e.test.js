let installMockFetch = function (payload) {
    const mockFetch = vi.fn((_input, _init) =>
      Promise.resolve({
        ok: true,
  ...
  createPerplexitySearchTool = function (perplexityConfig) {
  ...
  parseFirstRequestBody = function (mockFetch) {
    const request = mockFetch.mock.calls[0]?.[1];
    const requestBody = request?.body;
    return JSON.parse(typeof requestBody === "string" ? requestBody : "{}");
  },
  installPerplexitySuccessFetch = function () {
...
describe("web_search country and language parameters", () => {
  ...
  async function runBraveSearchAndGetUrl(params) {
    const mockFetch = installMockFetch({ web: { results: [] } });
    const tool = createWebSearchTool({ config: undefined, sandboxed: true });
    expect(tool).not.toBeNull();
    await tool?.execute?.("call-1", { query: "test", ...params });
    expect(mockFetch).toHaveBeenCalled();
    return new URL(mockFetch.mock.calls[0][0]);
  }
  it.each([
...
describe("web_search external content wrapping", () => {
  ...
  function installPerplexityFetch(payload) {
    const mock = vi.fn(async (_input, _init) =>
      Promise.resolve({
        ok: true,
        json: () => Promise.resolve(payload),
      }),
  ...
  async function executePerplexitySearchForWrapping(query) {
