let actionPrefix = function (action) {
    if (action === "history") {
  ...
  a2aDisabledMessage = function (action) {
    if (action === "history") {
  ...
  a2aDeniedMessage = function (action) {
    if (action === "history") {
...
export async function createSessionVisibilityGuard(params) {
  ...
  const check = (targetSessionKey) => {
    ...
    if (isCrossAgent) {
      // Parent can always reach sessions it spawned (e.g., subagents)
      if (spawnedKeys?.has(targetSessionKey)) {
        return { allowed: true };
      }
      // Cross-agent governed by A2A policy alone, not session visibility
