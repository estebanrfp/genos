...
const browserClientMocks = vi.hoisted(() => ({
  browserCloseTab: vi.fn(async (..._args) => ({})),
  browserFocusTab: vi.fn(async (..._args) => ({})),
  browserOpenTab: vi.fn(async (..._args) => ({})),
  browserProfiles: vi.fn(async (..._args) => []),
  browserSnapshot: vi.fn(async (..._args) => ({
    ok: true,
    format: "ai",
    targetId: "t1",
  ...
  browserStart: vi.fn(async (..._args) => ({})),
  browserStatus: vi.fn(async (..._args) => ({
    ok: true,
    running: true,
    pid: 1,
    cdpPort: 18792,
  ...
  browserStop: vi.fn(async (..._args) => ({})),
  browserTabs: vi.fn(async (..._args) => []),
}));
...
const nodesUtilsMocks = vi.hoisted(() => ({
  listNodes: vi.fn(async (..._args) => []),
}));
vi.mock("./nodes-utils.js", async () => {
  const actual = await vi.importActual("./nodes-utils.js");
  return {
    ...actual,
...
describe("browser tool snapshot maxChars", () => {
  ...
  it("skips the default when maxChars is explicitly zero", async () => {
    ...
    const opts = browserClientMocks.browserSnapshot.mock.calls.at(-1)?.[1];
