let resolveSessionEntry = function (params) {
  ...
  resolveSessionKeyFromSessionId = function (params) {
...
async function resolveModelOverride(params) {
  ...
  // Tier name resolution ("cambia a complex", "cambia a simple")
  const routing = params.cfg.agents?.defaults?.model?.routing;
  if (routing?.enabled && routing?.tiers) {
    const tierName = raw.toLowerCase();
    const tierValue = routing.tiers[tierName];
    if (tierValue) {
      const modelRef = extractTierModel(tierValue);
      const parsed = parseModelRef(modelRef, currentProvider);
      if (parsed) {
        const profile = normalizeTierProfile(tierValue);
        return {
          kind: "set",
          ...
          tierProfile: {
            ...
            ...(profile.verbose !== undefined ? { verbose: profile.verbose } : {}),
...
export function createSessionStatusTool(opts) {
  return {
    ...
    name: "session_status",
    description:
      "Show a /status-equivalent session status card (usage + time + cost when available). Use for model-use questions (\uD83D\uDCCA session_status). Set per-session model override: model=sonnet, model=opus, model=complex, model=default (resets). Tier names (normal/complex) apply full profiles (model + thinking + verbose + reasoning). model=default resets all overrides. THIS is the correct tool for switching models — not config_manage.",
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (typeof modelRaw === "string") {
        ...
        // Persist routing tier in session entry
        if (selection.kind === "reset") {
          delete nextEntry.routingTier;
        } else if (selection.tierName) {
          nextEntry.routingTier = selection.tierName;
        }
          ...
          selection:
            selection.kind === "reset"
              ...
              : {
                  ...
                  isDefault: false, // explicit request — always write override to bypass routing
                },
        });
        if (applied.updated || selection.tierName || selection.kind === "reset") {
          ...
          changedModel = true;
          // Build patch params — include tier profile fields if present
          const patchParams = { key: resolved.key };
          if (selection.kind === "reset") {
            // Reset all profile overrides alongside model
            patchParams.thinkingLevel = null;
            patchParams.verboseLevel = null;
            patchParams.reasoningLevel = null;
            patchParams.routingTier = null;
          } else {
            if (selection.tierName) {
              patchParams.routingTier = selection.tierName;
            }
            if (selection.tierProfile) {
              const tp = selection.tierProfile;
              if (tp.thinking !== undefined) {
                patchParams.thinkingLevel = tp.thinking;
              }
              if (tp.verbose !== undefined) {
                patchParams.verboseLevel = tp.verbose;
              }
              if (tp.reasoning !== undefined) {
                patchParams.reasoningLevel = tp.reasoning;
              }
            }
          }
          // Broadcast so the UI reflects the model change
          callGateway({
