let resolveImageToolMaxTokens = function (modelMaxTokens, requestedMaxTokens = 4096) {
    if (
  ...
  resolveDefaultModelRef = function (cfg) {
    if (cfg) {
  ...
  hasAuthForProvider = function (params) {
  ...
  pickMaxBytes = function (cfg, maxBytesMb) {
  ...
  buildImageContext = function (prompt, images) {
...
async function runImagePrompt(params) {
  const effectiveCfg = params.cfg
    ...
    : undefined;
  await ensureGenosOSModelsJson(effectiveCfg, params.agentDir);
