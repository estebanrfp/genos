let makeToolCallResultPairInput = function () {
    return [
      {
        role: "assistant",
        content: [
  ...
  expectToolCallAndResultIds = function (out, expectedId) {
  ...
  expectSingleAssistantContentEntry = function (out, expectEntry) {
    expect(out).toHaveLength(1);
    const content = out[0].content;
    expect(Array.isArray(content)).toBe(true);
    expect(content).toHaveLength(1);
    expectEntry(content[0] ?? {});
  };
