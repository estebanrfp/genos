let createProcessSessionHarness = function (sessionId) {
  ...
  retryMs = function (result) {
    return result.details.retryInMs;
  },
  pollStatus = function (result) {
...
async function pollSession(processTool, callId, sessionId, timeout) {
...
test("process poll exposes adaptive retryInMs for repeated no-output polls", async () => {
  ...
  expect(polls.map((poll) => retryMs(poll))).toEqual([5000, 1e4, 30000, 60000, 60000]);
});
