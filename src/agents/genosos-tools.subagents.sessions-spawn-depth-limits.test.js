let writeStore = function (agentId, store) {
  ...
  seedDepthTwoAncestryStore = function (params) {
...
describe("sessions_spawn depth + child limits", () => {
  beforeEach(() => {
    ...
    storeTemplatePath = path.join(
      os.tmpdir(),
      `genosos-subagent-depth-${Date.now()}-${Math.random().toString(16).slice(2)}-{agentId}.json`,
    );
    configOverride = {
      session: {
        mainKey: "main",
  ...
  it("allows depth-1 callers when maxSpawnDepth is 2", async () => {
    ...
    const calls = callGatewayMock.mock.calls.map((call) => call[0]);
  ...
  it("fails spawn when sessions.patch rejects the model", async () => {
    ...
    expect(String(result.details.error ?? "")).toContain("invalid model");
