let mockExistingClaudeKeychainItem = function () {
  ...
  getAddGenericPasswordCall = function () {
    return execFileSyncMock.mock.calls.find(
      ([binary, args]) =>
        String(binary) === "security" &&
        Array.isArray(args) &&
        args.map(String).includes("add-generic-password"),
    );
  };
...
const CLI_CREDENTIALS_CACHE_TTL_MS = 900000;
async function readCachedClaudeCliCredentials(allowKeychainPrompt) {
...
describe("cli credentials", () => {
  ...
  it("falls back to the file store when the keychain update fails", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-"));
    const credPath = path.join(tempDir, ".claude", ".credentials.json");
    fs.mkdirSync(path.dirname(credPath), { recursive: true, mode: 448 });
    fs.writeFileSync(
      credPath,
      `${JSON.stringify(
        {
  ...
  it("reads Codex credentials from keychain when available", async () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-codex-"));
  ...
  it("falls back to Codex auth.json when keychain is unavailable", async () => {
    ...
    fs.mkdirSync(tempHome, { recursive: true, mode: 448 });
    fs.writeFileSync(
      authPath,
      JSON.stringify({
        tokens: {
