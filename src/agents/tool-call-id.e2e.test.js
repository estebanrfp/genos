let expectCollisionIdsRemainDistinct = function (out, mode) {
  const assistant = out[0];
  const a = assistant.content?.[0];
  const b = assistant.content?.[1];
  expect(typeof a.id).toBe("string");
  expect(typeof b.id).toBe("string");
  expect(a.id).not.toBe(b.id);
  expect(isValidCloudCodeAssistToolId(a.id, mode)).toBe(true);
  expect(isValidCloudCodeAssistToolId(b.id, mode)).toBe(true);
  const r1 = out[1];
  const r2 = out[2];
...
describe("sanitizeToolCallIdsForCloudCodeAssist", () => {
  describe("strict mode (default)", () => {
    ...
    it("strips non-alphanumeric characters from tool call IDs", () => {
      ...
      expect(isValidCloudCodeAssistToolId(toolCall.id, "strict")).toBe(true);
      const result = out[1];
