let resolveOAuthStatus = function (expiresAt, now, warnAfterMs) {
  ...
  buildProfileHealth = function (params) {
...
export const DEFAULT_OAUTH_WARN_MS = 86400000;
export function resolveAuthProfileSource(_profileId) {
  return "store";
}
export function formatRemainingShort(remainingMs, opts) {
  ...
  const roundedMinutes = Math.round(remainingMs / 60000);
  if (roundedMinutes < 1) {
