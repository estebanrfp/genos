let resolveClaudeCliCredentialsPath = function (homeDir) {
    const baseDir = homeDir ?? resolveUserPath("~");
    return path.join(baseDir, CLAUDE_CLI_CREDENTIALS_RELATIVE_PATH);
  },
  parseClaudeCliOauthCredential = function (claudeOauth) {
  ...
  resolveCodexCliAuthPath = function () {
    return path.join(resolveCodexHomePath(), CODEX_CLI_AUTH_FILENAME);
  },
  resolveCodexHomePath = function () {
  ...
  resolveQwenCliCredentialsPath = function (homeDir) {
    const baseDir = homeDir ?? resolveUserPath("~");
    return path.join(baseDir, QWEN_CLI_CREDENTIALS_RELATIVE_PATH);
  },
  resolveMiniMaxCliCredentialsPath = function (homeDir) {
    const baseDir = homeDir ?? resolveUserPath("~");
    return path.join(baseDir, MINIMAX_CLI_CREDENTIALS_RELATIVE_PATH);
  },
  computeCodexKeychainAccount = function (codexHome) {
    const hash = createHash("sha256").update(codexHome).digest("hex");
    return `cli|${hash.slice(0, 16)}`;
  },
  readCodexKeychainCredentials = function (options) {
    ...
    try {
      ...
      const lastRefresh =
        typeof lastRefreshRaw === "string" || typeof lastRefreshRaw === "number"
          ...
          : Date.now();
      const expires = Number.isFinite(lastRefresh) ? lastRefresh + 3600000 : Date.now() + 3600000;
  ...
  readMiniMaxCliCredentials = function (options) {
    const credPath = resolveMiniMaxCliCredentialsPath(options?.homeDir);
    return readPortalCliOauthCredentials(credPath, "minimax-portal");
  },
  readClaudeCliKeychainCredentials = function (execSyncImpl = execSync) {
    try {
