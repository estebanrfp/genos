let resolveCleanupState = function () {
    const proc = process;
  ...
  resolvePositiveMs = function (value, fallback, opts = {}) {
  ...
  ensureWatchdogStarted = function (intervalMs) {
  ...
  handleTerminationSignal = function (signal) {
    releaseAllLocksSync();
  ...
  registerCleanupHandlers = function () {
    const cleanupState = resolveCleanupState();
  ...
  inspectLockPayload = function (payload, staleMs, nowMs) {
...
const CLEANUP_STATE_KEY = Symbol.for("genosos.sessionWriteLockCleanupState");
const HELD_LOCKS_KEY = Symbol.for("genosos.sessionWriteLockHeldLocks");
const WATCHDOG_STATE_KEY = Symbol.for("genosos.sessionWriteLockWatchdogState");
const DEFAULT_STALE_MS = 1800000;
const DEFAULT_MAX_HOLD_MS = 300000;
const DEFAULT_WATCHDOG_INTERVAL_MS = 60000;
const DEFAULT_TIMEOUT_GRACE_MS = 120000;
...
async function releaseHeldLock(normalizedSessionFile, held, opts = {}) {
  ...
  if (held.releasePromise) {
    await held.releasePromise.catch(() => {
      return;
    });
    return true;
  }
