...
describe("createGenosOSCodingTools", () => {
  it("preserves action enums in normalized schemas", () => {
    ...
    const collectActionValues = (schema, values) => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createGenosOSCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    expect(openAiTools.some((tool) => tool.name === "apply_patch")).toBe(true);
    const anthropicTools = createGenosOSCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createGenosOSCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    expect(allowed.some((tool) => tool.name === "apply_patch")).toBe(true);
    const denied = createGenosOSCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
