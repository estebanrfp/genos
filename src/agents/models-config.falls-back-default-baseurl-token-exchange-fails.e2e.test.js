...
describe("models-config", () => {
  it("falls back to default baseUrl when token exchange fails", async () => {
    await withTempHome(async () => {
      ...
      try {
        await ensureGenosOSModelsJson({ models: { providers: {} } });
        const agentDir = path.join(process.env.HOME, ".genosv1", "agents", "main", "agent");
  ...
  it("uses agentDir override auth profiles for copilot injection", async () => {
    await withTempHome(async (home) => {
      await withUnsetCopilotTokenEnv(async () => {
        ...
        await ensureGenosOSModelsJson({ models: { providers: {} } }, agentDir);
        const raw = await fs.readFile(path.join(agentDir, "models.json"), "utf8");
        const parsed = JSON.parse(raw);
