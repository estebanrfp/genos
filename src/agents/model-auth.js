let resolveProviderConfig = function (cfg, provider) {
    // New: cfg.providers takes precedence over cfg.models.providers
    const topProviders = cfg?.providers ?? {};
    const modelProviders = cfg?.models?.providers ?? {};
    const normalized = normalizeProviderId(provider);
    // Search in cfg.providers first
    const topDirect =
      topProviders[provider] ??
      topProviders[normalized] ??
      Object.entries(topProviders).find(([key]) => normalizeProviderId(key) === normalized)?.[1];
    if (topDirect) {
      return topDirect;
    }
    // Fallback: cfg.models.providers (legacy / not yet migrated)
    const direct = modelProviders[provider];
    if (direct) {
      return direct;
    }
    if (normalized === provider) {
      const matched = Object.entries(modelProviders).find(
        ([key]) => normalizeProviderId(key) === normalized,
      );
      return matched?.[1];
    }
    return (
      modelProviders[normalized] ??
      Object.entries(modelProviders).find(([key]) => normalizeProviderId(key) === normalized)?.[1]
    );
  },
  resolveProviderAuthOverride = function (cfg, provider) {
  ...
  resolveEnvSourceLabel = function (params) {
  ...
  resolveAwsSdkAuthInfo = function () {
...
export async function resolveApiKeyForProvider(params) {
  ...
  const store = params.store ?? ensureAuthProfileStore(params.agentDir, undefined, cfg);
  if (profileId) {
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("genosos agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
...
export function resolveModelAuthMode(provider, cfg, store) {
  ...
  const authStore = store ?? ensureAuthProfileStore(undefined, undefined, cfg);
