let toUsage = function (raw) {
  ...
  pickSessionId = function (parsed, backend) {
  ...
  resolveImageExtension = function (mimeType) {
...
export async function writeCliImages(images) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cli-images-"));
  const paths = [];
