let buildAddFilePatch = function (targetPath) {
  return `*** Begin Patch
*** Add File: ${targetPath}
+escaped
*** End Patch`;
...
async function withTempDir(fn) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-patch-"));
  try {
    return await fn(dir);
  } finally {
...
*** End Patch`;
  ...
  it("rejects absolute paths outside cwd by default", async () => {
    await withTempDir(async (dir) => {
      const escapedPath = path.join(os.tmpdir(), `genosos-apply-patch-${Date.now()}.txt`);
      try {
        await expectOutsideWriteRejected({
          dir,
...
*** End Patch`;
  ...
  it("allows deleting a symlink itself even if it points outside cwd", async () => {
    await withTempDir(async (dir) => {
      const outsideDir = await fs.mkdtemp(path.join(path.dirname(dir), "genosos-patch-outside-"));
      try {
