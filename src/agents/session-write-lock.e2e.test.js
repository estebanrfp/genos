...
describe("acquireSessionWriteLock", () => {
  it("reuses locks across symlinked session paths", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-lock-"));
    try {
  ...
  it("reclaims stale lock files", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        lockPath,
        JSON.stringify({ pid: 123456, createdAt: new Date(Date.now() - 60000).toISOString() }),
        "utf8",
      );
  ...
  it("derives max hold from timeout plus grace", () => {
    expect(resolveSessionLockMaxHoldFromTimeout({ timeoutMs: 600000 })).toBe(720000);
    expect(resolveSessionLockMaxHoldFromTimeout({ timeoutMs: 1000, minMs: 5000 })).toBe(123000);
  });
  it("clamps max hold for effectively no-timeout runs", () => {
    expect(
      resolveSessionLockMaxHoldFromTimeout({
        timeoutMs: 2147000000,
      }),
    ).toBe(2147000000);
  });
  ...
  it("removes held locks on termination signals", async () => {
    const signals = ["SIGINT", "SIGTERM", "SIGQUIT", "SIGABRT"];
    for (const signal of signals) {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-lock-cleanup-"));
      try {
        const sessionFile = path.join(root, "sessions.json");
        const lockPath = `${sessionFile}.lock`;
