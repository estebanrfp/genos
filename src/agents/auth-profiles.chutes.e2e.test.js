...
import { ensureAuthProfileStore, resolveApiKeyForProfile } from "./auth-profiles.js";
...
describe("auth-profiles (chutes)", () => {
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-chutes-"));
    process.env.GENOS_STATE_DIR = tempDir;
    process.env.GENOS_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
    ...
    const persisted = JSON.parse(await fs.readFile(authProfilePath, "utf8"));
