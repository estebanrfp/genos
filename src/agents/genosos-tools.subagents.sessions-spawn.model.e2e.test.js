let mockLongRunningSpawnFlow = function (params) {
  ...
  mockPatchAndSingleAgentRun = function (params) {
    callGatewayMock.mockImplementation(async (opts) => {
      const request = opts;
      params.calls.push(request);
      if (request.method === "sessions.patch") {
...
async function expectSpawnUsesConfiguredModel(params) {
  ...
  const patchCall = calls.find((call) => call.method === "sessions.patch" && call.params?.model);
  expect(patchCall?.params).toMatchObject({
    model: params.expectedModel,
  });
}
describe("genosos-tools: subagents (sessions_spawn model + thinking)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
