...
export async function prefetchMemoryContext({ prompt, config, sessionKey }) {
  ...
  try {
    ...
    if (!manager) {
      ...
      return { context: null, chunks: 0, searchMs: Date.now() - searchStart, queryVec: null };
    }
  } catch (err) {
  ...
  let queryVec = null;
  try {
    // Embed query for both memory search and semantic tool matching
    queryVec = await manager.embedQuery(userQuery).catch(() => null);
    ...
    return { context: null, chunks: 0, searchMs: Date.now() - searchStart, queryVec };
  }
  ...
  if (!chunks?.length) {
    ...
    return { context: null, chunks: 0, searchMs, queryVec };
  }
  ...
  return { context, chunks: chunks.length, searchMs, queryVec };
}
