let buildForwardCompatTemplate = function (params) {
    return {
      id: params.id,
      name: params.name,
  ...
  expectResolvedForwardCompatFallback = function (params) {
  ...
  expectUnknownModelError = function (provider, id) {
  ...
  it("model-level api takes precedence over provider-level api", () => {
    const providers = {
      custom: {
        ...
        models: [{ ...makeModel("custom-model"), api: "anthropic-messages" }],
      },
    };
    const result = buildInlineProviderModels(providers);
    expect(result).toHaveLength(1);
...
describe("resolveModel", () => {
  ...
  it("rejects unknown ollama models after eradication", () => {
