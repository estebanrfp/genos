...
import { TIER_CAPABILITY_DEFAULTS, extractTierModel } from "../tier-profiles.js";
...
export async function runEmbeddedPiAgent(params) {
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      // Smart model routing — only when enabled and no explicit override was applied
      if (!modelResolveOverride?.modelOverride && !params.hasSessionModelOverride) {
        ...
        if (routed) {
          ...
          log.info(`[model-routing] tier=${routed.tier} → ${provider}/${modelId}`);
          // Persist full tier profile so the UI reflects capabilities in real-time
          const patchParams = { key: params.sessionKey, routingTier: routed.tier };
          if (routed.thinking !== undefined) {
            patchParams.thinkingLevel = routed.thinking;
          }
          if (routed.reasoning !== undefined) {
            patchParams.reasoningLevel = routed.reasoning;
          }
          if (routed.verbose !== undefined) {
            patchParams.verboseLevel = routed.verbose;
          }
          callGateway({
      ...
      // Auto tier escalation + adaptive capability classification.
      // Runs for ALL sessions, every message. Single unified patch.
      // Config/implementation/security → complex (Opus). Everything else → default tier.
      // De-escalates automatically on the next non-config message.
      let escalatedTier = null;
      if (params.sessionKey && params.prompt) {
        const routing = params.config?.agents?.defaults?.model?.routing;
        const defaultTier = params.config?.agents?.defaults?.model?.defaultTier ?? "normal";
        // Tier escalation — config tasks → complex
        if (!params.hasSessionModelOverride) {
          const escalation = classifyTierEscalation(params.prompt);
          if (escalation === "complex" && routing?.tiers?.complex) {
            const complexModelRef = extractTierModel(routing.tiers.complex);
            const complexParsed = parseModelRef(complexModelRef, provider);
            if (complexParsed && complexParsed.model !== modelId) {
              provider = complexParsed.provider;
              modelId = complexParsed.model;
              escalatedTier = "complex";
              log.info(`[tier-escalation] config task detected → ${provider}/${modelId}`);
            }
          }
        }
        // Capability classification — thinking/reasoning/verbose from prompt analysis
        const caps = classifyPromptCapabilities(params.prompt, {
          imageCount: params.images?.length ?? 0,
        });
        const sessionTier = escalatedTier ?? defaultTier;
        const tierDefaults = TIER_CAPABILITY_DEFAULTS[sessionTier] ?? {};
        const thinkRank = { off: 0, minimal: 1, low: 2, medium: 3, high: 4, xhigh: 5 };
        const pickHigher = (a, b) => ((thinkRank[a] ?? 0) >= (thinkRank[b] ?? 0) ? a : b);
        const nextThink = pickHigher(caps.thinking ?? "off", tierDefaults.thinking ?? "off");
        const nextReason = caps.reasoning ?? tierDefaults.reasoning ?? "off";
        const nextVerbose = caps.verbose ?? tierDefaults.verbose ?? "off";
        // Build unified patch — tier + capabilities in one broadcast
        const unifiedPatch = { key: params.sessionKey };
        let patchNeeded = false;
        // Tier change (escalation or de-escalation)
        unifiedPatch.routingTier = escalatedTier ?? defaultTier;
        patchNeeded = true;
        // Thinking
        if (nextThink !== (params.thinkLevel ?? "off")) {
          unifiedPatch.thinkingLevel = nextThink === "off" ? null : nextThink;
          params.thinkLevel = nextThink;
        }
        // Reasoning
        if (nextReason !== (params.reasoningLevel ?? "off")) {
          unifiedPatch.reasoningLevel = nextReason === "off" ? null : nextReason;
          params.reasoningLevel = nextReason;
        }
        // Verbose
        if (nextVerbose !== (params.verboseLevel ?? "off")) {
          unifiedPatch.verboseLevel = nextVerbose === "off" ? null : nextVerbose;
          params.verboseLevel = nextVerbose;
        }
        if (patchNeeded) {
          const logParts = [
            `routingTier=${unifiedPatch.routingTier}`,
            `thinking=${nextThink}`,
            `reasoning=${nextReason}`,
            `verbose=${nextVerbose}`,
          ];
          log.info(`[adaptive-routing] ${logParts.join(" ")}`);
          callGateway({
            method: "sessions.patch",
            params: unifiedPatch,
            timeoutMs: 3e3,
