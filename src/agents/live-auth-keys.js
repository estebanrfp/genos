let parseKeyList = function (raw) {
  ...
  collectEnvPrefixedKeys = function (prefix) {
    const keys = [];
  ...
  resolveProviderApiKeyConfig = function (provider) {
    const normalized = normalizeProviderId(provider);
    const custom = PROVIDER_API_KEY_CONFIG[normalized];
    const base =
      PROVIDER_PREFIX_OVERRIDES[normalized] ?? normalized.toUpperCase().replace(/-/g, "_");
    const liveSingle = custom?.liveSingle ?? `GENOS_LIVE_${base}_KEY`;
...
const GOOGLE_LIVE_SINGLE_KEY = "GENOS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG = {
  anthropic: {
    liveSingle: "GENOS_LIVE_ANTHROPIC_KEY",
    listVar: "GENOS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "GENOS_LIVE_OPENAI_KEY",
