let mockSingleSuccessfulAttempt = function () {
...
const buildAssistant = (overrides) => ({
  role: "assistant",
  content: [],
...
const writeAuthStore = async (agentDir, opts) => {
...
async function readUsageStats(agentDir) {
  const stored = JSON.parse(await fs.readFile(path.join(agentDir, "auth-profiles.json"), "utf-8"));
  return stored.usageStats ?? {};
}
async function expectProfileP2UsageUpdated(agentDir) {
  const usageStats = await readUsageStats(agentDir);
  expect(typeof usageStats["openai:p2"]?.lastUsed).toBe("number");
}
async function expectProfileP2UsageUnchanged(agentDir) {
  const usageStats = await readUsageStats(agentDir);
  expect(usageStats["openai:p2"]?.lastUsed).toBe(2);
}
async function withTimedAgentWorkspace(run) {
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-workspace-"));
    const now = Date.now();
...
async function runTurnWithCooldownSeed(params) {
  return await withTimedAgentWorkspace(async ({ agentDir, workspaceDir, now }) => {
    await writeAuthStore(agentDir, {
      usageStats: {
        "openai:p1": { lastUsed: 1, cooldownUntil: now + 3600000 },
        "openai:p2": { lastUsed: 2 },
      },
    });
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("skips profiles in cooldown during initial selection", async () => {
    ...
    expect(usageStats["openai:p1"]?.cooldownUntil).toBe(now + 3600000);
    expect(typeof usageStats["openai:p2"]?.lastUsed).toBe("number");
  });
  it("fails over when all profiles are in cooldown and fallbacks are configured", async () => {
    await withTimedAgentWorkspace(async ({ agentDir, workspaceDir, now }) => {
      await writeAuthStore(agentDir, {
        usageStats: {
          "openai:p1": { lastUsed: 1, cooldownUntil: now + 3600000 },
          "openai:p2": { lastUsed: 2, cooldownUntil: now + 3600000 },
        },
      });
      await expect(
        runEmbeddedPiAgent({
  ...
  it("uses the active erroring model in billing failover errors", async () => {
    ...
    try {
      ...
      expect(thrown.message).toContain("openai (mock-rotated) returned a billing error");
      expect(runEmbeddedAttemptMock).toHaveBeenCalledTimes(1);
    } finally {
      await fs.rm(agentDir, { recursive: true, force: true });
      await fs.rm(workspaceDir, { recursive: true, force: true });
    ...
    try {
      ...
      try {
        ...
        const payload = {
          ...
          usageStats: {
            "openai:p1": { lastUsed: 1 },
            "openai:p2": { cooldownUntil: now + 3600000 },
        ...
        expect(usageStats["openai:p2"]?.cooldownUntil).toBe(now + 3600000);
      } finally {
        await fs.rm(agentDir, { recursive: true, force: true });
        await fs.rm(workspaceDir, { recursive: true, force: true });
      }
