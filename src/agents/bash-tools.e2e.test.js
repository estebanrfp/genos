...
const defaultShell = isWin
  ? undefined
  : process.env.GENOS_TEST_SHELL || resolveShellFromPath("bash") || process.env.SHELL || "sh";
...
async function runBackgroundEchoLines(lines) {
...
describe("exec tool backgrounding", () => {
      ...
      await expect
        .poll(
          async () => {
            ...
            const status = poll.details.status;
  ...
  it("defaults process log to a bounded tail when no window is provided", async () => {
    ...
    expect(log.details.totalLines).toBe(260);
  });
  ...
  it("scopes process sessions by scopeKey", async () => {
    ...
    const pollBDetails = pollB.details;
...
describe("buildDockerExecArgs", () => {
  it("prepends custom PATH after login shell sourcing to preserve both custom and system tools", () => {
    ...
    expect(args).toContain("GENOS_PREPEND_PATH=/custom/bin:/usr/local/bin:/usr/bin");
    expect(commandArg).toContain('export PATH="${GENOS_PREPEND_PATH}:$PATH"');
    expect(commandArg).toContain("echo hello");
    expect(commandArg).toBe(
      'export PATH="${GENOS_PREPEND_PATH}:$PATH"; unset GENOS_PREPEND_PATH; echo hello',
    );
  });
  it("does not interpolate PATH into the shell command", () => {
    const injectedPath = "$(touch /tmp/genosos-path-injection)";
    const args = buildDockerExecArgs({
      containerName: "test-container",
      command: "echo hello",
      env: {
    ...
    const commandArg = args[args.length - 1];
    expect(args).toContain(`GENOS_PREPEND_PATH=${injectedPath}`);
    expect(commandArg).not.toContain(injectedPath);
    expect(commandArg).toContain("GENOS_PREPEND_PATH");
  });
