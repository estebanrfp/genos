let asPositiveInt = function (value, fallback) {
    if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
      return fallback;
    }
    return value;
  },
  resolveLoopDetectionConfig = function (config) {
  ...
  digestStable = function (value) {
    const serialized = stableStringifyFallback(value);
    return createHash("sha256").update(serialized).digest("hex");
  },
  stableStringifyFallback = function (value) {
    try {
  ...
  isKnownPollToolCall = function (toolName, params) {
  ...
  formatErrorForHash = function (error) {
  ...
  hashToolOutcome = function (toolName, params, result, error) {
    if (error !== undefined) {
  ...
  getNoProgressStreak = function (history, toolName, argsHash) {
    let streak = 0;
  ...
  getPingPongStreak = function (history, currentSignature) {
  ...
  canonicalPairKey = function (signatureA, signatureB) {
