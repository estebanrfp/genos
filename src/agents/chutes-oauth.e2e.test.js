...
describe("chutes-oauth", () => {
  it("exchanges code for tokens and stores username as email", async () => {
    const fetchFn = withFetchPreconnect(async (input, init) => {
      ...
      if (url === CHUTES_TOKEN_ENDPOINT) {
        ...
        expect(String(init?.headers && init.headers["Content-Type"])).toContain(
      ...
      if (url === CHUTES_USERINFO_ENDPOINT) {
        expect(String(init?.headers && init.headers.Authorization)).toBe("Bearer at_123");
    ...
    expect(creds.accountId).toBe("sub_1");
    expect(creds.clientId).toBe("cid_test");
    expect(creds.expires).toBe(now + 3600000 - 300000);
  });
  it("refreshes tokens using stored client id and falls back to old refresh token", async () => {
    ...
    expect(refreshed.expires).toBe(now + 1800000 - 300000);
  });
});
