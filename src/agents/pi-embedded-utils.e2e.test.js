let makeAssistantMessage = function (message) {
  return {
    api: "responses",
    provider: "openai",
    model: "gpt-5",
  ...
  it("strips thinking tags from text content", () => {
    const msg = makeAssistantMessage({
      ...
      content: [
        {
          type: "text",
          text: "<think>El usuario quiere retomar una tarea...</think>Aqu\xED est\xE1 tu respuesta.",
        },
      ],
      timestamp: Date.now(),
    });
    const result = extractAssistantText(msg);
    expect(result).toBe("Aqu\xED est\xE1 tu respuesta.");
  });
