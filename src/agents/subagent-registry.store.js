let resolveSubagentStateDir = function (env = process.env) {
  const explicit = env.GENOS_STATE_DIR?.trim();
  if (explicit) {
    return resolveStateDir(env);
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), "genosos-test-state", String(process.pid));
  }
  return resolveStateDir(env);
};
