...
describe("resolveBundledSkillsDir", () => {
  const originalOverride = process.env.GENOS_BUNDLED_SKILLS_DIR;
  afterEach(() => {
    if (originalOverride === undefined) {
      delete process.env.GENOS_BUNDLED_SKILLS_DIR;
    } else {
      process.env.GENOS_BUNDLED_SKILLS_DIR = originalOverride;
    }
  });
  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.GENOS_BUNDLED_SKILLS_DIR;
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "genosos" }));
