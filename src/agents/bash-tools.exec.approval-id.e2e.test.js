...
vi.mock("./tools/nodes-utils.js", () => ({
  ...
  resolveNodeIdFromList: vi.fn((nodes) => nodes[0]?.nodeId),
}));
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-test-"));
  ...
  it("reuses approval id as the node runId", async () => {
    ...
    await expect
      .poll(() => invokeParams?.params?.runId, {
        timeout: 2000,
        interval: 20,
      })
      .toBe(approvalId);
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const { callGatewayTool } = await import("./tools/gateway.js");
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-test-bin-"));
