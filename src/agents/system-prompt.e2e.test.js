...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## GenosOS CLI Quick Reference");
    expect(prompt).toContain("genosos gateway restart");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("GenosOS docs: /tmp/genosos/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## GenosOS Self-Update");
  ...
  it("ignores context files with missing or blank paths", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/genosos",
      contextFiles: [
        { path: undefined, content: "Missing path" },
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/genosos. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
