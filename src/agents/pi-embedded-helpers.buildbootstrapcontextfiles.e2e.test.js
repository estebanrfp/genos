...
describe("buildBootstrapContextFiles", () => {
  ...
  it("keeps total injected bootstrap characters under the new default total cap", () => {
    const files = [
      makeFile({ name: "AGENTS.md", content: "a".repeat(1e4) }),
      makeFile({ name: "SOUL.md", path: "/tmp/SOUL.md", content: "b".repeat(1e4) }),
      makeFile({ name: "USER.md", path: "/tmp/USER.md", content: "c".repeat(1e4) }),
    ];
    const result = buildBootstrapContextFiles(files);
    ...
    expect(result[2]?.content).toBe("c".repeat(1e4));
  });
  it("caps total injected bootstrap characters when totalMaxChars is configured", () => {
    ...
    const result = buildBootstrapContextFiles(files, { totalMaxChars: 24000 });
    const totalChars = result.reduce((sum, entry) => sum + entry.content.length, 0);
  ...
  it("enforces strict total cap even when truncation markers are present", () => {
    const files = [
      makeFile({ name: "AGENTS.md", content: "a".repeat(1000) }),
      makeFile({ name: "SOUL.md", path: "/tmp/SOUL.md", content: "b".repeat(1000) }),
    ];
    const result = buildBootstrapContextFiles(files, {
      maxChars: 100,
      totalMaxChars: 150,
  ...
  it("skips bootstrap injection when remaining total budget is too small", () => {
    const files = [makeFile({ name: "AGENTS.md", content: "a".repeat(1000) })];
    const result = buildBootstrapContextFiles(files, {
      maxChars: 200,
      totalMaxChars: 40,
    });
