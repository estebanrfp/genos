let clampTtl = function (value) {
    if (!value || Number.isNaN(value)) {
      return DEFAULT_JOB_TTL_MS;
    }
    return Math.min(Math.max(value, MIN_JOB_TTL_MS), MAX_JOB_TTL_MS);
  },
  isSessionIdTaken = function (id) {
    return runningSessions.has(id) || finishedSessions.has(id);
  },
  moveToFinished = function (session, status) {
  ...
  sumPendingChars = function (buffer) {
    let total = 0;
  ...
  capPendingBuffer = function (buffer, pendingChars, cap) {
    if (pendingChars <= cap) {
      return pendingChars;
    }
  ...
  pruneFinishedSessions = function () {
...
const MIN_JOB_TTL_MS = 60000;
const MAX_JOB_TTL_MS = 10800000;
const DEFAULT_PENDING_OUTPUT_CHARS = 30000;
