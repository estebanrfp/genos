let isCronAddAction = function (args) {
  ...
  buildToolCallSummary = function (toolName, args, meta) {
  ...
  extendExecMeta = function (toolName, args, meta) {
    ...
    const suffix = flags.join(" \xB7 ");
    return meta ? `${meta} \xB7 ${suffix}` : suffix;
  },
  pushUniqueMediaUrl = function (urls, seen, value) {
  ...
  collectMessagingMediaUrlsFromRecord = function (record) {
    const urls = [];
    const seen = new Set();
  ...
  collectMessagingMediaUrlsFromToolResult = function (result) {
    const urls = [];
    const seen = new Set();
...
export async function handleToolExecutionEnd(ctx, evt) {
  ...
  const statusStr = isToolError ? " ✗" : " ✓";
  const metaTruncated = meta && meta.length > 120 ? `${meta.slice(0, 117)}…` : meta;
  const metaStr = metaTruncated ? ` → ${metaTruncated}` : "";
  ctx.log.info(`tool${statusStr} ${toolName}${metaStr}${durationStr}`);
