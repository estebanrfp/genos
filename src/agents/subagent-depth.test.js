...
describe("getSubagentDepthFromSessionStore", () => {
  ...
  it("resolves prefixed store keys when caller key omits the agent prefix", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-subagent-depth-"));
...
describe("resolveAgentTimeoutMs", () => {
  it("uses a timer-safe sentinel for no-timeout overrides", () => {
    expect(resolveAgentTimeoutMs({ overrideSeconds: 0 })).toBe(2147000000);
    expect(resolveAgentTimeoutMs({ overrideMs: 0 })).toBe(2147000000);
  });
  it("clamps very large timeout overrides to timer-safe values", () => {
    expect(resolveAgentTimeoutMs({ overrideSeconds: 9999999 })).toBe(2147000000);
    expect(resolveAgentTimeoutMs({ overrideMs: 9999999999 })).toBe(2147000000);
  });
});
