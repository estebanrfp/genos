let resolveLogSliceWindow = function (offset, limit) {
  ...
  defaultTailNote = function (totalLines, usingDefaultTail) {
  ...
  resolvePollWaitMs = function (value) {
  ...
  recordPollRetrySuggestion = function (sessionId, hasNewOutput) {
    try {
  ...
  resetPollRetrySuggestion = function (sessionId) {
    try {
      const sessionState = getDiagnosticSessionState({ sessionId });
