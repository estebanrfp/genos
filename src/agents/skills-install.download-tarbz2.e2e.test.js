let mockDownloadResponse = function () {
  ...
  runCommandResult = function (params) {
    return {
      code: 0,
      stdout: "",
      stderr: "",
  ...
  mockTarExtractionFlow = function (params) {
    mocks.runCommand.mockImplementation(async (argv) => {
      const cmd = argv;
  ...
  restoreGenosOSStateDir = function (originalValue) {
    if (originalValue === undefined) {
      delete process.env.GENOS_STATE_DIR;
      return;
    }
    process.env.GENOS_STATE_DIR = originalValue;
  };
...
async function withTempWorkspace(run) {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-skills-install-"));
  try {
...
const originalStateDir = process.env.GENOS_STATE_DIR;
afterEach(() => {
  restoreGenosOSStateDir(originalStateDir);
});
vi.mock("../process/exec.js", () => ({
  runCommandWithTimeout: (...args) => mocks.runCommand(...args),
}));
vi.mock("../infra/net/fetch-guard.js", () => ({
  fetchWithSsrFGuard: (...args) => mocks.fetchGuard(...args),
}));
vi.mock("../security/skill-scanner.js", async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
    scanDirectoryWithSummary: (...args) => mocks.scanSummary(...args),
  };
});
describe("installSkill download extraction safety (tar.bz2)", () => {
  ...
  it("rejects tar.bz2 traversal before extraction", async () => {
    await withTempWorkspace(async ({ workspaceDir, stateDir }) => {
      ...
      expect(mocks.runCommand.mock.calls.some((call) => call[0][1] === "xf")).toBe(false);
    });
  });
  ...
  it("extracts tar.bz2 with stripComponents safely (preflight only)", async () => {
    await withTempWorkspace(async ({ workspaceDir, stateDir }) => {
      ...
      expect(mocks.runCommand.mock.calls.some((call) => call[0][1] === "xf")).toBe(true);
    });
  });
