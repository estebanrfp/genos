let createReadTool = function () {
  ...
  enableAfterToolCallHook = function () {
    hookMocks.runner.hasHooks.mockImplementation((name) => name === "after_tool_call");
  },
  expectReadAfterToolCallPayload = function (result) {
...
const hookMocks = vi.hoisted(() => ({
  runner: {
    hasHooks: vi.fn((_) => false),
    runAfterToolCall: vi.fn(async () => {}),
  },
  isToolWrappedWithBeforeToolCallHook: vi.fn(() => false),
  consumeAdjustedParamsForToolCall: vi.fn((_) => {
    return;
  }),
  runBeforeToolCallHook: vi.fn(async ({ params }) => ({
    blocked: false,
    params,
  })),
}));
...
async function executeReadTool(callId) {
  const defs = toToolDefinitions([createReadTool()]);
  const def = defs[0];
  if (!def) {
    throw new Error("missing tool definition");
  }
  const execute = (...args) => def.execute(...args);
