let normalizeMode = function (mode) {
    return mode ?? DEFAULT_MODE;
  },
  matchSessionFilter = function (sessionKey, patterns) {
  ...
  buildTargetKey = function (target) {
    const channel = normalizeMessageChannel(target.channel) ?? target.channel;
    const accountId = target.accountId ?? "";
    const threadId = target.threadId ?? "";
    return [channel, target.to, accountId, threadId].join(":");
  },
  shouldSkipDiscordForwarding = function (target) {
    const channel = normalizeMessageChannel(target.channel) ?? target.channel;
    return channel === "discord";
  },
  formatApprovalCommand = function (command) {
    if (!command.includes("\n") && !command.includes("`")) {
  ...
  buildRequestMessage = function (request, nowMs) {
  ...
  buildResolvedMessage = function (resolved) {
    const base = `\u2705 Exec approval ${decisionLabel(resolved.decision)}.`;
    const by = resolved.resolvedBy ? ` Resolved by ${resolved.resolvedBy}.` : "";
    return `${base}${by} ID: ${resolved.id}`;
  },
  buildExpiredMessage = function (request) {
    return `\u23F1\uFE0F Exec approval expired. ID: ${request.id}`;
  },
  defaultResolveSessionTarget = function (params) {
...
export function createExecApprovalForwarder(deps = {}) {
  ...
  const handleRequested = async (request) => {
    ...
    timeoutId.unref?.();
    const pendingEntry = { request, targets: filteredTargets, timeoutId };
