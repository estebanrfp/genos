...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-archive-"));
});
afterAll(async () => {
...
describe("archive utils", () => {
  ...
  it("extracts zip archives", async () => {
    ...
    await extractArchive({ archivePath, destDir: extractDir, timeoutMs: 5000 });
  ...
  it("rejects zip path traversal (zip slip)", async () => {
    ...
    await fs.mkdir(extractDir, { recursive: true });
    await expect(
      extractArchive({ archivePath, destDir: extractDir, timeoutMs: 5000 }),
