let decodeDnsSdEscapes = function (value) {
    let decoded = false;
    const bytes = [];
    let pending = "";
    const flush = () => {
  ...
  parseDigShortLines = function (stdout) {
    return stdout
      .split("\n")
      .map((l) => l.trim())
      .filter(Boolean);
  },
  parseDigTxt = function (stdout) {
    const tokens = [];
  ...
  parseTailscaleStatusIPv4s = function (stdout) {
  ...
  parseTxtTokens = function (tokens) {
    const txt = {};
  ...
  parseDnsSdBrowse = function (stdout) {
    ...
    for (const raw of stdout.split("\n")) {
      ...
      const match = line.match(/_genosos-gw\._tcp\.?\s+(.+)$/);
      if (match?.[1]) {
  ...
  parseDnsSdResolve = function (stdout, instanceName) {
  ...
  parseAvahiBrowse = function (stdout) {
    const results = [];
    let current = null;
    for (const raw of stdout.split("\n")) {
...
const GATEWAY_SERVICE_TYPE = "_genosos-gw._tcp";
...
async function discoverWideAreaViaTailnetDns(domain, timeoutMs, run) {
  ...
  for (const ptr of ptrs) {
    ...
    const instanceName = ptrName.replace(/\.?_genosos-gw\._tcp\..*$/, "");
