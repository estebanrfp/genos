let createEndpointFetch = function (spec) {
  ...
  findWindow = function (snapshot, label) {
...
const createAntigravityFetch = (handler) => {
  const mockFetch = vi.fn(async (input, init) => handler(toRequestUrl(input), init));
  return withFetchPreconnect(mockFetch);
};
const getRequestBody = (init) => (typeof init?.body === "string" ? init.body : undefined);
async function runUsage(mockFetch) {
...
describe("fetchAntigravityUsage", () => {
  it("returns 3 windows when both endpoints succeed", async () => {
    ...
    expect(proWindow?.usedPercent).toBe(40);
    ...
    expect(flashWindow?.usedPercent).toBeCloseTo(20, 1);
  ...
  it("returns Credits only when loadCodeAssist succeeds but fetchAvailableModels fails", async () => {
    ...
    expect(creditsWindow?.usedPercent).toBe(75);
    expect(mockFetch).toHaveBeenCalledTimes(2);
  });
  it("returns model IDs when fetchAvailableModels succeeds but loadCodeAssist fails", async () => {
    ...
    const flashWindow = findWindow(snapshot, "gemini-flash-2.0");
    expect(flashWindow?.usedPercent).toBeCloseTo(30, 1);
    expect(mockFetch).toHaveBeenCalledTimes(2);
  });
  it.each([
    {
  ...
  it("parses string numbers correctly", async () => {
    ...
    expect(creditsWindow?.usedPercent).toBe(40);
    const flashWindow = snapshot.windows.find((w) => w.label === "gemini-flash-lite");
    expect(flashWindow?.usedPercent).toBeCloseTo(10, 1);
  });
  ...
  it("sorts models by usage and shows individual model IDs", async () => {
    ...
    expect(snapshot.windows[0]?.usedPercent).toBe(70);
    expect(snapshot.windows[1]?.label).toBe("gemini-flash-1.5");
    expect(snapshot.windows[1]?.usedPercent).toBe(40);
    expect(snapshot.windows[2]?.label).toBe("gemini-pro-1.0");
    expect(snapshot.windows[2]?.usedPercent).toBeCloseTo(20, 1);
    expect(snapshot.windows[3]?.label).toBe("gemini-flash-2.0");
    expect(snapshot.windows[3]?.usedPercent).toBeCloseTo(10, 1);
  });
