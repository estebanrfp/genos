let zoneFilenameForDomain = function (domain) {
    return `${domain.replace(/\.$/, "")}.db`;
  },
  dnsLabel = function (raw, fallback) {
  ...
  nextSerial = function (existingSerial, now) {
  ...
  extractSerial = function (zoneText) {
  ...
  extractContentHash = function (zoneText) {
    const match = zoneText.match(/^\s*;\s*genosos-content-hash:\s*(\S+)\s*$/m);
    return match?.[1] ?? null;
  },
  computeContentHash = function (body) {
    let h = 2166136261;
  ...
  renderZone = function (opts) {
    const hostname = os.hostname().split(".")[0] ?? "genosos";
    const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "genosos");
    const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "genosos-gw");
    ...
    records.push(`_genosos-gw._tcp IN PTR ${instanceLabel}._genosos-gw._tcp`);
    records.push(`${instanceLabel}._genosos-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
    records.push(`${instanceLabel}._genosos-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
    ...
    return `; genosos-content-hash: ${contentHash}\n${contentBody}`;
  };
...
export function resolveWideAreaDiscoveryDomain(params) {
  ...
  const candidate = params?.configDomain ?? env.GENOS_WIDE_AREA_DOMAIN ?? null;
