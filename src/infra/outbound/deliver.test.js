...
const telegramChunkConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};
const whatsappChunkConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    ...
    const cfg = { channels: { signal: { mediaMaxMb: 2 } } };
    const results = await deliverOutboundPayloads({
      cfg,
      channel: "signal",
      to: "+1555",
  ...
  it("preserves fenced blocks for markdown chunkers in newline mode", async () => {
    const chunker = vi.fn((text) => (text ? [text] : []));
    const sendText = vi.fn().mockImplementation(async ({ text }) => ({
      channel: "matrix",
      messageId: text,
      roomId: "r1",
    }));
    const sendMedia = vi.fn().mockImplementation(async ({ text }) => ({
      channel: "matrix",
      messageId: text,
      roomId: "r1",
    }));
  ...
  it("uses iMessage media maxBytes from agent fallback", async () => {
    ...
    expect(sendIMessage).toHaveBeenCalledWith(
      "chat_id:42",
      "hello",
      expect.objectContaining({ maxBytes: 3145728 }),
    );
  });
