let normalizeQuery = function (value) {
  ...
  stripTargetPrefixes = function (value) {
    return value
      .replace(/^(channel|user):/i, "")
      .replace(/^[@#]/, "")
      .trim();
  },
  preserveTargetCase = function (channel, raw, normalized) {
    if (channel !== "slack") {
      return normalized;
    }
  ...
  detectTargetKind = function (channel, raw, preferred) {
    if (preferred) {
      return preferred;
    }
    const trimmed = raw.trim();
  ...
  pickAmbiguousMatch = function (entries, mode) {
    if (entries.length === 0) {
      return null;
    }
    if (mode === "first") {
