let makePathEnv = function (binDir) {
  ...
  makeTempDir = function () {
    return fs.mkdtempSync(path.join(os.tmpdir(), "genosos-exec-approvals-"));
  };
...
describe("resolve exec approvals defaults", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    const prevGenosOSHome = process.env.GENOS_HOME;
    try {
      process.env.GENOS_HOME = dir;
      expect(path.normalize(resolveExecApprovalsPath())).toBe(
        path.normalize(path.join(dir, ".genosv1", "exec-approvals.json")),
      );
      expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
        path.normalize(path.join(dir, ".genosv1", "exec-approvals.sock")),
      );
    } finally {
      if (prevGenosOSHome === undefined) {
        delete process.env.GENOS_HOME;
      } else {
        process.env.GENOS_HOME = prevGenosOSHome;
      }
    }
  });
});
...
describe("exec approvals shell allowlist (chained commands)", () => {
  it("allows chained commands when all parts are allowlisted", () => {
    const allowlist = [{ pattern: "/usr/bin/obsidian-cli" }, { pattern: "/usr/bin/head" }];
  ...
  it("rejects chained commands when any part is not allowlisted", () => {
    const allowlist = [{ pattern: "/usr/bin/obsidian-cli" }];
    const result = evaluateShellAllowlist({
  ...
  it("returns analysisOk=false for malformed chains", () => {
    const allowlist = [{ pattern: "/usr/bin/echo" }];
    const result = evaluateShellAllowlist({
  ...
  it("rejects windows chain separators for allowlist analysis", () => {
    const allowlist = [{ pattern: "/usr/bin/ping" }];
    const result = evaluateShellAllowlist({
      command: "ping 127.0.0.1 -n 1 & whoami",
      allowlist,
      safeBins: new Set(),
...
describe("exec approvals allowlist evaluation", () => {
  it("satisfies allowlist on exact match", () => {
    ...
    const allowlist = [{ pattern: "/usr/bin/tool" }];
...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    try {
      process.env.GENOS_HOME = dir;
      const approvalsPath = path.join(dir, ".genosv1", "exec-approvals.json");
...
describe("exec approvals node host allowlist check", () => {
  it("satisfies allowlist when command matches exact path pattern", () => {
    ...
    const entries = [{ pattern: "/usr/bin/python3" }];
    const match = matchAllowlist(entries, resolution);
  ...
  it("satisfies allowlist when command matches ** wildcard pattern", () => {
    ...
    const entries = [{ pattern: "/opt/**/python*" }];
    const match = matchAllowlist(entries, resolution);
  ...
  it("does not satisfy allowlist when command is not in allowlist", () => {
    ...
    const entries = [{ pattern: "/usr/bin/python3" }, { pattern: "/opt/**/node" }];
    const match = matchAllowlist(entries, resolution);
    expect(match).toBeNull();
