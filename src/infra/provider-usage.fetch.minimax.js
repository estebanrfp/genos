let pickNumber = function (record, keys) {
  ...
  pickString = function (record, keys) {
    for (const key of keys) {
      const value = record[key];
  ...
  hasAny = function (record, keys) {
    return keys.some((key) => key in record);
  },
  scoreUsageRecord = function (record) {
    let score = 0;
  ...
  collectUsageCandidates = function (root) {
  ...
  deriveWindowLabel = function (payload) {
  ...
  deriveUsedPercent = function (payload) {
