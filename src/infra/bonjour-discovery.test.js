...
const WIDE_AREA_DOMAIN = "genosos.internal.";
describe("bonjour-discovery", () => {
  it("discovers beacons on darwin across local + wide-area domains", async () => {
    ...
    const run = vi.fn(async (argv, options) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B") {
        if (domain === "local.") {
          return {
            stdout: [
              "Add 2 3 local. _genosos-gw._tcp. Peter\\226\\128\\153s Mac Studio Gateway",
              "Add 2 3 local. _genosos-gw._tcp. Laptop Gateway",
              "",
            ].join("\n"),
            stderr: "",
            code: 0,
        ...
        if (domain === WIDE_AREA_DOMAIN) {
          return {
            stdout: [`Add 2 3 ${WIDE_AREA_DOMAIN} _genosos-gw._tcp. Tailnet Gateway`, ""].join(
              "\n",
            ),
            stderr: "",
            code: 0,
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        ...
        return {
          stdout: [
            `${instance}._genosos-gw._tcp. can be reached at ${host}:18789`,
            txtParts.join(" "),
            "",
          ].join("\n"),
          stderr: "",
  ...
  it("decodes dns-sd octal escapes in TXT displayName", async () => {
    const run = vi.fn(async (argv, options) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B" && domain === "local.") {
        return {
          stdout: ["Add 2 3 local. _genosos-gw._tcp. Studio Gateway", ""].join("\n"),
          stderr: "",
          code: 0,
          signal: null,
          killed: false,
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        return {
          stdout: [
            "Studio Gateway._genosos-gw._tcp. can be reached at studio.local:18789",
  ...
  it("falls back to tailnet DNS probing for wide-area when split DNS is not configured", async () => {
    ...
    const serviceBase = `_genosos-gw._tcp.${zone}`;
  ...
  it("normalizes domains and respects domains override", async () => {
    ...
    await discoverGatewayBeacons({
      platform: "darwin",
      timeoutMs: 1,
      domains: ["local", "genosos.internal"],
      run,
    });
    expect(calls.filter((c) => c[1] === "-B").map((c) => c[3])).toEqual(
      expect.arrayContaining(["local.", "genosos.internal."]),
    );
    calls.length = 0;
    await discoverGatewayBeacons({
      platform: "darwin",
