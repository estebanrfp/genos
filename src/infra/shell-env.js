let resolveTimeoutMs = function (timeoutMs) {
    if (typeof timeoutMs !== "number" || !Number.isFinite(timeoutMs)) {
      return DEFAULT_TIMEOUT_MS;
    }
    return Math.max(0, timeoutMs);
  },
  resolveShell = function (env) {
    const shell = env.SHELL?.trim();
    return shell && shell.length > 0 ? shell : "/bin/sh";
  },
  execLoginShellEnvZero = function (params) {
...
export function loadShellEnvFallback(opts) {
  ...
  try {
    ...
    logger.warn(`[genosos] shell env fallback failed: ${msg}`);
    lastAppliedKeys = [];
...
export function shouldEnableShellEnvFallback(env) {
  return isTruthyEnvValue(env.GENOS_LOAD_SHELL_ENV);
}
export function shouldDeferShellEnvFallback(env) {
  return isTruthyEnvValue(env.GENOS_DEFER_SHELL_ENV_FALLBACK);
}
export function resolveShellEnvFallbackTimeoutMs(env) {
  const raw = env.GENOS_SHELL_ENV_TIMEOUT_MS?.trim();
  if (!raw) {
    return DEFAULT_TIMEOUT_MS;
  }
