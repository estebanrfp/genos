...
describe("shell env fallback", () => {
  ...
  it("is disabled by default", () => {
    expect(shouldEnableShellEnvFallback({})).toBe(false);
    expect(shouldEnableShellEnvFallback({ GENOS_LOAD_SHELL_ENV: "0" })).toBe(false);
    expect(shouldEnableShellEnvFallback({ GENOS_LOAD_SHELL_ENV: "1" })).toBe(true);
  });
  it("resolves timeout from env with default fallback", () => {
    expect(resolveShellEnvFallbackTimeoutMs({})).toBe(15000);
    expect(resolveShellEnvFallbackTimeoutMs({ GENOS_SHELL_ENV_TIMEOUT_MS: "42" })).toBe(42);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        GENOS_SHELL_ENV_TIMEOUT_MS: "nope",
      }),
    ).toBe(15000);
  });
  it("skips when already has an expected key", () => {
    const env = { OPENAI_API_KEY: "set" };
