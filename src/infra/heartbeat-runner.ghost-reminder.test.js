...
describe("Ghost reminder bug (issue #13317)", () => {
  const createConfig = async (tmpDir) => {
  ...
  const expectCronEventPrompt = (getReplySpy, reminderText) => {
    expect(getReplySpy).toHaveBeenCalledTimes(1);
    const calledCtx = getReplySpy.mock.calls[0]?.[0] ?? null;
  ...
  const runCronReminderCase = async (tmpPrefix, enqueue) => {
  ...
  it("does not use CRON_EVENT_PROMPT when only a HEARTBEAT_OK event is present", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-ghost-"));
    const sendTelegram = vi.fn().mockResolvedValue({
      messageId: "m1",
      chatId: "155462274",
    });
  ...
  it("uses CRON_EVENT_PROMPT for tagged cron events on interval wake", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cron-interval-"));
