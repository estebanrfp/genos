...
const MIN_BUN = { major: 1, minor: 2, patch: 0 };
...
export function detectRuntime() {
  ...
  const kind = isBun ? "bun" : isNode ? "node" : "unknown";
  const version = isBun ? process.versions.bun : (process.versions?.node ?? null);
  return {
    kind,
    version,
...
export function assertSupportedRuntime(runtime = defaultRuntime, details = detectRuntime()) {
  ...
  runtime.error(
    [
      "genosos requires Bun >=1.2.0 or Node >=22.12.0.",
      `Detected: ${runtimeLabel} (exec: ${execLabel}).`,
      `PATH searched: ${details.pathEnv}`,
      "Install Bun: https://bun.sh   Install Node: https://nodejs.org/en/download",
      "Upgrade your runtime and re-run genosos.",
    ].join("\n"),
  );
  runtime.exit(1);
}
