let createRunner = function (responses) {
  const calls = [];
...
describe("runGatewayUpdate", () => {
  ...
  let tempDir;
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-update-"));
  });
  afterAll(async () => {
    if (fixtureRoot) {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "genosos.mjs"), "export {};\n", "utf-8");
  });
  afterEach(async () => {});
  function createStableTagRunner(params) {
    const calls = [];
    let uiBuildCount = 0;
    const doctorKey = `${process.execPath} ${path.join(tempDir, "genosos.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options) {
    await fs.mkdir(path.join(tempDir, ".git"));
    const pkg = { name: "genosos", version: "1.0.0" };
  ...
  function buildStableTagResponses(stableTag) {
    return {
  ...
  async function runWithRunner(runner, options) {
  ...
  async function runWithCommand(runCommand, options) {
    return runGatewayUpdate({
      cwd: options?.cwd ?? tempDir,
  ...
  async function seedGlobalPackageRoot(pkgRoot, version = "1.0.0") {
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "genosos", version }),
      "utf-8",
    );
  }
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "genosos.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "genosos", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params) {
    ...
    const pkgRoot = path.join(nodeModules, "genosos");
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "genosos", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g genosos@latest",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g genosos@beta",
      channel: "beta",
    },
    {
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "genosos");
    const staleDir = path.join(nodeModules, ".genos-stale");
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "genosos");
      await seedGlobalPackageRoot(pkgRoot);
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g genosos@latest",
        onInstall: async () => {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "genosos", version: "2.0.0" }),
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g genosos@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a genosos checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    cwdSpy.mockRestore();
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-genosos-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });
  it("fails with a clear reason when genosos.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "genosos.mjs"), { force: true });
    const stableTag = "v1.0.1-1";
    ...
    expect(result.steps.at(-1)?.name).toBe("genosos doctor entry");
  });
