let hashExecApprovalsRaw = function (raw) {
    return crypto
      .createHash("sha256")
      .update(raw ?? "")
      .digest("hex");
  },
  normalizeAllowlistPattern = function (value) {
    const trimmed = value?.trim() ?? "";
    return trimmed ? trimmed.toLowerCase() : null;
  },
  mergeLegacyAgent = function (current, legacy) {
    const allowlist = [];
    const seen = new Set();
  ...
  coerceAllowlistEntries = function (allowlist) {
  ...
  ensureAllowlistIds = function (allowlist) {
    if (!Array.isArray(allowlist) || allowlist.length === 0) {
      return allowlist;
    }
    let changed = false;
  ...
  normalizeSecurity = function (value, fallback) {
    if (value === "allowlist" || value === "full" || value === "deny") {
      return value;
    }
    return fallback;
  },
  normalizeAsk = function (value, fallback) {
...
export const DEFAULT_EXEC_APPROVAL_TIMEOUT_MS = 120000;
const DEFAULT_SECURITY = "deny";
const DEFAULT_ASK = "on-miss";
const DEFAULT_ASK_FALLBACK = "deny";
const DEFAULT_AUTO_ALLOW_SKILLS = false;
const DEFAULT_SOCKET = "~/.genosv1/exec-approvals.sock";
const DEFAULT_FILE = "~/.genosv1/exec-approvals.json";
...
export function saveExecApprovals(file) {
  const filePath = resolveExecApprovalsPath();
  ensureDir(filePath);
  secureWriteFileSync(filePath, `${JSON.stringify(file, null, 2)}\n`);
  try {
