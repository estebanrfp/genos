...
let fixtureRoot = "";
let fixtureCount = 0;
const createCaseDir = async (prefix, { skipHeartbeatFile = false } = {}) => {
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-heartbeat-suite-"));
});
beforeEach(() => {
...
describe("resolveHeartbeatIntervalMs", () => {
  it("returns default when unset", () => {
    expect(resolveHeartbeatIntervalMs({})).toBe(1800000);
  });
...
describe("resolveHeartbeatDeliveryTarget", () => {
  ...
  it("prefers per-agent heartbeat overrides when provided", () => {
    ...
    const heartbeat = { target: "whatsapp", to: "+1555" };
    expect(
      resolveHeartbeatDeliveryTarget({
        cfg,
...
describe("runHeartbeatOnce", () => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("genosos-hb");
