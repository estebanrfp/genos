...
describe("format-duration", () => {
  ...
  describe("formatDurationHuman", () => {
    ...
    it("uses 24h threshold for days", () => {
      expect(formatDurationHuman(82800000)).toBe("23h");
      expect(formatDurationHuman(86400000)).toBe("1d");
      expect(formatDurationHuman(90000000)).toBe("1d");
    });
  });
...
describe("format-datetime", () => {
  ...
  describe("formatZonedTimestamp", () => {
    ...
    ])("formats zoned timestamp", ({ date, options, expected }) => {
...
describe("format-relative", () => {
  describe("formatTimeAgo", () => {
    ...
    it("uses 48h threshold before switching to days", () => {
      expect(formatTimeAgo(169200000)).toBe("47h ago");
      expect(formatTimeAgo(172800000)).toBe("2d ago");
    });
  });
  describe("formatRelativeTimestamp", () => {
    ...
    it.each([
      { offsetMs: -1e4, expected: "just now" },
    ...
    it("falls back to date for old timestamps when enabled", () => {
      const oldDate = Date.now() - 2592000000;
