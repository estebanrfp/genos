...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cost-"));
    const sessionsDir = path.join(root, "agents", "main", "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const sessionFile = path.join(sessionsDir, "sess-1.jsonl");
    const now = new Date();
    const older = new Date(Date.now() - 3456000000);
    const entries = [
      {
        type: "message",
    ...
    const originalState = process.env.GENOS_STATE_DIR;
    process.env.GENOS_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) {
        ...
        process.env.GENOS_STATE_DIR = originalState;
      }
    }
  });
  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cost-session-"));
  ...
  it("captures message counts, tool usage, and model usage", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cost-session-meta-"));
    const sessionFile = path.join(root, "session.jsonl");
    ...
    expect(summary?.latency?.avgMs).toBe(300000);
    expect(summary?.latency?.p95Ms).toBe(300000);
  ...
  it("does not exclude sessions with mtime after endMs during discovery", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-discover-"));
    const sessionsDir = path.join(root, "agents", "main", "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("resolves non-main absolute sessionFile using explicit agentId for cost summary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cost-agent-"));
  ...
  it("resolves non-main absolute sessionFile using explicit agentId for timeseries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-timeseries-agent-"));
  ...
  it("resolves non-main absolute sessionFile using explicit agentId for logs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-logs-agent-"));
  ...
  it("preserves totals and cumulative values when downsampling timeseries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-timeseries-downsample-"));
    const sessionsDir = path.join(root, "agents", "main", "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
