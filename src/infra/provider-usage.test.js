...
async function expectMinimaxUsage(payload, expectedUsedPercent, expectedPlan) {
...
describe("provider usage formatting", () => {
  it("returns null when no usage is available", () => {
    const summary = { updatedAt: 0, providers: [] };
...
describe("provider usage loading", () => {
  ...
  it("discovers Claude usage from token auth profiles", async () => {
    await withTempHome(
      ...
      {
        env: {
          GENOS_STATE_DIR: (home) => path.join(home, ".genosv1"),
        },
        prefix: "genosos-provider-usage-",
      },
    );
  });
