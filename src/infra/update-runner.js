let normalizeDir = function (value) {
    if (!value) {
      return null;
    }
  ...
  resolveNodeModulesBinPackageRoot = function (argv1) {
  ...
  managerScriptArgs = function (manager, script, args = []) {
    if (manager === "pnpm") {
  ...
  managerInstallArgs = function (manager) {
    if (manager === "pnpm") {
      return ["pnpm", "install"];
    }
    if (manager === "bun") {
...
const DEFAULT_PACKAGE_NAME = "genosos";
...
export async function runGatewayUpdate(opts = {}) {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "genosos.mjs");
    ...
    const doctorStep = await runStep(
      step("genosos doctor", doctorArgv, gitRoot, { GENOS_UPDATE_IN_PROGRESS: "1" }),
    );
