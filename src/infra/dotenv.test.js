...
async function writeEnvFile(filePath, contents) {
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  await fs.writeFile(filePath, contents, "utf8");
}
async function withIsolatedEnvAndCwd(run) {
...
describe("loadDotEnv", () => {
  it("loads ~/.genosv1/.env as fallback without overriding CWD .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-dotenv-test-"));
