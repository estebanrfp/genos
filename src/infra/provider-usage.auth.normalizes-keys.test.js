...
describe("resolveProviderAuths key normalization", () => {
  ...
  let suiteCase = 0;
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-provider-auth-suite-"));
  });
  afterAll(async () => {
  ...
  async function withSuiteHome(fn, env) {
    const base = path.join(suiteRoot, `case-${++suiteCase}`);
    await fs.mkdir(base, { recursive: true });
    await fs.mkdir(path.join(base, ".genosv1", "agents", "main", "sessions"), { recursive: true });
    ...
    process.env.GENOS_STATE_DIR = path.join(base, ".genosv1");
  ...
  it("strips embedded CR/LF from stored auth profiles (token + api_key)", async () => {
    await withSuiteHome(
      async (home) => {
        const agentDir = path.join(home, ".genosv1", "agents", "main", "agent");
