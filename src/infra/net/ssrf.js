let normalizeHostnameSet = function (values) {
  ...
  isHostnameAllowedByPattern = function (hostname, pattern) {
  ...
  matchesHostnameAllowlist = function (hostname, allowlist) {
    if (allowlist.length === 0) {
      return true;
    }
    return allowlist.some((pattern) => isHostnameAllowedByPattern(hostname, pattern));
  },
  parseIpv4 = function (address) {
  ...
  stripIpv6ZoneId = function (address) {
    const index = address.indexOf("%");
    return index >= 0 ? address.slice(0, index) : address;
  },
  parseIpv6Hextets = function (address) {
  ...
  decodeIpv4FromHextets = function (high, low) {
    return [(high >>> 8) & 255, high & 255, (low >>> 8) & 255, low & 255];
  },
  extractIpv4FromEmbeddedIpv6 = function (hextets) {
