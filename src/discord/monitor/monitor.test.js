...
describe("discord component interactions", () => {
  ...
  const createModalInteraction = (overrides = {}) => {
    ...
    const fields = {
      getText: (key) => (key === "fld_1" ? "Casey" : undefined),
      getStringSelect: (_key) => {
        return;
      },
      getRoleSelect: (_key) => [],
      getUserSelect: (_key) => [],
  ...
  beforeEach(() => {
    ...
    upsertPairingRequestMock.mockReset().mockResolvedValue({ code: "PAIRCODE", created: true });
    enqueueSystemEventMock.mockReset();
    dispatchReplyMock.mockReset().mockImplementation(async (params) => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/genosos-sessions-test.json");
  });
