let resolveToolStatusEmoji = function (toolName) {
  ...
  createDiscordStatusReactionController = function (params) {
    ...
    const scheduleStallTimers = () => {
      ...
      softStallTimer = setTimeout(() => {
        ...
        requestEmoji(DISCORD_STATUS_STALL_SOFT_EMOJI, { immediate: true });
      }, DISCORD_STATUS_STALL_SOFT_MS);
      hardStallTimer = setTimeout(() => {
        if (finished) {
          return;
        }
        requestEmoji(DISCORD_STATUS_STALL_HARD_EMOJI, { immediate: true });
...
const DISCORD_STATUS_STALL_SOFT_MS = 1e4;
const DISCORD_STATUS_STALL_HARD_MS = 30000;
