let encodeCustomIdValue = function (value) {
    return encodeURIComponent(value);
  },
  decodeCustomIdValue = function (value) {
    try {
  ...
  resolveExecApprovalAccountId = function (params) {
  ...
  buildExecApprovalMetadataLines = function (request) {
    const lines = [];
  ...
  buildExecApprovalPayload = function (container) {
    const components = [container];
    return { components };
  },
  createExecApprovalRequestContainer = function (params) {
  ...
  createResolvedContainer = function (params) {
    const commandText = params.request.request.command;
    const commandPreview =
  ...
  createExpiredContainer = function (params) {
    const commandText = params.request.request.command;
    const commandPreview =
      commandText.length > 500 ? `${commandText.slice(0, 500)}...` : commandText;
    return new ExecApprovalContainer({
