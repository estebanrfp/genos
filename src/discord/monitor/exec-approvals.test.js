let createHandler = function (config, accountId = "default") {
  ...
  getHandlerInternals = function (handler) {
    return handler;
  },
  clearPendingTimeouts = function (handler) {
  ...
  createRequest = function (overrides = {}) {
    return {
      id: "test-id",
      request: {
        command: "echo hello",
...
const STORE_PATH = path.join(os.tmpdir(), "genosos-exec-approvals-test.json");
...
describe("parseExecApprovalData", () => {
  ...
  it("rejects null/undefined input", () => {
    ...
    expect(parseExecApprovalData(undefined)).toBeNull();
  });
...
describe("DiscordExecApprovalHandler delivery routing", () => {
  ...
  it("falls back to DM delivery when channel target has no channel id", async () => {
    ...
    const internals = getHandlerInternals(handler);
    mockRestPost.mockImplementation(async (route) => {
