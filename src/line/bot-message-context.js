let buildPeerId = function (source) {
  ...
  resolveLineInboundRoute = function (params) {
  ...
  describeStickerKeywords = function (sticker) {
  ...
  extractMediaPlaceholder = function (message) {
    switch (message.type) {
      case "image":
        return "<media:image>";
      case "video":
  ...
  resolveLineConversationLabel = function (params) {
  ...
  resolveLineAddresses = function (params) {
...
export async function buildLineMessageContext(params) {
  ...
  const { ctxPayload } = await finalizeLineInboundContext({
    ...
    media: {
      ...
      types: allMedia.length > 0 ? allMedia.map((m) => m.contentType).filter(Boolean) : undefined,
    },
    locationContext,
