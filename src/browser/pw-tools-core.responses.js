let matchUrlPattern = function (pattern, url) {
...
export async function responseBodyViaPlaywright(opts) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `Response not found for url pattern "${pattern}". Run '${formatCliCommand("genosos browser requests")}' to inspect recent network activity.`,
        ),
      );
    }, timeout);
  });
