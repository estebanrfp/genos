...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createMissingNameBody = await createMissingName.json();
    ...
    const createInvalidNameBody = await createInvalidName.json();
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "genosos" }),
    });
    expect(createDuplicate.status).toBe(409);
    const createDuplicateBody = await createDuplicate.json();
    ...
    const createRemoteBody = await createRemote.json();
    ...
    const createBadRemoteBody = await createBadRemote.json();
    ...
    const deleteDefault = await realFetch(`${base}/profiles/genosos`, {
      method: "DELETE",
    });
    expect(deleteDefault.status).toBe(400);
    const deleteDefaultBody = await deleteDefault.json();
    ...
    const deleteInvalidBody = await deleteInvalid.json();
