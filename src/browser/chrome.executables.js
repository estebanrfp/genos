let exists = function (filePath) {
  ...
  execText = function (command, args, timeoutMs = 1200, maxBuffer = 1048576) {
    try {
  ...
  inferKindFromIdentifier = function (identifier) {
  ...
  inferKindFromExecutableName = function (name) {
  ...
  detectDefaultChromiumExecutable = function (platform) {
    if (platform === "darwin") {
  ...
  detectDefaultChromiumExecutableMac = function () {
  ...
  detectDefaultBrowserBundleIdMac = function () {
  ...
  detectDefaultChromiumExecutableLinux = function () {
    const desktopId =
  ...
  detectDefaultChromiumExecutableWindows = function () {
  ...
  findDesktopFilePath = function (desktopId) {
    const candidates = [
  ...
  readDesktopExecLine = function (desktopPath) {
    try {
  ...
  extractExecutableFromExecLine = function (execLine) {
  ...
  resolveLinuxExecutablePath = function (command) {
  ...
  readWindowsCommandForProgId = function (progId) {
    const key =
      progId === "http"
  ...
  expandWindowsEnvVars = function (value) {
    return value.replace(/%([^%]+)%/g, (_match, name) => {
      const key = String(name ?? "").trim();
      return key ? (process.env[key] ?? `%${key}%`) : _match;
    });
  },
  extractWindowsExecutablePath = function (command) {
  ...
  findFirstExecutable = function (candidates) {
    for (const candidate of candidates) {
