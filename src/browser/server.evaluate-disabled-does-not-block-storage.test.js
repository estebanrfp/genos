...
let testPort = 0;
let prevGatewayPort;
let prevGatewayToken;
let prevGatewayPassword;
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          genosos: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.GENOS_GATEWAY_PORT;
    process.env.GENOS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.GENOS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.GENOS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.GENOS_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.GENOS_GATEWAY_TOKEN;
    } else {
      process.env.GENOS_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.GENOS_GATEWAY_PASSWORD;
    } else {
      process.env.GENOS_GATEWAY_PASSWORD = prevGatewayPassword;
    }
  ...
  it("blocks act:evaluate but still allows cookies/storage reads", async () => {
    ...
    const cookiesRes = await realFetch(`${base}/cookies`).then((r) => r.json());
    ...
    const storageRes = await realFetch(`${base}/storage/local?key=token`).then((r) => r.json());
