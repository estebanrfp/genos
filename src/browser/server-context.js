let normalizeWsUrl = function (raw, cdpBaseUrl) {
    if (!raw) {
      return;
    }
    try {
  ...
  createProfileContext = function (opts, profile) {
    const state = () => {
    ...
    const ensureBrowserAvailable = async () => {
      ...
      if (isExtension) {
        ...
        throw new Error(
          `Chrome extension relay is running, but no tab is connected. Click the GenosOS Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
        );
      }
      if (!httpReachable) {
        ...
        const launched = await launchGenosOSChrome(current.resolved, profile);
      ...
      if (!profileState.running) {
        throw new Error(
          `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by genosos. Run action=reset-profile profile=${profile.name} to kill the process.`,
        );
      }
      if (current.resolved.attachOnly || remoteCdp) {
      ...
      await stopGenosOSChrome(profileState.running);
      setProfileRunning(null);
      const relaunched = await launchGenosOSChrome(current.resolved, profile);
    ...
    const ensureTabAvailable = async (targetId) => {
      ...
      if (tabs1.length === 0) {
        if (profile.driver === "extension") {
          throw new Error(
            `tab not found (no attached Chrome tabs for profile "${profile.name}"). Click the GenosOS Browser Relay toolbar icon on the tab you want to control (badge ON).`,
          );
        }
    ...
    const resetProfile = async () => {
      ...
      const userDataDir = resolveGenosOSUserDataDir(profile.name);
      const profileState = getProfileState();
