...
describe("browser config", () => {
  ...
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const genosos = resolveProfile(resolved, "genosos");
    expect(genosos?.driver).toBe("genosos");
    expect(genosos?.cdpPort).toBe(18800);
    expect(genosos?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });
  it("derives default ports from GENOS_GATEWAY_PORT when unset", () => {
    const prev = process.env.GENOS_GATEWAY_PORT;
    process.env.GENOS_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const genosos = resolveProfile(resolved, "genosos");
      expect(genosos?.cdpPort).toBe(19012);
      expect(genosos?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.GENOS_GATEWAY_PORT;
      } else {
        process.env.GENOS_GATEWAY_PORT = prev;
      }
    }
  });
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const genosos = resolveProfile(resolved, "genosos");
      expect(genosos?.cdpPort).toBe(19022);
      expect(genosos?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.GENOS_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "genosos");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        genosos: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("genosos");
  });
  ...
  it("filters out non-string entries from extraArgs", () => {
    const resolved = resolveBrowserConfig({
      extraArgs: ["--flag", 42, null, undefined, true, "--other"],
    });
    expect(resolved.extraArgs).toEqual(["--flag", "--other"]);
  });
