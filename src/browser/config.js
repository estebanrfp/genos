let normalizeHexColor = function (raw) {
    const value = (raw ?? "").trim();
    if (!value) {
      return DEFAULT_GENOS_BROWSER_COLOR;
    }
  ...
  normalizeTimeoutMs = function (raw, fallback) {
    const value = typeof raw === "number" && Number.isFinite(raw) ? Math.floor(raw) : fallback;
    return value < 0 ? fallback : value;
  },
  ensureDefaultProfile = function (profiles, defaultColor, legacyCdpPort, derivedDefaultCdpPort) {
    const result = { ...profiles };
    if (!result[DEFAULT_GENOS_BROWSER_PROFILE_NAME]) {
      result[DEFAULT_GENOS_BROWSER_PROFILE_NAME] = {
  ...
  ensureDefaultChromeExtensionProfile = function (profiles, controlPort) {
    const result = { ...profiles };
    if (result.chrome) {
      return result;
    }
...
import {
  DEFAULT_GENOS_BROWSER_COLOR,
  DEFAULT_GENOS_BROWSER_ENABLED,
...
export function resolveBrowserConfig(cfg, rootConfig) {
  const enabled = cfg?.enabled ?? DEFAULT_GENOS_BROWSER_ENABLED;
...
export function resolveProfile(resolved, profileName) {
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "genosos";
  if (rawProfileUrl) {
