...
describe("fetchBrowserJson loopback auth (bridge auth registry)", () => {
  it("falls back to per-port bridge auth when config auth is not available", async () => {
    const port = 18765;
    const getBridgeAuthForPort = vi.fn((candidate) =>
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!genosos) {
      throw new Error("expected genosos profile");
    }
    const state = {
      server: null,
      port: 18791,
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...genos, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
    };
    expect(listKnownProfileNames(state).toSorted()).toEqual(["chrome", "genosos", "stale-removed"]);
  });
});
