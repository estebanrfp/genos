...
describe("browser control server", () => {
  installAgentContractHooks();
  const slowTimeoutMs = process.platform === "win32" ? 40000 : 20000;
  it(
  ...
  it("agent contract: hooks + response + downloads + screenshot", async () => {
    ...
    const consoleRes = await realFetch(`${base}/console?level=error`).then((r) => r.json());
  ...
  it("blocks file chooser traversal / absolute paths outside uploads dir", async () => {
    const base = await startServerAndBase();
    const traversal = await postJson(`${base}/hooks/file-chooser`, {
      paths: ["../../../../etc/passwd"],
    });
    ...
    const abs = await postJson(`${base}/hooks/file-chooser`, {
      paths: [absOutside],
    });
  ...
  it("trace stop rejects traversal path outside trace dir", async () => {
    const base = await startServerAndBase();
    const res = await postJson(`${base}/trace/stop`, {
      path: "../../pwned.zip",
    });
  ...
  it("wait/download rejects traversal path outside downloads dir", async () => {
    const base = await startServerAndBase();
    const waitRes = await postJson(`${base}/wait/download`, {
      path: "../../pwned.pdf",
    });
  ...
  it("download rejects traversal path outside downloads dir", async () => {
    const base = await startServerAndBase();
    const downloadRes = await postJson(`${base}/download`, {
      ref: "e12",
      path: "../../pwned.pdf",
    });
  ...
  it("wait/download accepts in-root relative output path", async () => {
    const base = await startServerAndBase();
    const res = await postJson(`${base}/wait/download`, {
      path: "safe-wait.pdf",
    });
    expect(res.ok).toBe(true);
  ...
  it("download accepts in-root relative output path", async () => {
    const base = await startServerAndBase();
    const res = await postJson(`${base}/download`, {
      ref: "e12",
