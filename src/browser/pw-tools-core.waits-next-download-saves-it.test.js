...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredGenosOSTmpDir: vi.fn(() => "/tmp/genosos"),
}));
vi.mock("../infra/tmp-genosos-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredGenosOSTmpDir.mockReturnValue("/tmp/genosos");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredGenosOSTmpDir.mockReturnValue("/tmp/genosos-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "genosos-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "genosos-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredGenosOSTmpDir).toHaveBeenCalled();
  });
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "genosos-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "genosos-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it("scrolls a ref into view (default timeout)", async () => {
    ...
    expect(scrollIntoViewIfNeeded).toHaveBeenCalledWith({ timeout: 20000 });
  });
