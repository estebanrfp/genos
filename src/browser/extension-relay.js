let headerValue = function (value) {
    if (!value) {
      return;
    }
  ...
  getHeader = function (req, name) {
    return headerValue(req.headers[name.toLowerCase()]);
  },
  getRelayAuthTokenFromRequest = function (req, url) {
  ...
  text = function (res, status, bodyText) {
  ...
  rejectUpgrade = function (socket, status, bodyText) {
    text(socket, status, bodyText);
    try {
      socket.destroy();
    } catch {}
  },
  resolveGatewayAuthToken = function () {
    const envToken =
      process.env.GENOS_GATEWAY_TOKEN?.trim() || process.env.GENOS_GATEWAY_TOKEN?.trim();
    if (envToken) {
      return envToken;
    }
    try {
  ...
  resolveRelayAuthToken = function () {
    const gatewayToken = resolveGatewayAuthToken();
    if (gatewayToken) {
      return gatewayToken;
    }
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or GENOS_GATEWAY_TOKEN)",
    );
  },
  isAddrInUseError = function (err) {
    return typeof err === "object" && err !== null && "code" in err && err.code === "EADDRINUSE";
  },
  relayAuthTokenForUrl = function (url) {
    try {
...
const RELAY_AUTH_HEADER = "x-genosos-relay-token";
const serversByPort = new Map();
async function looksLikeGenosOSRelay(baseUrl) {
...
export async function ensureChromeExtensionRelayServer(opts) {
  ...
  const routeCdpCommand = async (cmd) => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/GenosOS-Extension-Relay",
          revision: "0",
          userAgent: "GenosOS-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload = {
        Browser: "GenosOS/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeGenosOSRelay(info.baseUrl))) {
      const existingRelay = {
        host: info.host,
        port: info.port,
