...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved, name) => {
      if (name === "chrome") {
        return { driver: "extension", cdpUrl: "http://127.0.0.1:18888" };
      }
      return { driver: "genosos", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  ...
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["genosos", "chrome"]);
    const stopMap = {
      genosos: vi.fn(async () => {}),
      chrome: vi.fn(async () => {
    ...
    expect(stopMap.genos).toHaveBeenCalledTimes(1);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    const onWarn = vi.fn();
    await stopKnownBrowserProfiles({
      getState: () => ({ resolved: { profiles: {} }, profiles: new Map() }),
      onWarn,
    });
    expect(onWarn).toHaveBeenCalledWith("genosos browser stop failed: Error: oops");
  });
});
