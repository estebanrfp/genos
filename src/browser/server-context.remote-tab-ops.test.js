let makeState = function (profile) {
    return {
      server: null,
      port: 0,
      resolved: {
  ...
  makeUnexpectedFetchMock = function () {
    return vi.fn(async () => {
      throw new Error("unexpected fetch");
    });
  },
  createRemoteRouteHarness = function (fetchMock) {
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("genosos");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const genosos = ctx.forProfile("genosos");
    const opened = await genosos.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("genosos")?.lastTargetId).toBe("CREATED");
  });
});
