...
export const DEFAULT_BROWSER_SCREENSHOT_MAX_BYTES = 5242880;
export async function normalizeBrowserScreenshot(buffer, opts) {
  ...
  throw new Error(
    `Browser screenshot could not be reduced below ${(maxBytes / 1048576).toFixed(0)}MB (got ${(best.byteLength / 1048576).toFixed(2)}MB)`,
  );
}
