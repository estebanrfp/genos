let resolveTabsProfileContext = function (req, res, ctx) {
  ...
  handleTabsRouteError = function (ctx, res, err, opts) {
    if (opts?.mapTabError) {
  ...
  resolveIndexedTab = function (tabs, index) {
    return typeof index === "number" ? tabs[index] : tabs.at(0);
  },
  parseRequiredTargetId = function (res, rawTargetId) {
