let resolveDirectOrGroupChannelId = function (context) {
  ...
  buildIMessageThreadToolContext = function (params) {
    return {
  ...
  resolveCaseInsensitiveAccount = function (accounts, accountId) {
    if (!accounts) {
      return;
    }
  ...
  buildDockFromPlugin = function (plugin) {
    return {
      id: plugin.id,
  ...
  listPluginDockEntries = function () {
...
const formatDiscordAllowFrom = (allowFrom) =>
  allowFrom
    .map((entry) =>
      String(entry)
        .trim()
