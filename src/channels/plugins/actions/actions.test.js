let telegramCfg = function () {
    return { channels: { telegram: { botToken: "tok" } } };
  },
  slackHarness = function () {
    const cfg = { channels: { slack: { botToken: "tok" } } };
    const actions = createSlackActions("slack");
    return { cfg, actions };
  },
  expectFirstSlackAction = function (expected) {
    const [params] = handleSlackAction.mock.calls[0] ?? [];
    expect(params).toMatchObject(expected);
  },
  expectModerationActions = function (actions) {
    expect(actions).toContain("timeout");
    expect(actions).toContain("kick");
    expect(actions).toContain("ban");
  };
import { beforeEach, describe, expect, it, vi } from "vitest";
const handleDiscordAction = vi.fn(async (..._args) => ({ details: { ok: true } }));
const handleTelegramAction = vi.fn(async (..._args) => ({ ok: true }));
const sendReactionSignal = vi.fn(async (..._args) => ({ ok: true }));
const removeReactionSignal = vi.fn(async (..._args) => ({ ok: true }));
const handleSlackAction = vi.fn(async (..._args) => ({ details: { ok: true } }));
...
async function runSlackAction(action, params) {
...
async function expectSlackSendRejected(params, error) {
  const { cfg, actions } = slackHarness();
  await expect(
