let setSlackDmPolicy = function (cfg, dmPolicy) {
  ...
  buildSlackManifest = function (botName) {
    const safeName = botName.trim() || "GenosOS";
    const manifest = {
      display_information: {
        name: safeName,
        description: `${safeName} connector for GenosOS`,
      },
      features: {
        bot_user: {
  ...
  patchSlackConfigForAccount = function (cfg, accountId, patch) {
  ...
  setSlackGroupPolicy = function (cfg, accountId, groupPolicy) {
    return patchSlackConfigForAccount(cfg, accountId, { groupPolicy });
  },
  setSlackChannelAllowlist = function (cfg, accountId, channelKeys) {
    const channels = Object.fromEntries(channelKeys.map((key) => [key, { allow: true }]));
    return patchSlackConfigForAccount(cfg, accountId, { channels });
  },
  setSlackAllowFrom = function (cfg, allowFrom) {
    return {
      ...cfg,
      channels: {
        ...cfg.channels,
  ...
  parseSlackAllowFromInput = function (raw) {
    return raw
      .split(/[\n,;]+/g)
