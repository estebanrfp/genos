let setSignalDmPolicy = function (cfg, dmPolicy) {
    const allowFrom =
  ...
  setSignalAllowFrom = function (cfg, accountId, allowFrom) {
  ...
  parseSignalAllowFromInput = function (raw) {
    return raw
      .split(/[\n,;]+/g)
      .map((entry) => entry.trim())
      .filter(Boolean);
  },
  isUuidLike = function (value) {
...
async function promptSignalAllowFrom(params) {
  ...
  const unique = mergeAllowFromEntries(
    undefined,
    normalized.filter((part) => typeof part === "string" && part.trim().length > 0),
  );
...
export const signalOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "GenosOS"',
        "Scan QR in Signal \u2192 Linked Devices",
        `Then run: ${formatCliCommand("genosos gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
