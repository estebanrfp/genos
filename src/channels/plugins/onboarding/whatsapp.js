let setWhatsAppDmPolicy = function (cfg, dmPolicy) {
    return mergeWhatsAppConfig(cfg, { dmPolicy });
  },
  setWhatsAppAllowFrom = function (cfg, allowFrom) {
    return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
  },
  setWhatsAppSelfChatMode = function (cfg, selfChatMode) {
...
async function promptWhatsAppOwnerAllowFrom(params) {
  ...
  await prompter.note(
    "We need the sender/owner number so GenosOS can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppAllowFrom(cfg, _runtime, prompter, options) {
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for GenosOS" },
    ],
  });
...
export const whatsappOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("genosos channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
