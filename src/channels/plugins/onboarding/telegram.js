let setTelegramDmPolicy = function (cfg, dmPolicy) {
  const allowFrom =
...
async function noteTelegramUserIdHelp(prompter) {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("genosos logs --follow")}\` (safest)`,
  ...
  getStatus: async ({ cfg }) => {
    ...
    return {
      ...
      selectionHint: configured
        ? "recommended \xB7 configured"
        : "recommended \xB7 newcomer-friendly",
