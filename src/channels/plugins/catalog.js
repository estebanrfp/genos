let parseCatalogEntries = function (raw) {
  ...
  resolveExternalCatalogPaths = function (options) {
  ...
  loadExternalCatalogEntries = function (options) {
    ...
    for (const rawPath of paths) {
      ...
      try {
        const payload = JSON.parse(secureReadFileSync(resolved));
        entries.push(...parseCatalogEntries(payload));
      } catch {}
    }
    return entries;
  },
  toChannelMeta = function (params) {
  ...
  resolveInstallInfo = function (params) {
  ...
  buildCatalogEntry = function (candidate) {
  ...
  buildExternalCatalogEntry = function (entry) {
...
import { discoverGenosOSPlugins } from "../../plugins/discovery.js";
...
const ENV_CATALOG_PATHS = ["GENOS_PLUGIN_CATALOG_PATHS", "GENOS_MPM_CATALOG_PATHS"];
...
export function listChannelPluginCatalogEntries(options = {}) {
  const discovery = discoverGenosOSPlugins({ workspaceDir: options.workspaceDir });
