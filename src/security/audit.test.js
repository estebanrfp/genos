let stubChannelPlugin = function (params) {
    return {
      id: params.id,
      meta: {
        id: params.id,
      ...
      config: {
        listAccountIds: (cfg) => {
          const enabled = Boolean(cfg.channels?.[params.id]);
  ...
  successfulProbeResult = function (url) {
    return {
      ok: true,
      url,
      connectLatencyMs: 1,
...
describe("security audit", () => {
  let fixtureRoot = "";
  let caseId = 0;
  const makeTmpDir = async (label) => {
    const dir = path.join(fixtureRoot, `case-${caseId++}-${label}`);
    await fs.mkdir(dir, { recursive: true });
    return dir;
  };
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-security-audit-"));
  });
  afterAll(async () => {
    if (!fixtureRoot) {
      return;
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.GENOS_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevPassword === undefined) {
        ...
        process.env.GENOS_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "genosos.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
    const user = "DESKTOP-TEST\\Tester";
    const execIcacls = async (_cmd, args) => ({
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true, mode: 448 });
    const targetConfigPath = path.join(tmp, "managed-genosos.json");
    await fs.writeFile(targetConfigPath, "{}\n", "utf-8");
    await fs.chmod(targetConfigPath, 292);
    const configPath = path.join(stateDir, "genosos.json");
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.GENOS_STATE_DIR;
    const tmp = await makeTmpDir("discord");
    process.env.GENOS_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 448 });
    try {
      const cfg = {
        channels: {
          discord: {
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.GENOS_GATEWAY_TOKEN;
    process.env.GENOS_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.genos",
      configPath: "/Users/test/Dropbox/.genos/genosos.json",
    });
    expect(res.findings).toEqual(
      expect.arrayContaining([
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    if (isWindows) {
      ...
      await fs.chmod(includePath, 420);
    }
    const configPath = path.join(stateDir, "genosos.json");
    await fs.writeFile(configPath, `{ "$include": "./extra.json5" }\n`, "utf-8");
    await fs.chmod(configPath, 384);
    try {
      const cfg = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "genosos.json"),
      });
      expect(res.findings).toEqual(
        expect.arrayContaining([
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        genosos: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.GENOS_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.GENOS_GATEWAY_PASSWORD;
    beforeEach(() => {
      delete process.env.GENOS_GATEWAY_TOKEN;
      delete process.env.GENOS_GATEWAY_PASSWORD;
    });
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.GENOS_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.GENOS_GATEWAY_PASSWORD;
      } else {
        process.env.GENOS_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.GENOS_GATEWAY_TOKEN = "env-token";
      let capturedAuth;
      const cfg = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.GENOS_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth;
      const cfg = {
        gateway: {
          mode: "remote",
