let expandTilde = function (p, env) {
    if (!p.startsWith("~")) {
      return p;
    }
  ...
  formatCodeSafetyDetails = function (findings, rootDir) {
    return findings
      .map((finding) => {
  ...
  resolveToolPolicies = function (params) {
    ...
    const policies = [profilePolicy];
    return policies;
  },
  normalizePluginIdSet = function (entries) {
    return new Set(entries.map((entry) => entry.trim().toLowerCase()).filter(Boolean));
  },
  resolveEnabledExtensionPluginIds = function (params) {
  ...
  collectAllowEntries = function (config) {
    const out = [];
...
export async function collectPluginsTrustFindings(params) {
  ...
  if (enabledExtensionPluginIds.length > 0) {
    ...
    for (const context of contexts) {
      ...
      const broadPolicy = isToolAllowedByPolicies("__genosos_plugin_probe__", policies);
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `genosos security audit --deep`.",
      });
    },
  });
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all genosos.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your GenosOS extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params) {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (entry.skill.source === "genosos-bundled") {
        continue;
      }
      ...
      const summary = await skillScanner.scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `genosos security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
...
/**
 * Collect macOS-specific security findings (FileVault, Spotlight, Time Machine).
 * @param {{ stateDir: string }} params
 * @returns {Promise<Array<{ checkId: string, severity: string, title: string, detail: string, remediation: string }>>}
 */
export async function collectMacOsSecurityFindings(params) {
  const findings = [];
  ...
  // FileVault check
  try {
    const output = execFileSync("fdesetup", ["status"], { encoding: "utf-8" });
    if (!output.includes("FileVault is On")) {
      findings.push({
        checkId: "macos.filevault_disabled",
        severity: "critical",
        title: "FileVault disk encryption is not enabled",
        detail:
          "FileVault is not active. Without full-disk encryption, vault data can be read from a powered-off machine.",
        remediation: "Enable FileVault in System Settings > Privacy & Security > FileVault.",
      });
    }
  } catch {
    findings.push({
      checkId: "macos.filevault_check_failed",
      severity: "warn",
      title: "Could not determine FileVault status",
      detail: "fdesetup status command failed. This may require running as an admin user.",
      remediation: "Run `fdesetup status` manually to verify FileVault is enabled.",
    });
  }

  // Spotlight exclusion check
  const sentinel = path.join(params.stateDir, ".metadata_never_index");
  try {
    await fs.access(sentinel);
  } catch {
    findings.push({
      checkId: "macos.spotlight_not_excluded",
      severity: "warn",
      title: "State directory may be indexed by Spotlight",
      detail: `Missing .metadata_never_index sentinel in ${params.stateDir}. Spotlight may index sensitive file contents.`,
      remediation:
        "Restart the gateway to auto-create the sentinel, or run: touch ~/.genosv1/.metadata_never_index",
    });
  }

  // Time Machine exclusion check
  try {
    const output = execFileSync("tmutil", ["isexcluded", params.stateDir], { encoding: "utf-8" });
    if (!output.includes("[Excluded]")) {
      findings.push({
        checkId: "macos.timemachine_not_excluded",
        severity: "warn",
        title: "State directory is not excluded from Time Machine",
        detail: `${params.stateDir} may be backed up by Time Machine without additional encryption.`,
        remediation:
          "Run: tmutil addexclusion ~/.genos or restart the gateway to set the xattr automatically.",
      });
    }
  } catch {
    // tmutil may not be available or may fail — non-critical
  }
