let countBySeverity = function (findings) {
    let critical = 0;
    let warn = 0;
    let info = 0;
  ...
  normalizeAllowFromList = function (list) {
    if (!Array.isArray(list)) {
      return [];
    }
    return list.map((v) => String(v).trim()).filter(Boolean);
  },
  collectGatewayConfigFindings = function (cfg, env) {
    const findings = [];
    ...
    if (bind === "loopback" && controlUiEnabled && trustedProxies.length === 0) {
      findings.push({
        ...
        detail:
          "gateway.bind is loopback and gateway.trustedProxies is empty. Only relevant if you expose the Control UI through a reverse proxy.",
        remediation:
          "No action needed for local-only usage. Set gateway.trustedProxies if you add a reverse proxy.",
      });
    }
  ...
  collectBrowserControlFindings = function (cfg, env) {
    const findings = [];
    let resolved;
    try {
      ...
      findings.push({
        ...
        detail: String(err),
        remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("genosos security audit --deep")}".`,
      });
      return findings;
    }
    if (!resolved.enabled) {
  ...
  collectLoggingFindings = function (cfg) {
  ...
  collectElevatedFindings = function (cfg) {
    const findings = [];
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your GenosOS state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your GenosOS state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export async function runSecurityAudit(opts) {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("genosos status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("genosos security audit --deep")}".`,
    });
  }
