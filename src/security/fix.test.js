...
const expectPerms = (actual, expected) => {
  if (isWindows) {
    expect([expected, 438, 511]).toContain(actual);
    return;
  }
...
describe("security fix", () => {
  let fixtureRoot = "";
  let fixtureCount = 0;
  const createStateDir = async (prefix) => {
    const dir = path.join(fixtureRoot, `${prefix}-${fixtureCount++}`);
    await fs.mkdir(dir, { recursive: true });
    return dir;
  };
  const createFixEnv = (stateDir, configPath) => ({
    ...process.env,
    GENOS_STATE_DIR: stateDir,
    GENOS_CONFIG_PATH: configPath,
  });
  const writeJsonConfig = async (configPath, config) => {
    await fs.writeFile(configPath, `${JSON.stringify(config, null, 2)}\n`, "utf-8");
  };
  const writeWhatsAppConfig = async (configPath, whatsapp) => {
  ...
  const readParsedConfig = async (configPath) => JSON.parse(await fs.readFile(configPath, "utf-8"));
  const runFixAndReadChannels = async (stateDir, configPath) => {
  ...
  const writeWhatsAppAllowFromStore = async (stateDir, allowFrom) => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-security-fix-suite-"));
  });
  afterAll(async () => {
    if (fixtureRoot) {
  ...
  it("tightens groupPolicy + filesystem perms", async () => {
    ...
    const stateMode = (await fs.stat(stateDir)).mode & 511;
    expectPerms(stateMode, 448);
    const configMode = (await fs.stat(configPath)).mode & 511;
  ...
  it("tightens perms for credentials + agent auth/sessions + include files", async () => {
    ...
    const res = await fixSecurityFootguns({ env, stateDir, configPath });
    expect(res.ok).toBe(true);
    expectPerms((await fs.stat(credsDir)).mode & 511, 448);
    expectPerms((await fs.stat(allowFromPath)).mode & 511, 384);
    expectPerms((await fs.stat(authProfilesPath)).mode & 511, 384);
    expectPerms((await fs.stat(sessionsStorePath)).mode & 511, 384);
    expectPerms((await fs.stat(transcriptPath)).mode & 511, 384);
    expectPerms((await fs.stat(includePath)).mode & 511, 384);
  });
});
