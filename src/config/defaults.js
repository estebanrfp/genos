let isPositiveNumber = function (value) {
  ...
  resolveAnthropicDefaultAuthMode = function (cfg) {
    // New: check cfg.providers.anthropic.credentials first (unified format)
    const providerEntry = cfg.providers?.anthropic;
    if (Array.isArray(providerEntry?.credentials) && providerEntry.credentials.length > 0) {
      const failoverIds = providerEntry.failover ?? providerEntry.credentials.map((c) => c.id);
      for (const id of failoverIds) {
        const cred = providerEntry.credentials.find((c) => c.id === id);
        if (!cred) {
          continue;
        }
      ...
      const hasApiKey = providerEntry.credentials.some((c) => c.type === "api_key");
      const hasOauth = providerEntry.credentials.some(
        (c) => c.type === "oauth" || c.type === "token",
      );
    ...
    // Fallback: legacy auth.profiles + auth.order
  ...
  resolvePrimaryModelRef = function (raw) {
