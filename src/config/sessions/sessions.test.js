...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/genosos/agents/main/sessions";
  ...
  it("rejects absolute sessionFile paths outside known agent sessions dirs", () => {
    ...
    expect(() =>
      resolveSessionFilePath(
        "sess-1",
        { sessionFile: "/tmp/genosos/agents/work/not-sessions/abc-123.jsonl" },
        { sessionsDir },
      ),
...
describe("session store lock (Promise chain mutex)", () => {
  ...
  let lockCaseId = 0;
  let lockTmpDirs = [];
  async function makeTmpStore(initial = {}) {
  ...
  beforeAll(async () => {
    lockFixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "genosos-lock-test-"));
  });
  afterAll(async () => {
    if (lockFixtureRoot) {
      await fsPromises.rm(lockFixtureRoot, { recursive: true, force: true }).catch(() => {
        return;
      });
    }
  });
  ...
  it("multiple consecutive errors do not permanently poison the queue", async () => {
    ...
    const success = updateSessionStore(storePath, async (store) => {
      store[key] = { ...store[key], modelOverride: "recovered" };
    });
...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("creates transcript file and appends message for valid session", async () => {
    ...
    if (result.ok) {
      ...
      const sessionFileMode = fs.statSync(result.sessionFile).mode & 511;
      if (process.platform !== "win32") {
        expect(sessionFileMode).toBe(384);
      }
