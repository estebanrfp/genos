...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = GenosOSSchema.safeParse({
      $schema: "https://genosos.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://genosos.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = GenosOSSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = GenosOSSchema.safeParse({
      cron: {
        enabled: true,
