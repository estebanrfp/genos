let expectValidConfigValue = function (params) {
  ...
  expectInvalidIssuePath = function (config, expectedPath) {
...
async function expectLoadRejectionPreservesField(params) {
  await withTempHome(async (home) => {
    const configPath = path.join(home, ".genosv1", "genosos.json");
...
describe("legacy config detection", () => {
  ...
  it("rejects bindings[].match.provider on load", async () => {
    await expectLoadRejectionPreservesField({
      ...
      readValue: (parsed) => parsed.bindings?.[0]?.match?.provider,
      expectedValue: "slack",
    });
  });
  it("rejects bindings[].match.accountID on load", async () => {
    await expectLoadRejectionPreservesField({
      ...
      readValue: (parsed) => parsed.bindings?.[0]?.match?.accountID,
      expectedValue: "work",
    });
  });
