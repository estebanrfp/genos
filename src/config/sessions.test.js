...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-sessions-suite-"));
  });
  afterAll(async () => {
  ...
  it("derives session transcripts dir from GENOS_STATE_DIR", () => {
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.GENOS_STATE_DIR;
    const stateDir = path.resolve("/home/user/.genos");
  ...
  it("resolves cross-agent paths when GENOS_STATE_DIR differs from stored paths", () => {
    const prev = process.env.GENOS_STATE_DIR;
    process.env.GENOS_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.GENOS_STATE_DIR;
    process.env.GENOS_STATE_DIR = path.resolve("/home/user/.genos");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
