...
export const FIELD_HELP = {
  "meta.lastTouchedVersion": "Auto-set when GenosOS writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /genosos).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $GENOS_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.genosv1/memory/{agentId}.sqlite).",
  ...
  "memory.backend": 'Memory backend ("builtin" for GenosOS embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `genosos plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.genosv1/extensions/<id>).",
  ...
  "agents.defaults.model.routing.enabled":
    "Enable smart model routing. Classifies prompts as simple/normal/complex and routes to tier models.",
  "agents.defaults.model.routing.tiers.simple":
    "Model for simple tasks (greetings, short replies, cron jobs). E.g. 'anthropic/claude-haiku-4-5'.",
  "agents.defaults.model.routing.tiers.normal":
    "Model for general tasks (conversation, moderate complexity). E.g. 'anthropic/claude-sonnet-4-6'.",
  "agents.defaults.model.routing.tiers.complex":
    "Model for complex tasks (analysis, coding, reasoning). E.g. 'anthropic/claude-opus-4-6'.",
  ...
  "commands.config": "Allow /config chat command to read/write config on disk (default: true).",
  ...
  "session.dmScope":
    'DM session scoping (default: "per-channel-peer"). "per-channel-peer" isolates DM sessions per channel+sender; "per-peer" groups all channels for the same sender; "per-account-channel-peer" adds account isolation; "main" collapses all DMs to the main session (legacy).',
  ...
  // --- logging ---
  "logging.level": "Log verbosity (silent|fatal|error|warn|info|debug|trace). Default: info.",
  "logging.file":
    "Write logs to this file path (JSON-lines format, daily rotation). Default: /tmp/genosos/genosos-YYYY-MM-DD.log.",
  "logging.consoleLevel":
    "Console-only log level override (silent|fatal|error|warn|info|debug|trace). Independent from file log level.",
  "logging.consoleStyle":
    'Console log format (pretty|compact|json). "pretty" for dev, "json" for production. Default: pretty.',
  "logging.redactSensitive":
    'Redact sensitive tokens in tool summaries ("off" or "tools"). Keeps first 6 + last 4 chars. Default: tools.',
  "logging.redactPatterns":
    "Additional regex patterns for sensitive data redaction (merged with built-in patterns).",

  // --- hooks (webhook ingress) ---
  "hooks.enabled":
    "Enable inbound webhook endpoint (default: true). Required for Gmail push, GitHub webhooks, etc.",
  "hooks.path": 'URL path prefix for webhook endpoint (default: "/hooks").',
  "hooks.token": "Bearer token for webhook authentication (required for non-loopback).",
  "hooks.defaultSessionKey":
    "Default session key when webhook doesn't specify one (no default; resolves to undefined if omitted).",
  "hooks.allowRequestSessionKey":
    "Allow webhooks to specify a custom session key via request (default: false).",
  "hooks.allowedSessionKeyPrefixes":
    "Restrict explicit sessionKey values to these prefixes (array of strings).",
  "hooks.allowedAgentIds":
    "Restrict webhook routing to these agent IDs. Empty array denies all; omit for unrestricted.",
  "hooks.maxBodyBytes": "Max request body size in bytes for webhook payloads (default: 262144).",
  "hooks.presets": 'Bundled webhook integrations to enable (e.g. ["gmail"]).',
  "hooks.transformsDir":
    "Directory containing custom webhook transform scripts (default: ~/.genosv1/hooks/transforms).",
  "hooks.mappings":
    "Webhook routing rules: match conditions (path, source) → action (wake/agent), delivery target, model override.",
  "hooks.gmail.account": "Gmail address for push notification integration.",
  "hooks.gmail.topic": "Google Cloud Pub/Sub topic (format: projects/<id>/topics/<name>).",
  "hooks.gmail.subscription": "Google Cloud Pub/Sub subscription identifier.",
  "hooks.gmail.hookUrl": "Webhook callback URL for Gmail push notifications.",
  "hooks.gmail.includeBody": "Fetch full message content from Gmail (default: true).",
  "hooks.gmail.maxBytes": "Max message size in bytes for Gmail processing (default: 20000).",
  "hooks.gmail.renewEveryMinutes": "Gmail watch renewal interval in minutes (default: 720).",
  "hooks.gmail.model": "LLM model override for email processing (provider/model format).",
  "hooks.gmail.thinking": "AI thinking depth for Gmail processing (off|minimal|low|medium|high).",

  // --- approvals ---
  "approvals.exec.enabled":
    "Enable exec approval forwarding to external channels (default: false).",
  "approvals.exec.mode":
    'Forwarding mode: "session" (reply in-session), "targets" (forward to channels), "both".',
  "approvals.exec.agentFilter": "Only forward approvals from these agent IDs (empty = all).",
  "approvals.exec.sessionFilter": "Only forward approvals from sessions matching these prefixes.",
  "approvals.exec.targets":
    "Channel targets for approval forwarding (channel + to + optional accountId/threadId).",

  // --- canvasHost ---
  "canvasHost.enabled":
    "Enable the canvas host server for rendering agent-editable HTML/JS canvases (default: true).",
  "canvasHost.root": "Filesystem root for canvas assets (default: ~/.genosv1/canvas).",
  "canvasHost.port": "Port for the canvas host server.",
  "canvasHost.liveReload": "Auto-refresh served pages on file changes (default: true).",

  // --- talk (ElevenLabs TTS for real-time voice) ---
  "talk.voiceId":
    "Default ElevenLabs voice ID for real-time talk mode (fallback: ELEVENLABS_VOICE_ID env var).",
  "talk.voiceAliases":
    'Friendly name → voice ID mappings (e.g. {"Clawd": "EXAVITQu4vr4xnSDxMaL"}).',
  "talk.modelId": 'ElevenLabs TTS model for talk mode (default: "eleven_v3").',
  "talk.outputFormat":
    'Audio encoding format (default: "pcm_44100" macOS/iOS, "pcm_24000" Android). Options: mp3_44100_128, pcm_16000, pcm_22050, pcm_24000, pcm_44100.',
  "talk.apiKey": "ElevenLabs API key for talk mode (fallback: ELEVENLABS_API_KEY env var).",
  "talk.interruptOnSpeech": "Stop assistant playback when user speaks (default: true).",

  // --- web (WebSocket channel for remote clients) ---
  "web.enabled": "Enable WebSocket listener for remote clients (default: false).",
  "web.heartbeatSeconds": "WebSocket keepalive ping interval in seconds (default: 60).",
  "web.reconnect.initialMs": "Initial reconnect backoff delay in ms (default: 2000).",
  "web.reconnect.maxMs": "Maximum reconnect backoff cap in ms (default: 30000).",
  "web.reconnect.factor": "Exponential backoff multiplier for reconnect delays (default: 1.8).",
  "web.reconnect.jitter":
    "Random jitter factor (0–1) added to reconnect delays to avoid thundering herd (default: 0.25).",
  "web.reconnect.maxAttempts":
    "Maximum reconnect attempts before giving up (default: 0 = unlimited).",

  // --- env ---
  "env.shellEnv.enabled":
    "Import environment variables from login shell profile on startup (default: true). Never overrides existing values.",
  "env.shellEnv.timeoutMs": "Timeout in ms for shell environment capture (default: 20000).",

  // --- tools.exec.denyBins / agentToAgent ---
  "tools.exec.denyBins":
    "Binaries blocked from exec tool. Default: 14 dangerous bins (sudo, rm, etc.). Empty array = full trust.",
  "agents.defaults.tools.exec.denyBins":
    "Agent-level override for denied binaries. Cascades: agent → global → defaults.",
  "tools.agentToAgent.enabled":
    "Allow agents to communicate via inter-agent tool calls (default: false).",
  "tools.agentToAgent.allow":
    "Agent targeting allowlist for inter-agent communication (array of agent IDs).",

  // --- media / broadcast / discovery ---
  "media.preserveFilenames":
    "Preserve original filenames when saving media files (default: false).",
  "broadcast.strategy":
    'Message broadcast delivery strategy: "parallel" (all agents at once) or "sequential" (one after another). Default: parallel.',
  "discovery.wideArea.enabled": "Enable wide-area DNS-SD service discovery (default: false).",
