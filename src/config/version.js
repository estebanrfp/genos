...
export function parseGenosOSVersion(raw) {
  if (!raw) {
    return null;
  }
...
export function compareGenosOSVersions(a, b) {
  const parsedA = parseGenosOSVersion(a);
  const parsedB = parseGenosOSVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
