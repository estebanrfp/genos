let makeSnapshot = function (config, raw) {
    return {
      path: "/home/user/.genos/config.json5",
      exists: true,
  ...
  restoreRedactedValues = function (incoming, original, hints) {
...
describe("redactConfigSnapshot", () => {
  ...
  it("redacts botToken in channel configs", () => {
    ...
    const result = redactConfigSnapshot(snapshot);
    const channels = result.config.channels;
  ...
  it("redacts password fields", () => {
    ...
    const result = redactConfigSnapshot(snapshot);
    const gw = result.config.gateway;
  ...
  it("does not redact maxTokens-style fields", () => {
    ...
    const models = result.config.models;
    const providerList = models.providers.openai.models ?? [];
    ...
    expect(providers.openai.tokenLimit).toBe(1e5);
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/genosos/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/genosos/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/genosos/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/genosos/nickserv-password.txt");
  ...
  it("withholds resolved config for invalid snapshots", () => {
    const snapshot = {
      ...
      resolved: { gateway: { auth: { token: "leaky-secret" } } },
      valid: false,
      config: {},
...
describe("restoreRedactedValues", () => {
  ...
  it("round-trips with uiHints for custom sensitive fields", () => {
    ...
    const snapshot = makeSnapshot(originalConfig);
    const redacted = redactConfigSnapshot(snapshot, hints);
    const custom = redacted.config.custom;
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    ...
    const hints = mapSensitivePaths(GenosOSSchema, "", {});
    const snapshot = makeSnapshot({
      agents: {
        defaults: {
          memorySearch: {
