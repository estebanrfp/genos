let configPath = function (...parts) {
    return path.join(CONFIG_DIR, ...parts);
  },
  etcGenosOSPath = function (...parts) {
    return path.join(ETC_GENOS_DIR, ...parts);
  },
  sharedPath = function (...parts) {
    return path.join(SHARED_DIR, ...parts);
  },
  createMockResolver = function (files) {
    return {
      readFile: (filePath) => {
  ...
  resolve = function (obj, files = {}, basePath = DEFAULT_BASE_PATH) {
...
import { CircularIncludeError, ConfigIncludeError, resolveConfigIncludes } from "./includes.js";
const ROOT_DIR = path.parse(process.cwd()).root;
const CONFIG_DIR = path.join(ROOT_DIR, "config");
const ETC_GENOS_DIR = path.join(ROOT_DIR, "etc", "genosos");
const SHARED_DIR = path.join(ROOT_DIR, "shared");
const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "genosos.json");
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcGenosOSPath("agents.json");
  ...
  it("rejects parent directory traversal escaping config directory (CWE-22)", () => {
    ...
    expect(() => resolve(obj, files, configPath("sub", "genosos.json"))).toThrow(
      ConfigIncludeError,
    );
    expect(() => resolve(obj, files, configPath("sub", "genosos.json"))).toThrow(
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("error properties", () => {
    ...
    it("includes offending path in error", () => {
      ...
      try {
        ...
        expect(err).toBeInstanceOf(ConfigIncludeError);
        expect(err.includePath).toBe(maliciousPath);
      }
    });
    it("includes descriptive message", () => {
      ...
      try {
        ...
        expect(err).toBeInstanceOf(ConfigIncludeError);
        expect(err.message).toContain("escapes config directory");
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-includes-symlink-"));
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "genosos.json"),
        );
