let hashConfigRaw = function (raw) {
    return crypto
      .createHash("sha256")
      .update(raw ?? "")
      .digest("hex");
  },
  coerceConfig = function (value) {
  ...
  resolveGatewayMode = function (value) {
    if (!isPlainObject(value)) {
      return null;
    }
  ...
  cloneUnknown = function (value) {
    return structuredClone(value);
  },
  createMergePatch = function (base, target) {
  ...
  collectEnvRefPaths = function (value, path, output) {
  ...
  collectChangedPaths = function (base, target, path, output) {
  ...
  isPathChanged = function (path, changedPaths) {
  ...
  restoreEnvRefsFromMap = function (value, path, envRefMap, changedPaths) {
    if (typeof value === "string") {
  ...
  resolveConfigAuditLogPath = function (env, homedir) {
    return path.join(resolveStateDir(env, homedir), "logs", CONFIG_AUDIT_LOG_FILENAME);
  },
  resolveConfigWriteSuspiciousReasons = function (params) {
    const reasons = [];
  ...
  warnOnConfigMiskeys = function (raw, logger) {
  ...
  stampConfigVersion = function (cfg) {
  ...
  warnIfConfigFromFuture = function (cfg, logger) {
    const touched = cfg.meta?.lastTouchedVersion;
    if (!touched) {
      return;
    }
    const cmp = compareGenosOSVersions(VERSION, touched);
    if (cmp === null) {
      return;
    }
    if (cmp < 0) {
      logger.warn(
        `Config was last written by a newer GenosOS (${touched}); current version is ${VERSION}.`,
      );
    }
  },
  resolveConfigPathForDeps = function (deps) {
    if (deps.configPath) {
      return deps.configPath;
    }
    return resolveConfigPath(deps.env, resolveStateDir(deps.env, deps.homedir));
  },
  normalizeDeps = function (overrides = {}) {
    return {
      fs: overrides.fs ?? fs,
  ...
  maybeLoadDotEnvForConfig = function (env) {
    if (env !== process.env) {
      return;
    }
    loadDotEnv({ quiet: true });
  },
  resolveConfigIncludesForRead = function (parsed, configPath, deps) {
    return resolveConfigIncludes(parsed, configPath, {
      readFile: (candidate) => deps.fs.readFileSync(candidate, "utf-8"),
      parseJson: (raw) => deps.json5.parse(raw),
    });
  },
  resolveConfigForRead = function (resolvedIncludes, env) {
  ...
  resolveConfigCacheMs = function (env) {
    const raw = env.GENOS_CONFIG_CACHE_MS?.trim();
    if (raw === "" || raw === "0") {
      return 0;
    }
    if (!raw) {
  ...
  shouldUseConfigCache = function (env) {
    if (env.GENOS_DISABLE_CONFIG_CACHE?.trim()) {
      return false;
    }
...
import { resolvePassphrase as resolveVaultPassphrase } from "../infra/crypto-utils.js";
import { loadDotEnv } from "../infra/dotenv.js";
import { resolveRequiredHomeDir } from "../infra/home-dir.js";
import {
  isEncrypted as isNyxEncrypted,
  decryptContent as nyxDecrypt,
  encryptContent as nyxEncrypt,
} from "../infra/memory-encryption.js";
import {
...
import { compareGenosOSVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_APP_TOKEN",
  "GENOS_GATEWAY_TOKEN",
  "GENOS_GATEWAY_PASSWORD",
];
/**
 * Transparently decrypt raw config content if NYXENC1-encrypted.
 * @param {string} raw
 * @returns {string}
 */
const decryptConfigRaw = (raw) => {
  if (!isNyxEncrypted(raw)) {
    return raw;
  }
  try {
    return nyxDecrypt(raw, resolveVaultPassphrase());
  } catch {
    return raw;
  }
};
...
/**
 * Encrypt config content if vault passphrase is configured.
 * @param {string} content
 * @returns {string}
 */
const encryptConfigRaw = (content) => {
  try {
    const passphrase = resolveVaultPassphrase();
    return nyxEncrypt(content, passphrase);
  } catch {
    return content;
  }
};
...
async function appendConfigWriteAuditRecord(deps, record) {
  try {
    ...
    await deps.fs.promises.mkdir(path.dirname(auditPath), { recursive: true, mode: 448 });
...
export function createConfigIO(overrides = {}) {
  ...
  function loadConfig() {
    try {
      ...
      const rawEncrypted = deps.fs.readFileSync(configPath, "utf-8");
      const raw = decryptConfigRaw(rawEncrypted);
  ...
  async function writeConfigFile(cfg, options = {}) {
    ...
    try {
      if (deps.fs.existsSync(configPath)) {
        const currentRawEncrypted = await deps.fs.promises.readFile(configPath, "utf-8");
        const currentRaw = decryptConfigRaw(currentRawEncrypted);
    ...
    await deps.fs.promises.mkdir(dir, { recursive: true, mode: 448 });
    const outputConfig =
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.GENOS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.GENOS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.GENOS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.GENOS_WATCH_SESSION === "string" &&
        deps.env.GENOS_WATCH_SESSION.trim().length > 0
          ? deps.env.GENOS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.GENOS_WATCH_COMMAND === "string" &&
        deps.env.GENOS_WATCH_COMMAND.trim().length > 0
          ? deps.env.GENOS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
    ...
    const jsonToWrite = encryptConfigRaw(json);
    try {
      await deps.fs.promises.writeFile(tmp, jsonToWrite, {
        encoding: "utf-8",
        mode: 384,
      });
      if (deps.fs.existsSync(configPath)) {
