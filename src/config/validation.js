let isWorkspaceAvatarPath = function (value, workspaceDir) {
  ...
  validateIdentityAvatar = function (config) {
  ...
  validateConfigObjectWithPluginsBase = function (raw, opts) {
...
export function validateConfigObjectRaw(raw) {
  // Legacy issues are detected separately (findLegacyConfigIssues in io.js)
  // and auto-migrated at gateway startup (server.impl.js). Don't block
  // validation — legacy fields are still valid in the Zod schema.
  const validated = GenosOSSchema.safeParse(raw);
