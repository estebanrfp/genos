let normalizeElevenLabsBaseUrl = function (baseUrl) {
  ...
  requireInRange = function (value, min, max, label) {
    if (!Number.isFinite(value) || value < min || value > max) {
      throw new Error(`${label} must be between ${min} and ${max}`);
    }
  },
  assertElevenLabsVoiceSettings = function (settings) {
  ...
  normalizeApplyTextNormalization = function (mode) {
  ...
  isTextContentBlock = function (block) {
...
const TEMP_FILE_CLEANUP_DELAY_MS = 300000;
      ...
      try {
        switch (key) {
          ...
          case "kokorovoice":
            ...
            overrides.kokoro = { ...overrides.kokoro, voice: rawValue };
            break;
          case "model":
          case "modelid":
          case "model_id":
...
export const KOKORO_DEFAULT_BASE_URL = "http://localhost:8880/v1";
export const KOKORO_DEFAULT_MODEL = "kokoro";
...
/**
 * Generate audio using Kokoro TTS (local OpenAI-compatible endpoint).
 * @param {object} params
 * @param {string} params.text - Text to synthesize
 * @param {string} params.baseUrl - Kokoro API base URL
 * @param {string} params.model - Kokoro model name
 * @param {string} params.voice - Kokoro voice name
 * @param {string} [params.responseFormat] - Audio output format
 * @param {number} [params.speed] - Speech speed multiplier
 * @param {string} [params.language] - Language code (e.g. "es", "en")
 * @param {number} params.timeoutMs - Request timeout in ms
 * @returns {Promise<Buffer>}
 */
export async function kokoroTTS(params) {
  const { text, baseUrl, model, voice, responseFormat, speed, language, timeoutMs } = params;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const body = { model, input: text, voice, response_format: responseFormat };
    if (speed != null) {
      body.speed = speed;
    }
    if (language) {
    ...
    if (!response.ok) {
      throw new Error(`Kokoro TTS API error (${response.status})`);
    }
    return Buffer.from(await response.arrayBuffer());
  } finally {
    clearTimeout(timeout);
