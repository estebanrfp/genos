...
vi.mock("../agents/pi-embedded-runner/model.js", () => ({
  resolveModel: vi.fn((provider, modelId) => ({
    model: {
      provider,
      id: modelId,
      name: modelId,
...
vi.mock("../agents/model-auth.js", () => ({
  ...
  requireApiKey: vi.fn((auth) => auth.apiKey ?? ""),
}));
...
const mockAssistantMessage = (content) => ({
  role: "assistant",
  content,
  api: "openai-completions",
  provider: "openai",
...
describe("tts", () => {
  ...
  describe("parseTtsDirectives", () => {
    ...
    it("accepts kokoro as provider override", () => {
      const policy = resolveModelOverridePolicy({ enabled: true });
      const input = "Hello [[tts:provider=kokoro]] world";
      const result = parseTtsDirectives(input, policy);
      expect(result.overrides.provider).toBe("kokoro");
    });
    it("parses kokoro_voice directive", () => {
      const policy = resolveModelOverridePolicy({ enabled: true });
      const input = "Hello [[tts:provider=kokoro kokoro_voice=am_adam]] world";
      const result = parseTtsDirectives(input, policy);
      expect(result.overrides.provider).toBe("kokoro");
      expect(result.overrides.kokoro?.voice).toBe("am_adam");
    });
  });
  describe("resolveTtsConfig — kokoro", () => {
    const baseCfg = {
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: { tts: {} },
    };
    it("defaults kokoro to disabled", () => {
      const config = resolveTtsConfig(baseCfg);
      expect(config.kokoro.enabled).toBe(false);
      expect(config.kokoro.baseUrl).toBe("http://localhost:8880/v1");
      expect(config.kokoro.model).toBe("kokoro");
      expect(config.kokoro.voice).toBe("af_heart");
    });
    it("resolves kokoro config when provided", () => {
      const cfg = {
        ...baseCfg,
        messages: {
          tts: {
      ...
      expect(config.kokoro.enabled).toBe(true);
      expect(config.kokoro.baseUrl).toBe("http://myhost:9999/v1");
      expect(config.kokoro.voice).toBe("am_adam");
      expect(config.kokoro.speed).toBe(1.2);
      expect(config.kokoro.language).toBe("es");
    });
  });
  describe("getTtsProvider — kokoro", () => {
    it("prefers kokoro when enabled", () => {
      const cfg = {
        agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
        messages: { tts: { kokoro: { enabled: true } } },
      };
      withEnv(
        { OPENAI_API_KEY: "test-key", ELEVENLABS_API_KEY: undefined, XI_API_KEY: undefined },
        () => {
          const config = resolveTtsConfig(cfg);
          const provider = getTtsProvider(config, `/tmp/tts-prefs-kokoro-${Date.now()}.json`);
          expect(provider).toBe("kokoro");
        },
      );
    });
  });
  describe("isTtsProviderConfigured — kokoro", () => {
    it("returns true when kokoro is enabled", () => {
      const cfg = {
        agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
        messages: { tts: { kokoro: { enabled: true } } },
      };
      const config = resolveTtsConfig(cfg);
      expect(tts.isTtsProviderConfigured(config, "kokoro")).toBe(true);
    });
    it("returns false when kokoro is disabled", () => {
      const cfg = {
        agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
        messages: { tts: {} },
      };
      const config = resolveTtsConfig(cfg);
      expect(tts.isTtsProviderConfigured(config, "kokoro")).toBe(false);
    });
  });
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (run) => {
      const prevPrefs = process.env.GENOS_TTS_PREFS;
      process.env.GENOS_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.GENOS_TTS_PREFS = prevPrefs;
      }
    };
    const taggedCfg = {
      ...baseCfg,
