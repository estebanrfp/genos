let normalizeLaunchctlArgs = function (file, args) {
  if (file === "launchctl") {
    return args;
  }
...
vi.mock("./exec-file.js", () => ({
  execFileUtf8: vi.fn(async (file, args) => {
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.genos.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", GENOS_PROFILE: "default" },
    });
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when GENOS_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.genos.gateway.plist",
    );
  });
  it("uses profile-specific label when GENOS_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", GENOS_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.genos.jbphoenix.plist",
    );
  });
  it("prefers GENOS_LAUNCHD_LABEL over GENOS_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      GENOS_PROFILE: "jbphoenix",
      GENOS_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    );
  });
  it("trims whitespace from GENOS_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      GENOS_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty GENOS_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      GENOS_PROFILE: "myprofile",
      GENOS_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.genos.myprofile.plist",
    );
  });
});
