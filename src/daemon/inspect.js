let resolveHomeDir = function (env) {
  ...
  hasGatewayServiceMarker = function (content) {
    const lower = content.toLowerCase();
    const markerKeys = ["genosos_service_marker"];
    const kindKeys = ["genosos_service_kind"];
  ...
  isGenosOSGatewayLaunchdService = function (label, contents) {
    ...
    return label.startsWith("ai.genos.");
  },
  isGenosOSGatewaySystemdService = function (name, contents) {
    if (hasGatewayServiceMarker(contents)) {
      return true;
    }
    if (!name.startsWith("genosos-gateway")) {
      return false;
    }
    return contents.toLowerCase().includes("gateway");
  },
  isGenosOSGatewayTaskName = function (name) {
    const normalized = name.trim().toLowerCase();
    if (!normalized) {
      return false;
    }
    const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
    return normalized === defaultName || normalized.startsWith("genosos gateway");
  },
  tryExtractPlistLabel = function (contents) {
  ...
  isIgnoredLaunchdLabel = function (label) {
    return label === resolveGatewayLaunchAgentLabel();
  },
  isIgnoredSystemdName = function (name) {
    return name === resolveGatewaySystemdServiceName();
  },
  isLegacyLabel = function (label) {
    const lower = label.toLowerCase();
    return lower.includes("genosos") || lower.includes("genosos");
  },
  parseSchtasksList = function (output) {
    const tasks = [];
    let current = null;
...
const EXTRA_MARKERS = ["genosos", "genosos", "genosos"];
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "genosos" : "genosos",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "genosos" && isGenosOSGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "genosos" || isLegacyLabel(label),
    });
  }
  return results;
}
async function scanSystemdDir(params) {
  ...
  for (const entry of entries) {
    ...
    if (marker === "genosos" && isGenosOSGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(env, opts = {}) {
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isGenosOSGatewayTaskName(name)) {
        continue;
      }
