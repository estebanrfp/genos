let resolveLaunchAgentLabel = function (args) {
    const envLabel = args?.env?.GENOS_LAUNCHD_LABEL?.trim();
    if (envLabel) {
      return envLabel;
    }
    return resolveGatewayLaunchAgentLabel(args?.env?.GENOS_PROFILE);
  },
  resolveLaunchAgentPlistPathForLabel = function (env, label) {
    const home = toPosixPath(resolveHomeDir(env));
    return path.posix.join(home, "Library", "LaunchAgents", `${label}.plist`);
  },
  resolveGuiDomain = function () {
    if (typeof process.getuid !== "function") {
      return "gui/501";
    }
    return `gui/${process.getuid()}`;
  },
  isLaunchctlNotLoaded = function (res) {
...
export function resolveGatewayLogPaths(env) {
  ...
  const prefix = env.GENOS_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
export async function findLegacyLaunchAgents(env) {
  const domain = resolveGuiDomain();
  const results = [];
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.GENOS_PROFILE)) {
...
export async function installLaunchAgent({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.GENOS_PROFILE)) {
