let hasGatewaySubcommand = function (programArguments) {
    return Boolean(programArguments?.some((arg) => arg === "gateway"));
  },
  parseSystemdUnit = function (content) {
    const after = new Set();
    const wants = new Set();
    let restartSec;
  ...
  isRestartSecPreferred = function (value) {
    if (!value) {
      return false;
    }
  ...
  auditGatewayCommand = function (programArguments, issues) {
  ...
  auditGatewayToken = function (command, issues, expectedGatewayToken) {
    const expectedToken = expectedGatewayToken?.trim();
    if (!expectedToken) {
      return;
    }
    const serviceToken = command?.environment?.GENOS_GATEWAY_TOKEN?.trim();
    ...
    issues.push({
      ...
      message:
        "Gateway service GENOS_GATEWAY_TOKEN does not match gateway.auth.token in genosos.json",
      detail: serviceToken ? "service token is stale" : "service token is missing",
      level: "recommended",
    });
  },
  isNodeRuntime = function (execPath) {
    const base = path.basename(execPath).toLowerCase();
    return base === "node" || base === "node.exe";
  },
  isBunRuntime = function (execPath) {
    const base = path.basename(execPath).toLowerCase();
    return base === "bun" || base === "bun.exe";
  },
  getPathModule = function (platform) {
    return platform === "win32" ? path.win32 : path.posix;
  },
  normalizePathEntry = function (entry, platform) {
  ...
  auditGatewayServicePath = function (command, issues, env, platform) {
    if (platform === "win32") {
      return;
    }
...
export function checkTokenDrift(params) {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `genosos gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
  return null;
