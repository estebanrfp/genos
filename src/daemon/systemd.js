let resolveSystemdUnitPathForName = function (env, name) {
    const home = toPosixPath(resolveHomeDir(env));
    return path.posix.join(home, ".config", "systemd", "user", `${name}.service`);
  },
  resolveSystemdServiceName = function (env) {
    const override = env.GENOS_SYSTEMD_UNIT?.trim();
    if (override) {
      return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
    }
    return resolveGatewaySystemdServiceName(env.GENOS_PROFILE);
  },
  resolveSystemdUnitPath = function (env) {
...
export async function installSystemdService({
  ...
  const serviceName = resolveGatewaySystemdServiceName(env.GENOS_PROFILE);
