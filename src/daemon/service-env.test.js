...
describe("getMinimalServicePathParts - Linux user directories", () => {
  ...
  it("includes version manager directories on macOS when HOME is set", () => {
    ...
    expect(result).toContain("/Users/testuser/.fnm/aliases/default/bin");
    ...
    expect(result).toContain("/Users/testuser/.local/share/pnpm");
...
describe("buildMinimalServicePath", () => {
  const splitPath = (value, platform) =>
...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.GENOS_GATEWAY_PORT).toBe("18789");
    expect(env.GENOS_GATEWAY_TOKEN).toBe("secret");
    expect(env.GENOS_SERVICE_MARKER).toBe("genosos");
    expect(env.GENOS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.GENOS_SERVICE_VERSION).toBe("string");
    expect(env.GENOS_SYSTEMD_UNIT).toBe("genosos-gateway.service");
    if (process.platform === "darwin") {
      expect(env.GENOS_LAUNCHD_LABEL).toBe("ai.genos.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", GENOS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.GENOS_SYSTEMD_UNIT).toBe("genosos-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.GENOS_LAUNCHD_LABEL).toBe("ai.genos.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genosv1"));
  });
  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", GENOS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genos-rescue"));
  });
  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", GENOS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genosv1"));
  });
  it("uses GENOS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", GENOS_STATE_DIR: "/var/lib/genosos" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/genosos"));
  });
  it("expands ~ in GENOS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", GENOS_STATE_DIR: "~/genosos-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/genosos-state"));
  });
  it("preserves Windows absolute paths without HOME", () => {
    const env = { GENOS_STATE_DIR: "C:\\State\\genosos" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\genosos");
  });
});
