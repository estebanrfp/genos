let getPathModule = function (platform) {
    return platform === "win32" ? path.win32 : path.posix;
  },
  isNodeExecPath = function (execPath, platform) {
    const pathModule = getPathModule(platform);
    const base = pathModule.basename(execPath).toLowerCase();
    return base === "node" || base === "node.exe";
  },
  normalizeForCompare = function (input, platform) {
    const pathModule = getPathModule(platform);
  ...
  buildSystemNodeCandidates = function (env, platform) {
    if (platform === "darwin") {
