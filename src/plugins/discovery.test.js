let makeTempDir = function () {
  const dir = path.join(os.tmpdir(), `genosos-plugins-${randomUUID()}`);
...
const tempDirs = [];
async function withStateDir(stateDir, fn) {
  const prev = process.env.GENOS_STATE_DIR;
  const prevBundled = process.env.GENOS_BUNDLED_PLUGINS_DIR;
  ...
  try {
    ...
    if (prevBundled === undefined) {
      ...
      process.env.GENOS_BUNDLED_PLUGINS_DIR = prevBundled;
    }
  }
}
afterEach(() => {
...
describe("discoverGenosOSPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".genosv1", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverGenosOSPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        genosos: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        genosos: { extensions: ["./src/index.js"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverGenosOSPlugins({ extraPaths: [packDir] });
    });
    const ids = candidates.map((c) => c.idHint);
