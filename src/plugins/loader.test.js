let makeTempDir = function () {
  ...
  loadBundledMemoryPluginRegistry = function (options) {
    ...
    process.env.GENOS_BUNDLED_PLUGINS_DIR = bundledDir;
    return loadGenosOSPlugins({
      cache: false,
      config: {
        plugins: {
          slots: {
...
const fixtureRoot = path.join(os.tmpdir(), `genosos-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.GENOS_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.GENOS_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.GENOS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
afterAll(() => {
  try {
...
describe("loadGenosOSPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadGenosOSPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
