...
describe("compaction hook wiring", () => {
  ...
  it("calls runBeforeCompaction in handleAutoCompactionStart", async () => {
    ...
    const beforeCalls = hookMocks.runner.runBeforeCompaction.mock.calls;
    const event = beforeCalls[0]?.[0];
  ...
  it("calls runAfterCompaction when willRetry is false", async () => {
    ...
    const afterCalls = hookMocks.runner.runAfterCompaction.mock.calls;
    const event = afterCalls[0]?.[0];
