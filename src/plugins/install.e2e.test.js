let makeTempDir = function () {
    const dir = path.join(os.tmpdir(), `genosos-plugin-install-${randomUUID()}`);
    fs.mkdirSync(dir, { recursive: true });
    tempDirs.push(dir);
    return dir;
  },
  writePluginPackage = function (params) {
      ...
      JSON.stringify(
        {
          ...
          genosos: { extensions: params.extensions },
        },
        null,
        2,
      ),
  ...
  expectPluginFiles = function (result, stateDir, pluginId) {
  ...
  setupPluginInstallDirs = function () {
...
const tempDirs = [];
async function packToArchive({ pkgDir, outDir, outName }) {
...
async function expectArchiveInstallReservedSegmentRejection(params) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      genosos: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.genosv1/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without genosos.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@genosos/nope", version: "0.0.1" }),
      "utf-8",
    );
    const archivePath = await packToArchive({
      pkgDir,
    ...
    expect(result.error).toContain("genosos.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@genosos/voice-call@0.0.1",
    });
