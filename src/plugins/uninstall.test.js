let createSinglePluginEntries = function (pluginId = "my-plugin") {
    return {
      [pluginId]: { enabled: true },
    };
  },
  createSinglePluginWithEmptySlotsConfig = function () {
    return {
      plugins: {
...
async function createPluginDirFixture(baseDir, pluginId = "my-plugin") {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "genosos-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-genosos-extensions/my-plugin",
      },
      extensionsDir,
    });
