let resolveBundledPluginSources = function (params) {
    const discovery = discoverGenosOSPlugins({ workspaceDir: params.workspaceDir });
  ...
  pathsEqual = function (left, right) {
    if (!left || !right) {
      return false;
    }
    return resolveUserPath(left) === resolveUserPath(right);
  },
  buildLoadPathHelpers = function (existing) {
    let paths = [...existing];
