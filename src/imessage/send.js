let stripUnsafeReplyTagChars = function (value) {
    let next = "";
  ...
  sanitizeReplyToId = function (rawReplyToId) {
  ...
  prependReplyTagIfNeeded = function (message, replyToId) {
  ...
  resolveMessageId = function (result) {
    if (!result) {
      return null;
    }
    const raw =
