...
const embedBatch = vi.fn(async (_texts) => []);
...
describe("memory indexing with OpenAI batches", () => {
  ...
  function useFastShortTimeouts() {
    ...
    const spy = vi.spyOn(global, "setTimeout").mockImplementation((handler, timeout, ...args) => {
  ...
  async function readOpenAIBatchUploadRequests(body) {
    let uploadedRequests = [];
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-mem-batch-"));
  ...
  beforeEach(async () => {
    ...
    embedBatch.mockImplementation(async (texts) => texts.map((_text, index) => [index + 1, 0, 0]));
    ...
    manager.resetIndex();
    manager.dirty = true;
    manager.batchFailureCount = 0;
    manager.batchFailureLastError = undefined;
    manager.batchFailureLastProvider = undefined;
    manager.batch.enabled = true;
  });
  afterEach(async () => {
    vi.unstubAllGlobals();
  });
  ...
  it("tracks batch failures, resets on success, and disables after repeated failures", async () => {
    ...
    try {
      ...
      expect(embedBatch).not.toHaveBeenCalled();
      await manager.recordBatchFailure({
        provider: "openai",
