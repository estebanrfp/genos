let shouldIgnoreMemoryWatchPath = function (watchPath) {
...
const SESSION_DELTA_READ_CHUNK_BYTES = 65536;
const VECTOR_LOAD_TIMEOUT_MS = 30000;
...
export class MemoryManagerSyncOps {
    ...
    try {
      if (needsFullReindex) {
        if (
          ...
          process.env.GENOS_TEST_MEMORY_UNSAFE_REINDEX === "1"
        ) {
  ...
  async activateFallbackProvider(reason) {
    ...
    const fallbackModel =
      fallback === "gemini" ? DEFAULT_GEMINI_EMBEDDING_MODEL : DEFAULT_OPENAI_EMBEDDING_MODEL;
