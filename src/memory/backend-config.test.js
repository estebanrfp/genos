...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } };
  ...
  it("resolves qmd backend with default collections", () => {
    ...
    expect(resolved.qmd?.update.commandTimeoutMs).toBe(30000);
    expect(resolved.qmd?.update.updateTimeoutMs).toBe(120000);
    expect(resolved.qmd?.update.embedTimeoutMs).toBe(120000);
  ...
  it("resolves qmd update timeout overrides", () => {
      ...
      memory: {
        ...
        qmd: {
          update: {
            ...
            updateTimeoutMs: 480000,
            embedTimeoutMs: 360000,
          },
        },
      },
    };
    ...
    expect(resolved.qmd?.update.commandTimeoutMs).toBe(12000);
    expect(resolved.qmd?.update.updateTimeoutMs).toBe(480000);
    expect(resolved.qmd?.update.embedTimeoutMs).toBe(360000);
  });
