let sanitizeName = function (input) {
    const lower = input.toLowerCase().replace(/[^a-z0-9-]+/g, "-");
    const trimmed = lower.replace(/^-+|-+$/g, "");
    return trimmed || "collection";
  },
  scopeCollectionBase = function (base, agentId) {
    return `${base}-${sanitizeName(agentId)}`;
  },
  ensureUniqueName = function (base, existing) {
  ...
  resolvePath = function (raw, workspaceDir) {
  ...
  resolveIntervalMs = function (raw) {
  ...
  resolveEmbedIntervalMs = function (raw) {
    const value = raw?.trim();
    if (!value) {
  ...
  resolveDebounceMs = function (raw) {
    if (typeof raw === "number" && Number.isFinite(raw) && raw >= 0) {
      return Math.floor(raw);
    }
    return DEFAULT_QMD_DEBOUNCE_MS;
  },
  resolveTimeoutMs = function (raw, fallback) {
    if (typeof raw === "number" && Number.isFinite(raw) && raw > 0) {
      return Math.floor(raw);
    }
    return fallback;
  },
  resolveLimits = function (raw) {
  ...
  resolveSearchMode = function (raw) {
    if (raw === "search" || raw === "vsearch" || raw === "query") {
      return raw;
    }
    return DEFAULT_QMD_SEARCH_MODE;
  },
  resolveSessionConfig = function (cfg, workspaceDir) {
  ...
  resolveCustomPaths = function (rawPaths, workspaceDir, existing, agentId) {
    if (!rawPaths?.length) {
      return [];
    }
    const collections = [];
  ...
  resolveDefaultCollections = function (include, workspaceDir, existing, agentId) {
    if (!include) {
      return [];
    }
    const entries = [
