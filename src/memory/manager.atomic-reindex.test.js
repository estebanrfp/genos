...
describe.skip("memory manager atomic reindex", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-mem-atomic-"));
  });
  beforeEach(async () => {
    vi.stubEnv("GENOS_TEST_MEMORY_UNSAFE_REINDEX", "0");
    resetEmbeddingMocks();
    shouldFail = false;
    embedBatch.mockImplementation(async (texts) => {
      if (shouldFail) {
