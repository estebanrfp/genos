...
describe.skip("memory index", () => {
  ...
  let indexMainPath = "";
  let indexExtraPath = "";
  const managersByStorePath = new Map();
  const managersForCleanup = new Set();
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-mem-fixtures-"));
  ...
  beforeEach(async () => {
    vi.stubEnv("GENOS_TEST_MEMORY_UNSAFE_REINDEX", "1");
    embedBatchCalls = 0;
    await fs.mkdir(memoryDir, { recursive: true });
    await fs.rm(extraDir, { recursive: true, force: true });
  });
  function resetManagerForTest(manager) {
    manager.resetIndex();
    manager.db.exec("DELETE FROM embedding_cache");
    manager.dirty = true;
    manager.sessionsDirty = false;
  }
  ...
  async function getPersistentManager(cfg) {
