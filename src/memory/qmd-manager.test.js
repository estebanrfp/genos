let createMockChild = function (params) {
  ...
  emitAndClose = function (child, stream, data, code = 0) {
    queueMicrotask(() => {
...
describe("QmdMemoryManager", () => {
    ...
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          ...
          update: { interval: "0s", debounceMs: 60000, onBoot: false },
  ...
  it("debounces back-to-back sync calls", async () => {
    ...
    expect(spawnMock.mock.calls.length).toBe(baselineCalls + 2);
    manager.lastUpdateAt = Date.now() - (resolved.qmd?.update.debounceMs ?? 0) - 10;
  ...
  it("runs boot update in background by default", async () => {
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          update: { interval: "0s", debounceMs: 60000, onBoot: true },
          paths: [{ path: workspaceDir, pattern: "**/*.md", name: "workspace" }],
        },
      },
    };
  ...
  it("skips qmd command side effects in status mode initialization", async () => {
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          update: { interval: "5m", debounceMs: 60000, onBoot: true },
          paths: [{ path: workspaceDir, pattern: "**/*.md", name: "workspace" }],
        },
      },
    };
  ...
  it("can be configured to block startup on boot update", async () => {
    ...
    const updateSpawned = createDeferred();
    let releaseUpdate = null;
    spawnMock.mockImplementation((_cmd, args) => {
      if (args[0] === "update") {
        const child = createMockChild({ autoClose: false });
  ...
  it("rebinds sessions collection when existing collection path targets another agent", async () => {
    ...
    expect(manager).toBeTruthy();
    await manager?.close();
    const commands = spawnMock.mock.calls.map((call) => call[1]);
  ...
  it("retries search with qmd query when configured mode rejects flags", async () => {
    ...
    const searchAndQueryCalls = spawnMock.mock.calls
      .map((call) => call[1])
      .filter((args) => Array.isArray(args) && ["search", "query"].includes(args[0]));
  ...
  it("queues a forced sync behind an in-flight update", async () => {
    ...
    const firstUpdateSpawned = createDeferred();
    let updateCalls = 0;
    let releaseFirstUpdate = null;
    spawnMock.mockImplementation((_cmd, args) => {
      if (args[0] === "update") {
        updateCalls += 1;
        if (updateCalls === 1) {
  ...
  it("honors multiple forced sync requests while forced queue is active", async () => {
    ...
    const firstUpdateSpawned = createDeferred();
    const secondUpdateSpawned = createDeferred();
    let updateCalls = 0;
    let releaseFirstUpdate = null;
    let releaseSecondUpdate = null;
    spawnMock.mockImplementation((_cmd, args) => {
      if (args[0] === "update") {
        updateCalls += 1;
        if (updateCalls === 1) {
  ...
  it("scopes by channel for agent-prefixed session keys", async () => {
    ...
    const { manager } = await createManager();
    const isAllowed = (key) => manager.isScopeAllowed(key);
  ...
  it("errors when qmd output exceeds command output safety cap", async () => {
    const noisyPayload = "x".repeat(240000);
    spawnMock.mockImplementation((_cmd, args) => {
      if (args[0] === "search") {
        const child = createMockChild({ autoClose: false });
