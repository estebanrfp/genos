import { encryptChunkText, isVaultActive } from "../infra/secure-io.js";
...
const EMBEDDING_BATCH_TIMEOUT_REMOTE_MS = 120000;
...
export class MemoryManagerEmbeddingOps extends MemoryManagerSyncOps {
  ...
  resolveEmbeddingTimeout(kind) {
    return kind === "query" ? EMBEDDING_QUERY_TIMEOUT_REMOTE_MS : EMBEDDING_BATCH_TIMEOUT_REMOTE_MS;
  }
  /** Public: embed a query text and return the vector (reusable for semantic tool matching) */
  ...
  async indexFile(entry, options) {
    ...
    const content = options.content ?? (await readMemoryFile(entry.absPath));
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const storedText = encryptChunkText(chunk.text);
      this.db
      ...
      if (this.fts.enabled && this.fts.available && !vaultActive) {
        this.db
          .prepare(
