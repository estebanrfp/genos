let getGeminiUploadUrl = function (baseUrl) {
    if (baseUrl.includes("/v1beta")) {
      return baseUrl.replace(/\/v1beta\/?$/, "/upload/v1beta");
    }
    return `${baseUrl.replace(/\/$/, "")}/upload`;
  },
  buildGeminiUploadBody = function (params) {
    const boundary = `genosos-${hashText(params.displayName)}`;
  ...
  parseGeminiBatchOutput = function (text) {
    if (!text.trim()) {
      return [];
    }
    return text
