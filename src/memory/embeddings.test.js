let requireProvider = function (result) {
    if (!result.provider) {
      throw new Error("Expected embedding provider");
    }
    return result.provider;
  },
  mockResolvedProviderKey = function (apiKey = "provider-key") {
  ...
  mockMissingLocalEmbeddingDependency = function () {
  ...
  createLocalProvider = function (options) {
...
vi.mock("./node-llama.js", () => ({
  importNodeLlamaCpp: (...args) => importNodeLlamaCppMock(...args),
}));
const createFetchMock = () =>
  vi.fn(async (_input, _init) => ({
    ok: true,
    status: 200,
