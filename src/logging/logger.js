let attachExternalTransport = function (logger, transport) {
  ...
  resolveSettings = function () {
    ...
    const defaultLevel =
      process.env.VITEST === "true" && process.env.GENOS_TEST_FILE_LOG !== "1" ? "silent" : "info";
    const level = normalizeLogLevel(cfg?.level, defaultLevel);
    const file = cfg?.file ?? defaultRollingPathForToday();
    return { level, file };
  },
  settingsChanged = function (a, b) {
    if (!a) {
      return true;
    }
    return a.level !== b.level || a.file !== b.file;
  },
  buildLogger = function (settings) {
  ...
  defaultRollingPathForToday = function () {
    const today = formatLocalDate(new Date());
    return path.join(DEFAULT_LOG_DIR, `${LOG_PREFIX}-${today}${LOG_SUFFIX}`);
  },
  isRollingPath = function (file) {
  ...
  pruneOldRollingLogs = function (dir) {
    try {
...
export const DEFAULT_LOG_DIR = resolvePreferredGenosOSTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "genosos.log");
const LOG_PREFIX = "genosos";
const LOG_SUFFIX = ".log";
const MAX_LOG_AGE_MS = 86400000;
