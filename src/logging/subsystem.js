let shouldLogToConsole = function (level, settings) {
  ...
  pickSubsystemColor = function (color, subsystem) {
  ...
  formatSubsystemForConsole = function (subsystem) {
  ...
  formatConsoleLine = function (opts) {
  ...
  writeConsoleLine = function (level, line) {
    ...
    // Subsystem diagnostic logs always go to stderr to keep stdout clean for piping
    (sink.error ?? console.error)(sanitized);
  },
  logToFile = function (fileLogger, level, message, meta) {
    if (level === "silent") {
      return;
    }
