let moduleUrlFrom = function (root, relativePath) {
...
async function withTempDir(run) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({ name: "genosos", version: "1.2.3" }),
        "utf-8",
      );
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      ...
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ name: "genosos", version: "2.3.4" }),
        "utf-8",
      );
      await fs.writeFile(
  ...
  it("prefers GENOS_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        GENOS_VERSION: "9.9.9",
        GENOS_SERVICE_VERSION: "2.2.2",
