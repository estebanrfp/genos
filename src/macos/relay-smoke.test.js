...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { GENOS_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { GENOS_SMOKE_QR: "1" })).toBe(null);
  });
  it("supports GENOS_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { GENOS_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { GENOS_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { GENOS_SMOKE: "nope" })).toBe("qr");
  });
});
