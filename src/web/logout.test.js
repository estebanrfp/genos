...
describe("web logout", () => {
  ...
  let logoutWeb;
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "genosos-test-web-logout-"));
  ...
  it("deletes cached credentials when present", { timeout: 60000 }, async () => {
  ...
  it("no-ops when nothing to delete", { timeout: 60000 }, async () => {
    const authDir = await makeCaseDir();
    const result = await logoutWeb({ authDir, runtime });
