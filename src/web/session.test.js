let mockCredsJsonSpies = function (readContents) {
  const credsSuffix = path.join(".genosv1", "credentials", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    ...
    const passedLogger = passed.logger;
  ...
  it("waits for connection open", async () => {
    ...
    const promise = waitForWaConnection({ ev });
