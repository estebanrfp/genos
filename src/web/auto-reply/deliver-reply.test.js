let makeMsg = function () {
  ...
  mockFirstSendMediaFailure = function (msg, message) {
...
describe("deliverWebReply", () => {
  ...
  it("falls back to text-only when the first media send fails", async () => {
    ...
    expect(String(msg.reply.mock.calls[0]?.[0])).toContain("\u26A0\uFE0F Media failed");
