...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    const MESSAGE_TIMEOUT_MS = 1800000;
    ...
    // Connection status visible via Health modal — no need to pollute chat sessions.
    logVerbose(`WhatsApp gateway connected${selfE164 ? ` as ${selfE164}` : ""}.`);
    ...
    logVerbose(`WhatsApp gateway disconnected (status ${statusCode ?? "unknown"})`);
    if (loggedOut) {
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("genosos channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
