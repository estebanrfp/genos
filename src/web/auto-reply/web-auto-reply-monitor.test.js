let runGroupGating = function (params) {
  ...
  createGroupMessage = function (overrides = {}) {
    return {
      id: "g1",
      from: "123@g.us",
...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-group-gating-"));
...
describe("buildInboundLine", () => {
  it("prefixes group messages with sender", () => {
    const line = buildInboundLine({
      cfg: {
        agents: { defaults: { workspace: "/tmp/genosos" } },
