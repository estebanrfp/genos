...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\bgenosos\b/i] };
  it("ignores regex matches when other mentions are present", () => {
    const msg = makeMsg({
      body: "@GenosOS please help",
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\bgenosos\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("resolves mentionedJids via lid reverse mapping in authDir", async () => {
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-lid-mapping-"));
    try {
      await fs.writeFile(
