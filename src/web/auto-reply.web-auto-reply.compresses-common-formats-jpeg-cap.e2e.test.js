...
describe("web auto-reply", () => {
  ...
  async function setupSingleInboundMessage(params) {
    ...
    const sendComposing = vi.fn(async () => {
      return;
    });
  ...
  it("compresses common formats to jpeg under the cap", { timeout: 45000 }, async () => {
    const formats = [
      {
        name: "png",
        mime: "image/png",
    ...
    for (const fmt of formats) {
      ...
      expect(big.length).toBeGreaterThan(1048576);
      ...
      expect(payload.image.length).toBeLessThanOrEqual(1048576);
  ...
  it("honors mediaMaxMb from config", async () => {
    ...
    const bigPng = await sharp({
      ...
      .png({ compressionLevel: 0 })
      .toBuffer();
    expect(bigPng.length).toBeGreaterThan(1048576);
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
      body: true,
      arrayBuffer: async () =>
