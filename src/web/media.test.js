let buildDeterministicBytes = function (length) {
...
async function createLargeTestJpeg() {
  return { buffer: largeJpegBuffer, file: largeJpegFile };
}
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    stateDirSnapshot = captureEnv(["GENOS_STATE_DIR"]);
    process.env.GENOS_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "genosos-media-state-test",
    );
  });
  afterAll(() => {
  ...
  it("strips MEDIA: prefix before reading local file", async () => {
    const result = await loadWebMedia(`MEDIA:${tinyPngFile}`, 1048576);
  ...
  it("sniffs mime before extension when loading local files", async () => {
    const result = await loadWebMedia(tinyPngWrongExtFile, 1048576);
    expect(result.kind).toBe("image");
  ...
  it("includes URL + status in fetch errors", async () => {
    ...
    await expect(loadWebMedia("https://example.com/missing.jpg", 1048576)).rejects.toThrow(
  ...
  it("blocks private network URL fetches (SSRF guard)", async () => {
    ...
    await expect(loadWebMedia("http://127.0.0.1:8080/internal-api", 1048576)).rejects.toThrow(
  ...
  it("blocks cloud metadata hostnames (SSRF guard)", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch");
    await expect(
      loadWebMedia("http://metadata.google.internal/computeMetadata/v1/", 1048576),
  ...
  it("uses content-disposition filename when available", async () => {
    ...
    const result = await loadWebMedia("https://example.com/download?id=1", 1048576);
  ...
  it("preserves GIF from URL without JPEG conversion", async () => {
    const gifBytes = new Uint8Array([
      71, 73, 70, 56, 57, 97, 1, 0, 1, 0, 0, 0, 0, 44, 0, 0, 0, 0, 1, 0, 1, 0, 0, 2, 1, 68, 0, 59,
    ]);
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({
      ok: true,
      body: true,
    ...
    const result = await loadWebMedia("https://example.com/animation.gif", 1048576);
    expect(result.kind).toBe("image");
  ...
  it("preserves PNG alpha when under the cap", async () => {
    const result = await loadWebMedia(alphaPngFile, 1048576);
    expect(result.kind).toBe("image");
...
describe("local media root guard", () => {
  it("rejects local paths outside allowed roots", async () => {
    await expect(
      loadWebMedia(tinyPngFile, 1048576, { localRoots: ["/nonexistent-root"] }),
    ).rejects.toThrow(/not under an allowed directory/i);
  });
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1048576, { localRoots: [os.tmpdir()] });
    expect(result.kind).toBe("image");
  });
  ...
  it("rejects filesystem root entries in localRoots", async () => {
    await expect(
      loadWebMedia(tinyPngFile, 1048576, {
        localRoots: [path.parse(tinyPngFile).root],
      }),
    ).rejects.toThrow(/refuses filesystem root/i);
  });
  it("allows default GenosOS state workspace and sandbox roots", async () => {
  ...
  it("rejects default GenosOS state per-agent workspace-* roots without explicit local roots", async () => {
    const { resolveStateDir } = await import("../config/paths.js");
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    await expect(
