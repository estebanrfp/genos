let closeSocket = function (sock) {
    try {
      sock.ws?.close();
    } catch {}
  },
  isLoginFresh = function (login) {
    return Date.now() - login.startedAt < ACTIVE_LOGIN_TTL_MS;
  },
  attachLoginWaiter = function (accountId, login) {
...
const ACTIVE_LOGIN_TTL_MS = 180000;
const activeLogins = new Map();
async function resetActiveLogin(accountId, reason) {
  const login = activeLogins.get(accountId);
  if (login) {
    activeLogins.delete(accountId);
    // Delay socket close (same as CLI loginWeb) to let Baileys flush creds to disk
    setTimeout(() => closeSocket(login.sock), 500);
  }
  if (reason) {
    logInfo(reason);
  }
...
export async function startWebLoginWithQr(opts = {}) {
  ...
  const qrTimer = setTimeout(
    ...
    Math.max(opts.timeoutMs ?? 30000, 5000),
  );
  let sock;
  let pendingQr = null;
  try {
...
export async function waitForWebLogin(opts = {}) {
  ...
  while (true) {
    ...
    if (login.connected) {
      ...
      // Flush pending creds saves then ensure registered:true is persisted
      await flushCredsSaveQueue().catch(() => {});
      try {
        const credsPath = path.join(login.authDir, "creds.json");
