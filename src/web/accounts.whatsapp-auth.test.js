...
describe("hasAnyWhatsAppAuth", () => {
  let envSnapshot;
  let tempOauthDir;
  const writeCreds = (dir) => {
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, "creds.json"), JSON.stringify({ me: {} }));
  };
  beforeEach(() => {
    envSnapshot = captureEnv(["GENOS_OAUTH_DIR"]);
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-oauth-"));
    process.env.GENOS_OAUTH_DIR = tempOauthDir;
  });
  afterEach(() => {
    envSnapshot.restore();
    if (tempOauthDir) {
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-wa-auth-"));
    try {
