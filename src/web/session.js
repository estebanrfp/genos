let enqueueSaveCreds = function (authDir, saveCreds, logger) {
  ...
  safeStringify = function (value, limit = 800) {
    try {
  ...
  extractBoomDetails = function (err) {
...
/**
 * Wait for all pending credential saves to flush to disk.
 * @returns {Promise<void>}
 */
export const flushCredsSaveQueue = () => credsSaveQueue;
    ...
    try {
      ...
      if (connection === "close") {
        ...
        if (status === DisconnectReason.loggedOut) {
          console.error(
            danger(
              `WhatsApp session logged out. Run: ${formatCliCommand("genosos channels login")}`,
            ),
          );
        }
      }
...
export function getStatusCode(err) {
  return err?.output?.statusCode ?? err?.error?.output?.statusCode ?? err?.status;
}
