let extractDeliveryModes = function (schema) {
...
const UI_FILES = ["ui/src/ui/views/cron.js"];
...
describe("cron protocol conformance", () => {
  it("ui + swift include all cron delivery modes from gateway schema", async () => {
    const modes = extractDeliveryModes(CronDeliverySchema);
  ...
  it("cron status shape matches gateway fields in UI + Swift", async () => {
    const cwd = process.cwd();
    const uiController = await fs.readFile(
      path.join(cwd, "ui/src/ui/controllers/cron.js"),
      "utf-8",
    );
    expect(uiController.includes("loadCronStatus")).toBe(true);
    expect(uiController.includes("jobs")).toBe(true);
