let topOfHourOffsetMs = function (jobId) {
  ...
  createDueIsolatedJob = function (params) {
    return {
      id: params.id,
      name: params.id,
      enabled: true,
  ...
  createDefaultIsolatedRunner = function () {
    return vi.fn().mockResolvedValue({
      status: "ok",
      summary: "ok",
    });
  },
  createIsolatedRegressionJob = function (params) {
    return {
      id: params.id,
      name: params.name,
      enabled: true,
      createdAtMs: params.scheduledAt - 86400000,
      updatedAtMs: params.scheduledAt - 86400000,
...
const TOP_OF_HOUR_STAGGER_MS = 300000;
let fixtureRoot = "";
let fixtureCount = 0;
...
async function writeCronJobs(storePath, jobs) {
  await fs.writeFile(storePath, JSON.stringify({ version: 1, jobs }, null, 2), "utf-8");
}
async function startCronForStore(params) {
  const enqueueSystemEvent = params.enqueueSystemEvent ?? vi.fn();
  const requestHeartbeatNow = params.requestHeartbeatNow ?? vi.fn();
...
describe("Cron issue regressions", () => {
  ...
  it("does not advance unrelated due jobs when updating another job", async () => {
    ...
    const dueJob = await cron.add({
      name: "due-preserved",
      enabled: true,
      schedule: { kind: "every", everyMs: 60000, anchorMs: now },
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
    ...
    const storeData = JSON.parse(await fs.readFile(store.storePath, "utf8"));
  ...
  it("caps timer delay to 60s for far-future schedules", async () => {
    ...
    expect(delaysAfterAdd.some((delay) => delay === 60000)).toBe(true);
    cron.stop();
  ...
  it("prevents spin loop when cron job completes within the scheduled second (#17821)", async () => {
    ...
    const nextDay = scheduledAt + 86400000;
