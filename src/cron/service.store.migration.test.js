let makeLegacyJob = function (overrides) {
  return {
    id: "job-legacy",
    agentId: undefined,
    name: "Legacy job",
...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cron-migrate-"));
  return {
    dir,
...
async function writeLegacyStore(storePath, legacyJob) {
  await fs.mkdir(path.dirname(storePath), { recursive: true });
  await fs.writeFile(storePath, JSON.stringify({ version: 1, jobs: [legacyJob] }, null, 2));
}
async function migrateAndLoadFirstJob(storePath) {
  ...
  return loaded.jobs[0];
}
async function migrateLegacyJob(legacyJob) {
...
describe("cron store migration", () => {
  ...
  it("migrates isolated jobs to announce delivery and drops isolation", async () => {
    ...
    const schedule = migrated.schedule;
