let createMockState = function (now) {
  return {
    deps: {
      nowMs: () => now,
    },
...
describe("applyJobPatch", () => {
  const createIsolatedAgentTurnJob = (id, delivery, overrides) => {
    const now = Date.now();
    return {
      id,
      name: id,
...
describe("cron stagger defaults", () => {
  ...
  it("preserves existing stagger when editing cron expression without stagger", () => {
    ...
    const job = {
      ...
      createdAtMs: now,
      updatedAtMs: now,
      schedule: { kind: "cron", expr: "0 * * * *", tz: "UTC", staggerMs: 120000 },
      sessionTarget: "main",
      wakeMode: "now",
      payload: { kind: "systemEvent", text: "tick" },
      state: {},
    ...
    if (job.schedule.kind === "cron") {
      ...
      expect(job.schedule.staggerMs).toBe(120000);
    }
  });
