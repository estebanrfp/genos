...
describe("cron run log", () => {
  ...
  it("appends JSONL and prunes by line count", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cron-log-"));
  ...
  it("reads newest entries and filters by jobId", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cron-log-read-"));
    ...
    await appendCronRunLog(logPathA, {
      ...
      status: "skipped",
      sessionId: "run-123",
      sessionKey: "agent:default:cron:a:run:run-123",
    });
    ...
    expect(lastOne[0]?.sessionKey).toBe("agent:default:cron:a:run:run-123");
  ...
  it("reads telemetry fields", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cron-log-telemetry-"));
    const logPath = path.join(dir, "runs", "job-1.jsonl");
