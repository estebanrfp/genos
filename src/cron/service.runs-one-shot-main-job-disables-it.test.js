let bumpMtimeMs = function () {
  ...
  writeStoreFile = function (storePath, payload) {
  ...
  createCronEventHarness = function () {
    const events = [];
    const waiters = [];
    const onEvent = (evt) => {
      events.push(evt);
  ...
  createLegacyDeliveryMigrationJob = function (options) {
    return {
      id: options.id,
      name: "legacy",
      enabled: true,
...
const fixturesRoot = abs(path.join("__genosos_vitest__", "cron", "runs-one-shot"));
...
vi.mock("node:fs", async (importOriginal) => {
  ...
  const mkErr = (code, message) => Object.assign(new Error(message), { code });
  const promises = {
    ...actual.promises,
    mkdir: async (p) => {
...
async function createCronHarness(options = {}) {
  ensureDir(fixturesRoot);
...
async function createIsolatedAnnounceHarness(runIsolatedAgentJob) {
  const harness = await createCronHarness({
    runIsolatedAgentJob,
  });
  if (!harness.events) {
...
async function addDefaultIsolatedAnnounceJob(cron, name) {
...
async function addWakeModeNowMainSystemEventJob(cron, options) {
  return cron.add({
...
async function loadLegacyDeliveryMigration(rawJob) {
  ensureDir(fixturesRoot);
  const store = await makeStorePath();
...
// Skipped: vi.mock("node:fs", async factory) deadlocks with Fortress Mode import chain
// (secure-io → crypto-utils → keychain → child_process). Upstream test; runtime unaffected.
describe.skip("CronService", () => {
  ...
  it("passes agentId + sessionKey to runHeartbeatOnce for main-session wakeMode now jobs", async () => {
    const runHeartbeatOnce = vi.fn(async () => ({ status: "ran", durationMs: 1 }));
    const { store, cron, enqueueSystemEvent, requestHeartbeatNow } =
      await createWakeModeNowMainHarness({
        runHeartbeatOnce,
  ...
  it("runs an isolated job and posts summary to main", async () => {
    const runIsolatedAgentJob = vi.fn(async () => ({ status: "ok", summary: "done" }));
    const { store, cron, enqueueSystemEvent, requestHeartbeatNow, events } =
  ...
  it("rejects unsupported session/payload combinations", async () => {
    ...
    const cron = new CronService({
      ...
      enqueueSystemEvent: vi.fn(),
      requestHeartbeatNow: vi.fn(),
      runIsolatedAgentJob: vi.fn(async (_params) => ({
        status: "ok",
      })),
    });
    await cron.start();
