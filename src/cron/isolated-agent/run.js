let matchesMessagingToolDeliveryTarget = function (target, delivery) {
  ...
  resolveCronDeliveryBestEffort = function (job) {
...
export async function runCronIsolatedAgentTurn(params) {
  const isFastTestEnv = process.env.GENOS_TEST_FAST === "1";
  ...
  const withRunSession = (result) => ({
    ...
    cronSessionKey: agentSessionKey,
  });
  ...
  if (deliveryRequested && !skipHeartbeatDelivery && !skipMessagingToolDelivery) {
    ...
    if (deliveryPayloadHasStructuredContent) {
      ...
      try {
        ...
        logWarn(`[cron:${params.job.id}] cron announce error: ${String(err)}`);
      }
    }
  }
