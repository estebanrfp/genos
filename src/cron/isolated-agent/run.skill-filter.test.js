let makeJob = function (overrides) {
    return {
      id: "test-job",
      name: "Test Job",
...
vi.mock("../../agents/timeout.js", () => ({
  resolveAgentTimeoutMs: vi.fn().mockReturnValue(60000),
}));
...
describe("runCronIsolatedAgentTurn \u2014 skill filter", () => {
  let previousFastTestEnv;
  beforeEach(() => {
    vi.clearAllMocks();
    previousFastTestEnv = process.env.GENOS_TEST_FAST;
    delete process.env.GENOS_TEST_FAST;
  ...
  afterEach(() => {
    ...
    process.env.GENOS_TEST_FAST = previousFastTestEnv;
  });
  ...
  describe("model fallbacks", () => {
    ...
    async function expectPrimaryOverridePreservesDefaults(modelOverride) {
