let createStartedCron = function (storePath) {
...
const { makeStorePath } = createCronStoreHarness({ prefix: "genosos-cron-" });
installCronTestHooks({
  logger: noopLogger,
...
describe("CronService store migrations", () => {
  it("migrates legacy top-level agentTurn fields and initializes missing state", async () => {
    ...
    const persisted = JSON.parse(await fs.readFile(store.storePath, "utf-8"));
