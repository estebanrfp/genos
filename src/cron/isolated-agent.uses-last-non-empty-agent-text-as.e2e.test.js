let makeDeps = function () {
  ...
  mockEmbeddedTexts = function (texts) {
  ...
  mockEmbeddedOk = function () {
    mockEmbeddedTexts(["ok"]);
  },
  expectEmbeddedProviderModel = function (expected) {
...
async function writeSessionStore(home, entries = {}) {
...
async function readSessionEntry(storePath, key) {
...
const DEFAULT_AGENT_TURN_PAYLOAD = {
  kind: "agentTurn",
  message: DEFAULT_MESSAGE,
  deliver: false,
};
const GMAIL_MODEL = "openrouter/meta-llama/llama-3.3-70b:free";
async function runCronTurn(home, options = {}) {
...
async function runGmailHookTurn(home, storeEntries) {
...
async function runTurnWithStoredModelOverride(home, jobPayload, modelOverride = "gpt-4.1-mini") {
  return runCronTurn(home, {
    jobPayload,
    storeEntries: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".genosv1", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
