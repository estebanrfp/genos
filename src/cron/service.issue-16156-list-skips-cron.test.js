let createCronFromStorePath = function (storePath) {
...
async function writeJobsStore(storePath, jobs) {
  await fs.mkdir(path.dirname(storePath), { recursive: true });
...
describe("#16156: cron.list() must not silently advance past-due recurring jobs", () => {
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cron-16156-"));
  });
  afterAll(async () => {
    if (fixtureRoot) {
