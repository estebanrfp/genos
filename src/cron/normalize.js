let coerceSchedule = function (schedule) {
    const next = { ...schedule };
    const rawKind = typeof schedule.kind === "string" ? schedule.kind.trim().toLowerCase() : "";
    const kind =
      rawKind === "at" || rawKind === "every" || rawKind === "cron" ? rawKind : undefined;
  ...
  coercePayload = function (payload) {
    const next = { ...payload };
  ...
  coerceDelivery = function (delivery) {
    const next = { ...delivery };
  ...
  normalizeSessionTarget = function (raw) {
  ...
  normalizeWakeMode = function (raw) {
    if (typeof raw !== "string") {
      return;
    }
    const trimmed = raw.trim().toLowerCase();
  ...
  copyTopLevelAgentTurnFields = function (next, payload) {
  ...
  copyTopLevelLegacyDeliveryFields = function (next, payload) {
  ...
  stripLegacyTopLevelFields = function (next) {
    delete next.model;
    delete next.thinking;
    delete next.timeoutSeconds;
    delete next.allowUnsafeExternalContent;
