...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses GENOS_HOME for tilde expansion", () => {
    vi.stubEnv("GENOS_HOME", "/srv/genosos-home");
    vi.stubEnv("HOME", "/home/other");
    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/genosos-home", "cron", "jobs.json"));
  });
});
describe("cron store", () => {
