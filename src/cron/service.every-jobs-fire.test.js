...
describe("CronService interval/cron jobs fire on time", () => {
  const runLateTimerAndLoadJob = async ({ cron, finished, jobId, firstDueAt }) => {
  ...
  const expectMainSystemEvent = (enqueueSystemEvent, expectedText) => {
  ...
  it("fires an every-type main job when the timer fires a few ms late", async () => {
    ...
    const job = await cron.add({
      ...
      enabled: true,
      schedule: { kind: "every", everyMs: 1e4 },
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
      payload: { kind: "systemEvent", text: "tick" },
    });
    const firstDueAt = job.state.nextRunAtMs;
    expect(firstDueAt).toBe(Date.parse("2025-12-13T00:00:00.000Z") + 1e4);
    ...
    expect(updated?.state.nextRunAtMs).toBeGreaterThanOrEqual(firstDueAt + 1e4);
    cron.stop();
    await store.cleanup();
  });
  it("fires a cron-expression job when the timer fires a few ms late", async () => {
    ...
    expect(updated?.state.lastStatus).toBe("ok");
    expect(updated?.state.nextRunAtMs).toBe(firstDueAt + 60000);
    cron.stop();
    await store.cleanup();
  });
          ...
          jobs: [
            {
              ...
              state: { nextRunAtMs: nowMs + 120000 },
            },
            {
              id: "minute-cron",
              name: "minute cron",
