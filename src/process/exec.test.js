...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["GENOS_BASE_ENV"]);
    process.env.GENOS_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.GENOS_BASE_ENV ?? "") + "|" + (process.env.GENOS_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5000,
          env: { GENOS_TEST_ENV: "ok" },
        },
      );
