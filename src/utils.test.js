...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "genosos-test-"));
...
describe("assertWebChannel", () => {
  ...
  it("throws for invalid channel", () => {
    expect(() => assertWebChannel("bad")).toThrow();
  });
});
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.genos when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "genosos-config-dir-"));
    try {
      const newDir = path.join(root, ".genosv1");
      await fs.promises.mkdir(newDir, { recursive: true });
      const resolved = resolveConfigDir({}, () => root);
...
describe("resolveHomeDir", () => {
  it("prefers GENOS_HOME over HOME", () => {
    vi.stubEnv("GENOS_HOME", "/srv/genosos-home");
    vi.stubEnv("HOME", "/home/other");
    expect(resolveHomeDir()).toBe(path.resolve("/srv/genosos-home"));
    vi.unstubAllEnvs();
  });
});
describe("shortenHomePath", () => {
  it("uses $GENOS_HOME prefix when GENOS_HOME is set", () => {
    vi.stubEnv("GENOS_HOME", "/srv/genosos-home");
    vi.stubEnv("HOME", "/home/other");
    expect(shortenHomePath(`${path.resolve("/srv/genosos-home")}/.genos/genosos.json`)).toBe(
      "$GENOS_HOME/.genos/genosos.json",
    );
    vi.unstubAllEnvs();
  });
});
describe("shortenHomeInString", () => {
  it("uses $GENOS_HOME replacement when GENOS_HOME is set", () => {
    vi.stubEnv("GENOS_HOME", "/srv/genosos-home");
    vi.stubEnv("HOME", "/home/other");
    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/genosos-home")}/.genos/genosos.json`),
    ).toBe("config: $GENOS_HOME/.genos/genosos.json");
    vi.unstubAllEnvs();
  });
});
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/genosos")).toBe(path.resolve(os.homedir(), "genosos"));
  });
  it("resolves relative paths", () => {
    expect(resolveUserPath("tmp/dir")).toBe(path.resolve("tmp/dir"));
  });
  it("prefers GENOS_HOME for tilde expansion", () => {
    vi.stubEnv("GENOS_HOME", "/srv/genosos-home");
    vi.stubEnv("HOME", "/home/other");
    expect(resolveUserPath("~/genosos")).toBe(path.resolve("/srv/genosos-home", "genosos"));
    vi.unstubAllEnvs();
  });
