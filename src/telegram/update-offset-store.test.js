...
async function withTempStateDir(fn) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-tg-offset-"));
...
describe("deleteTelegramUpdateOffset", () => {
  it("removes the offset file so a new bot starts fresh", async () => {
    await withTempStateDir(async () => {
      await writeTelegramUpdateOffset({ accountId: "default", updateId: 432000000 });
      expect(await readTelegramUpdateOffset({ accountId: "default" })).toBe(432000000);
