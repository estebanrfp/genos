...
describe("createTelegramBot", () => {
  ...
  it("requests pairing by default for unknown DM senders", async () => {
    ...
    expect(pairingText).toContain("genosos pairing approve telegram PAIRME12");
  ...
  it("routes DMs by telegram accountId binding", async () => {
    ...
    expect(payload.SessionKey).toBe("agent:opie:telegram:direct:123");
  });
  ...
  async function dispatchMessage(params) {
    ...
    await handler({
      ...
      me: params.me ?? { username: "genosos_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors threaded replies for replyToMode=first/all", async () => {
    for (const [mode, messageId] of [
      ...
      expect(sendMessageSpy.mock.calls.length).toBeGreaterThan(1);
      for (const call of sendMessageSpy.mock.calls) {
        expect(call[2]?.reply_to_message_id).toBe(messageId);
      }
    }
  });
  ...
  it("honors routed group activation from session store", async () => {
    onSpy.mockReset();
    replySpy.mockReset();
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "genosos-telegram-"));
  ...
  it("buffers channel_post media groups and processes them together", async () => {
    ...
    try {
      ...
      const flushTimer = flushTimerCall?.[0];
