...
describe("resolveTelegramAutoSelectFamilyDecision", () => {
  it("prefers env enable over env disable", () => {
    const decision = resolveTelegramAutoSelectFamilyDecision({
      env: {
        GENOS_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1",
        GENOS_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1",
      },
      nodeMajor: 22,
    });
    expect(decision).toEqual({
      value: true,
      source: "env:GENOS_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
    });
  });
  it("uses env disable when set", () => {
    const decision = resolveTelegramAutoSelectFamilyDecision({
      env: { GENOS_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      nodeMajor: 22,
    });
    expect(decision).toEqual({
      value: false,
      source: "env:GENOS_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
    });
  });
  it("prefers env enable over config", () => {
    const decision = resolveTelegramAutoSelectFamilyDecision({
      env: { GENOS_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
