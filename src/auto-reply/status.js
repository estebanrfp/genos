let resolveRuntimeLabel = function () {
    return "direct";
  },
  groupCommandsByCategory = function (commands) {
  ...
  formatCommandEntry = function (command) {
  ...
  buildCommandItems = function (commands, pluginCommands) {
  ...
  formatCommandList = function (items) {
    const lines = [];
...
const readUsageFromSessionLog = (sessionId, sessionEntry, agentId, sessionKey, storePath) => {
  ...
  try {
    const lines = secureReadFileSync(logPath).split(/\n+/);
    let input = 0;
    let output = 0;
    let promptTokens = 0;
    let model;
...
const formatVoiceModeLine = (config, sessionEntry) => {
  ...
  return `\uD83D\uDD0A Voice: ${autoMode} \xB7 provider=${provider} \xB7 limit=${maxLength} \xB7 summary=${summarize}`;
};
export function buildStatusMessage(args) {
  ...
  const authLabel = authLabelValue ? ` \xB7 \uD83D\uDD11 ${authLabelValue}` : "";
  const modelLine = `\uD83E\uDDE0 Model: ${modelLabel}${authLabel}`;
  const commit = resolveCommitHash();
  const versionLine = `\uD83E\uDD9E GenosOS ${VERSION}${commit ? ` (${commit})` : ""}`;
  const usagePair = formatUsagePair(inputTokens, outputTokens);
  const costLine = costLabel ? `\uD83D\uDCB5 Cost: ${costLabel}` : null;
  const usageCostLine =
    usagePair && costLine ? `${usagePair} \xB7 ${costLine}` : (usagePair ?? costLine);
