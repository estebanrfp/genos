...
vi.mock("../agents/skills.js", () => {
  function resolveUniqueName(base, used) {
    let name = base;
    let suffix = 2;
  ...
  function resolveWorkspaceSkills(workspaceDir) {
...
let listSkillCommandsForAgents;
let resolveSkillCommandInvocation;
beforeAll(async () => {
...
describe("listSkillCommandsForAgents", () => {
  it("merges command names across agents and de-duplicates", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-skills-"));
