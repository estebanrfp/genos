let getTextAliasMap = function () {
  ...
  buildSkillCommandDefinitions = function (skillCommands) {
  ...
  resolveNativeName = function (command, provider) {
  ...
  toNativeCommandSpec = function (command, provider) {
    return {
  ...
  listNativeSpecsFromCommands = function (commands, provider) {
    return commands
      .filter((command) => command.scope !== "text" && command.nativeName)
      .map((command) => toNativeCommandSpec(command, provider));
  },
  parsePositionalArgs = function (definitions, raw) {
    const values = {};
  ...
  formatPositionalArgs = function (definitions, values) {
    const parts = [];
    for (const definition of definitions) {
  ...
  resolveDefaultCommandContext = function (cfg) {
