let formatMetaForDisplay = function (toolName, meta, markdown) {
    const normalized = (toolName ?? "").trim().toLowerCase();
    if (normalized === "exec" || normalized === "bash") {
      const { flags, body } = splitExecFlags(meta);
      if (flags.length > 0) {
        if (!body) {
          return flags.join(" \xB7 ");
        }
        return `${flags.join(" \xB7 ")} \xB7 ${maybeWrapMarkdown(body, markdown)}`;
      }
    }
    return maybeWrapMarkdown(meta, markdown);
  },
  splitExecFlags = function (meta) {
    const parts = meta
      .split(" \xB7 ")
      .map((part) => part.trim())
      .filter(Boolean);
    ...
    return { flags, body: bodyParts.join(" \xB7 ") };
  },
  isPathLike = function (value) {
    if (!value) {
      return false;
    }
    if (value.includes(" ")) {
  ...
  maybeWrapMarkdown = function (value, markdown) {
    if (!markdown) {
      return value;
    }
    if (value.includes("`")) {
