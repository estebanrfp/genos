let buildParams = function (commandBody, cfg, ctxOverrides) {
    return buildCommandTestParams(commandBody, cfg, ctxOverrides, {
      workspaceDir: testWorkspaceDir,
    });
  },
  buildPolicyParams = function (commandBody, cfg, ctxOverrides) {
    const ctx = {
      Body: commandBody,
...
vi.mock("../../pairing/pairing-store.js", async () => {
  const actual = await vi.importActual("../../pairing/pairing-store.js");
  return {
    ...actual,
...
vi.mock("../../channels/plugins/pairing.js", async () => {
  const actual = await vi.importActual("../../channels/plugins/pairing.js");
  return {
    ...actual,
...
vi.mock("../../agents/pi-embedded.js", () => {
  const resolveEmbeddedSessionLane = (key) => {
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-commands-"));
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/genosos-session-store.json" },
    };
...
describe("parseConfigCommand", () => {
  it("parses menu and section", () => {
    expect(parseConfigCommand("/config")).toEqual({ action: "menu" });
    expect(parseConfigCommand("/config 1")).toEqual({ action: "section", sectionNum: 1 });
    expect(parseConfigCommand("/config 13")).toEqual({ action: "section", sectionNum: 13 });
    expect(parseConfigCommand("/config 14")).toEqual({
      action: "error",
      message: expect.stringContaining("Invalid section number"),
    });
    expect(parseConfigCommand("/config 99")).toEqual({
      action: "error",
      message: expect.stringContaining("Invalid section number"),
    });
  });
...
describe("handleCommands /allowlist", () => {
  ...
  it("adds entries to config and pairing store", async () => {
    ...
    validateConfigObjectWithPluginsMock.mockImplementation((config) => ({
      ok: true,
      config,
    }));
...
describe("/models command", () => {
  ...
  it("lists providers on telegram (buttons)", async () => {
    ...
    const buttons = result.reply?.channelData?.telegram?.buttons;
...
describe("handleCommands subagents", () => {
  ...
  it("includes subagent details in /status when verbose", async () => {
    ...
    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("\uD83E\uDD16 Subagents: 1 active");
    expect(result.reply?.text).toContain("\xB7 1 done");
  });
  ...
  it("sends follow-up messages to finished subagents", async () => {
    ...
    const waitCall = callGatewayMock.mock.calls.find(
      (call) => call[0].method === "agent.wait" && call[0].params?.runId === "run-followup-1",
    );
  ...
  it("steers subagents via /steer alias", async () => {
    ...
    const steerWaitIndex = callGatewayMock.mock.calls.findIndex(
      (call) => call[0].method === "agent.wait" && call[0].params?.runId === "run-1",
    );
    ...
    expect(callGatewayMock.mock.calls[steerWaitIndex]?.[0]).toMatchObject({
      method: "agent.wait",
      params: { runId: "run-1", timeoutMs: 5000 },
      timeoutMs: 7000,
    });
