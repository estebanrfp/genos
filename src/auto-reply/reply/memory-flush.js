let formatDateStampInTimezone = function (nowMs, timezone) {
  ...
  ensureNoReplyHint = function (text) {
...
import { buildMemoryDocumentTemplate } from "../../agents/memory-document-template.js";
...
export function resolveMemoryFlushSettings(cfg) {
  ...
  const template = buildMemoryDocumentTemplate(cfg);
  const basePrompt = customPrompt || DEFAULT_MEMORY_FLUSH_PROMPT;
  const prompt = template && !customPrompt ? `${basePrompt}\n\n${template}` : basePrompt;
