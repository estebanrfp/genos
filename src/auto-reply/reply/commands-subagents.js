let compactLine = function (value) {
    return value.replace(/\s+/g, " ").trim();
  },
  formatTaskPreview = function (value) {
    return truncateLine(compactLine(value), SUBAGENT_TASK_PREVIEW_MAX);
  },
  resolveModelDisplay = function (entry, fallbackModel) {
  ...
  resolveDisplayStatus = function (entry) {
    const status = formatRunStatus(entry);
    return status === "error" ? "failed" : status;
  },
  formatSubagentListLine = function (params) {
  ...
  formatTimestamp = function (valueMs) {
    if (!valueMs || !Number.isFinite(valueMs) || valueMs <= 0) {
      return "n/a";
    }
    return new Date(valueMs).toISOString();
  },
  formatTimestampWithAge = function (valueMs) {
    if (!valueMs || !Number.isFinite(valueMs) || valueMs <= 0) {
      return "n/a";
    }
    return `${formatTimestamp(valueMs)} (${formatTimeAgo(Date.now() - valueMs, { fallback: "n/a" })})`;
  },
  resolveRequesterSessionKey = function (params, opts) {
  ...
  resolveSubagentTarget = function (runs, token) {
  ...
  formatLogLines = function (messages) {
    const lines = [];
  ...
  loadSubagentSessionEntry = function (params, childKey, storeCache) {
...
export const handleSubagentsCommand = async (params, allowTextCommands) => {
  ...
  if (action === "list") {
    ...
    const now = Date.now();
    const recentCutoff = now - RECENT_WINDOW_MINUTES * 60000;
    const storeCache = new Map();
    let index = 1;
    const mapRuns = (entries, runtimeMs) =>
      entries.map((entry) => {
