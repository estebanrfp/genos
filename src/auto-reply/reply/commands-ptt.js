let normalizeNodeKey = function (value) {
    return value
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+/, "")
  ...
  describeNodes = function (nodes) {
    return nodes
      .map((node) => node.displayName || node.remoteIp || node.nodeId)
      .filter(Boolean)
      .join(", ");
  },
  resolveNodeId = function (nodes, query) {
  ...
  parsePTTArgs = function (commandBody) {
