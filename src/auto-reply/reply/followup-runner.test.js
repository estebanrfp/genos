let mockCompactionRun = function (params) {
...
describe("createFollowupRunner compaction", () => {
  it("adds verbose auto-compaction notice and tracks count", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "genosos-compaction-")),
      "sessions.json",
    );
    const sessionEntry = {
      sessionId: "session",
...
describe("createFollowupRunner messaging tool dedupe", () => {
  ...
  it("persists usage even when replies are suppressed", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(tmpdir(), "genosos-followup-usage-")),
      "sessions.json",
    );
    ...
    expect(store[sessionKey]?.inputTokens).toBe(1000);
