let parseAllowlistCommand = function (raw) {
  ...
  normalizeAllowFrom = function (params) {
  ...
  formatEntryList = function (entries, resolved) {
    if (entries.length === 0) {
      return "(none)";
    }
    return entries
  ...
  resolveAccountTarget = function (parsed, channelId, accountId) {
  ...
  getNestedValue = function (root, path) {
    let current = root;
  ...
  ensureNestedObject = function (root, path) {
    let current = root;
    for (const key of path) {
  ...
  setNestedValue = function (root, path, value) {
    if (path.length === 0) {
      return;
    }
    if (path.length === 1) {
  ...
  deleteNestedValue = function (root, path) {
    if (path.length === 0) {
      return;
    }
    if (path.length === 1) {
  ...
  resolveChannelAllowFromPaths = function (channelId, scope) {
  ...
  mapResolvedAllowlistNames = function (entries) {
    const map = new Map();
...
export const handleAllowlistCommand = async (params, allowTextCommands) => {
  ...
  if (shouldUpdateConfig) {
    ...
    if (!configChanged && !shouldTouchStore) {
      const message =
        parsed.action === "add" ? "\u2705 Already allowlisted." : "\u26A0\uFE0F Entry not found.";
