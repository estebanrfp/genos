let createMinimalRun = function (params) {
...
vi.mock("../../agents/model-fallback.js", () => ({
  runWithModelFallback: async ({ provider, model, run }) => ({
...
vi.mock("../../agents/pi-embedded.js", () => ({
  ...
  runEmbeddedPiAgent: (params) => state.runEmbeddedPiAgentMock(params),
}));
vi.mock("../../agents/cli-runner.js", () => ({
  runCliAgent: (params) => state.runCliAgentMock(params),
}));
vi.mock("./queue.js", () => ({
...
describe("runReplyAgent typing (heartbeat)", () => {
  let fixtureRoot = "";
  let caseId = 0;
  function snapshotStateEnv() {
    return { GENOS_STATE_DIR: process.env.GENOS_STATE_DIR };
  }
  function restoreStateEnv(snapshot) {
    if (snapshot.GENOS_STATE_DIR === undefined) {
      delete process.env.GENOS_STATE_DIR;
    } else {
      process.env.GENOS_STATE_DIR = snapshot.GENOS_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "genosos-typing-heartbeat-"));
  });
  afterAll(async () => {
    if (fixtureRoot) {
  ...
  it("signals typing for normal runs", async () => {
    ...
    state.runEmbeddedPiAgentMock.mockImplementationOnce(async (params) => {
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "genosos-memory-flush-"));
  });
  afterAll(async () => {
    if (fixtureRoot) {
      await fs.rm(fixtureRoot, { recursive: true, force: true });
  ...
  it("skips memory flush for CLI providers", async () => {
    await withTempStore(async (storePath) => {
      ...
      const call = state.runCliAgentMock.mock.calls[0]?.[0];
  ...
  it("runs a memory flush turn and updates session metadata", async () => {
    await withTempStore(async (storePath) => {
      ...
      await seedSessionStore({ storePath, sessionKey, entry: sessionEntry });
      const calls = [];
      state.runEmbeddedPiAgentMock.mockImplementation(async (params) => {
  ...
  it("skips memory flush when disabled in config", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(state.runEmbeddedPiAgentMock).toHaveBeenCalledTimes(1);
      const call = state.runEmbeddedPiAgentMock.mock.calls[0]?.[0];
      expect(call?.prompt).toBe("hello");
      const stored = JSON.parse(await fs.readFile(storePath, "utf-8"));
