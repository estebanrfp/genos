...
const getLineData = (result) => result.channelData?.line ?? {};
...
describe("parseLineDirectives", () => {
  ...
  describe("buttons", () => {
    ...
    it("parses buttons with uri actions", () => {
      ...
      const templateMessage = getLineData(result).templateMessage;
  ...
  describe("event", () => {
    it("parses event with all fields", () => {
      ...
      const flexMessage = getLineData(result).flexMessage;
      expect(flexMessage).toBeDefined();
      expect(flexMessage?.altText).toBe(
        "\uD83D\uDCC5 Team Meeting - January 24, 2026 2:00 PM - 3:00 PM",
      );
    });
...
describe("createReplyDispatcher", () => {
  ...
  it("fires onIdle when the queue drains", async () => {
    const deliver = async () => await Promise.resolve();
    const onIdle = vi.fn();
