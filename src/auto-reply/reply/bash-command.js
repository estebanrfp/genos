let resolveForegroundMs = function (cfg) {
  ...
  formatSessionSnippet = function (sessionId) {
  ...
  formatOutputBlock = function (text) {
  ...
  resolveRawCommandBody = function (params) {
  ...
  getScopedSession = function (sessionId) {
  ...
  attachActiveWatcher = function (sessionId) {
...
export async function handleBashChatCommand(params) {
  if (params.cfg.commands?.bash !== true) {
    return {
      text: "\u26A0\uFE0F bash is disabled. Set commands.bash=true to enable. Docs: https://docs.genos.ai/tools/slash-commands#config",
    };
  }
  const agentId =
    params.agentId ??
