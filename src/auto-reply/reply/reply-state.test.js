...
async function createCompactionSessionFixture(entry) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-compact-"));
...
describe("shouldRunMemoryFlush", () => {
  ...
  it("ignores stale cached totals", () => {
    expect(
      shouldRunMemoryFlush({
        entry: { totalTokens: 96000, totalTokensFresh: false, compactionCount: 1 },
        contextWindowTokens: 1e5,
        reserveTokensFloor: 5000,
        softThresholdTokens: 2000,
      }),
...
describe("incrementCompactionCount", () => {
  it("increments compaction count", async () => {
    ...
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const entry = { sessionId: "s1", updatedAt: Date.now(), compactionCount: 2 };
    const sessionStore = { [sessionKey]: entry };
    await seedSessionStore({ storePath, sessionKey, entry });
    ...
    expect(stored[sessionKey].systemSent).toBe(false);
  });
  ...
  it("does not update totalTokens when tokensAfter is not provided", async () => {
    ...
    const stored = JSON.parse(await fs.readFile(storePath, "utf-8"));
    expect(stored[sessionKey].compactionCount).toBe(1);
    expect(stored[sessionKey].totalTokens).toBe(180000);
  });
});
