...
export async function runPreparedReply(params) {
  ...
  const ownerIdentityPrompt =
    ownerDisplayName && sessionCtx.Surface === INTERNAL_MESSAGE_CHANNEL
      ? `Messages from the user in this session come from ${ownerDisplayName} (the human owner).`
      : undefined;
    ...
    if (channel && to) {
      ...
      const text =
        modelLabel === defaultLabel
          ? `\u2705 New session started \xB7 model: ${modelLabel}`
          : `\u2705 New session started \xB7 model: ${modelLabel} (default: ${defaultLabel})`;
      await routeReply({
        payload: { text },
        channel,
        to,
