let hasUnbackedReminderCommitment = function (text) {
  ...
  appendUnscheduledReminderNote = function (payloads) {
    let appended = false;
...
const BLOCK_REPLY_SEND_TIMEOUT_MS = 15000;
...
const pendingPostCompactionAudits = new Map();
  ...
  try {
    ...
    if (responseUsageMode !== "off" && hasNonzeroUsage(usage)) {
      ...
      if (formatted && responseUsageMode === "full" && sessionKey) {
        formatted = `${formatted} \xB7 session ${sessionKey}`;
      }
      if (formatted) {
