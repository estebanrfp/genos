...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("GenosOS");
  ...
  it("includes media understanding decisions when present", () => {
    ...
    const normalized = normalizeTestText(text);
    expect(normalized).toContain("Media: image ok (openai/gpt-5.2) \xB7 audio skipped (maxBytes)");
  });
  ...
  it("shows queue details when overridden", () => {
    ...
    expect(text).toContain("Queue: collect (depth 3 \xB7 debounce 2s \xB7 cap 5 \xB7 drop old)");
  });
