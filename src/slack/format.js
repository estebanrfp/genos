let escapeSlackMrkdwnSegment = function (text) {
    return text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  },
  isAllowedSlackAngleToken = function (token) {
  ...
  escapeSlackMrkdwnContent = function (text) {
  ...
  escapeSlackMrkdwnText = function (text) {
    if (!text.includes("&") && !text.includes("<") && !text.includes(">")) {
      return text;
    }
    return text
  ...
  buildSlackLink = function (link, text) {
  ...
  buildSlackRenderOptions = function () {
    return {
      styleMarkers: {
        bold: { open: "*", close: "*" },
