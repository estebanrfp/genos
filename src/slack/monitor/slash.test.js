let encodeValue = function (parts) {
    return [
      "cmdarg",
  ...
  findFirstActionsBlock = function (payload) {
    return payload.blocks?.find((block) => block.type === "actions");
  },
  createArgMenusHarness = function () {
  ...
  requireHandler = function (handlers, key, label) {
  ...
  createSlashCommand = function (overrides = {}) {
    return {
      user_id: "U1",
      user_name: "Ada",
      channel_id: "C1",
  ...
  expectArgMenuLayout = function (respond) {
  ...
  createPolicyHarness = function (overrides) {
    const commands = new Map();
    const postEphemeral = vi.fn().mockResolvedValue({ ok: true });
    const app = {
      client: { chat: { postEphemeral } },
  ...
  expectChannelBlockedResponse = function (respond) {
  ...
  expectUnauthorizedResponse = function (respond) {
    expect(dispatchMock).not.toHaveBeenCalled();
    expect(respond).toHaveBeenCalledWith({
...
vi.mock("../../auto-reply/commands-registry.js", () => {
  ...
  const hasNonEmptyArgValue = (values, key) => {
    const raw = typeof values === "object" && values !== null ? values[key] : undefined;
...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "genosos_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "genosos_cmdarg", "arg-menu options");
  });
  beforeEach(() => {
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("genosos_cmdarg");
  ...
  it("serves filtered options for external_select menus", async () => {
    ...
    expect(blockId).toContain("genosos_cmdarg_ext:");
