let createContext = function () {
  ...
  let viewClosedHandler = null;
  const app = {
    action: vi.fn((_matcher, next) => {
      handler = next;
    }),
    view: vi.fn((_matcher, next) => {
      viewHandler = next;
    }),
    viewClosed: vi.fn((_matcher, next) => {
...
describe("registerSlackInteractionEvents", () => {
  it("enqueues structured events and updates button rows", async () => {
    ...
    registerSlackInteractionEvents({ ctx });
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "genosos:verify" }],
            },
          ],
        },
      },
    ...
    const [eventText] = enqueueSystemEventMock.mock.calls[0];
    expect(eventText.startsWith("Slack interaction: ")).toBe(true);
    const payload = JSON.parse(eventText.replace("Slack interaction: ", ""));
    expect(payload).toMatchObject({
      actionId: "genosos:verify",
      actionType: "button",
      value: "approved",
      userId: "U123",
      teamId: "T9",
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "genosos:multi" }],
            },
          ],
        },
      },
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "genosos:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "genosos:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "genosos:datetime" }],
            },
          ],
        },
      },
        ...
        blocks: [
          {
            ...
            elements: [
              {
                type: "mrkdwn",
                text: `:white_check_mark: *${new Date(selectedDateTimeEpoch * 1000).toISOString()}* selected by <@U333>`,
              },
            ],
          },
        ],
  ...
  it("captures expanded selection and temporal payload fields", async () => {
    ...
    expect(payload.selectedDateTime).toBe(1771700200);
  });
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    await viewHandler({
      ...
      body: {
        ...
        view: {
          id: "V123",
          callback_id: "genosos:deploy_form",
          root_view_id: "VROOT",
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:genosos:deploy_form",
      callbackId: "genosos:deploy_form",
      viewId: "V123",
      userId: "U777",
  ...
  it("captures modal input labels and picker values across block types", async () => {
    ...
    await viewHandler({
      ...
      body: {
        ...
        view: {
          id: "V400",
          callback_id: "genosos:routing_form",
          state: {
            values: {
              env_block: {
                env_select: {
              ...
              email_block: {
                email_input: {
                  ...
                  value: "team@genosos.ai",
                },
              },
              url_block: {
                url_input: {
                  type: "url_text_input",
                  value: "https://docs.genos.ai",
                },
              },
              richtext_block: {
                richtext_input: {
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ actionId: "datetime_select", selectedDateTime: 1771632300 }),
        expect.objectContaining({
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@genosos.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.genos.ai/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "genosos:long_richtext",
          state: {
            values: {
              richtext_block: {
                richtext_input: {
  ...
  it("captures modal close events and enqueues view closed event", async () => {
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledTimes(1);
    const [eventText, options] = enqueueSystemEventMock.mock.calls[0];
    const payload = JSON.parse(eventText.replace("Slack interaction: ", ""));
    expect(payload).toMatchObject({
