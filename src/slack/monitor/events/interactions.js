let readOptionValues = function (options) {
  ...
  readOptionLabels = function (options) {
    if (!Array.isArray(options)) {
      return;
    }
  ...
  uniqueNonEmptyStrings = function (values) {
    const unique = [];
    const seen = new Set();
  ...
  escapeSlackMrkdwn = function (value) {
    return value
      .replaceAll("\\", "\\\\")
      .replaceAll("&", "&amp;")
      .replaceAll("<", "&lt;")
  ...
  collectRichTextFragments = function (value, out) {
  ...
  summarizeRichTextPreview = function (value) {
    const fragments = [];
  ...
  readInteractionAction = function (raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
      return;
    }
    return raw;
  },
  summarizeAction = function (action) {
    const typed = action;
  ...
  isBulkActionsBlock = function (block) {
    return (
      block.type === "actions" &&
      Array.isArray(block.elements) &&
      block.elements.length > 0 &&
      block.elements.every(
        (el) => typeof el.action_id === "string" && el.action_id.includes("_all_"),
      )
    );
  },
  formatInteractionSelectionLabel = function (params) {
  ...
  formatInteractionConfirmationText = function (params) {
  ...
  summarizeViewState = function (values) {
  ...
  resolveModalSessionRouting = function (params) {
  ...
  summarizeSlackViewLifecycleContext = function (view) {
...
const GENOS_ACTION_PREFIX = "genosos:";
export function registerSlackInteractionEvents(params) {
  const { ctx } = params;
  if (typeof ctx.app.action !== "function") {
    return;
  }
  ctx.app.action(new RegExp(`^${GENOS_ACTION_PREFIX}`), async (args) => {
  ...
  ctx.app.view(new RegExp(`^${GENOS_ACTION_PREFIX}`), async ({ ack, body }) => {
    await ack();
    const typedBody = body;
    ...
    enqueueSystemEvent(`Slack interaction: ${JSON.stringify(eventPayload)}`, {
      ...
      contextKey: ["slack:interaction:view", callbackId, viewId, userId].filter(Boolean).join(":"),
    });
  });
  ...
  viewClosed(new RegExp(`^${GENOS_ACTION_PREFIX}`), async ({ ack, body }) => {
    await ack();
    const typedBody = body;
    const callbackId = typedBody.view?.callback_id ?? "unknown";
    const userId = typedBody.user?.id ?? "unknown";
    ...
    ctx.runtime.log?.(
      `slack:interaction view_closed callback=${callbackId} user=${userId} cleared=${typedBody.is_cleared === true}`,
    );
    enqueueSystemEvent(`Slack interaction: ${JSON.stringify(eventPayload)}`, {
      sessionKey: sessionRouting.sessionKey,
