let truncatePlainText = function (value, max) {
  ...
  buildSlackArgMenuConfirm = function (params) {
  ...
  pruneSlackExternalArgMenuStore = function (now = Date.now()) {
  ...
  storeSlackExternalArgMenu = function (params) {
  ...
  readSlackExternalArgMenuToken = function (raw) {
  ...
  encodeSlackCommandArgValue = function (parts) {
    return [
  ...
  parseSlackCommandArgValue = function (raw) {
    if (!raw) {
      return null;
    }
  ...
  buildSlackCommandArgMenuBlocks = function (params) {
...
const SLACK_COMMAND_ARG_ACTION_ID = "genosos_cmdarg";
...
const SLACK_COMMAND_ARG_EXTERNAL_PREFIX = "genosos_cmdarg_ext:";
const SLACK_COMMAND_ARG_EXTERNAL_TTL_MS = 600000;
