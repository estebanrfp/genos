...
describe("git-hooks/pre-commit (integration)", () => {
  it("does not treat staged filenames as git-add flags (e.g. --all)", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "genosos-pre-commit-"));
    run(dir, "git", ["init", "-q"]);
    ...
    chmodSync(path.join(dir, "git-hooks", "pre-commit"), 493);
    chmodSync(path.join(dir, "scripts", "pre-commit", "run-node-tool.sh"), 493);
