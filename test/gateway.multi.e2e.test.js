...
const GATEWAY_START_TIMEOUT_MS = 45000;
...
const spawnGatewayInstance = async (name) => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `genosos-e2e-${name}-`));
  const configDir = path.join(homeDir, ".genos");
...
const connectStatusClient = async (inst, timeoutMs = 5000) => {
  let settled = false;
  let timer = null;
  return await new Promise((resolve, reject) => {
    const finish = (err) => {
