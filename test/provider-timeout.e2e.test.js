let buildOpenAIResponsesSse = function (text) {
    const events = [
      {
...
describe("provider timeouts (e2e)", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-timeout-e2e-"));
      ...
      const configDir = path.join(tempHome, ".genos");
