...
const unitIsolatedFilesRaw = [
  "src/plugins/loader.test.js",
  "src/plugins/tools.optional.test.js",
  "src/agents/session-tool-result-guard.tool-result-persist-hook.test.js",
  "src/security/fix.test.js",
  "src/security/audit.test.js",
  "src/utils.test.js",
  "src/auto-reply/tool-meta.test.js",
  "src/auto-reply/envelope.test.js",
  "src/commands/auth-choice.test.js",
  "src/media/store.test.js",
  "src/media/store.header-ext.test.js",
  "src/web/media.test.js",
  "src/web/auto-reply.web-auto-reply.falls-back-text-media-send-fails.test.js",
  "src/browser/server.covers-additional-endpoint-branches.test.js",
  "src/browser/server.post-tabs-open-profile-unknown-returns-404.test.js",
  "src/browser/server.agent-contract-snapshot-endpoints.test.js",
  "src/browser/server.agent-contract-form-layout-act-commands.test.js",
  "src/browser/server.skips-default-maxchars-explicitly-set-zero.test.js",
  "src/browser/server.auth-token-gates-http.test.js",
  // Keep this high-variance heavy file off the unit-fast critical path.
  "src/auto-reply/reply.block-streaming.test.js",
  // Archive extraction/fixture-heavy suite; keep off unit-fast critical path.
  "src/hooks/install.test.js",
  // Setup-heavy bot bootstrap suite.
  "src/telegram/bot.create-telegram-bot.test.js",
  // Medium-heavy bot behavior suite; move off unit-fast critical path.
  "src/telegram/bot.test.js",
  // Slack slash registration tests are setup-heavy and can bottleneck unit-fast.
  "src/slack/monitor/slash.test.js",
  // Uses process-level unhandledRejection listeners; keep it off vmForks to avoid cross-file leakage.
  "src/imessage/monitor.shutdown.unhandled-rejection.test.js",
];
...
// GENOS_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.GENOS_TEST_VM_FORKS === "1" ||
  (process.env.GENOS_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks);
const disableIsolation = process.env.GENOS_TEST_NO_ISOLATE === "1";
const runs = [
  ...(useVmForks
    ? [
        {
...
const shardOverride = Number.parseInt(process.env.GENOS_TEST_SHARDS ?? "", 10);
...
const silentArgs = process.env.GENOS_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs =
  rawPassthroughArgs[0] === "--" ? rawPassthroughArgs.slice(1) : rawPassthroughArgs;
const rawTestProfile = process.env.GENOS_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
  ...
  rawTestProfile === "serial"
    ? rawTestProfile
    : "normal";
const overrideWorkers = Number.parseInt(process.env.GENOS_TEST_WORKERS ?? "", 10);
const resolvedOverride =
  Number.isFinite(overrideWorkers) && overrideWorkers > 0 ? overrideWorkers : null;
// Keep gateway serial on Windows CI and CI by default; run in parallel locally
// for lower wall-clock time. CI can opt in via GENOS_TEST_PARALLEL_GATEWAY=1.
const keepGatewaySerial =
  isWindowsCi ||
  process.env.GENOS_TEST_SERIAL_GATEWAY === "1" ||
  testProfile === "serial" ||
  (isCI && process.env.GENOS_TEST_PARALLEL_GATEWAY !== "1");
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.GENOS_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
...
function resolveReportDir() {
  const raw = process.env.GENOS_VITEST_REPORT_DIR?.trim();
  if (!raw) {
    return null;
  }
  try {
