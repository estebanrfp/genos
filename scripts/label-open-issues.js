let parseArgs = function (argv) {
  ...
  loadState = function (statePath) {
    ...
    const issues = Array.isArray(parsed.issues)
      ? parsed.issues.filter((value) => typeof value === "number" && Number.isFinite(value))
      : [];
    const pullRequests = Array.isArray(parsed.pullRequests)
      ? parsed.pullRequests.filter((value) => typeof value === "number" && Number.isFinite(value))
      : [];
    const state = {
      version: STATE_VERSION,
      issues,
  ...
  saveState = function (statePath, state) {
    mkdirSync(dirname(statePath), { recursive: true });
    writeFileSync(statePath, `${JSON.stringify(state, null, 2)}\n`);
  },
  buildStateSnapshot = function (issueSet, pullRequestSet) {
    return {
      version: STATE_VERSION,
  ...
  fetchIssuePage = function (repo, after) {
    const args = [
      "api",
      "graphql",
      "-f",
  ...
  fetchPullRequestPage = function (repo, after) {
    const args = [
      "api",
      "graphql",
      "-f",
  ...
  buildItemPrompt = function (item, kind) {
    const body = truncateBody(item.body?.trim() ?? "");
    return `Type: ${kind}\nTitle:\n${item.title.trim()}\n\nBody:\n${body}`;
  },
  extractResponseText = function (payload) {
  ...
  fallbackCategory = function (issueText) {
  ...
  normalizeClassification = function (raw, issueText) {
    ...
    const category =
      categoryRaw === "bug" || categoryRaw === "enhancement" ? categoryRaw : fallback;
    const isSupport = raw.isSupport === true;
    const isSkillOnly = raw.isSkillOnly === true;
    return { category, isSupport, isSkillOnly };
  },
  applyLabels = function (target, item, labelsToAdd, dryRun) {
...
const STATE_FILE_PATH = join(CONFIG_BASE_DIR, "genosos", STATE_FILE_NAME);
const ISSUE_QUERY = `
...
`;
function* fetchOpenIssueBatches(limit) {
...
function* fetchOpenPullRequestBatches(limit) {
  const repo = resolveRepo();
  const results = [];
  let page = 1;
  let after = null;
...
async function classifyItem(item, kind, options) {
      ...
      input: [
        {
          role: "system",
          content:
            "You classify GitHub issues and pull requests for GenosOS. Respond with JSON only, no extra text.",
        },
        {
          role: "user",
          content: [
...
async function main() {
  ...
  logHeader("GenosOS Issue Label Audit");
