let run = function (cmd) {
  ...
  parsePaginatedJson = function (raw) {
    const items = [];
  ...
  isGhostAvatar = function (url) {
    return url.toLowerCase().includes("ghost.png");
  },
  fetchUser = function (login) {
  ...
  resolveLogin = function (name, email, apiByLogin, nameToLogin, emailToLogin) {
  ...
  guessLoginFromEmailName = function (name, email, apiByLogin) {
  ...
  parseReadmeEntries = function (content) {
...
const REPO = "genosos/genosos";
const PER_LINE = 10;
const mapPath = resolve("scripts/clawtributors-map.json");
const mapConfig = JSON.parse(readFileSync(mapPath, "utf8"));
