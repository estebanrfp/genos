...
# GenosOS Auth Widget for Termux
...
SERVER="${GENOS_SERVER:-${GENOS_SERVER:-l36}}"

# Check auth status
termux-toast "Checking GenosOS auth..."

STATUS=$(ssh "$SERVER" '$HOME/genosos/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/genosos/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|GENOS_EXPIRING|GENOS_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/genosos/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|GENOS_EXPIRED|GENOS_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: GenosOS won't work until re-auth"
                ;;
        esac
        ;;
