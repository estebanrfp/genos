...
#
# Suggested cron: */30 * * * * /home/admin/genosos/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send GenosOS notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., genosos-alerts)
...
STATE_FILE="$HOME/.genos/auth-monitor-state"
...
send_notification() {
    ...
    # Send via GenosOS if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use genosos
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via GenosOS to $NOTIFY_PHONE..."
            genosos send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! GenosOS is down. Run: ssh l36 '~/genosos/scripts/mobile-reauth.sh'" "urgent"
    exit 1
