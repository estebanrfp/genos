...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/genosos/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "GenosOS Re-Auth" -c "After getting key, run: ssh $SERVER '~/genosos/scripts/mobile-reauth.sh'" --id genosos-auth
        ;;
    *)
