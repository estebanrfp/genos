...
BASE_IMAGE="${BASE_IMAGE:-genosos-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-genosos-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=genosos.sandbox=1)
NOTE
