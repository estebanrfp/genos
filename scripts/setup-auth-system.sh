#!/bin/bash
# Setup GenosOS Auth Management System
...
echo "=== GenosOS Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'genosos-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  GenosOS message: Send warning via GenosOS itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/genosos-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/genosos-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/genosos-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now genosos-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status genosos-auth-monitor.timer"
echo ""
echo "Quick commands:"
