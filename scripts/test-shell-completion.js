...
\`genosos update\`, \`genosos doctor\`, and \`genosos onboard\`.
...
${theme.heading("Examples:")}
  bun scripts/test-shell-completion.ts
  bun scripts/test-shell-completion.ts --check-only
  bun scripts/test-shell-completion.ts --force
`);
  },
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# GenosOS Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`genosos completion --install\` later to enable.`));
    return;
  }
