#!/usr/bin/env bash
# ClawDock - Docker helpers for GenosOS
# Inspired by Simon Willison's "Running GenosOS in Docker"
# https://til.simonwillison.net/llms/genosos-docker
#
# Installation:
...
# Common paths to check for GenosOS
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/genosos"
  "${HOME}/workspace/genosos"
  "${HOME}/projects/genosos"
  "${HOME}/dev/genosos"
  "${HOME}/code/genosos"
  "${HOME}/src/genosos"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found GenosOS at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/genosos"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ GenosOS not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/genosos/genosos.git ~/genosos"
    echo "  cd ~/genosos && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^GENOS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d genosos-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart genosos-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f genosos-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec genosos-gateway \
    bash -c 'echo "alias genosos=\"./genosos.mjs\"" > /tmp/.bashrc_genosos && bash --rcfile /tmp/.bashrc_genosos'
}
...
clawdock-cli() {
  _clawdock_compose run --rm genosos-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build genosos-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "GENOS_GATEWAY_TOKEN=$token" genosos-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" genosos-gateway \
    bash -c './genosos.mjs config set gateway.remote.token "$TOKEN" && ./genosos.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec genosos-gateway \
    bash -c "./genosos.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart genosos-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm genosos-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec genosos-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'genosos config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for GenosOS${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (genosos alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to genosos project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.genos)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'genosos channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'genosos status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.genos.ai${_CLR_RESET}"
  echo ""
}
