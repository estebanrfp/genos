...
12+ UI tabs eliminated and replaced with `config_manage` (25 actions, 164 blueprints, 12 operational guides). Most AI projects add UI on top of UI. GenosOS does the opposite: _configuration is conversation_. The blueprint system acts as a semantic bridge between raw JSON paths and natural language guidance — not just CRUD, but contextual validation with cross-field rules, auto-type coercion, and intelligent post-set defaults.
...
| Model for business           | routing: normal (balanced)                   | Nothing (picks the most efficient)     |
...
│   │   │   └── blueprints/     # 12 files, 164 declarative blueprints
│   │   ├── auto-config.js      # 6 pure functions for intelligent defaults
│   │   ├── model-routing.js    # Smart prompt tier classification
...
│   │   └── static-model-catalog.json # 4 providers × 3 tiers
...
- **Smart model routing** (tier classification per prompt)
...
```
classifyPromptTier(prompt) → "normal" | "complex"

Scoring:
  Token count >2000 (+4), >200 (+2)
  Images ≥2 (+3), =1 (+1)
  Code markers ≥3 (+3), ≥1 (+1)
  Analysis keywords ≥2 (+3), =1 (+1)
  Reasoning keywords (+4)
  Deep conversation >10 turns (+2)

Thresholds: ≤7 normal, >7 complex
```
...
- **Blueprints:** 164 declarative configs across 12 files + 16 operational guides in TOON format
- **Auto-config:** 6 pure functions for intelligent defaults
- **config_manage:** 25 RPC actions, all from chat
