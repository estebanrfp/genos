let getSectionIcon = function (key) {
    return sidebarIcons[key] ?? sidebarIcons.default;
  },
  resolveSectionMeta = function (key, schema) {
  ...
  resolveSubsections = function (params) {
  ...
  computeDiff = function (original, current) {
    if (!original || !current) {
      return [];
    }
    const changes = [];
  ...
  truncateValue = function (value, maxLen = 40) {
    let str;
    try {
...
import { hintForPath, humanize, schemaType } from "./config-form.shared.js";
...
// Only Providers benefits from visual form editing (managed panel with credentials).
// Everything else — including Agents — is managed conversationally via config_manage + blueprints.
const SECTIONS = [{ key: "providers", label: "Providers" }];
const ALL_SUBSECTION = "__all__";
export function renderConfig(props) {
  const validity = props.valid == null ? "unknown" : props.valid ? "valid" : "invalid";
  const analysis = analyzeConfigSchema(props.schema);
  // formUnsafe is always false: coerceFormValues preserves all uneditable fields as-is,
  // so saving from form mode never loses config entries. Fields the form can't render
  // are shown inline as "Unsupported schema node. Use Raw mode." but their values survive.
  const formUnsafe = false;
  // Sections fully managed by extraContent — skip config form + section hero for these
  const managedSet = new Set(props.managedPaths ?? []);
  const isManagedSection = managedSet.has(props.activeSection);
  const schemaProps = analysis.schema?.properties ?? {};
  // Whitelist: only SECTIONS entries present in the schema are shown.
  // All other sections are handled conversationally via config_manage + blueprints.
  const allSections = SECTIONS.filter((s) => s.key in schemaProps);
  ...
  const activeSectionMeta = props.activeSection
    ...
    : null;
  const hiddenForSection = new Set(props.hiddenSubsections?.[props.activeSection ?? ""] ?? []);
  const subsections = props.activeSection
    ? resolveSubsections({
        key: props.activeSection,
        schema: activeSectionSchema,
        uiHints: props.uiHints,
      }).filter((s) => !hiddenForSection.has(s.key))
    : [];
  const allowSubnav =
  ...
  // Contextual add button for subsections backed by a map field (hint has addLabel)
  const activeSubsectionAddLabel =
    props.activeSection && effectiveSubsection
      ? (props.uiHints?.[`${props.activeSection}.${effectiveSubsection}`]?.addLabel ?? null)
      : null;
  const onSubsectionAddEntry = activeSubsectionAddLabel
    ? () => {
        const current = props.formValue?.[props.activeSection]?.[effectiveSubsection] ?? {};
        let i = 1;
        let key = `custom-${i}`;
        ...
        props.onFormPatch([props.activeSection, effectiveSubsection, key], "");
      }
    : null;
  ...
  return html`
    <div class="config-layout">
      ...
      <main class="config-main">
              ...
              hasChanges
                ? html`
                  <span class="config-changes-badge"
                    >${props.formMode === "raw" ? "Unsaved changes" : `${diff.length} unsaved change${diff.length !== 1 ? "s" : ""}`}</span
                  >
                `
                : html`
        ...
        ${
          activeSectionMeta && props.formMode === "form" && !isManagedSection
            ? html`
          ...
          allowSubnav
            ? html`
              <div class="config-subnav">
                ...
                ${
                  onSubsectionAddEntry
                    ? html`<button
                      class="config-subnav__add"
                      ?disabled=${props.loading || !props.formValue}
                      @click=${onSubsectionAddEntry}
                    >+ ${activeSubsectionAddLabel}</button>`
                    : nothing
                }
              </div>
            `
        ...
        <!-- Form content -->
        <div class="config-content" data-section=${props.activeSection ?? ""}>
          ${
            props.formMode === "form"
              ? html`
                ${
                ...
                ${props.extraContent?.(props.activeSection) ?? nothing}
              `
              : html`
