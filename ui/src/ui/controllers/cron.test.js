let createState = function (overrides = {}) {
  return {
    client: null,
    connected: true,
    cronLoading: false,
...
import { addCronJob, normalizeCronFormState } from "./cron.js";
describe("cron controller", () => {
  ...
  it("forwards webhook delivery in cron.add payload", async () => {
    const request = vi.fn(async (method, _payload) => {
      if (method === "cron.add") {
        return { id: "job-1" };
      }
      if (method === "cron.list") {
  ...
  it("does not submit stale announce delivery when unsupported", async () => {
    ...
    expect(addCall?.[1]?.delivery).toBeUndefined();
