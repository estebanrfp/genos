...
/**
 * Send an internal message to the LLM via the agent endpoint.
 * No user message is saved in transcript or displayed in chat.
 * Streaming works via the shared broadcast mechanism (delta/final events).
 * @param {Object} state - App state
 * @param {string} message - Internal trigger text
 * @returns {Promise<string|null>} runId on success, null on failure
 */
export async function sendInternalMessage(state, message) {
  if (!state.client || !state.connected) {
    return null;
  }
