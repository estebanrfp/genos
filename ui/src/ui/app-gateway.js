let normalizeSessionKeyForDefaults = function (value, defaults) {
  ...
  handleGatewayEventUnsafe = function (host, evt) {
    ...
    if (evt.event === "chat") {
      ...
      if (state === "final") {
        if (host._internalRun) {
          host._internalRun = false;
          if (payload?.message) {
            host.chatMessages = [...host.chatMessages, payload.message];
          }
        } else {
...
import { loadChatHistory, sendInternalMessage } from "./controllers/chat.js";
...
export function connectGateway(host) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      // Clean stale localStorage — reset to server's main session if stored key doesn't exist
      const mainKey = hello.snapshot?.sessionDefaults?.mainSessionKey;
      if (mainKey && host.sessionKey !== mainKey) {
        host.client
          .request("sessions.list", {})
          .then((res) => {
            const keys = new Set((res?.sessions ?? []).map((s) => s.key));
            if (!keys.has(host.sessionKey)) {
      ...
      // Auto-start onboarding if session is empty (fresh install after provider setup)
      loadChatHistory(host).then(() => {
        if (!host.chatMessages?.length && !host.chatSending) {
          const locale = navigator.language ?? "en";
          sendInternalMessage(host, `/new [locale:${locale}]`);
        }
      });
    },
    onClose: ({ code, reason }) => {
