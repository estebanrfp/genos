let mountApp = function (pathname) {
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/genosos/cron");
    await app.updateComplete;
    expect(app.basePath).toBe("/apps/genosos");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/genosos/cron");
  });
  it("honors explicit base path overrides", async () => {
    window.__GENOS_CONTROL_UI_BASE_PATH__ = "/genosos";
    const app = mountApp("/genosos/sessions");
    await app.updateComplete;
    expect(app.basePath).toBe("/genosos");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/genosos/sessions");
  });
  ...
  it("resets to the main session when opening chat from sidebar navigation", async () => {
    ...
    await app.updateComplete;
    const link = app.querySelector('a.nav-item[href="/chat"]');
    expect(link).not.toBeNull();
    link?.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true, button: 0 }));
    await app.updateComplete;
  ...
  it("hydrates token from URL params and strips it", async () => {
    const app = mountApp("/ui/connection?token=abc123");
    await app.updateComplete;
    expect(app.settings.token).toBe("abc123");
    expect(window.location.pathname).toBe("/ui/connection");
    expect(window.location.search).toBe("");
  });
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "genosos.control.settings.v1",
