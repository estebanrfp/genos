...
GenosOS replaced 12+ UI tabs with `config_manage` — 30 actions, 190 blueprints, 15 operational guides. Configuration is conversation. The blueprint system acts as a semantic bridge between raw JSON paths and natural language guidance.
...
- Smart model routing — boost tool auto-escalates to advanced model when needed
- 3 curated providers (Anthropic, OpenAI, Gemini) with default/boost model pairs
...
- ✅ **Agent templates** — `skills/agent-templates/` with 12 templates (10 business + security guard + SEO specialist). One conversation from "I want an assistant for my X" to a fully configured agent.
