let buildAuthKey = function (account) {
  ...
  getAuthInstance = function (account) {
...
export async function getGoogleChatAccessToken(account) {
...
async function fetchChatCerts() {
  const now = Date.now();
  if (cachedCerts && now - cachedCerts.fetchedAt < 600000) {
