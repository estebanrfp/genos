...
async function withTempStateDir(fn) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-nostr-"));
  process.env.GENOS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.GENOS_STATE_DIR?.trim() || stateEnv.GENOS_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".genos");
      },
    },
  });
  try {
