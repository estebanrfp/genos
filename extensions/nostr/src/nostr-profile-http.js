let checkRateLimit = function (accountId) {
    const now = Date.now();
  ...
  validateUrlSafety = function (urlStr) {
    try {
  ...
  parseAccountIdFromPath = function (pathname) {
    const match = pathname.match(/^\/api\/channels\/nostr\/([^/]+)\/profile/);
    return match?.[1] ?? null;
  },
  isLoopbackRemoteAddress = function (remoteAddress) {
    if (!remoteAddress) {
      return false;
    }
  ...
  isLoopbackOriginLike = function (value) {
    try {
  ...
  enforceLoopbackMutationGuards = function (ctx, req, res) {
...
import { readJsonBodyWithLimit, requestBodyErrorToText } from "genosos/plugin-sdk";
import { z } from "zod";
...
async function readJsonBody(req, maxBytes = 65536, timeoutMs = 30000) {
