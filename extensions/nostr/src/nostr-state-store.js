let normalizeAccountId = function (accountId) {
  ...
  resolveNostrStatePath = function (accountId, env = process.env) {
  ...
  resolveNostrProfileStatePath = function (accountId, env = process.env) {
    const stateDir = getNostrRuntime().state.resolveStateDir(env, os.homedir);
    const normalized = normalizeAccountId(accountId);
    return path.join(stateDir, "nostr", `profile-state-${normalized}.json`);
  },
  safeParseState = function (raw) {
    try {
  ...
  safeParseProfileState = function (raw) {
    try {
      const parsed = JSON.parse(raw);
      if (parsed?.version === 1) {
        return {
...
export function computeSinceTimestamp(state, nowSec = Math.floor(Date.now() / 1000)) {
  if (!state) {
    return nowSec;
  }
