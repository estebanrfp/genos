...
export const discordPlugin = {
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      "- Forms: add `components.modal` (title, fields). GenosOS adds a trigger button and routes submissions as new messages.",
    ],
  },
  messaging: {
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      try {
        return await getDiscordRuntime().channel.discord.monitorDiscordProvider({
          token,
          accountId: account.accountId,
          config: ctx.cfg,
          runtime: ctx.runtime,
        ...
        ctx.log?.error(
          `[${account.accountId}] Discord provider crashed: ${err?.message ?? err}. Check intents and token in the Developer Portal.`,
        );
      }
    },
  },
