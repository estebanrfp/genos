...
export async function startCloudflareTunnel(config) {
  if (!config.token) {
    throw new Error(
      "Cloudflare Tunnel token is required (tunnel.cloudflareToken or CLOUDFLARE_TUNNEL_TOKEN)",
    );
  }
  if (!config.hostname) {
    throw new Error(
      "Cloudflare Tunnel hostname is required (tunnel.cloudflareHostname or CLOUDFLARE_TUNNEL_HOSTNAME)",
    );
  }
  const args = ["tunnel", "--no-autoupdate", "run", "--token", config.token];
  return new Promise((resolve, reject) => {
    ...
    const checkReady = (line) => {
      if (resolved) return;
      if (line.includes("Registered tunnel connection") || line.includes("connIndex=")) {
        resolved = true;
        clearTimeout(timeout);
        const publicUrl = `https://${config.hostname}${config.path}`;
        console.log(`[voice-call] Cloudflare Tunnel active: ${publicUrl}`);
        resolve({
          publicUrl,
          provider: "cloudflare",
          stop: async () => {
