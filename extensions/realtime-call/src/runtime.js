let isLoopbackBind = function (bind) {
    if (!bind) {
      return false;
    }
    return bind === "127.0.0.1" || bind === "::1" || bind === "localhost";
  },
  resolveProvider = function (config) {
    const allowNgrokFreeTierLoopbackBypass =
      config.tunnel?.provider === "ngrok" &&
      isLoopbackBind(config.serve?.bind) &&
      (config.tunnel?.allowNgrokFreeTierLoopbackBypass ?? true);
...
export async function createVoiceCallRuntime(params) {
  ...
  if (config.tunnel?.provider && config.tunnel.provider !== "none") {
    try {
      tunnelResult = await startTunnel({
        ...
        cloudflareToken: config.tunnel.cloudflareToken,
        cloudflareHostname: config.tunnel.cloudflareHostname,
      });
  ...
  publicUrl = publicUrl ?? config.publicUrl ?? null;
  ...
  if (provider.name === "twilio" && config.streaming?.enabled) {
    const streamMode = config.streaming?.mode ?? "realtime-conversation";
    const twilioProvider = provider;
    if (streamMode === "stt-only") {
      ...
      log.info("[voice-call] Realtime conversation mode — TTS/STT handled by OpenAI Realtime API");
    }
