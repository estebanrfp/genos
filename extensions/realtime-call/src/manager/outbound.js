...
export async function initiateCall(ctx, to, sessionKey, options) {
...
export async function speak(ctx, callId, text) {
...
export async function speakInitialMessage(ctx, providerCallId) {
...
export async function continueCall(ctx, callId, prompt) {
  const call = ctx.activeCalls.get(callId);
  if (!call) {
    return { success: false, error: "Call not found" };
  }
...
export async function endCall(ctx, callId) {
  const call = ctx.activeCalls.get(callId);
  if (!call) {
    return { success: false, error: "Call not found" };
  }
  ...
  try {
    ...
    // Call may already be ended on provider side (e.g. stream disconnect).
    // Still notify waiters so tools don't hang.
    if (!TerminalStates.has(call.state)) {
      call.state = "error";
      call.endedAt = Date.now();
      call.endReason = "hangup-bot";
