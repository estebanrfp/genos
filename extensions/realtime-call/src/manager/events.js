let shouldAcceptInbound = function (config, from) {
  ...
  createInboundCall = function (params) {
...
export function processEvent(ctx, event) {
  ...
  switch (event.type) {
    ...
    case "call.ended": {
      ...
      if (!alreadyEnded) ctx.onCallEnded?.(call);
