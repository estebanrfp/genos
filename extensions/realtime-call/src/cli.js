let resolveMode = function (input) {
  ...
  resolveDefaultStorePath = function (config) {
  ...
  summarizeSeries = function (values) {
    if (values.length === 0) {
...
import { sleep } from "genosos/plugin-sdk";
...
export function registerVoiceCallCli(params) {
  ...
  const root = program
    ...
    .addHelpText("after", () => `\nDocs: https://docs.genos.ai/cli/voicecall\n`);
  root
    .command("call")
