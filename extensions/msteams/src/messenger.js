let normalizeConversationId = function (rawId) {
    return rawId.split(";")[0] ?? rawId;
  },
  pushTextMessages = function (out, text, opts) {
    if (!text) {
      return;
    }
    if (opts.chunkText) {
  ...
  clampMs = function (value, maxMs) {
    if (!Number.isFinite(value) || value < 0) {
      return 0;
    }
    return Math.min(value, maxMs);
  },
  resolveRetryOptions = function (retry) {
    if (!retry) {
  ...
  computeRetryDelayMs = function (attempt, classification, opts) {
  ...
  shouldRetry = function (classification) {
    return classification.kind === "throttled" || classification.kind === "transient";
  };
import { isSilentReplyText, loadWebMedia, SILENT_REPLY_TOKEN, sleep } from "genosos/plugin-sdk";
