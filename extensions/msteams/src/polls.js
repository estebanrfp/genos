let isRecord = function (value) {
  ...
  extractSelections = function (value) {
  ...
  readNestedValue = function (value, keys) {
    let current = value;
  ...
  readNestedString = function (value, keys) {
...
const POLL_TTL_MS = 2592000000;
export function extractMSTeamsPollVote(activity) {
  const value = activity?.value;
  if (!value || !isRecord(value)) {
    return null;
  }
  const pollId =
    readNestedString(value, ["genososPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["genosos", "pollId"]) ??
    readNestedString(value, ["genosos", "poll", "id"]) ??
    readNestedString(value, ["data", "genososPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "genosos", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          genososPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "genosos poll vote",
          displayText: "Vote recorded",
          value: { genososPollId: pollId, pollId },
        },
      },
    ],
  };
...
export function normalizeMSTeamsPollSelections(poll, selections) {
