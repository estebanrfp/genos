let isHtmlAttachment = function (att) {
    const contentType = normalizeContentType(att.contentType) ?? "";
    return contentType.startsWith("text/html");
  },
  decodeDataImage = function (src) {
  ...
  normalizeAllowHost = function (value) {
  ...
  isHostAllowed = function (host, allowlist) {
...
export function isLikelyImageAttachment(att) {
  const contentType = normalizeContentType(att.contentType) ?? "";
...
export function isDownloadableAttachment(att) {
  const contentType = normalizeContentType(att.contentType) ?? "";
  if (
    contentType === "application/vnd.microsoft.teams.file.download.info" &&
    isRecord(att.content) &&
...
export function extractHtmlFromAttachment(att) {
...
export function extractInlineImageCandidates(attachments) {
  const out = [];
...
export function safeHostForUrl(url) {
  try {
...
export function resolveAuthAllowedHosts(input) {
  if (!Array.isArray(input) || input.length === 0) {
