...
export const telegramPlugin = {
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      // Clear allowFrom store + pending pairing requests on logout
      try {
        const { clearAllowFromStore, clearPairingRequests } = getTelegramRuntime().pairing;
        const storeAccountId = accountId === DEFAULT_ACCOUNT_ID ? undefined : accountId;
        await clearAllowFromStore("telegram", process.env, storeAccountId);
        await clearPairingRequests("telegram");
      } catch {
        /* best-effort */
      }
