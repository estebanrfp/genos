let sanitizePathSegment = function (value) {
    const cleaned = value
      .trim()
      .toLowerCase()
  ...
  resolveHomeserverKey = function (homeserver) {
    try {
  ...
  hashAccessToken = function (accessToken) {
    return crypto.createHash("sha256").update(accessToken).digest("hex").slice(0, 16);
  },
  resolveLegacyStoragePaths = function (env = process.env) {
