...
const runtimeStub = {
  ...
  media: {
    ...
    isVoiceCompatibleAudio: isVoiceCompatibleAudioMock,
  ...
  channel: {
    text: {
      ...
      chunkMarkdownText: (text) => (text ? [text] : []),
      chunkMarkdownTextWithMode: (text) => (text ? [text] : []),
      resolveMarkdownTableMode: () => "code",
      convertMarkdownTables: (text) => text,
    },
  },
};
let sendMessageMatrix;
const makeClient = () => {
...
describe("sendMessageMatrix media", () => {
  ...
  it("marks voice metadata and sends caption follow-up when audioAsVoice is compatible", async () => {
    ...
    const mediaContent = sendMessage.mock.calls[0]?.[1];
