...
import { EventType, MsgType, RelationType } from "./send/types.js";
const MATRIX_TEXT_LIMIT = 4000;
const getCore = () => getMatrixRuntime();
export { resolveMatrixRoomId } from "./send/targets.js";
export async function sendMessageMatrix(to, message, opts = {}) {
...
export async function sendPollMatrix(to, poll, opts = {}) {
...
export async function sendTypingMatrix(roomId, typing, timeoutMs, client) {
...
export async function sendReadReceiptMatrix(roomId, eventId, client) {
  if (!eventId?.trim()) {
    return;
  }
  const { client: resolved, stopOnDone } = await resolveMatrixClient({
...
export async function reactMatrixMessage(roomId, messageId, emoji, client) {
  if (!emoji.trim()) {
