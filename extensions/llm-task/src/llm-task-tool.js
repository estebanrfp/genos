let stripCodeFences = function (s) {
  ...
  collectText = function (payloads) {
    const texts = (payloads ?? [])
      .filter((p) => !p.isError && typeof p.text === "string")
      .map((p) => p.text ?? "");
    return texts.join("\n").trim();
  },
  toModelKey = function (provider, model) {
...
export function createLlmTaskTool(api) {
  return {
    ...
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via genosos.invoke.",
    ...
    async execute(_id, params) {
      ...
      let tmpDir = null;
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genosos-llm-task-"));
