...
Designed to be called from workflow engines (for example, Workflow via
`genosos.invoke --each`) without adding new GenosOS code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Workflow) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on GenosOS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** GenosOS extension (like `workflow`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.genosv1/extensions` as a standalone plugin directory.
