let generatePkce = function () {
  ...
  parseCallbackInput = function (input) {
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>GenosOS Antigravity OAuth</title>
  </head>
  <body>
    <main>
...
async function exchangeCode(params) {
  ...
  const expires = Date.now() + expiresIn * 1000 - 300000;
...
async function loginAntigravity(params) {
  ...
  if (!needsManual) {
    try {
      callbackServer = await startCallbackServer({ timeoutMs: 300000 });
    } catch {
      callbackServer = null;
    }
  }
