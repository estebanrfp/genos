...
export const whatsappPlugin = {
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // Skip starting if credentials are missing or not registered — login flow handles linking
      if (account.authDir) {
        try {
          const credsPath = await import("node:path").then((m) =>
            m.join(account.authDir, "creds.json"),
          );
          const raw = await import("node:fs/promises").then((m) => m.readFile(credsPath, "utf8"));
          const creds = JSON.parse(raw);
          if (creds.registered !== true) {
            ctx.log?.info(
              `[${account.accountId}] skipping start — credentials not registered (re-link needed)`,
            );
            return;
          }
        } catch {
          ctx.log?.info(
            `[${account.accountId}] skipping start — no credentials found (link needed)`,
          );
          return;
        }
      }
