---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for GenosOS deployments.
---

# GenosOS Host Hardening

Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, GenosOS cron scheduling for periodic checks, or version status checks on a machine running GenosOS (laptop, workstation, Pi, VPS).

## Overview

Assess and harden the host running GenosOS, then align it to a user-defined risk tolerance without breaking access. Use GenosOS security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim GenosOS changes the host firewall, SSH, or OS updates; it does not.
...
5. GenosOS gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if GenosOS can access sensitive data.
...
### 2) Run GenosOS security audits (read-only)

As part of the default read-only checks, run `genosos security audit --deep`. Only offer alternatives if the user requests them:

1. `genosos security audit` (faster, non-probing)
2. `genosos security audit --json` (structured output)

Offer to apply GenosOS safe defaults (numbered):

1. `genosos security audit --fix`

Be explicit that `--fix` only tightens GenosOS defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check GenosOS version/update status (read-only)

As part of the default read-only checks, run `genosos update status`.
...
- Credential hygiene notes (location of GenosOS creds, prefer disk encryption)
...
After GenosOS install or first hardening pass, run at least one baseline audit and version check:

- `genosos security audit`
- `genosos security audit --deep`
- `genosos update status`

Ongoing monitoring is recommended. Use the GenosOS cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `genosos cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `genosos update status`
...
Before creating, `genosos cron list` and match on exact `name`. If found, `genosos cron edit <id> ...`.
If not found, `genosos cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `genosos update status` (preferred for source checkouts and channels)
2. `npm view genosos version` (published npm version)

## GenosOS command accuracy

Use only supported commands and flags:

- `genosos security audit [--deep] [--fix] [--json]`
- `genosos status` / `genosos status --deep`
- `genosos health --json`
- `genosos update status`
- `genosos cron add|list|runs|run`

Do not invent CLI flags or imply GenosOS enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by GenosOS compaction:
