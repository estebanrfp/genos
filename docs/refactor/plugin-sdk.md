...
- Publish as `genosos/plugin-sdk` (or export from core under `genosos/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `GenosOSPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: GenosOSConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: GenosOSConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: GenosOSConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: GenosOSConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: GenosOSConfig): string;
  };
};
```
...
- Introduce `genosos/plugin-sdk`.
- Add `api.runtime` to `GenosOSPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `genososRuntime: ">=2026.2.0"`).
