...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.genosv1/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.genosv1/node.json` (node id + pairing token).
...
Path: `~/.genosv1/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.genosv1/exec-approvals.sock",
...
- Unix socket at `~/.genosv1/exec-approvals.sock` (0600).
