---
summary: "Chrome extension: let GenosOS drive your existing Chrome tab"
read_when:
...
The GenosOS Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate genosos-managed Chrome profile.
...
GenosOS then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the GenosOS release (npm package) as static files. There is no separate “build” step.

After upgrading GenosOS:

- Re-run `genosos browser extension install` to refresh the installed files under your GenosOS state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

GenosOS ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `GENOS_GATEWAY_TOKEN`)
...
- Open the tab you want GenosOS to control.
...
- `ON`: attached; GenosOS can drive that tab.
...
Debugging: `genosos sandbox explain`
...
`genosos browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `genosos browser extension install` first to copy the extension to a stable location under your GenosOS state directory.
...
- **This is not isolated** like the dedicated genosos-managed profile.
