...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `genosos sandbox explain`.
...
Legacy `agent.*` configs are migrated by `genosos doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=genosos-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${GENOS_STATE_DIR:-$HOME/.genos}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
