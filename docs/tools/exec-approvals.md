...
- **gateway host** → `genosos` process on the gateway machine
...
`~/.genosv1/exec-approvals.json` directly.

CLI: `genosos approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
