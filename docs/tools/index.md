---
summary: "Agent tool surface for GenosOS (browser, canvas, nodes, message, cron) replacing legacy `genosos-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `genosos-*` skills
title: "Tools"
---

# Tools (GenosOS)

GenosOS exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `genosos-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `genosos.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, GenosOS logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:genosos`: all built-in GenosOS tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`genosos node run`).
...
GenosOS tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `genosos configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated GenosOS-managed browser.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `genosos gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, GenosOS clamps `tools.sessions.visibility` to `tree`.
