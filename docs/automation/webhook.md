...
read_when:
  ...
  - Wiring external systems into GenosOS
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your GenosOS config directory (typically `~/.genosv1/hooks/transforms`).
...
- `genosos webhooks gmail setup` writes `hooks.gmail` config for `genosos webhooks gmail run`.
