...
- Jobs persist under `~/.genosv1/cron/` so restarts don’t lose schedules.
...
genosos cron list
genosos cron run <job-id>
genosos cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.genosv1/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `genosos cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, GenosOS applies a
...
If `delivery` is omitted for isolated jobs, GenosOS defaults to `announce`.
...
- Job store: `~/.genosv1/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.genosv1/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
genosos cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
genosos cron edit <jobId> --agent ops
genosos cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
genosos cron run <jobId>
genosos cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
genosos cron edit <jobId> \
...
```bash
genosos system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`genosos system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `GENOS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- GenosOS applies exponential retry backoff for recurring jobs after consecutive errors:
