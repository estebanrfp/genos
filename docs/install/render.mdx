...
Deploy GenosOS on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/genosos/genosos)
...
repository configures everything needed to run GenosOS:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: GENOS_STATE_DIR
        value: /data/.genos
      - key: GENOS_WORKSPACE_DIR
        value: /data/workspace
      - key: GENOS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/genosos`.
...
If you use the original GenosOS repository, Render will not auto-deploy your GenosOS. To update it, run a manual Blueprint sync from the dashboard.
...
For GenosOS, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any GenosOS host.
