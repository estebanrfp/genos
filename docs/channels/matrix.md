...
Matrix is an open, decentralized messaging protocol. GenosOS connects as a Matrix **user**
...
```bash
genosos plugins install @genosos/matrix
```

Local checkout (when running from a git repo):

```bash
genosos plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `genosos plugins install @genosos/matrix`
   - From a local checkout: `genosos plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: GenosOS calls the same
     login endpoint, stores the access token in `~/.genosv1/credentials/matrix/credentials.json`,
...
- On first connection, GenosOS requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  GenosOS logs a warning.
...
`~/.genosv1/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `genosos pairing list matrix`
  - `genosos pairing approve matrix <CODE>`
...
- On startup, GenosOS resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
