...
read_when:
  - Setting up Twitch chat integration for GenosOS
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. GenosOS connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
genosos plugins install @genosos/twitch
```

Local checkout (when running from a git repo):

```bash
genosos plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `GENOS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "genosos", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use GENOS_TWITCH_ACCESS_TOKEN env var)
...
```bash
GENOS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
