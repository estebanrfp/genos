...
read_when:
  - You want GenosOS to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables GenosOS to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`genosos onboard`) and `genosos channels add` list optional channel plugins.
...
```bash
genosos plugins install @genosos/nostr
```

Use a local checkout (dev workflows):

```bash
genosos plugins install --link <path-to-genosos>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "genosos@example.com",
        "lud16": "genosos@example.com"
      }
    }
  }
}
