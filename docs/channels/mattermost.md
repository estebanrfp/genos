---
summary: "Mattermost bot setup and GenosOS config"
read_when:
...
```bash
genosos plugins install @genosos/mattermost
```

Local checkout (when running from a git repo):

```bash
genosos plugins install ./extensions/mattermost
```
...
4. Configure GenosOS and start the gateway.
...
- Approve via:
  - `genosos pairing list mattermost`
  - `genosos pairing approve mattermost <CODE>`
...
Use these target formats with `genosos message send` or cron/webhooks:
