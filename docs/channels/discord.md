...
You will need to create a new application with a bot, add the bot to your server, and pair it to GenosOS. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "GenosOS".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your GenosOS agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to GenosOS in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with GenosOS. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Step 0: Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running GenosOS before messaging your agent.

```bash
genosos config set channels.discord.token '"YOUR_BOT_TOKEN"' --json
genosos config set channels.discord.enabled true --json
genosos gateway
```

    If GenosOS is already running as a background service, use `genosos gateway restart` instead.

  </Step>

  <Step title="Configure GenosOS and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your GenosOS agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
```bash
genosos pairing list discord
genosos pairing approve discord <CODE>
```
...
GenosOS supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- GenosOS adds a trigger button automatically
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in GenosOS config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces GenosOS auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while GenosOS is processing an inbound message.
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/genosos",
    },
  },
}
```
...
GenosOS uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires Carbon component instances), while legacy `embeds` remain available but are not recommended.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. GenosOS generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; GenosOS converts to OGG/Opus when needed.
...
- If command deploy/state is stale, restart gateway and re-check with `genosos channels status --probe`.
