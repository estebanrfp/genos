...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects GenosOS to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
genosos plugins install @genosos/feishu
```

Local checkout (when running from a git repo):

```bash
genosos plugins install ./extensions/feishu
```
...
If you just installed GenosOS, run the wizard:
...
2. Configuring app credentials in GenosOS
...
1. You already ran `genosos channels add` for Feishu
2. The gateway is running (`genosos gateway status`)
...
```bash
genosos pairing approve feishu <CODE>
```
...
2. Run `genosos logs --follow` and look for `chat_id`
...
2. Run `genosos logs --follow` and look for `open_id`
...
| Command                   | Description                   |
| ------------------------- | ----------------------------- |
| `genosos gateway status`  | Show gateway status           |
| `genosos gateway install` | Install/start gateway service |
| `genosos gateway stop`    | Stop gateway service          |
| `genosos gateway restart` | Restart gateway service       |
| `genosos logs --follow`   | Tail gateway logs             |
...
4. Check logs: `genosos logs --follow`
...
5. Ensure the gateway is running: `genosos gateway status`
6. Check logs: `genosos logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.genos/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.genos/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
