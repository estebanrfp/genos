...
Tlon is a decentralized messenger built on Urbit. GenosOS connects to your Urbit ship and can
...
```bash
genosos plugins install @genosos/tlon
```

Local checkout (when running from a git repo):

```bash
genosos plugins install ./extensions/tlon
```
...
By default, GenosOS blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `genosos message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, GenosOS replies in-thread.
