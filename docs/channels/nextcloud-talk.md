...
```bash
genosos plugins install @genosos/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
genosos plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "GenosOS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `genosos pairing list nextcloud-talk`
  - `genosos pairing approve nextcloud-talk <CODE>`
