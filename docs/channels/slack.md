...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure GenosOS HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `genosos pairing approve slack <code>`.
...
```json
{
  "display_information": {
    "name": "GenosOS",
    "description": "Slack connector for GenosOS"
  },
  "features": {
    "bot_user": {
      "display_name": "GenosOS",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/genosos",
        "description": "Send a message to GenosOS",
        "should_escape": false
      }
    ]
  },
...
GenosOS supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, GenosOS falls back to normal delivery for remaining payloads.
