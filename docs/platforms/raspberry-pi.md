---
summary: "GenosOS on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up GenosOS on a Raspberry Pi
  - Running GenosOS on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# GenosOS on Raspberry Pi

## Goal

Run a persistent, always-on GenosOS Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most GenosOS features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled genosos

# Enable if not
sudo systemctl enable genosos

# Start on boot
sudo systemctl start genosos
```
...
```bash
# Check logs
journalctl -u genosos --no-pager -n 100

# Common fix: rebuild
cd ~/genosos  # if using hackable install
npm run build
sudo systemctl restart genosos
```
