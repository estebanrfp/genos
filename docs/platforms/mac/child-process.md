...
service via the external `genosos` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`GENOS_PROFILE`; legacy `com.genos.*` is supported).
