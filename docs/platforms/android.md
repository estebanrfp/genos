...
- You can run the CLI (`genosos`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.genosv1/genosos.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `genosos.internal.`) on the gateway host and publish `_genosos-gw._tcp` records.
...
1. Create `~/.genosv1/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
genosos nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__genosos__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__genosos__/canvas/`.
