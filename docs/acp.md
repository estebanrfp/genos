...
This document describes how the GenosOS ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`genosos acp` exposes an ACP agent over stdio and forwards prompts to a running
GenosOS Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a GenosOS Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `genosos acp` over stdio.

Example config:

```bash
genosos config set gateway.remote.url wss://gateway-host:18789
genosos config set gateway.remote.token <token>
```

Example run:

```bash
genosos acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
genosos acp --session agent:main:main
genosos acp --session agent:design:main
genosos acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “GenosOS ACP” to start a thread.

## Execution Model

- ACP client spawns `genosos acp` and speaks ACP messages over stdio.
...
```bash
genosos acp --session agent:main:main
genosos acp --session-label "support inbox"
genosos acp --reset-session
```
...
`genosos acp` resolves the Gateway URL and auth from CLI flags or config:
