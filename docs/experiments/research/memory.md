...
read_when:
  - Designing workspace memory (~/.genosv1/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep GenosOS integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.genosv1/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.genosv1/workspace` as canonical human-readable memory.
...
```
~/.genosv1/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in GenosOS**, but keep a separable core library.

### Why integrate into GenosOS?

- GenosOS already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `genosos memory recall "…" --k 25 --since 30d`
  - `genosos memory reflect --since 7d`
...
Pragmatic take for `~/.genosv1/workspace`:
