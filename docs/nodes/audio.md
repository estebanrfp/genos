...
- **Media understanding (audio)**: If audio understanding is enabled (or auto‑detected), GenosOS:
...
GenosOS auto-detects in this order and stops at the first working option:
...
When `requireMention: true` is set for a group chat, GenosOS now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, GenosOS performs a "preflight" transcription.
