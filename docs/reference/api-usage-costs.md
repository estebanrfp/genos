...
GenosOS features that can generate provider usage or paid API calls.
...
- `genosos status --usage` and `genosos channels list` show provider **usage windows**
...
GenosOS can pick up credentials from:
...
- `genosos status --usage`
- `genosos models status --json`
...
`genosos models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
