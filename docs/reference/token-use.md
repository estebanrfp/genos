---
summary: "How GenosOS builds prompt context and reports token usage + costs"
read_when:
...
GenosOS tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

GenosOS assembles its own system prompt on every run. It includes:
...
For images, GenosOS downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from GenosOS session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `genosos status --usage` and `genosos channels list` show
...
`cacheWrite`. If pricing is missing, GenosOS shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. GenosOS can
...
Anthropic's 1M context window is currently beta-gated. GenosOS can inject the
