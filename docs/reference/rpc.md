...
GenosOS integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- GenosOS owns lifecycle when `channels.signal.autoStart=true`.
...
- GenosOS spawns `imsg rpc` as a child process (legacy iMessage integration).
