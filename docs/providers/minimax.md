---
summary: "Use MiniMax M2.1 in GenosOS"
read_when:
  - You want MiniMax models in GenosOS
...
```bash
genosos plugins enable minimax-portal-auth  # skip if already loaded.
genosos gateway restart  # restart if gateway is already running
genosos onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/estebanrfp/GenosOS/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `genosos.json`:
...
- Use `genosos models list` and `genosos models set minimax/MiniMax-M2.1` to switch.
...
- Running `genosos configure` and selecting **MiniMax M2.1**, or
