---
summary: "Use Synthetic's Anthropic-compatible API in GenosOS"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. GenosOS registers it as the
...
```bash
genosos onboard --auth-choice synthetic-api-key
```
...
Note: GenosOS's Anthropic client appends `/v1` to the base URL, so use
