---
summary: "Use Venice AI privacy-focused models in GenosOS"
read_when:
  - You want privacy-focused inference in GenosOS
...
```bash
genosos onboard --auth-choice venice-api-key
```
...
```bash
genosos chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, GenosOS shows all available Venice models. Pick based on your needs:
...
```bash
genosos models set venice/claude-opus-45
genosos models set venice/llama-3.3-70b
```

List all available models:

```bash
genosos models list | grep venice
```

## Configure via `genosos configure`

1. Run `genosos configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
GenosOS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
genosos chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
genosos chat --model venice/claude-opus-45

# Use uncensored model
genosos chat --model venice/venice-uncensored

# Use vision model with image
genosos chat --model venice/qwen3-vl-235b-a22b

# Use coding model
genosos chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `genosos models list` to see currently available models. Some models may be temporarily offline.
