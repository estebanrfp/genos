...
read_when:
  - You need detailed behavior for genosos onboard
...
This page is the full reference for `genosos onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.genosv1/genosos.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `genosos doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `genosos pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `genosos health`.
    - `genosos status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.genosv1/.env` so launchd can read it.
...
- OAuth credentials: `~/.genosv1/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.genosv1/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.genosv1/credentials/oauth.json` (or `$GENOS_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`genosos onboard`](/cli/onboard)
