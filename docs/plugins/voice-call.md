...
read_when:
  - You want to place an outbound voice call from GenosOS
...
Voice calls for GenosOS via a plugin. Supports outbound notifications and
...
- Use `genosos voicecall ...` or the `voice_call` tool
...
```bash
genosos plugins install ./extensions/voice-call
