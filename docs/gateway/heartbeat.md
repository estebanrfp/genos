...
- During heartbeat runs, GenosOS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `genosos sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, GenosOS skips the heartbeat run entirely (no model call).
...
```bash
genosos system event --text "Check for urgent follow-ups" --mode now
```
