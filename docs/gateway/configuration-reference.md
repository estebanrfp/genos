...
description: "Complete field-by-field reference for ~/.genosv1/genosos.json"
---

# Configuration Reference

Every field available in `~/.genosv1/genosos.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — GenosOS uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.genosv1/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `genosos doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["genosos-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-genosos",
          requireMention: false,
...
Mattermost ships as a plugin: `genosos plugins install @genosos/mattermost`.
...
GenosOS spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@genosos", "genosos"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@genosos"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `genosos.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, GenosOS auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/genosos" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, GenosOS assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.genosv1/sandboxes",
        docker: {
          image: "genosos-sandbox:bookworm-slim",
          containerPrefix: "genosos-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "genosos-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "genosos-sandbox-browser:bookworm-slim",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
- `none`: per-scope sandbox workspace under `~/.genosv1/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.genosv1/workspace",
        agentDir: "~/.genosv1/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@genosos"] },
        sandbox: { mode: "off" },
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.genosv1/workspace-home" },
      { id: "work", workspace: "~/.genosv1/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  session: {
    ...
    store: "~/.genosv1/agents/{agentId}/sessions/sessions.json",
    maintenance: {
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.genosv1/settings/tts.json",
      elevenlabs: {
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:genosos`    | All built-in tools (excludes provider plugins)                                           |
...
GenosOS uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.genosv1/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `GENOS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `genosos onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `genosos onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `genosos onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `genosos onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `genosos onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `genosos onboard --auth-choice minimax-api`.
...
- Loaded from `~/.genosv1/extensions`, `<workspace>/.genos/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or GENOS_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `GENOS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
```bash
GENOS_CONFIG_PATH=~/.genosv1/a.json \
GENOS_STATE_DIR=~/.genos-a \
genosos gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.genos-dev` + port `19001`), `--profile <name>` (uses `~/.genos-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.genosv1/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-genosos-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `GENOS_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.genosv1/workspace/canvas",
    liveReload: true,
    // enabled: false, // or GENOS_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__genosos__/canvas/`
- Local-only: keep `gateway.bind: "loopback"` (default).
- Non-loopback binds: canvas routes require Gateway auth (token/password/trusted-proxy), same as other Gateway HTTP surfaces.
- Node WebViews typically don't send auth headers; after a node is paired and connected, the Gateway allows a private-IP fallback so the node can load canvas without leaking secrets into URLs.
...
- Hostname defaults to `genosos`. Override with `GENOS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.genosv1/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `genosos dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.genosv1/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${GENOS_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.genosv1/credentials/oauth.json`.
...
- Default log file: `/tmp/genosos/genosos-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `genosos doctor --fix` can strip unknown keys).
