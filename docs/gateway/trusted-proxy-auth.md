...
read_when:
  - Running GenosOS behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of GenosOS
...
- You run GenosOS behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. GenosOS checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. GenosOS extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://genosos.example.com
    to: http://genosos-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`genosos security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update GenosOS config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `genosos security audit` and review findings
