...
## Quick check: `genosos security audit`
...
GenosOS is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, GenosOS also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.genosv1/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.genosv1/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.genosv1/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.genosv1/credentials/oauth.json`
- **Encrypted vault**: `~/.genosv1/vault.enc` (AES-256-GCM; managed via `genosos vault`)
...
| -------------------------------------------- | ------------- | -------------------------------------------------------- | ------------------------------------------------ | -------- |
| `fs.state_dir.perms_world_writable`          | critical      | Other users/processes can modify full GenosOS state      | filesystem perms on `~/.genos`                   | yes      |
| `fs.config.perms_writable`                   | critical      | Others can change auth/tool policy/config                | filesystem perms on `~/.genosv1/genosos.json`    | yes      |
...
`genosos security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${GENOS_GATEWAY_PASSWORD}
```
...
GenosOS stores session transcripts on disk under `~/.genosv1/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.genos` (see the audit section below). If you need
...
GenosOS can refresh the skills list mid-session:
...
- If you install plugins from npm (`genosos plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.genosv1/extensions/<pluginId>/` (or `$GENOS_STATE_DIR/extensions/<pluginId>/`).
  - GenosOS uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
By default, GenosOS routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
GenosOS has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.genosv1/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.genos or your logs.”

## Unsafe external content bypass flags

GenosOS includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.genosv1/genosos.json`: `600` (user read/write only)
- `~/.genos`: `700` (user only)

`genosos doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `GENOS_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__genosos__/canvas/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_genosos-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `GENOS_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `genosos doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `GENOS_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `GENOS_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), GenosOS
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. GenosOS verifies the identity by resolving the
...
- GenosOS will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.genosv1/` (or `$GENOS_STATE_DIR/`) may contain secrets or private data:

- `genosos.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- `vault.enc`: AES-256-GCM encrypted secret vault (managed via `genosos vault set|get|list|delete`). Uses PBKDF2 key derivation (100k iterations, SHA-512). Passphrase sourced from `VAULT_PASSPHRASE` env, `~/.genosv1/.env`, or interactive prompt. Each write generates a fresh IV; file is `chmod 600`.
...
- Store sensitive credentials in the encrypted vault (`genosos vault set <key> <value>`) instead of plaintext config.
...
- When sharing diagnostics, prefer `genosos status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@genosos", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.genosv1/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `genosos` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only GenosOS clients can connect.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.genosv1/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.genosv1/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.genosv1/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "none",
        },
        // Session tools can reveal sensitive data from transcripts. By default GenosOS limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `genosos gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `GENOS_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/genosos/genosos-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.genosv1/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `genosos security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + GenosOS version
...
Found a vulnerability in GenosOS? Please report responsibly:

1. Email: [security@genosos.ai](mailto:security@genosos.ai)
