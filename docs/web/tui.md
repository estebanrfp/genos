...
```bash
genosos tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `genosos tui --deliver`
...
- Check the Gateway logs: `genosos logs --follow`.
- Confirm the agent can run: `genosos status` and `genosos models status`.
...
- No agents in picker: check `genosos agents list` and your routing config.
