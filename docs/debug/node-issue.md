...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in GenosOS
...
Running GenosOS via Node with `tsx` fails at startup with:

```
[genosos] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `genosos status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch genosos.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node genosos.mjs status` works on Node 25.
