...
- `/usage cost` in chats: local cost summary aggregated from GenosOS session logs.
- CLI: `genosos status --usage` prints a full per-provider breakdown.
- CLI: `genosos channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
